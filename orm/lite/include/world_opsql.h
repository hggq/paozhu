#ifndef _ORM_LITE_WORLD_OPERATE_H
#define _ORM_LITE_WORLD_OPERATE_H
/*
 ...
 *  Creation time Thu, 17 Sep 2026 23:22:09 GMT
 */
#include <iostream>
#include <mutex>
#include <string>
...
{ /*tagnamespace_replace*/
    ...
    class world_opsql : public B_BASE
    {
      public:
        world_opsql(const std::string &tag) : dbtag(tag)
        {
            ...
            {
                //必须是 DB_TYPE::SQLITE 类型链接
                if (iter->second->conf_data[0].db_type != DB_TYPE::SQLITE)
                {
                    conn_obj  = nullptr;
                    iserror   = true;
        ...
        unsigned int count()
        {
            ...
            try
            {
                ...
                if (islock_conn)
                {
                    ...
                    {
                        select_conn = conn_obj->get_sqlite_select_conn();
                    }
                }
                else
                {
                ...
                bool ok = select_conn->query_scalar(sqlstring, scalar_result);
                if (!ok)
                {
                    iserror   = true;
                ...
                if (!islock_conn)
                {
                    conn_obj->back_sqlite_select_conn(std::move(select_conn));
                }
        ...
        unsigned int exec_count()
        {
            ...
            if (count_val == 0 && !conn->error_msg.empty())
            {
                ...
                if (!islock_conn)
                    conn_obj->back_sqlite_select_conn(std::move(conn));
                return 0;
            }
        ...
        asio::awaitable<unsigned int> async_count()
        {
            ...
            try
            {
                ...
                if (islock_conn)
                {
                    if (!select_conn || select_conn->isclose)
                    {
                        select_conn = co_await conn_obj->async_get_sqlite_select_conn();
                    }
                }
                else
                {
                ...
                sqlite_scalar_result scalar_result;
                bool ok = co_await select_conn->async_query_scalar(sqlstring, scalar_result);
                if (!ok)
                {
                    iserror   = true;
                    error_msg = select_conn->error_msg;
        ...
        asio::awaitable<unsigned int> async_exec_count()
        {
            ...
            std::shared_ptr<sqlite_conn_base> select_conn_l;
            if (islock_conn)
            {
                if (!this->select_conn || this->select_conn->isclose)
                    this->select_conn = co_await conn_obj->async_get_sqlite_select_conn();
                select_conn_l = this->select_conn;
            }
            else
            {
                select_conn_l = co_await conn_obj->async_get_sqlite_select_conn();
            }
            ...
            if (count_val == 0 && !select_conn_l->error_msg.empty())
            {
                ...
                if (!islock_conn)
                    conn_obj->back_sqlite_select_conn(std::move(select_conn_l));
                co_return 0;
            }
        ...
        unsigned int update_col(std::string colname, int num, char symbol = '+')
        {
            ...
            try
            {
                ...
                //auto conn = conn_obj->get_sqlite_edit_conn();

                if (islock_conn)
                {
                    if (!edit_conn || edit_conn->isclose)
                    {
                        edit_conn = conn_obj->get_sqlite_edit_conn();
                    }
                }
                else
                {
                ...
                else
                {
                    ...
                    if (!islock_conn)
                    {
                        conn_obj->back_sqlite_edit_conn(std::move(edit_conn));
                    }
                }
        ...
        asio::awaitable<unsigned int> async_update_col(std::string colname, int num, char symbol = '+')
        {
            ...
            try
            {
                ...
                if (islock_conn)
                {
                    if (!edit_conn || edit_conn->isclose)
                    {
                        edit_conn = co_await conn_obj->async_get_sqlite_edit_conn();
                    }
                }
                else
                {
        ...
        asio::awaitable<unsigned int> async_exec_update_col(std::string colname, int num, char symbol = '+')
        {
            ...
            std::shared_ptr<sqlite_conn_base> edit_conn_l;
            if (islock_conn)
            {
                if (!this->edit_conn || this->edit_conn->isclose)
                    this->edit_conn = co_await conn_obj->async_get_sqlite_edit_conn();
                edit_conn_l = this->edit_conn;
            }
            else
            {
                edit_conn_l = co_await conn_obj->async_get_sqlite_edit_conn();
            }
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, world_info::meta &data_temp)
    {
        ...
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, world_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
            ...
            case 1: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.randomnumber = static_cast<decltype(data_temp.randomnumber )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.randomnumber = static_cast<decltype(data_temp.randomnumber )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.randomnumber = static_cast<decltype(data_temp.randomnumber )>(v);
        ...
        http::obj_val get_field_value(unsigned char index_pos, const world_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
                ...
                break;
            case 1:
                return http::obj_val(static_cast<long long>(data_temp.randomnumber));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const world_info::meta &data_temp)
    {
...
M_MODEL& eqRandomnumber(const std::string &val)
	{return where(B_BASE::cols::randomnumber, orm::wq::eq, val);
	}

M_MODEL& nqRandomnumber(const std::string &val)
	{return where(B_BASE::cols::randomnumber, orm::wq::nq, val);
	}

M_MODEL& btRandomnumber(const std::string &val)
	{return where(B_BASE::cols::randomnumber, orm::wq::bt, val);
	}

M_MODEL& beRandomnumber(const std::string &val)
	{return where(B_BASE::cols::randomnumber, orm::wq::be, val);
	}

M_MODEL& ltRandomnumber(const std::string &val)
	{return where(B_BASE::cols::randomnumber, orm::wq::lt, val);
	}

M_MODEL& leRandomnumber(const std::string &val)
	{return where(B_BASE::cols::randomnumber, orm::wq::le, val);
	}
...
M_MODEL& nullRandomnumber()
	{return whereNull(B_BASE::cols::randomnumber);
	}

M_MODEL& notnullRandomnumber()
	{return whereNotNull(B_BASE::cols::randomnumber);
	}

M_MODEL& oreqRandomnumber(const std::string &val)
	{return whereOr(B_BASE::cols::randomnumber, orm::wq::eq, val);
	}

M_MODEL& ornqRandomnumber(const std::string &val)
	{return whereOr(B_BASE::cols::randomnumber, orm::wq::nq, val);
	}

M_MODEL& orbtRandomnumber(const std::string &val)
	{return whereOr(B_BASE::cols::randomnumber, orm::wq::bt, val);
	}

M_MODEL& orbeRandomnumber(const std::string &val)
	{return whereOr(B_BASE::cols::randomnumber, orm::wq::be, val);
	}

M_MODEL& orltRandomnumber(const std::string &val)
	{return whereOr(B_BASE::cols::randomnumber, orm::wq::lt, val);
	}

M_MODEL& orleRandomnumber(const std::string &val)
	{return whereOr(B_BASE::cols::randomnumber, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqRandomnumber(T val)
	{return whereOr(B_BASE::cols::randomnumber, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqRandomnumber(T val)
	{return whereOr(B_BASE::cols::randomnumber, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtRandomnumber(T val)
	{return whereOr(B_BASE::cols::randomnumber, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeRandomnumber(T val)
	{return whereOr(B_BASE::cols::randomnumber, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltRandomnumber(T val)
	{return whereOr(B_BASE::cols::randomnumber, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleRandomnumber(T val)
	{return whereOr(B_BASE::cols::randomnumber, orm::wq::le, val);
	}

M_MODEL& ornullRandomnumber()
	{return whereOrNull(B_BASE::cols::randomnumber);
	}

M_MODEL& ornotnullRandomnumber()
	{return whereOrNotNull(B_BASE::cols::randomnumber);
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = world_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &world_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &world_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(world_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(world_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(world_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(world_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &world_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &world_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &world_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &world_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &world_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(world_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &world_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(world_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &world_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(world_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &world_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(world_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &world_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(world_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(world_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(world_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(world_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(world_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(world_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(world_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(world_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(world_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(world_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &world_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &world_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        void build_text_where(std::string &where_clause)
        {
            ...
            // 有 leftjoin 时字段名前自动加主表名前缀，避免与 join 表字段歧义
            auto append_field = [&](const std::string &fname)
            {
                ...
                where_clause.append(escape_sqlite_col(fname));
            };
        ...
        M_MODEL &desc(world_info::cols field, world_info::cols field2)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &world_info::col_names> field1, orm::table_col<B_BASE, &world_info::col_names> field2)
        {
        ...
        M_MODEL &asc(world_info::cols field, world_info::cols field2)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &world_info::col_names> field1, orm::table_col<B_BASE, &world_info::col_names> field2)
        {
        ...
        M_MODEL &order(orm::table_col<B_BASE, &world_info::col_names> wq, const std::string &asc_or_desc)
        {
            ordersql.append(" ORDER BY ");
            ordersql.append(wq);
            ordersql.append(" ");
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &world_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &world_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &world_info::col_names> wq)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &world_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
                ...
                unsigned int fetch_count = select_conn->fetch_directly(sqlstring,
                                                                       [this, &temprecord, col_cache = sqlite_conn_col_name_cache_t{}](int col_count, char **col_names, auto get_data) mutable -> bool
                                                                       {
                                                                           ...
                                                                           for (int ij = 0; ij < col_count; ij++)
                                                                           {
                                                                               ...
                                                                               if (ptr == nullptr)
                                                                               {
                                                                                   data_temp.insert({col_cache[ij], ""});// NULL value -> keep column key
                                                                                   continue;
                                                                               }
        ...
        {
            model_meta_cache<world_info::meta> &data_cache = model_meta_cache<world_info::meta>::getinstance();
            data_cache.remove_exptime();

            model_meta_cache<std::vector<world_info::meta>> &record_cache = model_meta_cache<std::vector<world_info::meta>>::getinstance();
                ...
                unsigned int fetch_count = select_conn->fetch_directly(sqlstring,
                                                                       [this, &valuetemp, col_cache = sqlite_conn_col_name_cache_t{}](int col_count, char **col_names, auto get_data) mutable -> bool
                                                                       {
                                                                           ...
                                                                           for (int ij = 0; ij < col_count; ij++)
                                                                           {
                                                                               ...
                                                                               if (ptr == nullptr)
                                                                               {
                                                                                   json_temp_v[col_cache[ij]] = "";// NULL value -> empty string
                                                                                   continue;
                                                                               }
                                                                               if (!col_cache[ij].empty())
                                                                               {
        ...
        void build_join_on_sql(std::string &on_clause)
        {
            ...
            for (const auto &item : join_ptr->on_sqls)
            {
                ...
                // ON join_tbl.right_field = main_tbl.left_field
        ...
        void build_join_sub_sql(std::string &sub_where)
        {
            ...
            for (const auto &item : join_ptr->sub_sqls)
            {
                ...
                // 子查询单表，字段名不需要表名前缀
                sub_where.append(escape_sqlite_col(item.left_filed_name));
                ...
                case orm::wq::notin:
                    ...
                    {
                        // 兼容旧用法: joinWhere(field, wq::in, "a,b,c")
        ...
        M_MODEL &joinOn(std::string_view join_field, std::string_view main_field)
        {
            ...
            item.right_filed_name = join_field;// join 表字段
            item.left_filed_name  = main_field;// 主表字段
            join_ptr->on_sqls.push_back(std::move(item));

            return *mod;
        }

        // joinOn C++20 模板版: 主表字段用 cols 枚举, join表字段仍为 string
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = world_info::col_names[item.left_idx];
        ...
        M_MODEL &joinWhere(std::string_view field, orm::wq opwq, T2 &&value)
        {
            ...
            if (opwq == orm::wq::in || opwq == orm::wq::notin)
            {
                // IN 场景: value 是逗号分隔的字符串, build 时 split
                // 或调用者用 set_array + push 手动构建
            }
        ...
        world_opsql(const world_opsql &)            = delete;
        world_opsql &operator=(const world_opsql &) = delete;
        world_opsql(world_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        world_opsql &operator=(world_opsql &&o) noexcept
        {
            if (this != &o)
            {
      ...
      public:
        ...
        template <typename T>
        M_MODEL &AND(orm::table_col<B_BASE, &world_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = opwq;
        ...
        template <typename T>
        M_MODEL &OR(orm::table_col<B_BASE, &world_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = opwq;
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < world_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == world_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(world_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(world_info::col_names[idx]);
                sql.append(" = ?");
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_exec_insert()
        {
            ...
            try
            {
                ...
                long long last_id = co_await edit_conn->async_last_insert_rowid();
                B_BASE::setPK(last_id);
                effect_num = affected;
                if (!islock_conn)
                {
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = world_info::col_names.size();
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_exec_insert_batch()
        {
            ...
            try
            {
                ...
                long long first_id = co_await edit_conn->async_last_insert_rowid();
                effect_num         = affected;
                if (!islock_conn)
                {
                    conn_obj->back_sqlite_edit_conn(std::move(edit_conn));
      ...
      public:
        ...
        std::shared_ptr<sqlite_conn_base> select_conn;
        std::shared_ptr<sqlite_conn_base> edit_conn;
        std::shared_ptr<orm_conn_pool> conn_obj;

        static constexpr DB_TYPE db_type = DB_TYPE::SQLITE;
    };
} /*tagnamespace_replace*/
}// namespace orm
#endif
