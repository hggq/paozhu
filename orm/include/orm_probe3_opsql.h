#ifndef _ORM_DEFAULT_ORM_PROBE3_OPERATE_H
#define _ORM_DEFAULT_ORM_PROBE3_OPERATE_H
/*
...
//{ /*tagnamespace_replace*/
    ...
    class orm_probe3_opsql : public B_BASE
    {
      public:
        orm_probe3_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        orm_probe3_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, orm_probe3_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            data_temp.tag.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 1:
        ...
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, orm_probe3_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.tag.assign(v.data(), v.size());
                    else
                        data_temp.tag.clear();
                }, val);
            } break;
        ...
        http::obj_val get_field_value(unsigned char index_pos, const orm_probe3_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
                return http::obj_val(std::string(data_temp.tag));
                break;
            case 1:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const orm_probe3_info::meta &data_temp)
    {
...
M_MODEL& eqTag(const std::string &val)
	{return where(B_BASE::cols::tag, orm::wq::eq, val);
	}

M_MODEL& nqTag(const std::string &val)
	{return where(B_BASE::cols::tag, orm::wq::nq, val);
	}

M_MODEL& btTag(const std::string &val)
	{return where(B_BASE::cols::tag, orm::wq::bt, val);
	}

M_MODEL& beTag(const std::string &val)
	{return where(B_BASE::cols::tag, orm::wq::be, val);
	}

M_MODEL& ltTag(const std::string &val)
	{return where(B_BASE::cols::tag, orm::wq::lt, val);
	}

M_MODEL& leTag(const std::string &val)
	{return where(B_BASE::cols::tag, orm::wq::le, val);
	}

M_MODEL& likeTag(const std::string &val)
	{return where(B_BASE::cols::tag, orm::wq::like, val);
	}

M_MODEL& nullTag()
	{return whereNull(B_BASE::cols::tag);
	}

M_MODEL& notnullTag()
	{return whereNotNull(B_BASE::cols::tag);
	}

M_MODEL& oreqTag(const std::string &val)
	{return whereOr(B_BASE::cols::tag, orm::wq::eq, val);
	}

M_MODEL& ornqTag(const std::string &val)
	{return whereOr(B_BASE::cols::tag, orm::wq::nq, val);
	}

M_MODEL& orbtTag(const std::string &val)
	{return whereOr(B_BASE::cols::tag, orm::wq::bt, val);
	}

M_MODEL& orbeTag(const std::string &val)
	{return whereOr(B_BASE::cols::tag, orm::wq::be, val);
	}

M_MODEL& orltTag(const std::string &val)
	{return whereOr(B_BASE::cols::tag, orm::wq::lt, val);
	}

M_MODEL& orleTag(const std::string &val)
	{return whereOr(B_BASE::cols::tag, orm::wq::le, val);
	}

M_MODEL& orlikeTag(const std::string &val)
	{return whereOr(B_BASE::cols::tag, orm::wq::like, val);
	}

M_MODEL& ornullTag()
	{return whereOrNull(B_BASE::cols::tag);
	}

M_MODEL& ornotnullTag()
	{return whereOrNotNull(B_BASE::cols::tag);
	}
...
M_MODEL& ornotnullV()
	{return whereOrNotNull(B_BASE::cols::v);
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = orm_probe3_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(orm_probe3_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(orm_probe3_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(orm_probe3_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(orm_probe3_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(orm_probe3_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm_probe3_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(orm_probe3_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm_probe3_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(orm_probe3_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(orm_probe3_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(orm_probe3_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(orm_probe3_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(orm_probe3_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(orm_probe3_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(orm_probe3_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(orm_probe3_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(orm_probe3_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(orm_probe3_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &order(orm_probe3_info::cols field, const std::string &asc_or_desc)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
            
			case orm_probe3_info::cols::tag:
				ordersql.append("tag");
				break;
			case orm_probe3_info::cols::v:
				ordersql.append("v");
				break;
            default:
                return *mod;
        ...
        M_MODEL &asc(orm_probe3_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &orm_probe3_info::col_names> field1, orm::table_col<B_BASE, &orm_probe3_info::col_names> field2)
        {
        ...
        M_MODEL &asc(orm_probe3_info::cols field, orm_probe3_info::cols field2)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(orm_probe3_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(orm_probe3_info::cols field, orm_probe3_info::cols field2)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &orm_probe3_info::col_names> field1, orm::table_col<B_BASE, &orm_probe3_info::col_names> field2)
        {
        ...
        M_MODEL &order(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq, const std::string &asc_or_desc)
        {
            ordersql.append(" ORDER BY ");
            ordersql.append(wq);
            ordersql.append(" ");
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm_probe3_info::cols field)
        {
            groupsql.append(" HAVING ");
            switch (field)
            {
        ...
        M_MODEL &group(orm_probe3_info::cols field)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &orm_probe3_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
            ...
            try
            {
                ...
                unsigned int fetch_count = select_conn->fetch_directly(sqlstring,
                                                                       [this, col_pos_map = std::vector<int>{}, first_row = true](int col_count, char **col_names, auto get_data) mutable -> bool
                                                                       {
                                                                           ...
                                                                           orm_probe3_info::meta data_temp;
                                                                           for (int ij = 0; ij < col_count; ij++)
                                                                           {
                                                                               auto [ptr, len] = get_data(ij);
                                                                               if (ptr == nullptr)
        ...
        {
            model_meta_cache<orm_probe3_info::meta> &data_cache = model_meta_cache<orm_probe3_info::meta>::getinstance();
            data_cache.remove_exptime();

            model_meta_cache<std::vector<orm_probe3_info::meta>> &record_cache = model_meta_cache<std::vector<orm_probe3_info::meta>>::getinstance();
        ...
        {
            model_meta_cache<orm_probe3_info::meta> &temp_cache = model_meta_cache<orm_probe3_info::meta>::getinstance();
        ...
        {
            model_meta_cache<std::vector<orm_probe3_info::meta>> &temp_cache = model_meta_cache<std::vector<orm_probe3_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(sqlstring);
        ...
        bool save_data_cache(const std::string &cache_key_name, const orm_probe3_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<orm_probe3_info::meta> &temp_cache = model_meta_cache<orm_probe3_info::meta>::getinstance();
        ...
        bool save_cache(std::size_t cache_key_name, const std::vector<orm_probe3_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<orm_probe3_info::meta>> &temp_cache = model_meta_cache<std::vector<orm_probe3_info::meta>>::getinstance();
            temp_cache.save(cache_key_name, cache_data, exp_time);
            return true;
        }
        bool save_cache(const std::string cache_key_name, const std::vector<orm_probe3_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<orm_probe3_info::meta>> &temp_cache = model_meta_cache<std::vector<orm_probe3_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_vector_cache(const std::string cache_key_name, const std::vector<orm_probe3_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<orm_probe3_info::meta>> &temp_cache = model_meta_cache<std::vector<orm_probe3_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_cache(const std::string cache_key_name, const orm_probe3_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<orm_probe3_info::meta> &temp_cache = model_meta_cache<orm_probe3_info::meta>::getinstance();
            std::size_t sqlhashid                               = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        const orm_probe3_info::meta get_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<orm_probe3_info::meta> &temp_cache = model_meta_cache<orm_probe3_info::meta>::getinstance();
        ...
        const std::vector<orm_probe3_info::meta> get_vector_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<std::vector<orm_probe3_info::meta>> &temp_cache = model_meta_cache<std::vector<orm_probe3_info::meta>>::getinstance();
        ...
        std::tuple<unsigned int, unsigned long long> insert(orm_probe3_info::meta &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(orm_probe3_info::meta &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_data_insert_sql(insert_data);
        ...
        std::tuple<unsigned int, unsigned long long> insert(std::vector<orm_probe3_info::meta> &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(std::vector<orm_probe3_info::meta> &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_vector_insert_sql(insert_data);
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = orm_probe3_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(orm_probe3_info::meta &insert_data)
        {
        ...
        M_MODEL &set_data(orm_probe3_info::meta indata)
        {
            B_BASE::data = indata;
            return *mod;
        }
        ...
        orm_probe3_opsql(const orm_probe3_opsql &)            = delete;
        orm_probe3_opsql &operator=(const orm_probe3_opsql &) = delete;
        orm_probe3_opsql(orm_probe3_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        orm_probe3_opsql &operator=(orm_probe3_opsql &&o) noexcept
        {
            if (this != &o)
            {
        ...
        template <typename T>
        M_MODEL &AND(orm::table_col<B_BASE, &orm_probe3_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = opwq;
        ...
        template <typename T>
        M_MODEL &OR(orm::table_col<B_BASE, &orm_probe3_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = opwq;
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < orm_probe3_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == orm_probe3_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(orm_probe3_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(orm_probe3_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = orm_probe3_info::col_names.size();
