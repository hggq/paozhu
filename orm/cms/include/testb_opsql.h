#ifndef _ORM_CMS_TESTB_OPERATE_H
#define _ORM_CMS_TESTB_OPERATE_H
/*
...
{ /*tagnamespace_replace*/
    ...
    class testb_opsql : public B_BASE
    {
      public:
        testb_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        testb_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, testb_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            break;
                case 5:
            data_temp.subprice.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                
        }
    }
    
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, testb_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.tid = static_cast<decltype(data_temp.tid )>(v);
            ...
            case 1: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.score = static_cast<decltype(data_temp.score )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.score = static_cast<decltype(data_temp.score )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.score = static_cast<decltype(data_temp.score )>(v);
            ...
            case 3: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.pricenum = static_cast<double>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.pricenum = static_cast<double>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.pricenum = v;
                    else
                        data_temp.pricenum = 0.0;
                }, val);
            } break;
            case 4: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.orgprice = static_cast<double>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.orgprice = static_cast<double>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.orgprice = v;
                    else
                        data_temp.orgprice = 0.0;
                }, val);
            } break;
            case 5: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.subprice.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.subprice.assign(v.data(), v.size());
                    else
                        data_temp.subprice.clear();
                }, val);
            } break;
        ...
        http::obj_val get_field_value(unsigned char index_pos, const testb_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
                return http::obj_val(static_cast<long long>(data_temp.tid));
                break;
            case 1:
                return http::obj_val(static_cast<long long>(data_temp.score));
                break;
            case 2:
                return http::obj_val(std::string(data_temp.name));
                break;
            case 3:
                return http::obj_val(static_cast<double>(data_temp.pricenum));
                break;
            case 4:
                return http::obj_val(static_cast<double>(data_temp.orgprice));
                break;
            case 5:
                return http::obj_val(std::string(data_temp.subprice));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const testb_info::meta &data_temp)
    {
        // Auto-increment primary key tid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.tid == 0)
            return http::obj_val(nullptr);
        return get_field_value(index_pos, data_temp);
    }
    

M_MODEL& eqTid(const std::string &val)
	{return where(B_BASE::cols::tid, orm::wq::eq, val);
	}

M_MODEL& nqTid(const std::string &val)
	{return where(B_BASE::cols::tid, orm::wq::nq, val);
	}

M_MODEL& btTid(const std::string &val)
	{return where(B_BASE::cols::tid, orm::wq::bt, val);
	}

M_MODEL& beTid(const std::string &val)
	{return where(B_BASE::cols::tid, orm::wq::be, val);
	}

M_MODEL& ltTid(const std::string &val)
	{return where(B_BASE::cols::tid, orm::wq::lt, val);
	}

M_MODEL& leTid(const std::string &val)
	{return where(B_BASE::cols::tid, orm::wq::le, val);
	}
...
M_MODEL& nullTid()
	{return whereNull(B_BASE::cols::tid);
	}

M_MODEL& notnullTid()
	{return whereNotNull(B_BASE::cols::tid);
	}

M_MODEL& oreqTid(const std::string &val)
	{return whereOr(B_BASE::cols::tid, orm::wq::eq, val);
	}

M_MODEL& ornqTid(const std::string &val)
	{return whereOr(B_BASE::cols::tid, orm::wq::nq, val);
	}

M_MODEL& orbtTid(const std::string &val)
	{return whereOr(B_BASE::cols::tid, orm::wq::bt, val);
	}

M_MODEL& orbeTid(const std::string &val)
	{return whereOr(B_BASE::cols::tid, orm::wq::be, val);
	}

M_MODEL& orltTid(const std::string &val)
	{return whereOr(B_BASE::cols::tid, orm::wq::lt, val);
	}

M_MODEL& orleTid(const std::string &val)
	{return whereOr(B_BASE::cols::tid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqTid(T val)
	{return whereOr(B_BASE::cols::tid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqTid(T val)
	{return whereOr(B_BASE::cols::tid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtTid(T val)
	{return whereOr(B_BASE::cols::tid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeTid(T val)
	{return whereOr(B_BASE::cols::tid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltTid(T val)
	{return whereOr(B_BASE::cols::tid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleTid(T val)
	{return whereOr(B_BASE::cols::tid, orm::wq::le, val);
	}

M_MODEL& ornullTid()
	{return whereOrNull(B_BASE::cols::tid);
	}

M_MODEL& ornotnullTid()
	{return whereOrNotNull(B_BASE::cols::tid);
	}

M_MODEL& eqScore(const std::string &val)
	{return where(B_BASE::cols::score, orm::wq::eq, val);
	}

M_MODEL& nqScore(const std::string &val)
	{return where(B_BASE::cols::score, orm::wq::nq, val);
	}

M_MODEL& btScore(const std::string &val)
	{return where(B_BASE::cols::score, orm::wq::bt, val);
	}

M_MODEL& beScore(const std::string &val)
	{return where(B_BASE::cols::score, orm::wq::be, val);
	}

M_MODEL& ltScore(const std::string &val)
	{return where(B_BASE::cols::score, orm::wq::lt, val);
	}

M_MODEL& leScore(const std::string &val)
	{return where(B_BASE::cols::score, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqScore(T val)
	{return where(B_BASE::cols::score, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqScore(T val)
	{return where(B_BASE::cols::score, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btScore(T val)
	{return where(B_BASE::cols::score, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beScore(T val)
	{return where(B_BASE::cols::score, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltScore(T val)
	{return where(B_BASE::cols::score, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leScore(T val)
	{return where(B_BASE::cols::score, orm::wq::le, val);
	}

M_MODEL& nullScore()
	{return whereNull(B_BASE::cols::score);
	}

M_MODEL& notnullScore()
	{return whereNotNull(B_BASE::cols::score);
	}

M_MODEL& oreqScore(const std::string &val)
	{return whereOr(B_BASE::cols::score, orm::wq::eq, val);
	}

M_MODEL& ornqScore(const std::string &val)
	{return whereOr(B_BASE::cols::score, orm::wq::nq, val);
	}

M_MODEL& orbtScore(const std::string &val)
	{return whereOr(B_BASE::cols::score, orm::wq::bt, val);
	}

M_MODEL& orbeScore(const std::string &val)
	{return whereOr(B_BASE::cols::score, orm::wq::be, val);
	}

M_MODEL& orltScore(const std::string &val)
	{return whereOr(B_BASE::cols::score, orm::wq::lt, val);
	}

M_MODEL& orleScore(const std::string &val)
	{return whereOr(B_BASE::cols::score, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqScore(T val)
	{return whereOr(B_BASE::cols::score, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqScore(T val)
	{return whereOr(B_BASE::cols::score, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtScore(T val)
	{return whereOr(B_BASE::cols::score, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeScore(T val)
	{return whereOr(B_BASE::cols::score, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltScore(T val)
	{return whereOr(B_BASE::cols::score, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleScore(T val)
	{return whereOr(B_BASE::cols::score, orm::wq::le, val);
	}

M_MODEL& ornullScore()
	{return whereOrNull(B_BASE::cols::score);
	}

M_MODEL& ornotnullScore()
	{return whereOrNotNull(B_BASE::cols::score);
	}
...
M_MODEL& eqPricenum(const std::string &val)
	{return where(B_BASE::cols::pricenum, orm::wq::eq, val);
	}

M_MODEL& nqPricenum(const std::string &val)
	{return where(B_BASE::cols::pricenum, orm::wq::nq, val);
	}

M_MODEL& btPricenum(const std::string &val)
	{return where(B_BASE::cols::pricenum, orm::wq::bt, val);
	}

M_MODEL& bePricenum(const std::string &val)
	{return where(B_BASE::cols::pricenum, orm::wq::be, val);
	}

M_MODEL& ltPricenum(const std::string &val)
	{return where(B_BASE::cols::pricenum, orm::wq::lt, val);
	}

M_MODEL& lePricenum(const std::string &val)
	{return where(B_BASE::cols::pricenum, orm::wq::le, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& eqPricenum(T val)
	{return where(B_BASE::cols::pricenum, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& nqPricenum(T val)
	{return where(B_BASE::cols::pricenum, orm::wq::nq, val);
	}
...
template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& lePricenum(T val)
	{return where(B_BASE::cols::pricenum, orm::wq::le, val);
	}

M_MODEL& nullPricenum()
	{return whereNull(B_BASE::cols::pricenum);
	}

M_MODEL& notnullPricenum()
	{return whereNotNull(B_BASE::cols::pricenum);
	}

M_MODEL& oreqPricenum(const std::string &val)
	{return whereOr(B_BASE::cols::pricenum, orm::wq::eq, val);
	}

M_MODEL& ornqPricenum(const std::string &val)
	{return whereOr(B_BASE::cols::pricenum, orm::wq::nq, val);
	}

M_MODEL& orbtPricenum(const std::string &val)
	{return whereOr(B_BASE::cols::pricenum, orm::wq::bt, val);
	}

M_MODEL& orbePricenum(const std::string &val)
	{return whereOr(B_BASE::cols::pricenum, orm::wq::be, val);
	}

M_MODEL& orltPricenum(const std::string &val)
	{return whereOr(B_BASE::cols::pricenum, orm::wq::lt, val);
	}

M_MODEL& orlePricenum(const std::string &val)
	{return whereOr(B_BASE::cols::pricenum, orm::wq::le, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& oreqPricenum(T val)
	{return whereOr(B_BASE::cols::pricenum, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& ornqPricenum(T val)
	{return whereOr(B_BASE::cols::pricenum, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& orbtPricenum(T val)
	{return whereOr(B_BASE::cols::pricenum, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& orbePricenum(T val)
	{return whereOr(B_BASE::cols::pricenum, orm::wq::be, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& orltPricenum(T val)
	{return whereOr(B_BASE::cols::pricenum, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& orlePricenum(T val)
	{return whereOr(B_BASE::cols::pricenum, orm::wq::le, val);
	}

M_MODEL& ornullPricenum()
	{return whereOrNull(B_BASE::cols::pricenum);
	}

M_MODEL& ornotnullPricenum()
	{return whereOrNotNull(B_BASE::cols::pricenum);
	}

M_MODEL& eqOrgprice(const std::string &val)
	{return where(B_BASE::cols::orgprice, orm::wq::eq, val);
	}

M_MODEL& nqOrgprice(const std::string &val)
	{return where(B_BASE::cols::orgprice, orm::wq::nq, val);
	}

M_MODEL& btOrgprice(const std::string &val)
	{return where(B_BASE::cols::orgprice, orm::wq::bt, val);
	}

M_MODEL& beOrgprice(const std::string &val)
	{return where(B_BASE::cols::orgprice, orm::wq::be, val);
	}

M_MODEL& ltOrgprice(const std::string &val)
	{return where(B_BASE::cols::orgprice, orm::wq::lt, val);
	}

M_MODEL& leOrgprice(const std::string &val)
	{return where(B_BASE::cols::orgprice, orm::wq::le, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& eqOrgprice(T val)
	{return where(B_BASE::cols::orgprice, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& nqOrgprice(T val)
	{return where(B_BASE::cols::orgprice, orm::wq::nq, val);
	}
...
template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& leOrgprice(T val)
	{return where(B_BASE::cols::orgprice, orm::wq::le, val);
	}

M_MODEL& nullOrgprice()
	{return whereNull(B_BASE::cols::orgprice);
	}

M_MODEL& notnullOrgprice()
	{return whereNotNull(B_BASE::cols::orgprice);
	}

M_MODEL& oreqOrgprice(const std::string &val)
	{return whereOr(B_BASE::cols::orgprice, orm::wq::eq, val);
	}

M_MODEL& ornqOrgprice(const std::string &val)
	{return whereOr(B_BASE::cols::orgprice, orm::wq::nq, val);
	}

M_MODEL& orbtOrgprice(const std::string &val)
	{return whereOr(B_BASE::cols::orgprice, orm::wq::bt, val);
	}

M_MODEL& orbeOrgprice(const std::string &val)
	{return whereOr(B_BASE::cols::orgprice, orm::wq::be, val);
	}

M_MODEL& orltOrgprice(const std::string &val)
	{return whereOr(B_BASE::cols::orgprice, orm::wq::lt, val);
	}

M_MODEL& orleOrgprice(const std::string &val)
	{return whereOr(B_BASE::cols::orgprice, orm::wq::le, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& oreqOrgprice(T val)
	{return whereOr(B_BASE::cols::orgprice, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& ornqOrgprice(T val)
	{return whereOr(B_BASE::cols::orgprice, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& orbtOrgprice(T val)
	{return whereOr(B_BASE::cols::orgprice, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& orbeOrgprice(T val)
	{return whereOr(B_BASE::cols::orgprice, orm::wq::be, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& orltOrgprice(T val)
	{return whereOr(B_BASE::cols::orgprice, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& orleOrgprice(T val)
	{return whereOr(B_BASE::cols::orgprice, orm::wq::le, val);
	}

M_MODEL& ornullOrgprice()
	{return whereOrNull(B_BASE::cols::orgprice);
	}

M_MODEL& ornotnullOrgprice()
	{return whereOrNotNull(B_BASE::cols::orgprice);
	}

M_MODEL& eqSubprice(const std::string &val)
	{return where(B_BASE::cols::subprice, orm::wq::eq, val);
	}

M_MODEL& nqSubprice(const std::string &val)
	{return where(B_BASE::cols::subprice, orm::wq::nq, val);
	}

M_MODEL& btSubprice(const std::string &val)
	{return where(B_BASE::cols::subprice, orm::wq::bt, val);
	}

M_MODEL& beSubprice(const std::string &val)
	{return where(B_BASE::cols::subprice, orm::wq::be, val);
	}

M_MODEL& ltSubprice(const std::string &val)
	{return where(B_BASE::cols::subprice, orm::wq::lt, val);
	}

M_MODEL& leSubprice(const std::string &val)
	{return where(B_BASE::cols::subprice, orm::wq::le, val);
	}

M_MODEL& likeSubprice(const std::string &val)
	{return where(B_BASE::cols::subprice, orm::wq::like, val);
	}

M_MODEL& nullSubprice()
	{return whereNull(B_BASE::cols::subprice);
	}

M_MODEL& notnullSubprice()
	{return whereNotNull(B_BASE::cols::subprice);
	}

M_MODEL& oreqSubprice(const std::string &val)
	{return whereOr(B_BASE::cols::subprice, orm::wq::eq, val);
	}

M_MODEL& ornqSubprice(const std::string &val)
	{return whereOr(B_BASE::cols::subprice, orm::wq::nq, val);
	}

M_MODEL& orbtSubprice(const std::string &val)
	{return whereOr(B_BASE::cols::subprice, orm::wq::bt, val);
	}

M_MODEL& orbeSubprice(const std::string &val)
	{return whereOr(B_BASE::cols::subprice, orm::wq::be, val);
	}

M_MODEL& orltSubprice(const std::string &val)
	{return whereOr(B_BASE::cols::subprice, orm::wq::lt, val);
	}

M_MODEL& orleSubprice(const std::string &val)
	{return whereOr(B_BASE::cols::subprice, orm::wq::le, val);
	}

M_MODEL& orlikeSubprice(const std::string &val)
	{return whereOr(B_BASE::cols::subprice, orm::wq::like, val);
	}

M_MODEL& ornullSubprice()
	{return whereOrNull(B_BASE::cols::subprice);
	}

M_MODEL& ornotnullSubprice()
	{return whereOrNotNull(B_BASE::cols::subprice);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = testb_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &testb_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &testb_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(testb_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(testb_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(testb_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(testb_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &testb_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &testb_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &testb_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &testb_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &testb_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(testb_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &testb_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(testb_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &testb_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(testb_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &testb_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(testb_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &testb_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(testb_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(testb_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(testb_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(testb_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(testb_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(testb_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(testb_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(testb_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(testb_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(testb_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &testb_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &testb_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &testb_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &testb_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &testb_info::col_names> wq)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &testb_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
        ...
        {
            model_meta_cache<testb_info::meta> &temp_cache = model_meta_cache<testb_info::meta>::getinstance();
        ...
        {
            model_meta_cache<std::vector<testb_info::meta>> &temp_cache = model_meta_cache<std::vector<testb_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(sqlstring);
        ...
        bool save_data_cache(const std::string &cache_key_name, const testb_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<testb_info::meta> &temp_cache = model_meta_cache<testb_info::meta>::getinstance();
        ...
        bool save_cache(std::size_t cache_key_name, const std::vector<testb_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<testb_info::meta>> &temp_cache = model_meta_cache<std::vector<testb_info::meta>>::getinstance();
            temp_cache.save(cache_key_name, cache_data, exp_time);
            return true;
        }
        bool save_cache(const std::string cache_key_name, const std::vector<testb_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<testb_info::meta>> &temp_cache = model_meta_cache<std::vector<testb_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_vector_cache(const std::string cache_key_name, const std::vector<testb_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<testb_info::meta>> &temp_cache = model_meta_cache<std::vector<testb_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_cache(const std::string cache_key_name, const testb_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<testb_info::meta> &temp_cache = model_meta_cache<testb_info::meta>::getinstance();
            std::size_t sqlhashid                               = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        const testb_info::meta get_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<testb_info::meta> &temp_cache = model_meta_cache<testb_info::meta>::getinstance();
        ...
        const std::vector<testb_info::meta> get_vector_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<std::vector<testb_info::meta>> &temp_cache = model_meta_cache<std::vector<testb_info::meta>>::getinstance();
        ...
        std::tuple<unsigned int, unsigned long long> insert(testb_info::meta &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(testb_info::meta &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_data_insert_sql(insert_data);
        ...
        std::tuple<unsigned int, unsigned long long> insert(std::vector<testb_info::meta> &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(std::vector<testb_info::meta> &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_vector_insert_sql(insert_data);
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = testb_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(testb_info::meta &insert_data)
        {
        ...
        M_MODEL &set_data(testb_info::meta indata)
        {
            B_BASE::data = indata;
            return *mod;
        }
        ...
        testb_opsql(const testb_opsql &)            = delete;
        testb_opsql &operator=(const testb_opsql &) = delete;
        testb_opsql(testb_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        testb_opsql &operator=(testb_opsql &&o) noexcept
        {
            if (this != &o)
            {
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < testb_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == testb_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(testb_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(testb_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = testb_info::col_names.size();
