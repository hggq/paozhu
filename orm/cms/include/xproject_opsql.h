#ifndef _ORM_CMS_XPROJECT_OPERATE_H
#define _ORM_CMS_XPROJECT_OPERATE_H
/*
...
{ /*tagnamespace_replace*/
    ...
    class xproject_opsql : public B_BASE
    {
      public:
        xproject_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        xproject_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, xproject_info::meta &data_temp)
    {
        switch(index_pos)
        {
            ...
            break;
                case 17:
            data_temp.giturl.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 18:
            data_temp.gitname.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 19:
            data_temp.gitpwd.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 20:
            data_temp.xcolor.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 21:
            data_temp.fupan.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 22:
                {
                    ...
                    try {
                        ...
                        if (value_size == 0 || *p == ' ' || *p == '\t' || *p == '\n' || *p == '\r') {
                            ...
                            if (idx > 0 && idx <= value_size) {
                                data_temp.totalvalue = static_cast<double>(parsed);
                            } else {
                                data_temp.totalvalue = 0.0;
                            }
                        }
        ...
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, xproject_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
            ...
            case 2: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.prexpjid = static_cast<decltype(data_temp.prexpjid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.prexpjid = static_cast<decltype(data_temp.prexpjid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.prexpjid = static_cast<decltype(data_temp.prexpjid )>(v);
            ...
            case 3: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.dpid = static_cast<decltype(data_temp.dpid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.dpid = static_cast<decltype(data_temp.dpid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.dpid = static_cast<decltype(data_temp.dpid )>(v);
            ...
            case 4: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.grouptype = static_cast<decltype(data_temp.grouptype )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.grouptype = static_cast<decltype(data_temp.grouptype )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.grouptype = static_cast<decltype(data_temp.grouptype )>(v);
            ...
            case 6: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.adminuserid = static_cast<decltype(data_temp.adminuserid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.adminuserid = static_cast<decltype(data_temp.adminuserid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.adminuserid = static_cast<decltype(data_temp.adminuserid )>(v);
            ...
            case 7: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.regdate = static_cast<decltype(data_temp.regdate )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.regdate = static_cast<decltype(data_temp.regdate )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.regdate = static_cast<decltype(data_temp.regdate )>(v);
            ...
            case 9: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.expiredate = static_cast<decltype(data_temp.expiredate )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.expiredate = static_cast<decltype(data_temp.expiredate )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.expiredate = static_cast<decltype(data_temp.expiredate )>(v);
            ...
            case 11: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.clientid = static_cast<decltype(data_temp.clientid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.clientid = static_cast<decltype(data_temp.clientid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.clientid = static_cast<decltype(data_temp.clientid )>(v);
            ...
            case 12: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.totalnum = static_cast<decltype(data_temp.totalnum )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.totalnum = static_cast<decltype(data_temp.totalnum )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.totalnum = static_cast<decltype(data_temp.totalnum )>(v);
            ...
            case 13: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.referdocverion = static_cast<decltype(data_temp.referdocverion )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.referdocverion = static_cast<decltype(data_temp.referdocverion )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.referdocverion = static_cast<decltype(data_temp.referdocverion )>(v);
            ...
            case 14: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.xtheme = static_cast<decltype(data_temp.xtheme )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.xtheme = static_cast<decltype(data_temp.xtheme )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.xtheme = static_cast<decltype(data_temp.xtheme )>(v);
            ...
            case 17: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.giturl.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.giturl.assign(v.data(), v.size());
                    else
                        data_temp.giturl.clear();
                }, val);
            } break;
            case 18: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.gitname.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.gitname.assign(v.data(), v.size());
                    else
                        data_temp.gitname.clear();
                }, val);
            } break;
            case 19: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.gitpwd.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.gitpwd.assign(v.data(), v.size());
                    else
                        data_temp.gitpwd.clear();
                }, val);
            } break;
            ...
            case 21: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.fupan.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.fupan.assign(v.data(), v.size());
                    else
                        data_temp.fupan.clear();
                }, val);
            } break;
            case 22: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.totalvalue = static_cast<double>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.totalvalue = static_cast<double>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.totalvalue = v;
                    else
                        data_temp.totalvalue = 0.0;
                }, val);
            } break;
        ...
        http::obj_val get_field_value(unsigned char index_pos, const xproject_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            case 1:
                ...
                break;
            case 2:
                return http::obj_val(static_cast<long long>(data_temp.prexpjid));
                break;
            case 3:
                return http::obj_val(static_cast<long long>(data_temp.dpid));
                break;
            case 4:
                return http::obj_val(static_cast<long long>(data_temp.grouptype));
                break;
            case 5:
                return http::obj_val(std::string(data_temp.title));
                break;
            case 6:
                return http::obj_val(static_cast<long long>(data_temp.adminuserid));
                break;
            case 7:
                return http::obj_val(static_cast<long long>(data_temp.regdate));
                break;
            case 8:
                return http::obj_val(static_cast<long long>(data_temp.begindate));
                break;
            case 9:
                return http::obj_val(static_cast<long long>(data_temp.expiredate));
                break;
            case 10:
                return http::obj_val(static_cast<long long>(data_temp.isopen));
                break;
            case 11:
                return http::obj_val(static_cast<long long>(data_temp.clientid));
                break;
            case 12:
                return http::obj_val(static_cast<long long>(data_temp.totalnum));
                break;
            case 13:
                return http::obj_val(static_cast<long long>(data_temp.referdocverion));
                break;
            case 14:
                return http::obj_val(static_cast<long long>(data_temp.xtheme));
                break;
            case 15:
            ...
            case 16:
                ...
                break;
            case 17:
                return http::obj_val(std::string(data_temp.giturl));
                break;
            case 18:
                return http::obj_val(std::string(data_temp.gitname));
                break;
            case 19:
                return http::obj_val(std::string(data_temp.gitpwd));
                break;
            case 20:
                return http::obj_val(std::string(data_temp.xcolor));
                break;
            case 21:
                return http::obj_val(std::string(data_temp.fupan));
                break;
            case 22:
                return http::obj_val(static_cast<double>(data_temp.totalvalue));
                break;
            case 23:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const xproject_info::meta &data_temp)
    {
        // Auto-increment primary key xpjid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.xpjid == 0)
            return http::obj_val(nullptr);
...
M_MODEL& eqPrexpjid(const std::string &val)
	{return where(B_BASE::cols::prexpjid, orm::wq::eq, val);
	}

M_MODEL& nqPrexpjid(const std::string &val)
	{return where(B_BASE::cols::prexpjid, orm::wq::nq, val);
	}

M_MODEL& btPrexpjid(const std::string &val)
	{return where(B_BASE::cols::prexpjid, orm::wq::bt, val);
	}

M_MODEL& bePrexpjid(const std::string &val)
	{return where(B_BASE::cols::prexpjid, orm::wq::be, val);
	}

M_MODEL& ltPrexpjid(const std::string &val)
	{return where(B_BASE::cols::prexpjid, orm::wq::lt, val);
	}

M_MODEL& lePrexpjid(const std::string &val)
	{return where(B_BASE::cols::prexpjid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqPrexpjid(T val)
	{return where(B_BASE::cols::prexpjid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqPrexpjid(T val)
	{return where(B_BASE::cols::prexpjid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btPrexpjid(T val)
	{return where(B_BASE::cols::prexpjid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& bePrexpjid(T val)
	{return where(B_BASE::cols::prexpjid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltPrexpjid(T val)
	{return where(B_BASE::cols::prexpjid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& lePrexpjid(T val)
	{return where(B_BASE::cols::prexpjid, orm::wq::le, val);
	}

M_MODEL& nullPrexpjid()
	{return whereNull(B_BASE::cols::prexpjid);
	}

M_MODEL& notnullPrexpjid()
	{return whereNotNull(B_BASE::cols::prexpjid);
	}

M_MODEL& oreqPrexpjid(const std::string &val)
	{return whereOr(B_BASE::cols::prexpjid, orm::wq::eq, val);
	}

M_MODEL& ornqPrexpjid(const std::string &val)
	{return whereOr(B_BASE::cols::prexpjid, orm::wq::nq, val);
	}

M_MODEL& orbtPrexpjid(const std::string &val)
	{return whereOr(B_BASE::cols::prexpjid, orm::wq::bt, val);
	}

M_MODEL& orbePrexpjid(const std::string &val)
	{return whereOr(B_BASE::cols::prexpjid, orm::wq::be, val);
	}

M_MODEL& orltPrexpjid(const std::string &val)
	{return whereOr(B_BASE::cols::prexpjid, orm::wq::lt, val);
	}

M_MODEL& orlePrexpjid(const std::string &val)
	{return whereOr(B_BASE::cols::prexpjid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqPrexpjid(T val)
	{return whereOr(B_BASE::cols::prexpjid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqPrexpjid(T val)
	{return whereOr(B_BASE::cols::prexpjid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtPrexpjid(T val)
	{return whereOr(B_BASE::cols::prexpjid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbePrexpjid(T val)
	{return whereOr(B_BASE::cols::prexpjid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltPrexpjid(T val)
	{return whereOr(B_BASE::cols::prexpjid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orlePrexpjid(T val)
	{return whereOr(B_BASE::cols::prexpjid, orm::wq::le, val);
	}

M_MODEL& ornullPrexpjid()
	{return whereOrNull(B_BASE::cols::prexpjid);
	}

M_MODEL& ornotnullPrexpjid()
	{return whereOrNotNull(B_BASE::cols::prexpjid);
	}

M_MODEL& eqDpid(const std::string &val)
	{return where(B_BASE::cols::dpid, orm::wq::eq, val);
	}

M_MODEL& nqDpid(const std::string &val)
	{return where(B_BASE::cols::dpid, orm::wq::nq, val);
	}

M_MODEL& btDpid(const std::string &val)
	{return where(B_BASE::cols::dpid, orm::wq::bt, val);
	}

M_MODEL& beDpid(const std::string &val)
	{return where(B_BASE::cols::dpid, orm::wq::be, val);
	}

M_MODEL& ltDpid(const std::string &val)
	{return where(B_BASE::cols::dpid, orm::wq::lt, val);
	}

M_MODEL& leDpid(const std::string &val)
	{return where(B_BASE::cols::dpid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqDpid(T val)
	{return where(B_BASE::cols::dpid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqDpid(T val)
	{return where(B_BASE::cols::dpid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btDpid(T val)
	{return where(B_BASE::cols::dpid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beDpid(T val)
	{return where(B_BASE::cols::dpid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltDpid(T val)
	{return where(B_BASE::cols::dpid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leDpid(T val)
	{return where(B_BASE::cols::dpid, orm::wq::le, val);
	}

M_MODEL& nullDpid()
	{return whereNull(B_BASE::cols::dpid);
	}

M_MODEL& notnullDpid()
	{return whereNotNull(B_BASE::cols::dpid);
	}

M_MODEL& oreqDpid(const std::string &val)
	{return whereOr(B_BASE::cols::dpid, orm::wq::eq, val);
	}

M_MODEL& ornqDpid(const std::string &val)
	{return whereOr(B_BASE::cols::dpid, orm::wq::nq, val);
	}

M_MODEL& orbtDpid(const std::string &val)
	{return whereOr(B_BASE::cols::dpid, orm::wq::bt, val);
	}

M_MODEL& orbeDpid(const std::string &val)
	{return whereOr(B_BASE::cols::dpid, orm::wq::be, val);
	}

M_MODEL& orltDpid(const std::string &val)
	{return whereOr(B_BASE::cols::dpid, orm::wq::lt, val);
	}

M_MODEL& orleDpid(const std::string &val)
	{return whereOr(B_BASE::cols::dpid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqDpid(T val)
	{return whereOr(B_BASE::cols::dpid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqDpid(T val)
	{return whereOr(B_BASE::cols::dpid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtDpid(T val)
	{return whereOr(B_BASE::cols::dpid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeDpid(T val)
	{return whereOr(B_BASE::cols::dpid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltDpid(T val)
	{return whereOr(B_BASE::cols::dpid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleDpid(T val)
	{return whereOr(B_BASE::cols::dpid, orm::wq::le, val);
	}

M_MODEL& ornullDpid()
	{return whereOrNull(B_BASE::cols::dpid);
	}

M_MODEL& ornotnullDpid()
	{return whereOrNotNull(B_BASE::cols::dpid);
	}

M_MODEL& eqGrouptype(const std::string &val)
	{return where(B_BASE::cols::grouptype, orm::wq::eq, val);
	}

M_MODEL& nqGrouptype(const std::string &val)
	{return where(B_BASE::cols::grouptype, orm::wq::nq, val);
	}

M_MODEL& btGrouptype(const std::string &val)
	{return where(B_BASE::cols::grouptype, orm::wq::bt, val);
	}

M_MODEL& beGrouptype(const std::string &val)
	{return where(B_BASE::cols::grouptype, orm::wq::be, val);
	}

M_MODEL& ltGrouptype(const std::string &val)
	{return where(B_BASE::cols::grouptype, orm::wq::lt, val);
	}

M_MODEL& leGrouptype(const std::string &val)
	{return where(B_BASE::cols::grouptype, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqGrouptype(T val)
	{return where(B_BASE::cols::grouptype, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqGrouptype(T val)
	{return where(B_BASE::cols::grouptype, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btGrouptype(T val)
	{return where(B_BASE::cols::grouptype, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beGrouptype(T val)
	{return where(B_BASE::cols::grouptype, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltGrouptype(T val)
	{return where(B_BASE::cols::grouptype, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leGrouptype(T val)
	{return where(B_BASE::cols::grouptype, orm::wq::le, val);
	}

M_MODEL& nullGrouptype()
	{return whereNull(B_BASE::cols::grouptype);
	}

M_MODEL& notnullGrouptype()
	{return whereNotNull(B_BASE::cols::grouptype);
	}

M_MODEL& oreqGrouptype(const std::string &val)
	{return whereOr(B_BASE::cols::grouptype, orm::wq::eq, val);
	}

M_MODEL& ornqGrouptype(const std::string &val)
	{return whereOr(B_BASE::cols::grouptype, orm::wq::nq, val);
	}

M_MODEL& orbtGrouptype(const std::string &val)
	{return whereOr(B_BASE::cols::grouptype, orm::wq::bt, val);
	}

M_MODEL& orbeGrouptype(const std::string &val)
	{return whereOr(B_BASE::cols::grouptype, orm::wq::be, val);
	}

M_MODEL& orltGrouptype(const std::string &val)
	{return whereOr(B_BASE::cols::grouptype, orm::wq::lt, val);
	}

M_MODEL& orleGrouptype(const std::string &val)
	{return whereOr(B_BASE::cols::grouptype, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqGrouptype(T val)
	{return whereOr(B_BASE::cols::grouptype, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqGrouptype(T val)
	{return whereOr(B_BASE::cols::grouptype, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtGrouptype(T val)
	{return whereOr(B_BASE::cols::grouptype, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeGrouptype(T val)
	{return whereOr(B_BASE::cols::grouptype, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltGrouptype(T val)
	{return whereOr(B_BASE::cols::grouptype, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleGrouptype(T val)
	{return whereOr(B_BASE::cols::grouptype, orm::wq::le, val);
	}

M_MODEL& ornullGrouptype()
	{return whereOrNull(B_BASE::cols::grouptype);
	}

M_MODEL& ornotnullGrouptype()
	{return whereOrNotNull(B_BASE::cols::grouptype);
	}
...
M_MODEL& eqAdminuserid(const std::string &val)
	{return where(B_BASE::cols::adminuserid, orm::wq::eq, val);
	}

M_MODEL& nqAdminuserid(const std::string &val)
	{return where(B_BASE::cols::adminuserid, orm::wq::nq, val);
	}

M_MODEL& btAdminuserid(const std::string &val)
	{return where(B_BASE::cols::adminuserid, orm::wq::bt, val);
	}

M_MODEL& beAdminuserid(const std::string &val)
	{return where(B_BASE::cols::adminuserid, orm::wq::be, val);
	}

M_MODEL& ltAdminuserid(const std::string &val)
	{return where(B_BASE::cols::adminuserid, orm::wq::lt, val);
	}

M_MODEL& leAdminuserid(const std::string &val)
	{return where(B_BASE::cols::adminuserid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqAdminuserid(T val)
	{return where(B_BASE::cols::adminuserid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqAdminuserid(T val)
	{return where(B_BASE::cols::adminuserid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btAdminuserid(T val)
	{return where(B_BASE::cols::adminuserid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beAdminuserid(T val)
	{return where(B_BASE::cols::adminuserid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltAdminuserid(T val)
	{return where(B_BASE::cols::adminuserid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leAdminuserid(T val)
	{return where(B_BASE::cols::adminuserid, orm::wq::le, val);
	}

M_MODEL& nullAdminuserid()
	{return whereNull(B_BASE::cols::adminuserid);
	}

M_MODEL& notnullAdminuserid()
	{return whereNotNull(B_BASE::cols::adminuserid);
	}

M_MODEL& oreqAdminuserid(const std::string &val)
	{return whereOr(B_BASE::cols::adminuserid, orm::wq::eq, val);
	}

M_MODEL& ornqAdminuserid(const std::string &val)
	{return whereOr(B_BASE::cols::adminuserid, orm::wq::nq, val);
	}

M_MODEL& orbtAdminuserid(const std::string &val)
	{return whereOr(B_BASE::cols::adminuserid, orm::wq::bt, val);
	}

M_MODEL& orbeAdminuserid(const std::string &val)
	{return whereOr(B_BASE::cols::adminuserid, orm::wq::be, val);
	}

M_MODEL& orltAdminuserid(const std::string &val)
	{return whereOr(B_BASE::cols::adminuserid, orm::wq::lt, val);
	}

M_MODEL& orleAdminuserid(const std::string &val)
	{return whereOr(B_BASE::cols::adminuserid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqAdminuserid(T val)
	{return whereOr(B_BASE::cols::adminuserid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqAdminuserid(T val)
	{return whereOr(B_BASE::cols::adminuserid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtAdminuserid(T val)
	{return whereOr(B_BASE::cols::adminuserid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeAdminuserid(T val)
	{return whereOr(B_BASE::cols::adminuserid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltAdminuserid(T val)
	{return whereOr(B_BASE::cols::adminuserid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleAdminuserid(T val)
	{return whereOr(B_BASE::cols::adminuserid, orm::wq::le, val);
	}

M_MODEL& ornullAdminuserid()
	{return whereOrNull(B_BASE::cols::adminuserid);
	}

M_MODEL& ornotnullAdminuserid()
	{return whereOrNotNull(B_BASE::cols::adminuserid);
	}

M_MODEL& eqRegdate(const std::string &val)
	{return where(B_BASE::cols::regdate, orm::wq::eq, val);
	}

M_MODEL& nqRegdate(const std::string &val)
	{return where(B_BASE::cols::regdate, orm::wq::nq, val);
	}

M_MODEL& btRegdate(const std::string &val)
	{return where(B_BASE::cols::regdate, orm::wq::bt, val);
	}

M_MODEL& beRegdate(const std::string &val)
	{return where(B_BASE::cols::regdate, orm::wq::be, val);
	}

M_MODEL& ltRegdate(const std::string &val)
	{return where(B_BASE::cols::regdate, orm::wq::lt, val);
	}

M_MODEL& leRegdate(const std::string &val)
	{return where(B_BASE::cols::regdate, orm::wq::le, val);
	}
...
M_MODEL& nullRegdate()
	{return whereNull(B_BASE::cols::regdate);
	}

M_MODEL& notnullRegdate()
	{return whereNotNull(B_BASE::cols::regdate);
	}

M_MODEL& oreqRegdate(const std::string &val)
	{return whereOr(B_BASE::cols::regdate, orm::wq::eq, val);
	}

M_MODEL& ornqRegdate(const std::string &val)
	{return whereOr(B_BASE::cols::regdate, orm::wq::nq, val);
	}

M_MODEL& orbtRegdate(const std::string &val)
	{return whereOr(B_BASE::cols::regdate, orm::wq::bt, val);
	}

M_MODEL& orbeRegdate(const std::string &val)
	{return whereOr(B_BASE::cols::regdate, orm::wq::be, val);
	}

M_MODEL& orltRegdate(const std::string &val)
	{return whereOr(B_BASE::cols::regdate, orm::wq::lt, val);
	}

M_MODEL& orleRegdate(const std::string &val)
	{return whereOr(B_BASE::cols::regdate, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqRegdate(T val)
	{return whereOr(B_BASE::cols::regdate, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqRegdate(T val)
	{return whereOr(B_BASE::cols::regdate, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtRegdate(T val)
	{return whereOr(B_BASE::cols::regdate, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeRegdate(T val)
	{return whereOr(B_BASE::cols::regdate, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltRegdate(T val)
	{return whereOr(B_BASE::cols::regdate, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleRegdate(T val)
	{return whereOr(B_BASE::cols::regdate, orm::wq::le, val);
	}

M_MODEL& ornullRegdate()
	{return whereOrNull(B_BASE::cols::regdate);
	}

M_MODEL& ornotnullRegdate()
	{return whereOrNotNull(B_BASE::cols::regdate);
	}
...
M_MODEL& eqExpiredate(const std::string &val)
	{return where(B_BASE::cols::expiredate, orm::wq::eq, val);
	}

M_MODEL& nqExpiredate(const std::string &val)
	{return where(B_BASE::cols::expiredate, orm::wq::nq, val);
	}

M_MODEL& btExpiredate(const std::string &val)
	{return where(B_BASE::cols::expiredate, orm::wq::bt, val);
	}

M_MODEL& beExpiredate(const std::string &val)
	{return where(B_BASE::cols::expiredate, orm::wq::be, val);
	}

M_MODEL& ltExpiredate(const std::string &val)
	{return where(B_BASE::cols::expiredate, orm::wq::lt, val);
	}

M_MODEL& leExpiredate(const std::string &val)
	{return where(B_BASE::cols::expiredate, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqExpiredate(T val)
	{return where(B_BASE::cols::expiredate, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqExpiredate(T val)
	{return where(B_BASE::cols::expiredate, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btExpiredate(T val)
	{return where(B_BASE::cols::expiredate, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beExpiredate(T val)
	{return where(B_BASE::cols::expiredate, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltExpiredate(T val)
	{return where(B_BASE::cols::expiredate, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leExpiredate(T val)
	{return where(B_BASE::cols::expiredate, orm::wq::le, val);
	}

M_MODEL& nullExpiredate()
	{return whereNull(B_BASE::cols::expiredate);
	}

M_MODEL& notnullExpiredate()
	{return whereNotNull(B_BASE::cols::expiredate);
	}

M_MODEL& oreqExpiredate(const std::string &val)
	{return whereOr(B_BASE::cols::expiredate, orm::wq::eq, val);
	}

M_MODEL& ornqExpiredate(const std::string &val)
	{return whereOr(B_BASE::cols::expiredate, orm::wq::nq, val);
	}

M_MODEL& orbtExpiredate(const std::string &val)
	{return whereOr(B_BASE::cols::expiredate, orm::wq::bt, val);
	}

M_MODEL& orbeExpiredate(const std::string &val)
	{return whereOr(B_BASE::cols::expiredate, orm::wq::be, val);
	}

M_MODEL& orltExpiredate(const std::string &val)
	{return whereOr(B_BASE::cols::expiredate, orm::wq::lt, val);
	}

M_MODEL& orleExpiredate(const std::string &val)
	{return whereOr(B_BASE::cols::expiredate, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqExpiredate(T val)
	{return whereOr(B_BASE::cols::expiredate, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqExpiredate(T val)
	{return whereOr(B_BASE::cols::expiredate, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtExpiredate(T val)
	{return whereOr(B_BASE::cols::expiredate, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeExpiredate(T val)
	{return whereOr(B_BASE::cols::expiredate, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltExpiredate(T val)
	{return whereOr(B_BASE::cols::expiredate, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleExpiredate(T val)
	{return whereOr(B_BASE::cols::expiredate, orm::wq::le, val);
	}

M_MODEL& ornullExpiredate()
	{return whereOrNull(B_BASE::cols::expiredate);
	}

M_MODEL& ornotnullExpiredate()
	{return whereOrNotNull(B_BASE::cols::expiredate);
	}
...
M_MODEL& eqClientid(const std::string &val)
	{return where(B_BASE::cols::clientid, orm::wq::eq, val);
	}

M_MODEL& nqClientid(const std::string &val)
	{return where(B_BASE::cols::clientid, orm::wq::nq, val);
	}

M_MODEL& btClientid(const std::string &val)
	{return where(B_BASE::cols::clientid, orm::wq::bt, val);
	}

M_MODEL& beClientid(const std::string &val)
	{return where(B_BASE::cols::clientid, orm::wq::be, val);
	}

M_MODEL& ltClientid(const std::string &val)
	{return where(B_BASE::cols::clientid, orm::wq::lt, val);
	}

M_MODEL& leClientid(const std::string &val)
	{return where(B_BASE::cols::clientid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqClientid(T val)
	{return where(B_BASE::cols::clientid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqClientid(T val)
	{return where(B_BASE::cols::clientid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btClientid(T val)
	{return where(B_BASE::cols::clientid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beClientid(T val)
	{return where(B_BASE::cols::clientid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltClientid(T val)
	{return where(B_BASE::cols::clientid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leClientid(T val)
	{return where(B_BASE::cols::clientid, orm::wq::le, val);
	}

M_MODEL& nullClientid()
	{return whereNull(B_BASE::cols::clientid);
	}

M_MODEL& notnullClientid()
	{return whereNotNull(B_BASE::cols::clientid);
	}

M_MODEL& oreqClientid(const std::string &val)
	{return whereOr(B_BASE::cols::clientid, orm::wq::eq, val);
	}

M_MODEL& ornqClientid(const std::string &val)
	{return whereOr(B_BASE::cols::clientid, orm::wq::nq, val);
	}

M_MODEL& orbtClientid(const std::string &val)
	{return whereOr(B_BASE::cols::clientid, orm::wq::bt, val);
	}

M_MODEL& orbeClientid(const std::string &val)
	{return whereOr(B_BASE::cols::clientid, orm::wq::be, val);
	}

M_MODEL& orltClientid(const std::string &val)
	{return whereOr(B_BASE::cols::clientid, orm::wq::lt, val);
	}

M_MODEL& orleClientid(const std::string &val)
	{return whereOr(B_BASE::cols::clientid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqClientid(T val)
	{return whereOr(B_BASE::cols::clientid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqClientid(T val)
	{return whereOr(B_BASE::cols::clientid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtClientid(T val)
	{return whereOr(B_BASE::cols::clientid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeClientid(T val)
	{return whereOr(B_BASE::cols::clientid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltClientid(T val)
	{return whereOr(B_BASE::cols::clientid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleClientid(T val)
	{return whereOr(B_BASE::cols::clientid, orm::wq::le, val);
	}

M_MODEL& ornullClientid()
	{return whereOrNull(B_BASE::cols::clientid);
	}

M_MODEL& ornotnullClientid()
	{return whereOrNotNull(B_BASE::cols::clientid);
	}

M_MODEL& eqTotalnum(const std::string &val)
	{return where(B_BASE::cols::totalnum, orm::wq::eq, val);
	}

M_MODEL& nqTotalnum(const std::string &val)
	{return where(B_BASE::cols::totalnum, orm::wq::nq, val);
	}

M_MODEL& btTotalnum(const std::string &val)
	{return where(B_BASE::cols::totalnum, orm::wq::bt, val);
	}

M_MODEL& beTotalnum(const std::string &val)
	{return where(B_BASE::cols::totalnum, orm::wq::be, val);
	}

M_MODEL& ltTotalnum(const std::string &val)
	{return where(B_BASE::cols::totalnum, orm::wq::lt, val);
	}

M_MODEL& leTotalnum(const std::string &val)
	{return where(B_BASE::cols::totalnum, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqTotalnum(T val)
	{return where(B_BASE::cols::totalnum, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqTotalnum(T val)
	{return where(B_BASE::cols::totalnum, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btTotalnum(T val)
	{return where(B_BASE::cols::totalnum, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beTotalnum(T val)
	{return where(B_BASE::cols::totalnum, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltTotalnum(T val)
	{return where(B_BASE::cols::totalnum, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leTotalnum(T val)
	{return where(B_BASE::cols::totalnum, orm::wq::le, val);
	}

M_MODEL& nullTotalnum()
	{return whereNull(B_BASE::cols::totalnum);
	}

M_MODEL& notnullTotalnum()
	{return whereNotNull(B_BASE::cols::totalnum);
	}

M_MODEL& oreqTotalnum(const std::string &val)
	{return whereOr(B_BASE::cols::totalnum, orm::wq::eq, val);
	}

M_MODEL& ornqTotalnum(const std::string &val)
	{return whereOr(B_BASE::cols::totalnum, orm::wq::nq, val);
	}

M_MODEL& orbtTotalnum(const std::string &val)
	{return whereOr(B_BASE::cols::totalnum, orm::wq::bt, val);
	}

M_MODEL& orbeTotalnum(const std::string &val)
	{return whereOr(B_BASE::cols::totalnum, orm::wq::be, val);
	}

M_MODEL& orltTotalnum(const std::string &val)
	{return whereOr(B_BASE::cols::totalnum, orm::wq::lt, val);
	}

M_MODEL& orleTotalnum(const std::string &val)
	{return whereOr(B_BASE::cols::totalnum, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqTotalnum(T val)
	{return whereOr(B_BASE::cols::totalnum, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqTotalnum(T val)
	{return whereOr(B_BASE::cols::totalnum, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtTotalnum(T val)
	{return whereOr(B_BASE::cols::totalnum, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeTotalnum(T val)
	{return whereOr(B_BASE::cols::totalnum, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltTotalnum(T val)
	{return whereOr(B_BASE::cols::totalnum, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleTotalnum(T val)
	{return whereOr(B_BASE::cols::totalnum, orm::wq::le, val);
	}

M_MODEL& ornullTotalnum()
	{return whereOrNull(B_BASE::cols::totalnum);
	}

M_MODEL& ornotnullTotalnum()
	{return whereOrNotNull(B_BASE::cols::totalnum);
	}

M_MODEL& eqReferdocverion(const std::string &val)
	{return where(B_BASE::cols::referdocverion, orm::wq::eq, val);
	}

M_MODEL& nqReferdocverion(const std::string &val)
	{return where(B_BASE::cols::referdocverion, orm::wq::nq, val);
	}

M_MODEL& btReferdocverion(const std::string &val)
	{return where(B_BASE::cols::referdocverion, orm::wq::bt, val);
	}

M_MODEL& beReferdocverion(const std::string &val)
	{return where(B_BASE::cols::referdocverion, orm::wq::be, val);
	}

M_MODEL& ltReferdocverion(const std::string &val)
	{return where(B_BASE::cols::referdocverion, orm::wq::lt, val);
	}

M_MODEL& leReferdocverion(const std::string &val)
	{return where(B_BASE::cols::referdocverion, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqReferdocverion(T val)
	{return where(B_BASE::cols::referdocverion, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqReferdocverion(T val)
	{return where(B_BASE::cols::referdocverion, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btReferdocverion(T val)
	{return where(B_BASE::cols::referdocverion, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beReferdocverion(T val)
	{return where(B_BASE::cols::referdocverion, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltReferdocverion(T val)
	{return where(B_BASE::cols::referdocverion, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leReferdocverion(T val)
	{return where(B_BASE::cols::referdocverion, orm::wq::le, val);
	}

M_MODEL& nullReferdocverion()
	{return whereNull(B_BASE::cols::referdocverion);
	}

M_MODEL& notnullReferdocverion()
	{return whereNotNull(B_BASE::cols::referdocverion);
	}

M_MODEL& oreqReferdocverion(const std::string &val)
	{return whereOr(B_BASE::cols::referdocverion, orm::wq::eq, val);
	}

M_MODEL& ornqReferdocverion(const std::string &val)
	{return whereOr(B_BASE::cols::referdocverion, orm::wq::nq, val);
	}

M_MODEL& orbtReferdocverion(const std::string &val)
	{return whereOr(B_BASE::cols::referdocverion, orm::wq::bt, val);
	}

M_MODEL& orbeReferdocverion(const std::string &val)
	{return whereOr(B_BASE::cols::referdocverion, orm::wq::be, val);
	}

M_MODEL& orltReferdocverion(const std::string &val)
	{return whereOr(B_BASE::cols::referdocverion, orm::wq::lt, val);
	}

M_MODEL& orleReferdocverion(const std::string &val)
	{return whereOr(B_BASE::cols::referdocverion, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqReferdocverion(T val)
	{return whereOr(B_BASE::cols::referdocverion, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqReferdocverion(T val)
	{return whereOr(B_BASE::cols::referdocverion, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtReferdocverion(T val)
	{return whereOr(B_BASE::cols::referdocverion, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeReferdocverion(T val)
	{return whereOr(B_BASE::cols::referdocverion, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltReferdocverion(T val)
	{return whereOr(B_BASE::cols::referdocverion, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleReferdocverion(T val)
	{return whereOr(B_BASE::cols::referdocverion, orm::wq::le, val);
	}

M_MODEL& ornullReferdocverion()
	{return whereOrNull(B_BASE::cols::referdocverion);
	}

M_MODEL& ornotnullReferdocverion()
	{return whereOrNotNull(B_BASE::cols::referdocverion);
	}

M_MODEL& eqXtheme(const std::string &val)
	{return where(B_BASE::cols::xtheme, orm::wq::eq, val);
	}

M_MODEL& nqXtheme(const std::string &val)
	{return where(B_BASE::cols::xtheme, orm::wq::nq, val);
	}

M_MODEL& btXtheme(const std::string &val)
	{return where(B_BASE::cols::xtheme, orm::wq::bt, val);
	}

M_MODEL& beXtheme(const std::string &val)
	{return where(B_BASE::cols::xtheme, orm::wq::be, val);
	}

M_MODEL& ltXtheme(const std::string &val)
	{return where(B_BASE::cols::xtheme, orm::wq::lt, val);
	}

M_MODEL& leXtheme(const std::string &val)
	{return where(B_BASE::cols::xtheme, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqXtheme(T val)
	{return where(B_BASE::cols::xtheme, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqXtheme(T val)
	{return where(B_BASE::cols::xtheme, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btXtheme(T val)
	{return where(B_BASE::cols::xtheme, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beXtheme(T val)
	{return where(B_BASE::cols::xtheme, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltXtheme(T val)
	{return where(B_BASE::cols::xtheme, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leXtheme(T val)
	{return where(B_BASE::cols::xtheme, orm::wq::le, val);
	}

M_MODEL& nullXtheme()
	{return whereNull(B_BASE::cols::xtheme);
	}

M_MODEL& notnullXtheme()
	{return whereNotNull(B_BASE::cols::xtheme);
	}

M_MODEL& oreqXtheme(const std::string &val)
	{return whereOr(B_BASE::cols::xtheme, orm::wq::eq, val);
	}

M_MODEL& ornqXtheme(const std::string &val)
	{return whereOr(B_BASE::cols::xtheme, orm::wq::nq, val);
	}

M_MODEL& orbtXtheme(const std::string &val)
	{return whereOr(B_BASE::cols::xtheme, orm::wq::bt, val);
	}

M_MODEL& orbeXtheme(const std::string &val)
	{return whereOr(B_BASE::cols::xtheme, orm::wq::be, val);
	}

M_MODEL& orltXtheme(const std::string &val)
	{return whereOr(B_BASE::cols::xtheme, orm::wq::lt, val);
	}

M_MODEL& orleXtheme(const std::string &val)
	{return whereOr(B_BASE::cols::xtheme, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqXtheme(T val)
	{return whereOr(B_BASE::cols::xtheme, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqXtheme(T val)
	{return whereOr(B_BASE::cols::xtheme, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtXtheme(T val)
	{return whereOr(B_BASE::cols::xtheme, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeXtheme(T val)
	{return whereOr(B_BASE::cols::xtheme, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltXtheme(T val)
	{return whereOr(B_BASE::cols::xtheme, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleXtheme(T val)
	{return whereOr(B_BASE::cols::xtheme, orm::wq::le, val);
	}

M_MODEL& ornullXtheme()
	{return whereOrNull(B_BASE::cols::xtheme);
	}

M_MODEL& ornotnullXtheme()
	{return whereOrNotNull(B_BASE::cols::xtheme);
	}
...
M_MODEL& eqGiturl(const std::string &val)
	{return where(B_BASE::cols::giturl, orm::wq::eq, val);
	}

M_MODEL& nqGiturl(const std::string &val)
	{return where(B_BASE::cols::giturl, orm::wq::nq, val);
	}

M_MODEL& btGiturl(const std::string &val)
	{return where(B_BASE::cols::giturl, orm::wq::bt, val);
	}

M_MODEL& beGiturl(const std::string &val)
	{return where(B_BASE::cols::giturl, orm::wq::be, val);
	}

M_MODEL& ltGiturl(const std::string &val)
	{return where(B_BASE::cols::giturl, orm::wq::lt, val);
	}

M_MODEL& leGiturl(const std::string &val)
	{return where(B_BASE::cols::giturl, orm::wq::le, val);
	}

M_MODEL& likeGiturl(const std::string &val)
	{return where(B_BASE::cols::giturl, orm::wq::like, val);
	}

M_MODEL& nullGiturl()
	{return whereNull(B_BASE::cols::giturl);
	}

M_MODEL& notnullGiturl()
	{return whereNotNull(B_BASE::cols::giturl);
	}

M_MODEL& oreqGiturl(const std::string &val)
	{return whereOr(B_BASE::cols::giturl, orm::wq::eq, val);
	}

M_MODEL& ornqGiturl(const std::string &val)
	{return whereOr(B_BASE::cols::giturl, orm::wq::nq, val);
	}

M_MODEL& orbtGiturl(const std::string &val)
	{return whereOr(B_BASE::cols::giturl, orm::wq::bt, val);
	}

M_MODEL& orbeGiturl(const std::string &val)
	{return whereOr(B_BASE::cols::giturl, orm::wq::be, val);
	}

M_MODEL& orltGiturl(const std::string &val)
	{return whereOr(B_BASE::cols::giturl, orm::wq::lt, val);
	}

M_MODEL& orleGiturl(const std::string &val)
	{return whereOr(B_BASE::cols::giturl, orm::wq::le, val);
	}

M_MODEL& orlikeGiturl(const std::string &val)
	{return whereOr(B_BASE::cols::giturl, orm::wq::like, val);
	}

M_MODEL& ornullGiturl()
	{return whereOrNull(B_BASE::cols::giturl);
	}

M_MODEL& ornotnullGiturl()
	{return whereOrNotNull(B_BASE::cols::giturl);
	}

M_MODEL& eqGitname(const std::string &val)
	{return where(B_BASE::cols::gitname, orm::wq::eq, val);
	}

M_MODEL& nqGitname(const std::string &val)
	{return where(B_BASE::cols::gitname, orm::wq::nq, val);
	}

M_MODEL& btGitname(const std::string &val)
	{return where(B_BASE::cols::gitname, orm::wq::bt, val);
	}

M_MODEL& beGitname(const std::string &val)
	{return where(B_BASE::cols::gitname, orm::wq::be, val);
	}

M_MODEL& ltGitname(const std::string &val)
	{return where(B_BASE::cols::gitname, orm::wq::lt, val);
	}

M_MODEL& leGitname(const std::string &val)
	{return where(B_BASE::cols::gitname, orm::wq::le, val);
	}

M_MODEL& likeGitname(const std::string &val)
	{return where(B_BASE::cols::gitname, orm::wq::like, val);
	}

M_MODEL& nullGitname()
	{return whereNull(B_BASE::cols::gitname);
	}

M_MODEL& notnullGitname()
	{return whereNotNull(B_BASE::cols::gitname);
	}

M_MODEL& oreqGitname(const std::string &val)
	{return whereOr(B_BASE::cols::gitname, orm::wq::eq, val);
	}

M_MODEL& ornqGitname(const std::string &val)
	{return whereOr(B_BASE::cols::gitname, orm::wq::nq, val);
	}

M_MODEL& orbtGitname(const std::string &val)
	{return whereOr(B_BASE::cols::gitname, orm::wq::bt, val);
	}

M_MODEL& orbeGitname(const std::string &val)
	{return whereOr(B_BASE::cols::gitname, orm::wq::be, val);
	}

M_MODEL& orltGitname(const std::string &val)
	{return whereOr(B_BASE::cols::gitname, orm::wq::lt, val);
	}

M_MODEL& orleGitname(const std::string &val)
	{return whereOr(B_BASE::cols::gitname, orm::wq::le, val);
	}

M_MODEL& orlikeGitname(const std::string &val)
	{return whereOr(B_BASE::cols::gitname, orm::wq::like, val);
	}

M_MODEL& ornullGitname()
	{return whereOrNull(B_BASE::cols::gitname);
	}

M_MODEL& ornotnullGitname()
	{return whereOrNotNull(B_BASE::cols::gitname);
	}

M_MODEL& eqGitpwd(const std::string &val)
	{return where(B_BASE::cols::gitpwd, orm::wq::eq, val);
	}

M_MODEL& nqGitpwd(const std::string &val)
	{return where(B_BASE::cols::gitpwd, orm::wq::nq, val);
	}

M_MODEL& btGitpwd(const std::string &val)
	{return where(B_BASE::cols::gitpwd, orm::wq::bt, val);
	}

M_MODEL& beGitpwd(const std::string &val)
	{return where(B_BASE::cols::gitpwd, orm::wq::be, val);
	}

M_MODEL& ltGitpwd(const std::string &val)
	{return where(B_BASE::cols::gitpwd, orm::wq::lt, val);
	}

M_MODEL& leGitpwd(const std::string &val)
	{return where(B_BASE::cols::gitpwd, orm::wq::le, val);
	}

M_MODEL& likeGitpwd(const std::string &val)
	{return where(B_BASE::cols::gitpwd, orm::wq::like, val);
	}

M_MODEL& nullGitpwd()
	{return whereNull(B_BASE::cols::gitpwd);
	}

M_MODEL& notnullGitpwd()
	{return whereNotNull(B_BASE::cols::gitpwd);
	}

M_MODEL& oreqGitpwd(const std::string &val)
	{return whereOr(B_BASE::cols::gitpwd, orm::wq::eq, val);
	}

M_MODEL& ornqGitpwd(const std::string &val)
	{return whereOr(B_BASE::cols::gitpwd, orm::wq::nq, val);
	}

M_MODEL& orbtGitpwd(const std::string &val)
	{return whereOr(B_BASE::cols::gitpwd, orm::wq::bt, val);
	}

M_MODEL& orbeGitpwd(const std::string &val)
	{return whereOr(B_BASE::cols::gitpwd, orm::wq::be, val);
	}

M_MODEL& orltGitpwd(const std::string &val)
	{return whereOr(B_BASE::cols::gitpwd, orm::wq::lt, val);
	}

M_MODEL& orleGitpwd(const std::string &val)
	{return whereOr(B_BASE::cols::gitpwd, orm::wq::le, val);
	}

M_MODEL& orlikeGitpwd(const std::string &val)
	{return whereOr(B_BASE::cols::gitpwd, orm::wq::like, val);
	}

M_MODEL& ornullGitpwd()
	{return whereOrNull(B_BASE::cols::gitpwd);
	}

M_MODEL& ornotnullGitpwd()
	{return whereOrNotNull(B_BASE::cols::gitpwd);
	}
...
M_MODEL& eqFupan(const std::string &val)
	{return where(B_BASE::cols::fupan, orm::wq::eq, val);
	}

M_MODEL& nqFupan(const std::string &val)
	{return where(B_BASE::cols::fupan, orm::wq::nq, val);
	}

M_MODEL& btFupan(const std::string &val)
	{return where(B_BASE::cols::fupan, orm::wq::bt, val);
	}

M_MODEL& beFupan(const std::string &val)
	{return where(B_BASE::cols::fupan, orm::wq::be, val);
	}

M_MODEL& ltFupan(const std::string &val)
	{return where(B_BASE::cols::fupan, orm::wq::lt, val);
	}

M_MODEL& leFupan(const std::string &val)
	{return where(B_BASE::cols::fupan, orm::wq::le, val);
	}

M_MODEL& likeFupan(const std::string &val)
	{return where(B_BASE::cols::fupan, orm::wq::like, val);
	}

M_MODEL& nullFupan()
	{return whereNull(B_BASE::cols::fupan);
	}

M_MODEL& notnullFupan()
	{return whereNotNull(B_BASE::cols::fupan);
	}

M_MODEL& oreqFupan(const std::string &val)
	{return whereOr(B_BASE::cols::fupan, orm::wq::eq, val);
	}

M_MODEL& ornqFupan(const std::string &val)
	{return whereOr(B_BASE::cols::fupan, orm::wq::nq, val);
	}

M_MODEL& orbtFupan(const std::string &val)
	{return whereOr(B_BASE::cols::fupan, orm::wq::bt, val);
	}

M_MODEL& orbeFupan(const std::string &val)
	{return whereOr(B_BASE::cols::fupan, orm::wq::be, val);
	}

M_MODEL& orltFupan(const std::string &val)
	{return whereOr(B_BASE::cols::fupan, orm::wq::lt, val);
	}

M_MODEL& orleFupan(const std::string &val)
	{return whereOr(B_BASE::cols::fupan, orm::wq::le, val);
	}

M_MODEL& orlikeFupan(const std::string &val)
	{return whereOr(B_BASE::cols::fupan, orm::wq::like, val);
	}

M_MODEL& ornullFupan()
	{return whereOrNull(B_BASE::cols::fupan);
	}

M_MODEL& ornotnullFupan()
	{return whereOrNotNull(B_BASE::cols::fupan);
	}

M_MODEL& eqTotalvalue(const std::string &val)
	{return where(B_BASE::cols::totalvalue, orm::wq::eq, val);
	}

M_MODEL& nqTotalvalue(const std::string &val)
	{return where(B_BASE::cols::totalvalue, orm::wq::nq, val);
	}

M_MODEL& btTotalvalue(const std::string &val)
	{return where(B_BASE::cols::totalvalue, orm::wq::bt, val);
	}

M_MODEL& beTotalvalue(const std::string &val)
	{return where(B_BASE::cols::totalvalue, orm::wq::be, val);
	}

M_MODEL& ltTotalvalue(const std::string &val)
	{return where(B_BASE::cols::totalvalue, orm::wq::lt, val);
	}

M_MODEL& leTotalvalue(const std::string &val)
	{return where(B_BASE::cols::totalvalue, orm::wq::le, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& eqTotalvalue(T val)
	{return where(B_BASE::cols::totalvalue, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& nqTotalvalue(T val)
	{return where(B_BASE::cols::totalvalue, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& btTotalvalue(T val)
	{return where(B_BASE::cols::totalvalue, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& beTotalvalue(T val)
	{return where(B_BASE::cols::totalvalue, orm::wq::be, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& ltTotalvalue(T val)
	{return where(B_BASE::cols::totalvalue, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& leTotalvalue(T val)
	{return where(B_BASE::cols::totalvalue, orm::wq::le, val);
	}

M_MODEL& nullTotalvalue()
	{return whereNull(B_BASE::cols::totalvalue);
	}

M_MODEL& notnullTotalvalue()
	{return whereNotNull(B_BASE::cols::totalvalue);
	}

M_MODEL& oreqTotalvalue(const std::string &val)
	{return whereOr(B_BASE::cols::totalvalue, orm::wq::eq, val);
	}

M_MODEL& ornqTotalvalue(const std::string &val)
	{return whereOr(B_BASE::cols::totalvalue, orm::wq::nq, val);
	}

M_MODEL& orbtTotalvalue(const std::string &val)
	{return whereOr(B_BASE::cols::totalvalue, orm::wq::bt, val);
	}

M_MODEL& orbeTotalvalue(const std::string &val)
	{return whereOr(B_BASE::cols::totalvalue, orm::wq::be, val);
	}

M_MODEL& orltTotalvalue(const std::string &val)
	{return whereOr(B_BASE::cols::totalvalue, orm::wq::lt, val);
	}

M_MODEL& orleTotalvalue(const std::string &val)
	{return whereOr(B_BASE::cols::totalvalue, orm::wq::le, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& oreqTotalvalue(T val)
	{return whereOr(B_BASE::cols::totalvalue, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& ornqTotalvalue(T val)
	{return whereOr(B_BASE::cols::totalvalue, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& orbtTotalvalue(T val)
	{return whereOr(B_BASE::cols::totalvalue, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& orbeTotalvalue(T val)
	{return whereOr(B_BASE::cols::totalvalue, orm::wq::be, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& orltTotalvalue(T val)
	{return whereOr(B_BASE::cols::totalvalue, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_arithmetic_v<T>
M_MODEL& orleTotalvalue(T val)
	{return whereOr(B_BASE::cols::totalvalue, orm::wq::le, val);
	}

M_MODEL& ornullTotalvalue()
	{return whereOrNull(B_BASE::cols::totalvalue);
	}

M_MODEL& ornotnullTotalvalue()
	{return whereOrNotNull(B_BASE::cols::totalvalue);
	}
...
M_MODEL& ornotnullRealday()
	{return whereOrNotNull(B_BASE::cols::realday);
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = xproject_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &xproject_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &xproject_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(xproject_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(xproject_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(xproject_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(xproject_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &xproject_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &xproject_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &xproject_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &xproject_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &xproject_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(xproject_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &xproject_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(xproject_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &xproject_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(xproject_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &xproject_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(xproject_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &xproject_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(xproject_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(xproject_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(xproject_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(xproject_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(xproject_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(xproject_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(xproject_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(xproject_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(xproject_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(xproject_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &xproject_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &xproject_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &xproject_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &xproject_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &xproject_info::col_names> wq)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &xproject_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = xproject_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(xproject_info::meta &insert_data)
        {
        ...
        xproject_opsql(const xproject_opsql &)            = delete;
        xproject_opsql &operator=(const xproject_opsql &) = delete;
        xproject_opsql(xproject_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        xproject_opsql &operator=(xproject_opsql &&o) noexcept
        {
            if (this != &o)
            {
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < xproject_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == xproject_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(xproject_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(xproject_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = xproject_info::col_names.size();
