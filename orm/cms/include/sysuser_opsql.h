#ifndef _ORM_CMS_SYSUSER_OPERATE_H
#define _ORM_CMS_SYSUSER_OPERATE_H
/*
...
{ /*tagnamespace_replace*/
    ...
    class sysuser_opsql : public B_BASE
    {
      public:
        sysuser_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        sysuser_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, sysuser_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            break;
                case 2:
            data_temp.password.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 3:
            data_temp.textword.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 4:
             {
               data_temp.isopen = 0;
            ...
            break;
                case 15:
            data_temp.nickname.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 16:
            data_temp.realname.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 17:
            data_temp.avatar.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 18:
            ...
            break;
                case 20:
            data_temp.wxuuid.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                
        }
    }
    
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, sysuser_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
            ...
            case 2: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.password.assign(v.data(), v.size());
                    else
                        data_temp.password.clear();
                }, val);
            } break;
            case 3: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.textword.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.textword.assign(v.data(), v.size());
                    else
                        data_temp.textword.clear();
                }, val);
            } break;
            ...
            case 5: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.level = static_cast<decltype(data_temp.level )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.level = static_cast<decltype(data_temp.level )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.level = static_cast<decltype(data_temp.level )>(v);
            ...
            case 6: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.companyid = static_cast<decltype(data_temp.companyid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.companyid = static_cast<decltype(data_temp.companyid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.companyid = static_cast<decltype(data_temp.companyid )>(v);
            ...
            case 8: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.jobid = static_cast<decltype(data_temp.jobid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.jobid = static_cast<decltype(data_temp.jobid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.jobid = static_cast<decltype(data_temp.jobid )>(v);
            ...
            case 9: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.roleid = static_cast<decltype(data_temp.roleid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.roleid = static_cast<decltype(data_temp.roleid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.roleid = static_cast<decltype(data_temp.roleid )>(v);
            ...
            case 10: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.postid = static_cast<decltype(data_temp.postid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.postid = static_cast<decltype(data_temp.postid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.postid = static_cast<decltype(data_temp.postid )>(v);
            ...
            case 13: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.qrtemp = static_cast<decltype(data_temp.qrtemp )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.qrtemp = static_cast<decltype(data_temp.qrtemp )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.qrtemp = static_cast<decltype(data_temp.qrtemp )>(v);
            ...
            case 14: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.gender = static_cast<decltype(data_temp.gender )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.gender = static_cast<decltype(data_temp.gender )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.gender = static_cast<decltype(data_temp.gender )>(v);
            ...
            case 15: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.nickname.assign(v.data(), v.size());
                    else
                        data_temp.nickname.clear();
                }, val);
            } break;
            case 16: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.realname.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.realname.assign(v.data(), v.size());
                    else
                        data_temp.realname.clear();
                }, val);
            } break;
            case 17: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.avatar.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.avatar.assign(v.data(), v.size());
                    else
                        data_temp.avatar.clear();
                }, val);
            } break;
            ...
            case 20: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.wxuuid.assign(v.data(), v.size());
                    else
                        data_temp.wxuuid.clear();
                }, val);
            } break;
        ...
        http::obj_val get_field_value(unsigned char index_pos, const sysuser_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            case 1:
                ...
                break;
            case 2:
                return http::obj_val(std::string(data_temp.password));
                break;
            case 3:
                return http::obj_val(std::string(data_temp.textword));
                break;
            case 4:
                return http::obj_val(static_cast<long long>(data_temp.isopen));
                break;
            case 5:
                return http::obj_val(static_cast<long long>(data_temp.level));
                break;
            case 6:
                return http::obj_val(static_cast<long long>(data_temp.companyid));
                break;
            case 7:
                return http::obj_val(static_cast<long long>(data_temp.dpid));
                break;
            case 8:
                return http::obj_val(static_cast<long long>(data_temp.jobid));
                break;
            case 9:
                return http::obj_val(static_cast<long long>(data_temp.roleid));
                break;
            case 10:
                return http::obj_val(static_cast<long long>(data_temp.postid));
                break;
            case 11:
            ...
            case 12:
                ...
                break;
            case 13:
                return http::obj_val(static_cast<long long>(data_temp.qrtemp));
                break;
            case 14:
                return http::obj_val(static_cast<long long>(data_temp.gender));
                break;
            case 15:
                return http::obj_val(std::string(data_temp.nickname));
                break;
            case 16:
                return http::obj_val(std::string(data_temp.realname));
                break;
            case 17:
                return http::obj_val(std::string(data_temp.avatar));
                break;
            case 18:
            ...
            case 19:
                ...
                break;
            case 20:
                return http::obj_val(std::string(data_temp.wxuuid));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const sysuser_info::meta &data_temp)
    {
        // Auto-increment primary key adminid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.adminid == 0)
            return http::obj_val(nullptr);
...
M_MODEL& eqPassword(const std::string &val)
	{return where(B_BASE::cols::password, orm::wq::eq, val);
	}

M_MODEL& nqPassword(const std::string &val)
	{return where(B_BASE::cols::password, orm::wq::nq, val);
	}

M_MODEL& btPassword(const std::string &val)
	{return where(B_BASE::cols::password, orm::wq::bt, val);
	}

M_MODEL& bePassword(const std::string &val)
	{return where(B_BASE::cols::password, orm::wq::be, val);
	}

M_MODEL& ltPassword(const std::string &val)
	{return where(B_BASE::cols::password, orm::wq::lt, val);
	}

M_MODEL& lePassword(const std::string &val)
	{return where(B_BASE::cols::password, orm::wq::le, val);
	}

M_MODEL& likePassword(const std::string &val)
	{return where(B_BASE::cols::password, orm::wq::like, val);
	}

M_MODEL& nullPassword()
	{return whereNull(B_BASE::cols::password);
	}

M_MODEL& notnullPassword()
	{return whereNotNull(B_BASE::cols::password);
	}

M_MODEL& oreqPassword(const std::string &val)
	{return whereOr(B_BASE::cols::password, orm::wq::eq, val);
	}

M_MODEL& ornqPassword(const std::string &val)
	{return whereOr(B_BASE::cols::password, orm::wq::nq, val);
	}

M_MODEL& orbtPassword(const std::string &val)
	{return whereOr(B_BASE::cols::password, orm::wq::bt, val);
	}

M_MODEL& orbePassword(const std::string &val)
	{return whereOr(B_BASE::cols::password, orm::wq::be, val);
	}

M_MODEL& orltPassword(const std::string &val)
	{return whereOr(B_BASE::cols::password, orm::wq::lt, val);
	}

M_MODEL& orlePassword(const std::string &val)
	{return whereOr(B_BASE::cols::password, orm::wq::le, val);
	}

M_MODEL& orlikePassword(const std::string &val)
	{return whereOr(B_BASE::cols::password, orm::wq::like, val);
	}

M_MODEL& ornullPassword()
	{return whereOrNull(B_BASE::cols::password);
	}

M_MODEL& ornotnullPassword()
	{return whereOrNotNull(B_BASE::cols::password);
	}

M_MODEL& eqTextword(const std::string &val)
	{return where(B_BASE::cols::textword, orm::wq::eq, val);
	}

M_MODEL& nqTextword(const std::string &val)
	{return where(B_BASE::cols::textword, orm::wq::nq, val);
	}

M_MODEL& btTextword(const std::string &val)
	{return where(B_BASE::cols::textword, orm::wq::bt, val);
	}

M_MODEL& beTextword(const std::string &val)
	{return where(B_BASE::cols::textword, orm::wq::be, val);
	}

M_MODEL& ltTextword(const std::string &val)
	{return where(B_BASE::cols::textword, orm::wq::lt, val);
	}

M_MODEL& leTextword(const std::string &val)
	{return where(B_BASE::cols::textword, orm::wq::le, val);
	}

M_MODEL& likeTextword(const std::string &val)
	{return where(B_BASE::cols::textword, orm::wq::like, val);
	}

M_MODEL& nullTextword()
	{return whereNull(B_BASE::cols::textword);
	}

M_MODEL& notnullTextword()
	{return whereNotNull(B_BASE::cols::textword);
	}

M_MODEL& oreqTextword(const std::string &val)
	{return whereOr(B_BASE::cols::textword, orm::wq::eq, val);
	}

M_MODEL& ornqTextword(const std::string &val)
	{return whereOr(B_BASE::cols::textword, orm::wq::nq, val);
	}

M_MODEL& orbtTextword(const std::string &val)
	{return whereOr(B_BASE::cols::textword, orm::wq::bt, val);
	}

M_MODEL& orbeTextword(const std::string &val)
	{return whereOr(B_BASE::cols::textword, orm::wq::be, val);
	}

M_MODEL& orltTextword(const std::string &val)
	{return whereOr(B_BASE::cols::textword, orm::wq::lt, val);
	}

M_MODEL& orleTextword(const std::string &val)
	{return whereOr(B_BASE::cols::textword, orm::wq::le, val);
	}

M_MODEL& orlikeTextword(const std::string &val)
	{return whereOr(B_BASE::cols::textword, orm::wq::like, val);
	}

M_MODEL& ornullTextword()
	{return whereOrNull(B_BASE::cols::textword);
	}

M_MODEL& ornotnullTextword()
	{return whereOrNotNull(B_BASE::cols::textword);
	}
...
M_MODEL& eqLevel(const std::string &val)
	{return where(B_BASE::cols::level, orm::wq::eq, val);
	}

M_MODEL& nqLevel(const std::string &val)
	{return where(B_BASE::cols::level, orm::wq::nq, val);
	}

M_MODEL& btLevel(const std::string &val)
	{return where(B_BASE::cols::level, orm::wq::bt, val);
	}

M_MODEL& beLevel(const std::string &val)
	{return where(B_BASE::cols::level, orm::wq::be, val);
	}

M_MODEL& ltLevel(const std::string &val)
	{return where(B_BASE::cols::level, orm::wq::lt, val);
	}

M_MODEL& leLevel(const std::string &val)
	{return where(B_BASE::cols::level, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqLevel(T val)
	{return where(B_BASE::cols::level, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqLevel(T val)
	{return where(B_BASE::cols::level, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btLevel(T val)
	{return where(B_BASE::cols::level, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beLevel(T val)
	{return where(B_BASE::cols::level, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltLevel(T val)
	{return where(B_BASE::cols::level, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leLevel(T val)
	{return where(B_BASE::cols::level, orm::wq::le, val);
	}

M_MODEL& nullLevel()
	{return whereNull(B_BASE::cols::level);
	}

M_MODEL& notnullLevel()
	{return whereNotNull(B_BASE::cols::level);
	}

M_MODEL& oreqLevel(const std::string &val)
	{return whereOr(B_BASE::cols::level, orm::wq::eq, val);
	}

M_MODEL& ornqLevel(const std::string &val)
	{return whereOr(B_BASE::cols::level, orm::wq::nq, val);
	}

M_MODEL& orbtLevel(const std::string &val)
	{return whereOr(B_BASE::cols::level, orm::wq::bt, val);
	}

M_MODEL& orbeLevel(const std::string &val)
	{return whereOr(B_BASE::cols::level, orm::wq::be, val);
	}

M_MODEL& orltLevel(const std::string &val)
	{return whereOr(B_BASE::cols::level, orm::wq::lt, val);
	}

M_MODEL& orleLevel(const std::string &val)
	{return whereOr(B_BASE::cols::level, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqLevel(T val)
	{return whereOr(B_BASE::cols::level, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqLevel(T val)
	{return whereOr(B_BASE::cols::level, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtLevel(T val)
	{return whereOr(B_BASE::cols::level, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeLevel(T val)
	{return whereOr(B_BASE::cols::level, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltLevel(T val)
	{return whereOr(B_BASE::cols::level, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleLevel(T val)
	{return whereOr(B_BASE::cols::level, orm::wq::le, val);
	}

M_MODEL& ornullLevel()
	{return whereOrNull(B_BASE::cols::level);
	}

M_MODEL& ornotnullLevel()
	{return whereOrNotNull(B_BASE::cols::level);
	}

M_MODEL& eqCompanyid(const std::string &val)
	{return where(B_BASE::cols::companyid, orm::wq::eq, val);
	}

M_MODEL& nqCompanyid(const std::string &val)
	{return where(B_BASE::cols::companyid, orm::wq::nq, val);
	}

M_MODEL& btCompanyid(const std::string &val)
	{return where(B_BASE::cols::companyid, orm::wq::bt, val);
	}

M_MODEL& beCompanyid(const std::string &val)
	{return where(B_BASE::cols::companyid, orm::wq::be, val);
	}

M_MODEL& ltCompanyid(const std::string &val)
	{return where(B_BASE::cols::companyid, orm::wq::lt, val);
	}

M_MODEL& leCompanyid(const std::string &val)
	{return where(B_BASE::cols::companyid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqCompanyid(T val)
	{return where(B_BASE::cols::companyid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqCompanyid(T val)
	{return where(B_BASE::cols::companyid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btCompanyid(T val)
	{return where(B_BASE::cols::companyid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beCompanyid(T val)
	{return where(B_BASE::cols::companyid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltCompanyid(T val)
	{return where(B_BASE::cols::companyid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leCompanyid(T val)
	{return where(B_BASE::cols::companyid, orm::wq::le, val);
	}

M_MODEL& nullCompanyid()
	{return whereNull(B_BASE::cols::companyid);
	}

M_MODEL& notnullCompanyid()
	{return whereNotNull(B_BASE::cols::companyid);
	}

M_MODEL& oreqCompanyid(const std::string &val)
	{return whereOr(B_BASE::cols::companyid, orm::wq::eq, val);
	}

M_MODEL& ornqCompanyid(const std::string &val)
	{return whereOr(B_BASE::cols::companyid, orm::wq::nq, val);
	}

M_MODEL& orbtCompanyid(const std::string &val)
	{return whereOr(B_BASE::cols::companyid, orm::wq::bt, val);
	}

M_MODEL& orbeCompanyid(const std::string &val)
	{return whereOr(B_BASE::cols::companyid, orm::wq::be, val);
	}

M_MODEL& orltCompanyid(const std::string &val)
	{return whereOr(B_BASE::cols::companyid, orm::wq::lt, val);
	}

M_MODEL& orleCompanyid(const std::string &val)
	{return whereOr(B_BASE::cols::companyid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqCompanyid(T val)
	{return whereOr(B_BASE::cols::companyid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqCompanyid(T val)
	{return whereOr(B_BASE::cols::companyid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtCompanyid(T val)
	{return whereOr(B_BASE::cols::companyid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeCompanyid(T val)
	{return whereOr(B_BASE::cols::companyid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltCompanyid(T val)
	{return whereOr(B_BASE::cols::companyid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleCompanyid(T val)
	{return whereOr(B_BASE::cols::companyid, orm::wq::le, val);
	}

M_MODEL& ornullCompanyid()
	{return whereOrNull(B_BASE::cols::companyid);
	}

M_MODEL& ornotnullCompanyid()
	{return whereOrNotNull(B_BASE::cols::companyid);
	}
...
M_MODEL& eqJobid(const std::string &val)
	{return where(B_BASE::cols::jobid, orm::wq::eq, val);
	}

M_MODEL& nqJobid(const std::string &val)
	{return where(B_BASE::cols::jobid, orm::wq::nq, val);
	}

M_MODEL& btJobid(const std::string &val)
	{return where(B_BASE::cols::jobid, orm::wq::bt, val);
	}

M_MODEL& beJobid(const std::string &val)
	{return where(B_BASE::cols::jobid, orm::wq::be, val);
	}

M_MODEL& ltJobid(const std::string &val)
	{return where(B_BASE::cols::jobid, orm::wq::lt, val);
	}

M_MODEL& leJobid(const std::string &val)
	{return where(B_BASE::cols::jobid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqJobid(T val)
	{return where(B_BASE::cols::jobid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqJobid(T val)
	{return where(B_BASE::cols::jobid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btJobid(T val)
	{return where(B_BASE::cols::jobid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beJobid(T val)
	{return where(B_BASE::cols::jobid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltJobid(T val)
	{return where(B_BASE::cols::jobid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leJobid(T val)
	{return where(B_BASE::cols::jobid, orm::wq::le, val);
	}

M_MODEL& nullJobid()
	{return whereNull(B_BASE::cols::jobid);
	}

M_MODEL& notnullJobid()
	{return whereNotNull(B_BASE::cols::jobid);
	}

M_MODEL& oreqJobid(const std::string &val)
	{return whereOr(B_BASE::cols::jobid, orm::wq::eq, val);
	}

M_MODEL& ornqJobid(const std::string &val)
	{return whereOr(B_BASE::cols::jobid, orm::wq::nq, val);
	}

M_MODEL& orbtJobid(const std::string &val)
	{return whereOr(B_BASE::cols::jobid, orm::wq::bt, val);
	}

M_MODEL& orbeJobid(const std::string &val)
	{return whereOr(B_BASE::cols::jobid, orm::wq::be, val);
	}

M_MODEL& orltJobid(const std::string &val)
	{return whereOr(B_BASE::cols::jobid, orm::wq::lt, val);
	}

M_MODEL& orleJobid(const std::string &val)
	{return whereOr(B_BASE::cols::jobid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqJobid(T val)
	{return whereOr(B_BASE::cols::jobid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqJobid(T val)
	{return whereOr(B_BASE::cols::jobid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtJobid(T val)
	{return whereOr(B_BASE::cols::jobid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeJobid(T val)
	{return whereOr(B_BASE::cols::jobid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltJobid(T val)
	{return whereOr(B_BASE::cols::jobid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleJobid(T val)
	{return whereOr(B_BASE::cols::jobid, orm::wq::le, val);
	}

M_MODEL& ornullJobid()
	{return whereOrNull(B_BASE::cols::jobid);
	}

M_MODEL& ornotnullJobid()
	{return whereOrNotNull(B_BASE::cols::jobid);
	}

M_MODEL& eqRoleid(const std::string &val)
	{return where(B_BASE::cols::roleid, orm::wq::eq, val);
	}

M_MODEL& nqRoleid(const std::string &val)
	{return where(B_BASE::cols::roleid, orm::wq::nq, val);
	}

M_MODEL& btRoleid(const std::string &val)
	{return where(B_BASE::cols::roleid, orm::wq::bt, val);
	}

M_MODEL& beRoleid(const std::string &val)
	{return where(B_BASE::cols::roleid, orm::wq::be, val);
	}

M_MODEL& ltRoleid(const std::string &val)
	{return where(B_BASE::cols::roleid, orm::wq::lt, val);
	}

M_MODEL& leRoleid(const std::string &val)
	{return where(B_BASE::cols::roleid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqRoleid(T val)
	{return where(B_BASE::cols::roleid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqRoleid(T val)
	{return where(B_BASE::cols::roleid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btRoleid(T val)
	{return where(B_BASE::cols::roleid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beRoleid(T val)
	{return where(B_BASE::cols::roleid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltRoleid(T val)
	{return where(B_BASE::cols::roleid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leRoleid(T val)
	{return where(B_BASE::cols::roleid, orm::wq::le, val);
	}

M_MODEL& nullRoleid()
	{return whereNull(B_BASE::cols::roleid);
	}

M_MODEL& notnullRoleid()
	{return whereNotNull(B_BASE::cols::roleid);
	}

M_MODEL& oreqRoleid(const std::string &val)
	{return whereOr(B_BASE::cols::roleid, orm::wq::eq, val);
	}

M_MODEL& ornqRoleid(const std::string &val)
	{return whereOr(B_BASE::cols::roleid, orm::wq::nq, val);
	}

M_MODEL& orbtRoleid(const std::string &val)
	{return whereOr(B_BASE::cols::roleid, orm::wq::bt, val);
	}

M_MODEL& orbeRoleid(const std::string &val)
	{return whereOr(B_BASE::cols::roleid, orm::wq::be, val);
	}

M_MODEL& orltRoleid(const std::string &val)
	{return whereOr(B_BASE::cols::roleid, orm::wq::lt, val);
	}

M_MODEL& orleRoleid(const std::string &val)
	{return whereOr(B_BASE::cols::roleid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqRoleid(T val)
	{return whereOr(B_BASE::cols::roleid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqRoleid(T val)
	{return whereOr(B_BASE::cols::roleid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtRoleid(T val)
	{return whereOr(B_BASE::cols::roleid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeRoleid(T val)
	{return whereOr(B_BASE::cols::roleid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltRoleid(T val)
	{return whereOr(B_BASE::cols::roleid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleRoleid(T val)
	{return whereOr(B_BASE::cols::roleid, orm::wq::le, val);
	}

M_MODEL& ornullRoleid()
	{return whereOrNull(B_BASE::cols::roleid);
	}

M_MODEL& ornotnullRoleid()
	{return whereOrNotNull(B_BASE::cols::roleid);
	}

M_MODEL& eqPostid(const std::string &val)
	{return where(B_BASE::cols::postid, orm::wq::eq, val);
	}

M_MODEL& nqPostid(const std::string &val)
	{return where(B_BASE::cols::postid, orm::wq::nq, val);
	}

M_MODEL& btPostid(const std::string &val)
	{return where(B_BASE::cols::postid, orm::wq::bt, val);
	}

M_MODEL& bePostid(const std::string &val)
	{return where(B_BASE::cols::postid, orm::wq::be, val);
	}

M_MODEL& ltPostid(const std::string &val)
	{return where(B_BASE::cols::postid, orm::wq::lt, val);
	}

M_MODEL& lePostid(const std::string &val)
	{return where(B_BASE::cols::postid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqPostid(T val)
	{return where(B_BASE::cols::postid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqPostid(T val)
	{return where(B_BASE::cols::postid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btPostid(T val)
	{return where(B_BASE::cols::postid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& bePostid(T val)
	{return where(B_BASE::cols::postid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltPostid(T val)
	{return where(B_BASE::cols::postid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& lePostid(T val)
	{return where(B_BASE::cols::postid, orm::wq::le, val);
	}

M_MODEL& nullPostid()
	{return whereNull(B_BASE::cols::postid);
	}

M_MODEL& notnullPostid()
	{return whereNotNull(B_BASE::cols::postid);
	}

M_MODEL& oreqPostid(const std::string &val)
	{return whereOr(B_BASE::cols::postid, orm::wq::eq, val);
	}

M_MODEL& ornqPostid(const std::string &val)
	{return whereOr(B_BASE::cols::postid, orm::wq::nq, val);
	}

M_MODEL& orbtPostid(const std::string &val)
	{return whereOr(B_BASE::cols::postid, orm::wq::bt, val);
	}

M_MODEL& orbePostid(const std::string &val)
	{return whereOr(B_BASE::cols::postid, orm::wq::be, val);
	}

M_MODEL& orltPostid(const std::string &val)
	{return whereOr(B_BASE::cols::postid, orm::wq::lt, val);
	}

M_MODEL& orlePostid(const std::string &val)
	{return whereOr(B_BASE::cols::postid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqPostid(T val)
	{return whereOr(B_BASE::cols::postid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqPostid(T val)
	{return whereOr(B_BASE::cols::postid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtPostid(T val)
	{return whereOr(B_BASE::cols::postid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbePostid(T val)
	{return whereOr(B_BASE::cols::postid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltPostid(T val)
	{return whereOr(B_BASE::cols::postid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orlePostid(T val)
	{return whereOr(B_BASE::cols::postid, orm::wq::le, val);
	}

M_MODEL& ornullPostid()
	{return whereOrNull(B_BASE::cols::postid);
	}

M_MODEL& ornotnullPostid()
	{return whereOrNotNull(B_BASE::cols::postid);
	}
...
M_MODEL& eqQrtemp(const std::string &val)
	{return where(B_BASE::cols::qrtemp, orm::wq::eq, val);
	}

M_MODEL& nqQrtemp(const std::string &val)
	{return where(B_BASE::cols::qrtemp, orm::wq::nq, val);
	}

M_MODEL& btQrtemp(const std::string &val)
	{return where(B_BASE::cols::qrtemp, orm::wq::bt, val);
	}

M_MODEL& beQrtemp(const std::string &val)
	{return where(B_BASE::cols::qrtemp, orm::wq::be, val);
	}

M_MODEL& ltQrtemp(const std::string &val)
	{return where(B_BASE::cols::qrtemp, orm::wq::lt, val);
	}

M_MODEL& leQrtemp(const std::string &val)
	{return where(B_BASE::cols::qrtemp, orm::wq::le, val);
	}
...
M_MODEL& nullQrtemp()
	{return whereNull(B_BASE::cols::qrtemp);
	}

M_MODEL& notnullQrtemp()
	{return whereNotNull(B_BASE::cols::qrtemp);
	}

M_MODEL& oreqQrtemp(const std::string &val)
	{return whereOr(B_BASE::cols::qrtemp, orm::wq::eq, val);
	}

M_MODEL& ornqQrtemp(const std::string &val)
	{return whereOr(B_BASE::cols::qrtemp, orm::wq::nq, val);
	}

M_MODEL& orbtQrtemp(const std::string &val)
	{return whereOr(B_BASE::cols::qrtemp, orm::wq::bt, val);
	}

M_MODEL& orbeQrtemp(const std::string &val)
	{return whereOr(B_BASE::cols::qrtemp, orm::wq::be, val);
	}

M_MODEL& orltQrtemp(const std::string &val)
	{return whereOr(B_BASE::cols::qrtemp, orm::wq::lt, val);
	}

M_MODEL& orleQrtemp(const std::string &val)
	{return whereOr(B_BASE::cols::qrtemp, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqQrtemp(T val)
	{return whereOr(B_BASE::cols::qrtemp, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqQrtemp(T val)
	{return whereOr(B_BASE::cols::qrtemp, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtQrtemp(T val)
	{return whereOr(B_BASE::cols::qrtemp, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeQrtemp(T val)
	{return whereOr(B_BASE::cols::qrtemp, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltQrtemp(T val)
	{return whereOr(B_BASE::cols::qrtemp, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleQrtemp(T val)
	{return whereOr(B_BASE::cols::qrtemp, orm::wq::le, val);
	}

M_MODEL& ornullQrtemp()
	{return whereOrNull(B_BASE::cols::qrtemp);
	}

M_MODEL& ornotnullQrtemp()
	{return whereOrNotNull(B_BASE::cols::qrtemp);
	}

M_MODEL& eqGender(const std::string &val)
	{return where(B_BASE::cols::gender, orm::wq::eq, val);
	}

M_MODEL& nqGender(const std::string &val)
	{return where(B_BASE::cols::gender, orm::wq::nq, val);
	}

M_MODEL& btGender(const std::string &val)
	{return where(B_BASE::cols::gender, orm::wq::bt, val);
	}

M_MODEL& beGender(const std::string &val)
	{return where(B_BASE::cols::gender, orm::wq::be, val);
	}

M_MODEL& ltGender(const std::string &val)
	{return where(B_BASE::cols::gender, orm::wq::lt, val);
	}

M_MODEL& leGender(const std::string &val)
	{return where(B_BASE::cols::gender, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqGender(T val)
	{return where(B_BASE::cols::gender, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqGender(T val)
	{return where(B_BASE::cols::gender, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btGender(T val)
	{return where(B_BASE::cols::gender, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beGender(T val)
	{return where(B_BASE::cols::gender, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltGender(T val)
	{return where(B_BASE::cols::gender, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leGender(T val)
	{return where(B_BASE::cols::gender, orm::wq::le, val);
	}

M_MODEL& nullGender()
	{return whereNull(B_BASE::cols::gender);
	}

M_MODEL& notnullGender()
	{return whereNotNull(B_BASE::cols::gender);
	}

M_MODEL& oreqGender(const std::string &val)
	{return whereOr(B_BASE::cols::gender, orm::wq::eq, val);
	}

M_MODEL& ornqGender(const std::string &val)
	{return whereOr(B_BASE::cols::gender, orm::wq::nq, val);
	}

M_MODEL& orbtGender(const std::string &val)
	{return whereOr(B_BASE::cols::gender, orm::wq::bt, val);
	}

M_MODEL& orbeGender(const std::string &val)
	{return whereOr(B_BASE::cols::gender, orm::wq::be, val);
	}

M_MODEL& orltGender(const std::string &val)
	{return whereOr(B_BASE::cols::gender, orm::wq::lt, val);
	}

M_MODEL& orleGender(const std::string &val)
	{return whereOr(B_BASE::cols::gender, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqGender(T val)
	{return whereOr(B_BASE::cols::gender, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqGender(T val)
	{return whereOr(B_BASE::cols::gender, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtGender(T val)
	{return whereOr(B_BASE::cols::gender, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeGender(T val)
	{return whereOr(B_BASE::cols::gender, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltGender(T val)
	{return whereOr(B_BASE::cols::gender, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleGender(T val)
	{return whereOr(B_BASE::cols::gender, orm::wq::le, val);
	}

M_MODEL& ornullGender()
	{return whereOrNull(B_BASE::cols::gender);
	}

M_MODEL& ornotnullGender()
	{return whereOrNotNull(B_BASE::cols::gender);
	}

M_MODEL& eqNickname(const std::string &val)
	{return where(B_BASE::cols::nickname, orm::wq::eq, val);
	}

M_MODEL& nqNickname(const std::string &val)
	{return where(B_BASE::cols::nickname, orm::wq::nq, val);
	}

M_MODEL& btNickname(const std::string &val)
	{return where(B_BASE::cols::nickname, orm::wq::bt, val);
	}

M_MODEL& beNickname(const std::string &val)
	{return where(B_BASE::cols::nickname, orm::wq::be, val);
	}

M_MODEL& ltNickname(const std::string &val)
	{return where(B_BASE::cols::nickname, orm::wq::lt, val);
	}

M_MODEL& leNickname(const std::string &val)
	{return where(B_BASE::cols::nickname, orm::wq::le, val);
	}

M_MODEL& likeNickname(const std::string &val)
	{return where(B_BASE::cols::nickname, orm::wq::like, val);
	}

M_MODEL& nullNickname()
	{return whereNull(B_BASE::cols::nickname);
	}

M_MODEL& notnullNickname()
	{return whereNotNull(B_BASE::cols::nickname);
	}

M_MODEL& oreqNickname(const std::string &val)
	{return whereOr(B_BASE::cols::nickname, orm::wq::eq, val);
	}

M_MODEL& ornqNickname(const std::string &val)
	{return whereOr(B_BASE::cols::nickname, orm::wq::nq, val);
	}

M_MODEL& orbtNickname(const std::string &val)
	{return whereOr(B_BASE::cols::nickname, orm::wq::bt, val);
	}

M_MODEL& orbeNickname(const std::string &val)
	{return whereOr(B_BASE::cols::nickname, orm::wq::be, val);
	}

M_MODEL& orltNickname(const std::string &val)
	{return whereOr(B_BASE::cols::nickname, orm::wq::lt, val);
	}

M_MODEL& orleNickname(const std::string &val)
	{return whereOr(B_BASE::cols::nickname, orm::wq::le, val);
	}

M_MODEL& orlikeNickname(const std::string &val)
	{return whereOr(B_BASE::cols::nickname, orm::wq::like, val);
	}

M_MODEL& ornullNickname()
	{return whereOrNull(B_BASE::cols::nickname);
	}

M_MODEL& ornotnullNickname()
	{return whereOrNotNull(B_BASE::cols::nickname);
	}

M_MODEL& eqRealname(const std::string &val)
	{return where(B_BASE::cols::realname, orm::wq::eq, val);
	}

M_MODEL& nqRealname(const std::string &val)
	{return where(B_BASE::cols::realname, orm::wq::nq, val);
	}

M_MODEL& btRealname(const std::string &val)
	{return where(B_BASE::cols::realname, orm::wq::bt, val);
	}

M_MODEL& beRealname(const std::string &val)
	{return where(B_BASE::cols::realname, orm::wq::be, val);
	}

M_MODEL& ltRealname(const std::string &val)
	{return where(B_BASE::cols::realname, orm::wq::lt, val);
	}

M_MODEL& leRealname(const std::string &val)
	{return where(B_BASE::cols::realname, orm::wq::le, val);
	}

M_MODEL& likeRealname(const std::string &val)
	{return where(B_BASE::cols::realname, orm::wq::like, val);
	}

M_MODEL& nullRealname()
	{return whereNull(B_BASE::cols::realname);
	}

M_MODEL& notnullRealname()
	{return whereNotNull(B_BASE::cols::realname);
	}

M_MODEL& oreqRealname(const std::string &val)
	{return whereOr(B_BASE::cols::realname, orm::wq::eq, val);
	}

M_MODEL& ornqRealname(const std::string &val)
	{return whereOr(B_BASE::cols::realname, orm::wq::nq, val);
	}

M_MODEL& orbtRealname(const std::string &val)
	{return whereOr(B_BASE::cols::realname, orm::wq::bt, val);
	}

M_MODEL& orbeRealname(const std::string &val)
	{return whereOr(B_BASE::cols::realname, orm::wq::be, val);
	}

M_MODEL& orltRealname(const std::string &val)
	{return whereOr(B_BASE::cols::realname, orm::wq::lt, val);
	}

M_MODEL& orleRealname(const std::string &val)
	{return whereOr(B_BASE::cols::realname, orm::wq::le, val);
	}

M_MODEL& orlikeRealname(const std::string &val)
	{return whereOr(B_BASE::cols::realname, orm::wq::like, val);
	}

M_MODEL& ornullRealname()
	{return whereOrNull(B_BASE::cols::realname);
	}

M_MODEL& ornotnullRealname()
	{return whereOrNotNull(B_BASE::cols::realname);
	}

M_MODEL& eqAvatar(const std::string &val)
	{return where(B_BASE::cols::avatar, orm::wq::eq, val);
	}

M_MODEL& nqAvatar(const std::string &val)
	{return where(B_BASE::cols::avatar, orm::wq::nq, val);
	}

M_MODEL& btAvatar(const std::string &val)
	{return where(B_BASE::cols::avatar, orm::wq::bt, val);
	}

M_MODEL& beAvatar(const std::string &val)
	{return where(B_BASE::cols::avatar, orm::wq::be, val);
	}

M_MODEL& ltAvatar(const std::string &val)
	{return where(B_BASE::cols::avatar, orm::wq::lt, val);
	}

M_MODEL& leAvatar(const std::string &val)
	{return where(B_BASE::cols::avatar, orm::wq::le, val);
	}

M_MODEL& likeAvatar(const std::string &val)
	{return where(B_BASE::cols::avatar, orm::wq::like, val);
	}

M_MODEL& nullAvatar()
	{return whereNull(B_BASE::cols::avatar);
	}

M_MODEL& notnullAvatar()
	{return whereNotNull(B_BASE::cols::avatar);
	}

M_MODEL& oreqAvatar(const std::string &val)
	{return whereOr(B_BASE::cols::avatar, orm::wq::eq, val);
	}

M_MODEL& ornqAvatar(const std::string &val)
	{return whereOr(B_BASE::cols::avatar, orm::wq::nq, val);
	}

M_MODEL& orbtAvatar(const std::string &val)
	{return whereOr(B_BASE::cols::avatar, orm::wq::bt, val);
	}

M_MODEL& orbeAvatar(const std::string &val)
	{return whereOr(B_BASE::cols::avatar, orm::wq::be, val);
	}

M_MODEL& orltAvatar(const std::string &val)
	{return whereOr(B_BASE::cols::avatar, orm::wq::lt, val);
	}

M_MODEL& orleAvatar(const std::string &val)
	{return whereOr(B_BASE::cols::avatar, orm::wq::le, val);
	}

M_MODEL& orlikeAvatar(const std::string &val)
	{return whereOr(B_BASE::cols::avatar, orm::wq::like, val);
	}

M_MODEL& ornullAvatar()
	{return whereOrNull(B_BASE::cols::avatar);
	}

M_MODEL& ornotnullAvatar()
	{return whereOrNotNull(B_BASE::cols::avatar);
	}
...
M_MODEL& eqWxuuid(const std::string &val)
	{return where(B_BASE::cols::wxuuid, orm::wq::eq, val);
	}

M_MODEL& nqWxuuid(const std::string &val)
	{return where(B_BASE::cols::wxuuid, orm::wq::nq, val);
	}

M_MODEL& btWxuuid(const std::string &val)
	{return where(B_BASE::cols::wxuuid, orm::wq::bt, val);
	}

M_MODEL& beWxuuid(const std::string &val)
	{return where(B_BASE::cols::wxuuid, orm::wq::be, val);
	}

M_MODEL& ltWxuuid(const std::string &val)
	{return where(B_BASE::cols::wxuuid, orm::wq::lt, val);
	}

M_MODEL& leWxuuid(const std::string &val)
	{return where(B_BASE::cols::wxuuid, orm::wq::le, val);
	}

M_MODEL& likeWxuuid(const std::string &val)
	{return where(B_BASE::cols::wxuuid, orm::wq::like, val);
	}

M_MODEL& nullWxuuid()
	{return whereNull(B_BASE::cols::wxuuid);
	}

M_MODEL& notnullWxuuid()
	{return whereNotNull(B_BASE::cols::wxuuid);
	}

M_MODEL& oreqWxuuid(const std::string &val)
	{return whereOr(B_BASE::cols::wxuuid, orm::wq::eq, val);
	}

M_MODEL& ornqWxuuid(const std::string &val)
	{return whereOr(B_BASE::cols::wxuuid, orm::wq::nq, val);
	}

M_MODEL& orbtWxuuid(const std::string &val)
	{return whereOr(B_BASE::cols::wxuuid, orm::wq::bt, val);
	}

M_MODEL& orbeWxuuid(const std::string &val)
	{return whereOr(B_BASE::cols::wxuuid, orm::wq::be, val);
	}

M_MODEL& orltWxuuid(const std::string &val)
	{return whereOr(B_BASE::cols::wxuuid, orm::wq::lt, val);
	}

M_MODEL& orleWxuuid(const std::string &val)
	{return whereOr(B_BASE::cols::wxuuid, orm::wq::le, val);
	}

M_MODEL& orlikeWxuuid(const std::string &val)
	{return whereOr(B_BASE::cols::wxuuid, orm::wq::like, val);
	}

M_MODEL& ornullWxuuid()
	{return whereOrNull(B_BASE::cols::wxuuid);
	}

M_MODEL& ornotnullWxuuid()
	{return whereOrNotNull(B_BASE::cols::wxuuid);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = sysuser_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &sysuser_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &sysuser_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(sysuser_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(sysuser_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(sysuser_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(sysuser_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &sysuser_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &sysuser_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &sysuser_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &sysuser_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &sysuser_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(sysuser_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &sysuser_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(sysuser_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &sysuser_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(sysuser_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &sysuser_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(sysuser_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &sysuser_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(sysuser_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(sysuser_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(sysuser_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(sysuser_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(sysuser_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(sysuser_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(sysuser_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(sysuser_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(sysuser_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(sysuser_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &sysuser_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &sysuser_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &sysuser_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &sysuser_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &sysuser_info::col_names> wq)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &sysuser_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
        ...
        bool save_data_cache(const std::string &cache_key_name, const sysuser_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<sysuser_info::meta> &temp_cache = model_meta_cache<sysuser_info::meta>::getinstance();
        ...
        bool save_cache(std::size_t cache_key_name, const std::vector<sysuser_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<sysuser_info::meta>> &temp_cache = model_meta_cache<std::vector<sysuser_info::meta>>::getinstance();
            temp_cache.save(cache_key_name, cache_data, exp_time);
            return true;
        }
        bool save_cache(const std::string cache_key_name, const std::vector<sysuser_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<sysuser_info::meta>> &temp_cache = model_meta_cache<std::vector<sysuser_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_vector_cache(const std::string cache_key_name, const std::vector<sysuser_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<sysuser_info::meta>> &temp_cache = model_meta_cache<std::vector<sysuser_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_cache(const std::string cache_key_name, const sysuser_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<sysuser_info::meta> &temp_cache = model_meta_cache<sysuser_info::meta>::getinstance();
            std::size_t sqlhashid                               = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        std::tuple<unsigned int, unsigned long long> insert(sysuser_info::meta &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(sysuser_info::meta &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_data_insert_sql(insert_data);
        ...
        std::tuple<unsigned int, unsigned long long> insert(std::vector<sysuser_info::meta> &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(std::vector<sysuser_info::meta> &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_vector_insert_sql(insert_data);
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = sysuser_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(sysuser_info::meta &insert_data)
        {
        ...
        M_MODEL &set_data(sysuser_info::meta indata)
        {
            B_BASE::data = indata;
            return *mod;
        }
        ...
        sysuser_opsql(const sysuser_opsql &)            = delete;
        sysuser_opsql &operator=(const sysuser_opsql &) = delete;
        sysuser_opsql(sysuser_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        sysuser_opsql &operator=(sysuser_opsql &&o) noexcept
        {
            if (this != &o)
            {
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < sysuser_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == sysuser_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(sysuser_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(sysuser_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = sysuser_info::col_names.size();
