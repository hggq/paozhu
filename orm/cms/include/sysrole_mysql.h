...
{ /*tagnamespace_replace*/
    ...
    class sysrole_mysql : public B_BASE
    {
      public:
        sysrole_mysql(const std::string &tag) : dbtag(tag)
        {
        ...
        sysrole_mysql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned int value_size, sysrole_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            data_temp.status=0;
                    ...
                    {
                        decltype(data_temp.status) _tmp{};
                        auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            _tmp);
                        if (result.ec == std::errc()) {
                            data_temp.status = _tmp;
                        }
                    }
            break;
                case 4:
            data_temp.rolecode=0;
                    ...
                    {
                        decltype(data_temp.rolecode) _tmp{};
                        auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            _tmp);
                        if (result.ec == std::errc()) {
                            data_temp.rolecode = _tmp;
                        }
                    }
            break;
                case 5:
            ...
            data_temp.rolevalue=0;
                    ...
                    {
                        decltype(data_temp.rolevalue) _tmp{};
                        auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            _tmp);
                        if (result.ec == std::errc()) {
                            data_temp.rolevalue = _tmp;
                        }
                    }
            break;
