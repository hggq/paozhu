#ifndef _ORM_CMS_BLOGCATALOG_OPERATE_H
#define _ORM_CMS_BLOGCATALOG_OPERATE_H
/*
...
{ /*tagnamespace_replace*/
    ...
    class blogcatalog_opsql : public B_BASE
    {
      public:
        blogcatalog_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        blogcatalog_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, blogcatalog_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            break;
                case 3:
             {
               data_temp.articlenum = 0;
            
                    auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            data_temp.articlenum);
                        if (result.ec == std::errc()) {
            ...
            break;
                case 4:
             {
               data_temp.languageid = 0;
            
                    auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            data_temp.languageid);
                        if (result.ec == std::errc()) {
        ...
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, blogcatalog_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.bid = static_cast<decltype(data_temp.bid )>(v);
            ...
            case 2: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.dateid = static_cast<decltype(data_temp.dateid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.dateid = static_cast<decltype(data_temp.dateid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.dateid = static_cast<decltype(data_temp.dateid )>(v);
            ...
            case 3: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.articlenum = static_cast<decltype(data_temp.articlenum )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.articlenum = static_cast<decltype(data_temp.articlenum )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.articlenum = static_cast<decltype(data_temp.articlenum )>(v);
            ...
            case 4: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.languageid = static_cast<decltype(data_temp.languageid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.languageid = static_cast<decltype(data_temp.languageid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.languageid = static_cast<decltype(data_temp.languageid )>(v);
        ...
        http::obj_val get_field_value(unsigned char index_pos, const blogcatalog_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
                return http::obj_val(static_cast<long long>(data_temp.bid));
                break;
            case 1:
                return http::obj_val(static_cast<long long>(data_temp.userid));
                break;
            case 2:
                return http::obj_val(static_cast<long long>(data_temp.dateid));
                break;
            case 3:
                return http::obj_val(static_cast<long long>(data_temp.articlenum));
                break;
            case 4:
                return http::obj_val(static_cast<long long>(data_temp.languageid));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const blogcatalog_info::meta &data_temp)
    {
        // Auto-increment primary key bid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.bid == 0)
            return http::obj_val(nullptr);
        return get_field_value(index_pos, data_temp);
    }
    

M_MODEL& eqBid(const std::string &val)
	{return where(B_BASE::cols::bid, orm::wq::eq, val);
	}

M_MODEL& nqBid(const std::string &val)
	{return where(B_BASE::cols::bid, orm::wq::nq, val);
	}

M_MODEL& btBid(const std::string &val)
	{return where(B_BASE::cols::bid, orm::wq::bt, val);
	}

M_MODEL& beBid(const std::string &val)
	{return where(B_BASE::cols::bid, orm::wq::be, val);
	}

M_MODEL& ltBid(const std::string &val)
	{return where(B_BASE::cols::bid, orm::wq::lt, val);
	}

M_MODEL& leBid(const std::string &val)
	{return where(B_BASE::cols::bid, orm::wq::le, val);
	}
...
M_MODEL& nullBid()
	{return whereNull(B_BASE::cols::bid);
	}

M_MODEL& notnullBid()
	{return whereNotNull(B_BASE::cols::bid);
	}

M_MODEL& oreqBid(const std::string &val)
	{return whereOr(B_BASE::cols::bid, orm::wq::eq, val);
	}

M_MODEL& ornqBid(const std::string &val)
	{return whereOr(B_BASE::cols::bid, orm::wq::nq, val);
	}

M_MODEL& orbtBid(const std::string &val)
	{return whereOr(B_BASE::cols::bid, orm::wq::bt, val);
	}

M_MODEL& orbeBid(const std::string &val)
	{return whereOr(B_BASE::cols::bid, orm::wq::be, val);
	}

M_MODEL& orltBid(const std::string &val)
	{return whereOr(B_BASE::cols::bid, orm::wq::lt, val);
	}

M_MODEL& orleBid(const std::string &val)
	{return whereOr(B_BASE::cols::bid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqBid(T val)
	{return whereOr(B_BASE::cols::bid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqBid(T val)
	{return whereOr(B_BASE::cols::bid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtBid(T val)
	{return whereOr(B_BASE::cols::bid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeBid(T val)
	{return whereOr(B_BASE::cols::bid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltBid(T val)
	{return whereOr(B_BASE::cols::bid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleBid(T val)
	{return whereOr(B_BASE::cols::bid, orm::wq::le, val);
	}

M_MODEL& ornullBid()
	{return whereOrNull(B_BASE::cols::bid);
	}

M_MODEL& ornotnullBid()
	{return whereOrNotNull(B_BASE::cols::bid);
	}
...
M_MODEL& eqDateid(const std::string &val)
	{return where(B_BASE::cols::dateid, orm::wq::eq, val);
	}

M_MODEL& nqDateid(const std::string &val)
	{return where(B_BASE::cols::dateid, orm::wq::nq, val);
	}

M_MODEL& btDateid(const std::string &val)
	{return where(B_BASE::cols::dateid, orm::wq::bt, val);
	}

M_MODEL& beDateid(const std::string &val)
	{return where(B_BASE::cols::dateid, orm::wq::be, val);
	}

M_MODEL& ltDateid(const std::string &val)
	{return where(B_BASE::cols::dateid, orm::wq::lt, val);
	}

M_MODEL& leDateid(const std::string &val)
	{return where(B_BASE::cols::dateid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqDateid(T val)
	{return where(B_BASE::cols::dateid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqDateid(T val)
	{return where(B_BASE::cols::dateid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btDateid(T val)
	{return where(B_BASE::cols::dateid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beDateid(T val)
	{return where(B_BASE::cols::dateid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltDateid(T val)
	{return where(B_BASE::cols::dateid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leDateid(T val)
	{return where(B_BASE::cols::dateid, orm::wq::le, val);
	}

M_MODEL& nullDateid()
	{return whereNull(B_BASE::cols::dateid);
	}

M_MODEL& notnullDateid()
	{return whereNotNull(B_BASE::cols::dateid);
	}

M_MODEL& oreqDateid(const std::string &val)
	{return whereOr(B_BASE::cols::dateid, orm::wq::eq, val);
	}

M_MODEL& ornqDateid(const std::string &val)
	{return whereOr(B_BASE::cols::dateid, orm::wq::nq, val);
	}

M_MODEL& orbtDateid(const std::string &val)
	{return whereOr(B_BASE::cols::dateid, orm::wq::bt, val);
	}

M_MODEL& orbeDateid(const std::string &val)
	{return whereOr(B_BASE::cols::dateid, orm::wq::be, val);
	}

M_MODEL& orltDateid(const std::string &val)
	{return whereOr(B_BASE::cols::dateid, orm::wq::lt, val);
	}

M_MODEL& orleDateid(const std::string &val)
	{return whereOr(B_BASE::cols::dateid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqDateid(T val)
	{return whereOr(B_BASE::cols::dateid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqDateid(T val)
	{return whereOr(B_BASE::cols::dateid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtDateid(T val)
	{return whereOr(B_BASE::cols::dateid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeDateid(T val)
	{return whereOr(B_BASE::cols::dateid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltDateid(T val)
	{return whereOr(B_BASE::cols::dateid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleDateid(T val)
	{return whereOr(B_BASE::cols::dateid, orm::wq::le, val);
	}

M_MODEL& ornullDateid()
	{return whereOrNull(B_BASE::cols::dateid);
	}

M_MODEL& ornotnullDateid()
	{return whereOrNotNull(B_BASE::cols::dateid);
	}

M_MODEL& eqArticlenum(const std::string &val)
	{return where(B_BASE::cols::articlenum, orm::wq::eq, val);
	}

M_MODEL& nqArticlenum(const std::string &val)
	{return where(B_BASE::cols::articlenum, orm::wq::nq, val);
	}

M_MODEL& btArticlenum(const std::string &val)
	{return where(B_BASE::cols::articlenum, orm::wq::bt, val);
	}

M_MODEL& beArticlenum(const std::string &val)
	{return where(B_BASE::cols::articlenum, orm::wq::be, val);
	}

M_MODEL& ltArticlenum(const std::string &val)
	{return where(B_BASE::cols::articlenum, orm::wq::lt, val);
	}

M_MODEL& leArticlenum(const std::string &val)
	{return where(B_BASE::cols::articlenum, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqArticlenum(T val)
	{return where(B_BASE::cols::articlenum, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqArticlenum(T val)
	{return where(B_BASE::cols::articlenum, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btArticlenum(T val)
	{return where(B_BASE::cols::articlenum, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beArticlenum(T val)
	{return where(B_BASE::cols::articlenum, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltArticlenum(T val)
	{return where(B_BASE::cols::articlenum, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leArticlenum(T val)
	{return where(B_BASE::cols::articlenum, orm::wq::le, val);
	}

M_MODEL& nullArticlenum()
	{return whereNull(B_BASE::cols::articlenum);
	}

M_MODEL& notnullArticlenum()
	{return whereNotNull(B_BASE::cols::articlenum);
	}

M_MODEL& oreqArticlenum(const std::string &val)
	{return whereOr(B_BASE::cols::articlenum, orm::wq::eq, val);
	}

M_MODEL& ornqArticlenum(const std::string &val)
	{return whereOr(B_BASE::cols::articlenum, orm::wq::nq, val);
	}

M_MODEL& orbtArticlenum(const std::string &val)
	{return whereOr(B_BASE::cols::articlenum, orm::wq::bt, val);
	}

M_MODEL& orbeArticlenum(const std::string &val)
	{return whereOr(B_BASE::cols::articlenum, orm::wq::be, val);
	}

M_MODEL& orltArticlenum(const std::string &val)
	{return whereOr(B_BASE::cols::articlenum, orm::wq::lt, val);
	}

M_MODEL& orleArticlenum(const std::string &val)
	{return whereOr(B_BASE::cols::articlenum, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqArticlenum(T val)
	{return whereOr(B_BASE::cols::articlenum, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqArticlenum(T val)
	{return whereOr(B_BASE::cols::articlenum, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtArticlenum(T val)
	{return whereOr(B_BASE::cols::articlenum, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeArticlenum(T val)
	{return whereOr(B_BASE::cols::articlenum, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltArticlenum(T val)
	{return whereOr(B_BASE::cols::articlenum, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleArticlenum(T val)
	{return whereOr(B_BASE::cols::articlenum, orm::wq::le, val);
	}

M_MODEL& ornullArticlenum()
	{return whereOrNull(B_BASE::cols::articlenum);
	}

M_MODEL& ornotnullArticlenum()
	{return whereOrNotNull(B_BASE::cols::articlenum);
	}

M_MODEL& eqLanguageid(const std::string &val)
	{return where(B_BASE::cols::languageid, orm::wq::eq, val);
	}

M_MODEL& nqLanguageid(const std::string &val)
	{return where(B_BASE::cols::languageid, orm::wq::nq, val);
	}

M_MODEL& btLanguageid(const std::string &val)
	{return where(B_BASE::cols::languageid, orm::wq::bt, val);
	}

M_MODEL& beLanguageid(const std::string &val)
	{return where(B_BASE::cols::languageid, orm::wq::be, val);
	}

M_MODEL& ltLanguageid(const std::string &val)
	{return where(B_BASE::cols::languageid, orm::wq::lt, val);
	}

M_MODEL& leLanguageid(const std::string &val)
	{return where(B_BASE::cols::languageid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqLanguageid(T val)
	{return where(B_BASE::cols::languageid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqLanguageid(T val)
	{return where(B_BASE::cols::languageid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btLanguageid(T val)
	{return where(B_BASE::cols::languageid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beLanguageid(T val)
	{return where(B_BASE::cols::languageid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltLanguageid(T val)
	{return where(B_BASE::cols::languageid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leLanguageid(T val)
	{return where(B_BASE::cols::languageid, orm::wq::le, val);
	}

M_MODEL& nullLanguageid()
	{return whereNull(B_BASE::cols::languageid);
	}

M_MODEL& notnullLanguageid()
	{return whereNotNull(B_BASE::cols::languageid);
	}

M_MODEL& oreqLanguageid(const std::string &val)
	{return whereOr(B_BASE::cols::languageid, orm::wq::eq, val);
	}

M_MODEL& ornqLanguageid(const std::string &val)
	{return whereOr(B_BASE::cols::languageid, orm::wq::nq, val);
	}

M_MODEL& orbtLanguageid(const std::string &val)
	{return whereOr(B_BASE::cols::languageid, orm::wq::bt, val);
	}

M_MODEL& orbeLanguageid(const std::string &val)
	{return whereOr(B_BASE::cols::languageid, orm::wq::be, val);
	}

M_MODEL& orltLanguageid(const std::string &val)
	{return whereOr(B_BASE::cols::languageid, orm::wq::lt, val);
	}

M_MODEL& orleLanguageid(const std::string &val)
	{return whereOr(B_BASE::cols::languageid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqLanguageid(T val)
	{return whereOr(B_BASE::cols::languageid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqLanguageid(T val)
	{return whereOr(B_BASE::cols::languageid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtLanguageid(T val)
	{return whereOr(B_BASE::cols::languageid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeLanguageid(T val)
	{return whereOr(B_BASE::cols::languageid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltLanguageid(T val)
	{return whereOr(B_BASE::cols::languageid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleLanguageid(T val)
	{return whereOr(B_BASE::cols::languageid, orm::wq::le, val);
	}

M_MODEL& ornullLanguageid()
	{return whereOrNull(B_BASE::cols::languageid);
	}

M_MODEL& ornotnullLanguageid()
	{return whereOrNotNull(B_BASE::cols::languageid);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = blogcatalog_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(blogcatalog_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(blogcatalog_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(blogcatalog_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(blogcatalog_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(blogcatalog_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(blogcatalog_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(blogcatalog_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(blogcatalog_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(blogcatalog_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(blogcatalog_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(blogcatalog_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(blogcatalog_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(blogcatalog_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(blogcatalog_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(blogcatalog_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(blogcatalog_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(blogcatalog_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(blogcatalog_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &order(blogcatalog_info::cols field, const std::string &asc_or_desc)
        {
			...
			case blogcatalog_info::cols::dateid:
				ordersql.append("dateid");
				break;
			case blogcatalog_info::cols::articlenum:
				ordersql.append("articlenum");
				break;
			case blogcatalog_info::cols::languageid:
				ordersql.append("languageid");
				break;
            default:
                return *mod;
                break;
        ...
        M_MODEL &asc(blogcatalog_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &blogcatalog_info::col_names> field1, orm::table_col<B_BASE, &blogcatalog_info::col_names> field2)
        {
        ...
        M_MODEL &asc(blogcatalog_info::cols field, blogcatalog_info::cols field2)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(blogcatalog_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(blogcatalog_info::cols field, blogcatalog_info::cols field2)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &blogcatalog_info::col_names> field1, orm::table_col<B_BASE, &blogcatalog_info::col_names> field2)
        {
        ...
        M_MODEL &order(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq, const std::string &asc_or_desc)
        {
            ordersql.append(" ORDER BY ");
            ordersql.append(wq);
            ordersql.append(" ");
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq)
        {
        ...
        M_MODEL &having(blogcatalog_info::cols field)
        {
            groupsql.append(" HAVING ");
            switch (field)
            {
			...
			case blogcatalog_info::cols::userid:
				...
				break;
			case blogcatalog_info::cols::dateid:
				groupsql.append("dateid");
				break;
			case blogcatalog_info::cols::articlenum:
				groupsql.append("articlenum");
				break;
			case blogcatalog_info::cols::languageid:
				groupsql.append("languageid");
				break;
            default:
                return *mod;
                break;
        ...
        M_MODEL &group(blogcatalog_info::cols field)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &blogcatalog_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
            ...
            try
            {
                ...
                unsigned int fetch_count = select_conn->fetch_directly(sqlstring,
                                                                       [this, col_pos_map = std::vector<int>{}, first_row = true](int col_count, char **col_names, auto get_data) mutable -> bool
                                                                       {
                                                                           ...
                                                                           blogcatalog_info::meta data_temp;
                                                                           for (int ij = 0; ij < col_count; ij++)
                                                                           {
                                                                               auto [ptr, len] = get_data(ij);
                                                                               if (ptr == nullptr)
        ...
        {
            model_meta_cache<blogcatalog_info::meta> &data_cache = model_meta_cache<blogcatalog_info::meta>::getinstance();
            data_cache.remove_exptime();

            model_meta_cache<std::vector<blogcatalog_info::meta>> &record_cache = model_meta_cache<std::vector<blogcatalog_info::meta>>::getinstance();
        ...
        {
            model_meta_cache<blogcatalog_info::meta> &temp_cache = model_meta_cache<blogcatalog_info::meta>::getinstance();
        ...
        {
            model_meta_cache<std::vector<blogcatalog_info::meta>> &temp_cache = model_meta_cache<std::vector<blogcatalog_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(sqlstring);
        ...
        bool save_data_cache(const std::string &cache_key_name, const blogcatalog_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<blogcatalog_info::meta> &temp_cache = model_meta_cache<blogcatalog_info::meta>::getinstance();
        ...
        bool save_cache(std::size_t cache_key_name, const std::vector<blogcatalog_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<blogcatalog_info::meta>> &temp_cache = model_meta_cache<std::vector<blogcatalog_info::meta>>::getinstance();
            temp_cache.save(cache_key_name, cache_data, exp_time);
            return true;
        }
        bool save_cache(const std::string cache_key_name, const std::vector<blogcatalog_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<blogcatalog_info::meta>> &temp_cache = model_meta_cache<std::vector<blogcatalog_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_vector_cache(const std::string cache_key_name, const std::vector<blogcatalog_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<blogcatalog_info::meta>> &temp_cache = model_meta_cache<std::vector<blogcatalog_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_cache(const std::string cache_key_name, const blogcatalog_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<blogcatalog_info::meta> &temp_cache = model_meta_cache<blogcatalog_info::meta>::getinstance();
            std::size_t sqlhashid                               = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        const blogcatalog_info::meta get_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<blogcatalog_info::meta> &temp_cache = model_meta_cache<blogcatalog_info::meta>::getinstance();
        ...
        const std::vector<blogcatalog_info::meta> get_vector_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<std::vector<blogcatalog_info::meta>> &temp_cache = model_meta_cache<std::vector<blogcatalog_info::meta>>::getinstance();
        ...
        std::tuple<unsigned int, unsigned long long> insert(blogcatalog_info::meta &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(blogcatalog_info::meta &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_data_insert_sql(insert_data);
        ...
        std::tuple<unsigned int, unsigned long long> insert(std::vector<blogcatalog_info::meta> &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(std::vector<blogcatalog_info::meta> &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_vector_insert_sql(insert_data);
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = blogcatalog_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(blogcatalog_info::meta &insert_data)
        {
        ...
        M_MODEL &set_data(blogcatalog_info::meta indata)
        {
            B_BASE::data = indata;
            return *mod;
        }
        ...
        blogcatalog_opsql(const blogcatalog_opsql &)            = delete;
        blogcatalog_opsql &operator=(const blogcatalog_opsql &) = delete;
        blogcatalog_opsql(blogcatalog_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        blogcatalog_opsql &operator=(blogcatalog_opsql &&o) noexcept
        {
            if (this != &o)
            {
        ...
        template <typename T>
        M_MODEL &AND(orm::table_col<B_BASE, &blogcatalog_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = opwq;
        ...
        template <typename T>
        M_MODEL &OR(orm::table_col<B_BASE, &blogcatalog_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = opwq;
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < blogcatalog_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == blogcatalog_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(blogcatalog_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(blogcatalog_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = blogcatalog_info::col_names.size();
