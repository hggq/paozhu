...
{ /*tagnamespace_replace*/
    ...
    class productparam_opsql : public B_BASE
    {
      public:
        productparam_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        productparam_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, productparam_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            break;
                case 5:
            data_temp.attachfiles.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 6:
            data_temp.name.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 7:
            data_temp.attachdate.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 8:
             {
               data_temp.sortid = 0;
            ...
            break;
                case 10:
            data_temp.fileext.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                
        }
    }
    
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, productparam_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.ppid = static_cast<decltype(data_temp.ppid )>(v);
            ...
            case 5: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.attachfiles.assign(v.data(), v.size());
                    else
                        data_temp.attachfiles.clear();
                }, val);
            } break;
            ...
            case 7: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.attachdate.assign(v.data(), v.size());
                    else
                        data_temp.attachdate.clear();
                }, val);
            } break;
            ...
            case 9: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.filesize = static_cast<decltype(data_temp.filesize )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.filesize = static_cast<decltype(data_temp.filesize )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.filesize = static_cast<decltype(data_temp.filesize )>(v);
            ...
            case 10: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.fileext.assign(v.data(), v.size());
                    else
                        data_temp.fileext.clear();
                }, val);
            } break;
        ...
        http::obj_val get_field_value(unsigned char index_pos, const productparam_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
                return http::obj_val(static_cast<long long>(data_temp.ppid));
                break;
            case 1:
            ...
            case 4:
                ...
                break;
            case 5:
                return http::obj_val(std::string(data_temp.attachfiles));
                break;
            case 6:
                return http::obj_val(std::string(data_temp.name));
                break;
            case 7:
                return http::obj_val(std::string(data_temp.attachdate));
                break;
            case 8:
                return http::obj_val(static_cast<long long>(data_temp.sortid));
                break;
            case 9:
                return http::obj_val(static_cast<long long>(data_temp.filesize));
                break;
            case 10:
                return http::obj_val(std::string(data_temp.fileext));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const productparam_info::meta &data_temp)
    {
        // Auto-increment primary key ppid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.ppid == 0)
            return http::obj_val(nullptr);
...
M_MODEL& eqPpid(const std::string &val)
	{return where(B_BASE::cols::ppid, orm::wq::eq, val);
	}

M_MODEL& nqPpid(const std::string &val)
	{return where(B_BASE::cols::ppid, orm::wq::nq, val);
	}

M_MODEL& btPpid(const std::string &val)
	{return where(B_BASE::cols::ppid, orm::wq::bt, val);
	}

M_MODEL& bePpid(const std::string &val)
	{return where(B_BASE::cols::ppid, orm::wq::be, val);
	}

M_MODEL& ltPpid(const std::string &val)
	{return where(B_BASE::cols::ppid, orm::wq::lt, val);
	}

M_MODEL& lePpid(const std::string &val)
	{return where(B_BASE::cols::ppid, orm::wq::le, val);
	}
...
M_MODEL& nullPpid()
	{return whereNull(B_BASE::cols::ppid);
	}

M_MODEL& notnullPpid()
	{return whereNotNull(B_BASE::cols::ppid);
	}

M_MODEL& oreqPpid(const std::string &val)
	{return whereOr(B_BASE::cols::ppid, orm::wq::eq, val);
	}

M_MODEL& ornqPpid(const std::string &val)
	{return whereOr(B_BASE::cols::ppid, orm::wq::nq, val);
	}

M_MODEL& orbtPpid(const std::string &val)
	{return whereOr(B_BASE::cols::ppid, orm::wq::bt, val);
	}

M_MODEL& orbePpid(const std::string &val)
	{return whereOr(B_BASE::cols::ppid, orm::wq::be, val);
	}

M_MODEL& orltPpid(const std::string &val)
	{return whereOr(B_BASE::cols::ppid, orm::wq::lt, val);
	}

M_MODEL& orlePpid(const std::string &val)
	{return whereOr(B_BASE::cols::ppid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqPpid(T val)
	{return whereOr(B_BASE::cols::ppid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqPpid(T val)
	{return whereOr(B_BASE::cols::ppid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtPpid(T val)
	{return whereOr(B_BASE::cols::ppid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbePpid(T val)
	{return whereOr(B_BASE::cols::ppid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltPpid(T val)
	{return whereOr(B_BASE::cols::ppid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orlePpid(T val)
	{return whereOr(B_BASE::cols::ppid, orm::wq::le, val);
	}

M_MODEL& ornullPpid()
	{return whereOrNull(B_BASE::cols::ppid);
	}

M_MODEL& ornotnullPpid()
	{return whereOrNotNull(B_BASE::cols::ppid);
	}
...
M_MODEL& eqAttachfiles(const std::string &val)
	{return where(B_BASE::cols::attachfiles, orm::wq::eq, val);
	}

M_MODEL& nqAttachfiles(const std::string &val)
	{return where(B_BASE::cols::attachfiles, orm::wq::nq, val);
	}

M_MODEL& btAttachfiles(const std::string &val)
	{return where(B_BASE::cols::attachfiles, orm::wq::bt, val);
	}

M_MODEL& beAttachfiles(const std::string &val)
	{return where(B_BASE::cols::attachfiles, orm::wq::be, val);
	}

M_MODEL& ltAttachfiles(const std::string &val)
	{return where(B_BASE::cols::attachfiles, orm::wq::lt, val);
	}

M_MODEL& leAttachfiles(const std::string &val)
	{return where(B_BASE::cols::attachfiles, orm::wq::le, val);
	}

M_MODEL& likeAttachfiles(const std::string &val)
	{return where(B_BASE::cols::attachfiles, orm::wq::like, val);
	}

M_MODEL& nullAttachfiles()
	{return whereNull(B_BASE::cols::attachfiles);
	}

M_MODEL& notnullAttachfiles()
	{return whereNotNull(B_BASE::cols::attachfiles);
	}

M_MODEL& oreqAttachfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attachfiles, orm::wq::eq, val);
	}

M_MODEL& ornqAttachfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attachfiles, orm::wq::nq, val);
	}

M_MODEL& orbtAttachfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attachfiles, orm::wq::bt, val);
	}

M_MODEL& orbeAttachfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attachfiles, orm::wq::be, val);
	}

M_MODEL& orltAttachfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attachfiles, orm::wq::lt, val);
	}

M_MODEL& orleAttachfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attachfiles, orm::wq::le, val);
	}

M_MODEL& orlikeAttachfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attachfiles, orm::wq::like, val);
	}

M_MODEL& ornullAttachfiles()
	{return whereOrNull(B_BASE::cols::attachfiles);
	}

M_MODEL& ornotnullAttachfiles()
	{return whereOrNotNull(B_BASE::cols::attachfiles);
	}
...
M_MODEL& eqAttachdate(const std::string &val)
	{return where(B_BASE::cols::attachdate, orm::wq::eq, val);
	}

M_MODEL& nqAttachdate(const std::string &val)
	{return where(B_BASE::cols::attachdate, orm::wq::nq, val);
	}

M_MODEL& btAttachdate(const std::string &val)
	{return where(B_BASE::cols::attachdate, orm::wq::bt, val);
	}

M_MODEL& beAttachdate(const std::string &val)
	{return where(B_BASE::cols::attachdate, orm::wq::be, val);
	}

M_MODEL& ltAttachdate(const std::string &val)
	{return where(B_BASE::cols::attachdate, orm::wq::lt, val);
	}

M_MODEL& leAttachdate(const std::string &val)
	{return where(B_BASE::cols::attachdate, orm::wq::le, val);
	}

M_MODEL& likeAttachdate(const std::string &val)
	{return where(B_BASE::cols::attachdate, orm::wq::like, val);
	}

M_MODEL& nullAttachdate()
	{return whereNull(B_BASE::cols::attachdate);
	}

M_MODEL& notnullAttachdate()
	{return whereNotNull(B_BASE::cols::attachdate);
	}

M_MODEL& oreqAttachdate(const std::string &val)
	{return whereOr(B_BASE::cols::attachdate, orm::wq::eq, val);
	}

M_MODEL& ornqAttachdate(const std::string &val)
	{return whereOr(B_BASE::cols::attachdate, orm::wq::nq, val);
	}

M_MODEL& orbtAttachdate(const std::string &val)
	{return whereOr(B_BASE::cols::attachdate, orm::wq::bt, val);
	}

M_MODEL& orbeAttachdate(const std::string &val)
	{return whereOr(B_BASE::cols::attachdate, orm::wq::be, val);
	}

M_MODEL& orltAttachdate(const std::string &val)
	{return whereOr(B_BASE::cols::attachdate, orm::wq::lt, val);
	}

M_MODEL& orleAttachdate(const std::string &val)
	{return whereOr(B_BASE::cols::attachdate, orm::wq::le, val);
	}

M_MODEL& orlikeAttachdate(const std::string &val)
	{return whereOr(B_BASE::cols::attachdate, orm::wq::like, val);
	}

M_MODEL& ornullAttachdate()
	{return whereOrNull(B_BASE::cols::attachdate);
	}

M_MODEL& ornotnullAttachdate()
	{return whereOrNotNull(B_BASE::cols::attachdate);
	}
...
M_MODEL& eqFilesize(const std::string &val)
	{return where(B_BASE::cols::filesize, orm::wq::eq, val);
	}

M_MODEL& nqFilesize(const std::string &val)
	{return where(B_BASE::cols::filesize, orm::wq::nq, val);
	}

M_MODEL& btFilesize(const std::string &val)
	{return where(B_BASE::cols::filesize, orm::wq::bt, val);
	}

M_MODEL& beFilesize(const std::string &val)
	{return where(B_BASE::cols::filesize, orm::wq::be, val);
	}

M_MODEL& ltFilesize(const std::string &val)
	{return where(B_BASE::cols::filesize, orm::wq::lt, val);
	}

M_MODEL& leFilesize(const std::string &val)
	{return where(B_BASE::cols::filesize, orm::wq::le, val);
	}
...
M_MODEL& nullFilesize()
	{return whereNull(B_BASE::cols::filesize);
	}

M_MODEL& notnullFilesize()
	{return whereNotNull(B_BASE::cols::filesize);
	}

M_MODEL& oreqFilesize(const std::string &val)
	{return whereOr(B_BASE::cols::filesize, orm::wq::eq, val);
	}

M_MODEL& ornqFilesize(const std::string &val)
	{return whereOr(B_BASE::cols::filesize, orm::wq::nq, val);
	}

M_MODEL& orbtFilesize(const std::string &val)
	{return whereOr(B_BASE::cols::filesize, orm::wq::bt, val);
	}

M_MODEL& orbeFilesize(const std::string &val)
	{return whereOr(B_BASE::cols::filesize, orm::wq::be, val);
	}

M_MODEL& orltFilesize(const std::string &val)
	{return whereOr(B_BASE::cols::filesize, orm::wq::lt, val);
	}

M_MODEL& orleFilesize(const std::string &val)
	{return whereOr(B_BASE::cols::filesize, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqFilesize(T val)
	{return whereOr(B_BASE::cols::filesize, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqFilesize(T val)
	{return whereOr(B_BASE::cols::filesize, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtFilesize(T val)
	{return whereOr(B_BASE::cols::filesize, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeFilesize(T val)
	{return whereOr(B_BASE::cols::filesize, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltFilesize(T val)
	{return whereOr(B_BASE::cols::filesize, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleFilesize(T val)
	{return whereOr(B_BASE::cols::filesize, orm::wq::le, val);
	}

M_MODEL& ornullFilesize()
	{return whereOrNull(B_BASE::cols::filesize);
	}

M_MODEL& ornotnullFilesize()
	{return whereOrNotNull(B_BASE::cols::filesize);
	}

M_MODEL& eqFileext(const std::string &val)
	{return where(B_BASE::cols::fileext, orm::wq::eq, val);
	}

M_MODEL& nqFileext(const std::string &val)
	{return where(B_BASE::cols::fileext, orm::wq::nq, val);
	}

M_MODEL& btFileext(const std::string &val)
	{return where(B_BASE::cols::fileext, orm::wq::bt, val);
	}

M_MODEL& beFileext(const std::string &val)
	{return where(B_BASE::cols::fileext, orm::wq::be, val);
	}

M_MODEL& ltFileext(const std::string &val)
	{return where(B_BASE::cols::fileext, orm::wq::lt, val);
	}

M_MODEL& leFileext(const std::string &val)
	{return where(B_BASE::cols::fileext, orm::wq::le, val);
	}

M_MODEL& likeFileext(const std::string &val)
	{return where(B_BASE::cols::fileext, orm::wq::like, val);
	}

M_MODEL& nullFileext()
	{return whereNull(B_BASE::cols::fileext);
	}

M_MODEL& notnullFileext()
	{return whereNotNull(B_BASE::cols::fileext);
	}

M_MODEL& oreqFileext(const std::string &val)
	{return whereOr(B_BASE::cols::fileext, orm::wq::eq, val);
	}

M_MODEL& ornqFileext(const std::string &val)
	{return whereOr(B_BASE::cols::fileext, orm::wq::nq, val);
	}

M_MODEL& orbtFileext(const std::string &val)
	{return whereOr(B_BASE::cols::fileext, orm::wq::bt, val);
	}

M_MODEL& orbeFileext(const std::string &val)
	{return whereOr(B_BASE::cols::fileext, orm::wq::be, val);
	}

M_MODEL& orltFileext(const std::string &val)
	{return whereOr(B_BASE::cols::fileext, orm::wq::lt, val);
	}

M_MODEL& orleFileext(const std::string &val)
	{return whereOr(B_BASE::cols::fileext, orm::wq::le, val);
	}

M_MODEL& orlikeFileext(const std::string &val)
	{return whereOr(B_BASE::cols::fileext, orm::wq::like, val);
	}

M_MODEL& ornullFileext()
	{return whereOrNull(B_BASE::cols::fileext);
	}

M_MODEL& ornotnullFileext()
	{return whereOrNotNull(B_BASE::cols::fileext);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = productparam_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &productparam_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &productparam_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(productparam_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(productparam_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(productparam_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(productparam_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &productparam_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &productparam_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &productparam_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &productparam_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &productparam_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(productparam_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &productparam_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(productparam_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &productparam_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(productparam_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &productparam_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(productparam_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &productparam_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(productparam_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(productparam_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(productparam_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(productparam_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(productparam_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(productparam_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(productparam_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(productparam_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(productparam_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(productparam_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &productparam_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &productparam_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &order(productparam_info::cols field, const std::string &asc_or_desc)
        {
        ...
        M_MODEL &asc(productparam_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(productparam_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &productparam_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &productparam_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &productparam_info::col_names> wq)
        {
        ...
        M_MODEL &having(productparam_info::cols field)
        {
            groupsql.append(" HAVING ");
            switch (field)
            {
			...
			case productparam_info::cols::price:
				...
				break;
			case productparam_info::cols::attachfiles:
				groupsql.append("attachfiles");
				break;
			case productparam_info::cols::name:
				groupsql.append("name");
				break;
			case productparam_info::cols::attachdate:
				groupsql.append("attachdate");
				break;
			case productparam_info::cols::sortid:
				groupsql.append("sortid");
				break;
			case productparam_info::cols::filesize:
				groupsql.append("filesize");
				break;
			case productparam_info::cols::fileext:
				groupsql.append("fileext");
				break;
            default:
                return *mod;
                break;
        ...
        M_MODEL &group(productparam_info::cols field)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &productparam_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
            ...
            try
            {
                ...
                unsigned int fetch_count = select_conn->fetch_directly(sqlstring,
                                                                       [this, col_pos_map = std::vector<int>{}, first_row = true](int col_count, char **col_names, auto get_data) mutable -> bool
                                                                       {
                                                                           ...
                                                                           productparam_info::meta data_temp;
                                                                           for (int ij = 0; ij < col_count; ij++)
                                                                           {
                                                                               auto [ptr, len] = get_data(ij);
                                                                               if (ptr == nullptr)
        ...
        {
            model_meta_cache<productparam_info::meta> &temp_cache = model_meta_cache<productparam_info::meta>::getinstance();
        ...
        {
            model_meta_cache<std::vector<productparam_info::meta>> &temp_cache = model_meta_cache<std::vector<productparam_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(sqlstring);
        ...
        bool save_data_cache(const std::string &cache_key_name, const productparam_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<productparam_info::meta> &temp_cache = model_meta_cache<productparam_info::meta>::getinstance();
        ...
        bool save_cache(std::size_t cache_key_name, const std::vector<productparam_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<productparam_info::meta>> &temp_cache = model_meta_cache<std::vector<productparam_info::meta>>::getinstance();
            temp_cache.save(cache_key_name, cache_data, exp_time);
            return true;
        }
        bool save_cache(const std::string cache_key_name, const std::vector<productparam_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<productparam_info::meta>> &temp_cache = model_meta_cache<std::vector<productparam_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_vector_cache(const std::string cache_key_name, const std::vector<productparam_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<productparam_info::meta>> &temp_cache = model_meta_cache<std::vector<productparam_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_cache(const std::string cache_key_name, const productparam_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<productparam_info::meta> &temp_cache = model_meta_cache<productparam_info::meta>::getinstance();
            std::size_t sqlhashid                               = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        const productparam_info::meta get_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<productparam_info::meta> &temp_cache = model_meta_cache<productparam_info::meta>::getinstance();
        ...
        const std::vector<productparam_info::meta> get_vector_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<std::vector<productparam_info::meta>> &temp_cache = model_meta_cache<std::vector<productparam_info::meta>>::getinstance();
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = productparam_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(productparam_info::meta &insert_data)
        {
        ...
        M_MODEL &set_data(productparam_info::meta indata)
        {
            B_BASE::data = indata;
            return *mod;
        }
        ...
        productparam_opsql(const productparam_opsql &)            = delete;
        productparam_opsql &operator=(const productparam_opsql &) = delete;
        productparam_opsql(productparam_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        productparam_opsql &operator=(productparam_opsql &&o) noexcept
        {
            if (this != &o)
            {
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < productparam_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == productparam_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(productparam_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(productparam_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = productparam_info::col_names.size();
