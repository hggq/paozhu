...
{ /*tagnamespace_replace*/
    ...
    class product_opsql : public B_BASE
    {
      public:
        product_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        product_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, product_info::meta &data_temp)
    {
        switch(index_pos)
        {
            ...
            break;
                case 10:
            data_temp.sntype.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 11:
            ...
            break;
                case 14:
            data_temp.listimg.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 15:
            data_temp.bigimg.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 16:
            data_temp.maincontent.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 17:
            data_temp.paracontent.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 18:
            data_temp.samepro.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 19:
            data_temp.attatchfiles.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 20:
             {
               data_temp.price = 0;
            ...
            break;
                case 22:
            data_temp.adddate.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 23:
            data_temp.editdate.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                
        }
    }
    
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, product_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.pid = static_cast<decltype(data_temp.pid )>(v);
            ...
            case 3: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.bigid = static_cast<decltype(data_temp.bigid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.bigid = static_cast<decltype(data_temp.bigid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.bigid = static_cast<decltype(data_temp.bigid )>(v);
            ...
            case 4: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.smallid = static_cast<decltype(data_temp.smallid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.smallid = static_cast<decltype(data_temp.smallid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.smallid = static_cast<decltype(data_temp.smallid )>(v);
            ...
            case 5: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.brandid = static_cast<decltype(data_temp.brandid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.brandid = static_cast<decltype(data_temp.brandid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.brandid = static_cast<decltype(data_temp.brandid )>(v);
            ...
            case 6: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.isview = static_cast<decltype(data_temp.isview )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.isview = static_cast<decltype(data_temp.isview )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.isview = static_cast<decltype(data_temp.isview )>(v);
            ...
            case 7: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.isstore = static_cast<decltype(data_temp.isstore )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.isstore = static_cast<decltype(data_temp.isstore )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.isstore = static_cast<decltype(data_temp.isstore )>(v);
            ...
            case 10: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.sntype.assign(v.data(), v.size());
                    else
                        data_temp.sntype.clear();
                }, val);
            } break;
            case 11: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.name.assign(v.data(), v.size());
                    else
                        data_temp.name.clear();
                }, val);
            } break;
            ...
            case 14: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.listimg.assign(v.data(), v.size());
                    else
                        data_temp.listimg.clear();
                }, val);
            } break;
            case 15: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.bigimg.assign(v.data(), v.size());
                    else
                        data_temp.bigimg.clear();
                }, val);
            } break;
            case 16: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.maincontent.assign(v.data(), v.size());
                    else
                        data_temp.maincontent.clear();
                }, val);
            } break;
            case 17: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.paracontent.assign(v.data(), v.size());
                    else
                        data_temp.paracontent.clear();
                }, val);
            } break;
            case 18: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.samepro.assign(v.data(), v.size());
                    else
                        data_temp.samepro.clear();
                }, val);
            } break;
            case 19: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.attatchfiles.assign(v.data(), v.size());
                    else
                        data_temp.attatchfiles.clear();
                }, val);
            } break;
            case 20: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.price = static_cast<decltype(data_temp.price )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.price = static_cast<decltype(data_temp.price )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.price = static_cast<decltype(data_temp.price )>(v);
            ...
            case 22: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.adddate.assign(v.data(), v.size());
                    else
                        data_temp.adddate.clear();
                }, val);
            } break;
            case 23: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.editdate.assign(v.data(), v.size());
                    else
                        data_temp.editdate.clear();
                }, val);
            } break;
        ...
        http::obj_val get_field_value(unsigned char index_pos, const product_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
                return http::obj_val(static_cast<long long>(data_temp.pid));
                break;
            case 1:
            ...
            case 2:
                ...
                break;
            case 3:
                return http::obj_val(static_cast<long long>(data_temp.bigid));
                break;
            case 4:
                return http::obj_val(static_cast<long long>(data_temp.smallid));
                break;
            case 5:
                return http::obj_val(static_cast<long long>(data_temp.brandid));
                break;
            case 6:
                return http::obj_val(static_cast<long long>(data_temp.isview));
                break;
            case 7:
                return http::obj_val(static_cast<long long>(data_temp.isstore));
                break;
            case 8:
            ...
            case 9:
                ...
                break;
            case 10:
                return http::obj_val(std::string(data_temp.sntype));
                break;
            case 11:
                return http::obj_val(std::string(data_temp.name));
                break;
            case 12:
            ...
            case 13:
                ...
                break;
            case 14:
                return http::obj_val(std::string(data_temp.listimg));
                break;
            case 15:
                return http::obj_val(std::string(data_temp.bigimg));
                break;
            case 16:
                return http::obj_val(std::string(data_temp.maincontent));
                break;
            case 17:
                return http::obj_val(std::string(data_temp.paracontent));
                break;
            case 18:
                return http::obj_val(std::string(data_temp.samepro));
                break;
            case 19:
                return http::obj_val(std::string(data_temp.attatchfiles));
                break;
            case 20:
                return http::obj_val(static_cast<long long>(data_temp.price));
                break;
            case 21:
                return http::obj_val(static_cast<long long>(data_temp.sortid));
                break;
            case 22:
                return http::obj_val(std::string(data_temp.adddate));
                break;
            case 23:
                return http::obj_val(std::string(data_temp.editdate));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const product_info::meta &data_temp)
    {
        // Auto-increment primary key pid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.pid == 0)
            return http::obj_val(nullptr);
...
M_MODEL& eqPid(const std::string &val)
	{return where(B_BASE::cols::pid, orm::wq::eq, val);
	}

M_MODEL& nqPid(const std::string &val)
	{return where(B_BASE::cols::pid, orm::wq::nq, val);
	}

M_MODEL& btPid(const std::string &val)
	{return where(B_BASE::cols::pid, orm::wq::bt, val);
	}

M_MODEL& bePid(const std::string &val)
	{return where(B_BASE::cols::pid, orm::wq::be, val);
	}

M_MODEL& ltPid(const std::string &val)
	{return where(B_BASE::cols::pid, orm::wq::lt, val);
	}

M_MODEL& lePid(const std::string &val)
	{return where(B_BASE::cols::pid, orm::wq::le, val);
	}
...
M_MODEL& nullPid()
	{return whereNull(B_BASE::cols::pid);
	}

M_MODEL& notnullPid()
	{return whereNotNull(B_BASE::cols::pid);
	}

M_MODEL& oreqPid(const std::string &val)
	{return whereOr(B_BASE::cols::pid, orm::wq::eq, val);
	}

M_MODEL& ornqPid(const std::string &val)
	{return whereOr(B_BASE::cols::pid, orm::wq::nq, val);
	}

M_MODEL& orbtPid(const std::string &val)
	{return whereOr(B_BASE::cols::pid, orm::wq::bt, val);
	}

M_MODEL& orbePid(const std::string &val)
	{return whereOr(B_BASE::cols::pid, orm::wq::be, val);
	}

M_MODEL& orltPid(const std::string &val)
	{return whereOr(B_BASE::cols::pid, orm::wq::lt, val);
	}

M_MODEL& orlePid(const std::string &val)
	{return whereOr(B_BASE::cols::pid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqPid(T val)
	{return whereOr(B_BASE::cols::pid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqPid(T val)
	{return whereOr(B_BASE::cols::pid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtPid(T val)
	{return whereOr(B_BASE::cols::pid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbePid(T val)
	{return whereOr(B_BASE::cols::pid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltPid(T val)
	{return whereOr(B_BASE::cols::pid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orlePid(T val)
	{return whereOr(B_BASE::cols::pid, orm::wq::le, val);
	}

M_MODEL& ornullPid()
	{return whereOrNull(B_BASE::cols::pid);
	}

M_MODEL& ornotnullPid()
	{return whereOrNotNull(B_BASE::cols::pid);
	}
...
M_MODEL& eqBigid(const std::string &val)
	{return where(B_BASE::cols::bigid, orm::wq::eq, val);
	}

M_MODEL& nqBigid(const std::string &val)
	{return where(B_BASE::cols::bigid, orm::wq::nq, val);
	}

M_MODEL& btBigid(const std::string &val)
	{return where(B_BASE::cols::bigid, orm::wq::bt, val);
	}

M_MODEL& beBigid(const std::string &val)
	{return where(B_BASE::cols::bigid, orm::wq::be, val);
	}

M_MODEL& ltBigid(const std::string &val)
	{return where(B_BASE::cols::bigid, orm::wq::lt, val);
	}

M_MODEL& leBigid(const std::string &val)
	{return where(B_BASE::cols::bigid, orm::wq::le, val);
	}
...
M_MODEL& nullBigid()
	{return whereNull(B_BASE::cols::bigid);
	}

M_MODEL& notnullBigid()
	{return whereNotNull(B_BASE::cols::bigid);
	}

M_MODEL& oreqBigid(const std::string &val)
	{return whereOr(B_BASE::cols::bigid, orm::wq::eq, val);
	}

M_MODEL& ornqBigid(const std::string &val)
	{return whereOr(B_BASE::cols::bigid, orm::wq::nq, val);
	}

M_MODEL& orbtBigid(const std::string &val)
	{return whereOr(B_BASE::cols::bigid, orm::wq::bt, val);
	}

M_MODEL& orbeBigid(const std::string &val)
	{return whereOr(B_BASE::cols::bigid, orm::wq::be, val);
	}

M_MODEL& orltBigid(const std::string &val)
	{return whereOr(B_BASE::cols::bigid, orm::wq::lt, val);
	}

M_MODEL& orleBigid(const std::string &val)
	{return whereOr(B_BASE::cols::bigid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqBigid(T val)
	{return whereOr(B_BASE::cols::bigid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqBigid(T val)
	{return whereOr(B_BASE::cols::bigid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtBigid(T val)
	{return whereOr(B_BASE::cols::bigid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeBigid(T val)
	{return whereOr(B_BASE::cols::bigid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltBigid(T val)
	{return whereOr(B_BASE::cols::bigid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleBigid(T val)
	{return whereOr(B_BASE::cols::bigid, orm::wq::le, val);
	}

M_MODEL& ornullBigid()
	{return whereOrNull(B_BASE::cols::bigid);
	}

M_MODEL& ornotnullBigid()
	{return whereOrNotNull(B_BASE::cols::bigid);
	}

M_MODEL& eqSmallid(const std::string &val)
	{return where(B_BASE::cols::smallid, orm::wq::eq, val);
	}

M_MODEL& nqSmallid(const std::string &val)
	{return where(B_BASE::cols::smallid, orm::wq::nq, val);
	}

M_MODEL& btSmallid(const std::string &val)
	{return where(B_BASE::cols::smallid, orm::wq::bt, val);
	}

M_MODEL& beSmallid(const std::string &val)
	{return where(B_BASE::cols::smallid, orm::wq::be, val);
	}

M_MODEL& ltSmallid(const std::string &val)
	{return where(B_BASE::cols::smallid, orm::wq::lt, val);
	}

M_MODEL& leSmallid(const std::string &val)
	{return where(B_BASE::cols::smallid, orm::wq::le, val);
	}
...
M_MODEL& nullSmallid()
	{return whereNull(B_BASE::cols::smallid);
	}

M_MODEL& notnullSmallid()
	{return whereNotNull(B_BASE::cols::smallid);
	}

M_MODEL& oreqSmallid(const std::string &val)
	{return whereOr(B_BASE::cols::smallid, orm::wq::eq, val);
	}

M_MODEL& ornqSmallid(const std::string &val)
	{return whereOr(B_BASE::cols::smallid, orm::wq::nq, val);
	}

M_MODEL& orbtSmallid(const std::string &val)
	{return whereOr(B_BASE::cols::smallid, orm::wq::bt, val);
	}

M_MODEL& orbeSmallid(const std::string &val)
	{return whereOr(B_BASE::cols::smallid, orm::wq::be, val);
	}

M_MODEL& orltSmallid(const std::string &val)
	{return whereOr(B_BASE::cols::smallid, orm::wq::lt, val);
	}

M_MODEL& orleSmallid(const std::string &val)
	{return whereOr(B_BASE::cols::smallid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqSmallid(T val)
	{return whereOr(B_BASE::cols::smallid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqSmallid(T val)
	{return whereOr(B_BASE::cols::smallid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtSmallid(T val)
	{return whereOr(B_BASE::cols::smallid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeSmallid(T val)
	{return whereOr(B_BASE::cols::smallid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltSmallid(T val)
	{return whereOr(B_BASE::cols::smallid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleSmallid(T val)
	{return whereOr(B_BASE::cols::smallid, orm::wq::le, val);
	}

M_MODEL& ornullSmallid()
	{return whereOrNull(B_BASE::cols::smallid);
	}

M_MODEL& ornotnullSmallid()
	{return whereOrNotNull(B_BASE::cols::smallid);
	}

M_MODEL& eqBrandid(const std::string &val)
	{return where(B_BASE::cols::brandid, orm::wq::eq, val);
	}

M_MODEL& nqBrandid(const std::string &val)
	{return where(B_BASE::cols::brandid, orm::wq::nq, val);
	}

M_MODEL& btBrandid(const std::string &val)
	{return where(B_BASE::cols::brandid, orm::wq::bt, val);
	}

M_MODEL& beBrandid(const std::string &val)
	{return where(B_BASE::cols::brandid, orm::wq::be, val);
	}

M_MODEL& ltBrandid(const std::string &val)
	{return where(B_BASE::cols::brandid, orm::wq::lt, val);
	}

M_MODEL& leBrandid(const std::string &val)
	{return where(B_BASE::cols::brandid, orm::wq::le, val);
	}
...
M_MODEL& nullBrandid()
	{return whereNull(B_BASE::cols::brandid);
	}

M_MODEL& notnullBrandid()
	{return whereNotNull(B_BASE::cols::brandid);
	}

M_MODEL& oreqBrandid(const std::string &val)
	{return whereOr(B_BASE::cols::brandid, orm::wq::eq, val);
	}

M_MODEL& ornqBrandid(const std::string &val)
	{return whereOr(B_BASE::cols::brandid, orm::wq::nq, val);
	}

M_MODEL& orbtBrandid(const std::string &val)
	{return whereOr(B_BASE::cols::brandid, orm::wq::bt, val);
	}

M_MODEL& orbeBrandid(const std::string &val)
	{return whereOr(B_BASE::cols::brandid, orm::wq::be, val);
	}

M_MODEL& orltBrandid(const std::string &val)
	{return whereOr(B_BASE::cols::brandid, orm::wq::lt, val);
	}

M_MODEL& orleBrandid(const std::string &val)
	{return whereOr(B_BASE::cols::brandid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqBrandid(T val)
	{return whereOr(B_BASE::cols::brandid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqBrandid(T val)
	{return whereOr(B_BASE::cols::brandid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtBrandid(T val)
	{return whereOr(B_BASE::cols::brandid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeBrandid(T val)
	{return whereOr(B_BASE::cols::brandid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltBrandid(T val)
	{return whereOr(B_BASE::cols::brandid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleBrandid(T val)
	{return whereOr(B_BASE::cols::brandid, orm::wq::le, val);
	}

M_MODEL& ornullBrandid()
	{return whereOrNull(B_BASE::cols::brandid);
	}

M_MODEL& ornotnullBrandid()
	{return whereOrNotNull(B_BASE::cols::brandid);
	}

M_MODEL& eqIsview(const std::string &val)
	{return where(B_BASE::cols::isview, orm::wq::eq, val);
	}

M_MODEL& nqIsview(const std::string &val)
	{return where(B_BASE::cols::isview, orm::wq::nq, val);
	}

M_MODEL& btIsview(const std::string &val)
	{return where(B_BASE::cols::isview, orm::wq::bt, val);
	}

M_MODEL& beIsview(const std::string &val)
	{return where(B_BASE::cols::isview, orm::wq::be, val);
	}

M_MODEL& ltIsview(const std::string &val)
	{return where(B_BASE::cols::isview, orm::wq::lt, val);
	}

M_MODEL& leIsview(const std::string &val)
	{return where(B_BASE::cols::isview, orm::wq::le, val);
	}
...
M_MODEL& nullIsview()
	{return whereNull(B_BASE::cols::isview);
	}

M_MODEL& notnullIsview()
	{return whereNotNull(B_BASE::cols::isview);
	}

M_MODEL& oreqIsview(const std::string &val)
	{return whereOr(B_BASE::cols::isview, orm::wq::eq, val);
	}

M_MODEL& ornqIsview(const std::string &val)
	{return whereOr(B_BASE::cols::isview, orm::wq::nq, val);
	}

M_MODEL& orbtIsview(const std::string &val)
	{return whereOr(B_BASE::cols::isview, orm::wq::bt, val);
	}

M_MODEL& orbeIsview(const std::string &val)
	{return whereOr(B_BASE::cols::isview, orm::wq::be, val);
	}

M_MODEL& orltIsview(const std::string &val)
	{return whereOr(B_BASE::cols::isview, orm::wq::lt, val);
	}

M_MODEL& orleIsview(const std::string &val)
	{return whereOr(B_BASE::cols::isview, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqIsview(T val)
	{return whereOr(B_BASE::cols::isview, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqIsview(T val)
	{return whereOr(B_BASE::cols::isview, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtIsview(T val)
	{return whereOr(B_BASE::cols::isview, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeIsview(T val)
	{return whereOr(B_BASE::cols::isview, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltIsview(T val)
	{return whereOr(B_BASE::cols::isview, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleIsview(T val)
	{return whereOr(B_BASE::cols::isview, orm::wq::le, val);
	}

M_MODEL& ornullIsview()
	{return whereOrNull(B_BASE::cols::isview);
	}

M_MODEL& ornotnullIsview()
	{return whereOrNotNull(B_BASE::cols::isview);
	}

M_MODEL& eqIsstore(const std::string &val)
	{return where(B_BASE::cols::isstore, orm::wq::eq, val);
	}

M_MODEL& nqIsstore(const std::string &val)
	{return where(B_BASE::cols::isstore, orm::wq::nq, val);
	}

M_MODEL& btIsstore(const std::string &val)
	{return where(B_BASE::cols::isstore, orm::wq::bt, val);
	}

M_MODEL& beIsstore(const std::string &val)
	{return where(B_BASE::cols::isstore, orm::wq::be, val);
	}

M_MODEL& ltIsstore(const std::string &val)
	{return where(B_BASE::cols::isstore, orm::wq::lt, val);
	}

M_MODEL& leIsstore(const std::string &val)
	{return where(B_BASE::cols::isstore, orm::wq::le, val);
	}
...
M_MODEL& nullIsstore()
	{return whereNull(B_BASE::cols::isstore);
	}

M_MODEL& notnullIsstore()
	{return whereNotNull(B_BASE::cols::isstore);
	}

M_MODEL& oreqIsstore(const std::string &val)
	{return whereOr(B_BASE::cols::isstore, orm::wq::eq, val);
	}

M_MODEL& ornqIsstore(const std::string &val)
	{return whereOr(B_BASE::cols::isstore, orm::wq::nq, val);
	}

M_MODEL& orbtIsstore(const std::string &val)
	{return whereOr(B_BASE::cols::isstore, orm::wq::bt, val);
	}

M_MODEL& orbeIsstore(const std::string &val)
	{return whereOr(B_BASE::cols::isstore, orm::wq::be, val);
	}

M_MODEL& orltIsstore(const std::string &val)
	{return whereOr(B_BASE::cols::isstore, orm::wq::lt, val);
	}

M_MODEL& orleIsstore(const std::string &val)
	{return whereOr(B_BASE::cols::isstore, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqIsstore(T val)
	{return whereOr(B_BASE::cols::isstore, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqIsstore(T val)
	{return whereOr(B_BASE::cols::isstore, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtIsstore(T val)
	{return whereOr(B_BASE::cols::isstore, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeIsstore(T val)
	{return whereOr(B_BASE::cols::isstore, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltIsstore(T val)
	{return whereOr(B_BASE::cols::isstore, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleIsstore(T val)
	{return whereOr(B_BASE::cols::isstore, orm::wq::le, val);
	}

M_MODEL& ornullIsstore()
	{return whereOrNull(B_BASE::cols::isstore);
	}

M_MODEL& ornotnullIsstore()
	{return whereOrNotNull(B_BASE::cols::isstore);
	}
...
M_MODEL& eqSntype(const std::string &val)
	{return where(B_BASE::cols::sntype, orm::wq::eq, val);
	}

M_MODEL& nqSntype(const std::string &val)
	{return where(B_BASE::cols::sntype, orm::wq::nq, val);
	}

M_MODEL& btSntype(const std::string &val)
	{return where(B_BASE::cols::sntype, orm::wq::bt, val);
	}

M_MODEL& beSntype(const std::string &val)
	{return where(B_BASE::cols::sntype, orm::wq::be, val);
	}

M_MODEL& ltSntype(const std::string &val)
	{return where(B_BASE::cols::sntype, orm::wq::lt, val);
	}

M_MODEL& leSntype(const std::string &val)
	{return where(B_BASE::cols::sntype, orm::wq::le, val);
	}

M_MODEL& likeSntype(const std::string &val)
	{return where(B_BASE::cols::sntype, orm::wq::like, val);
	}

M_MODEL& nullSntype()
	{return whereNull(B_BASE::cols::sntype);
	}

M_MODEL& notnullSntype()
	{return whereNotNull(B_BASE::cols::sntype);
	}

M_MODEL& oreqSntype(const std::string &val)
	{return whereOr(B_BASE::cols::sntype, orm::wq::eq, val);
	}

M_MODEL& ornqSntype(const std::string &val)
	{return whereOr(B_BASE::cols::sntype, orm::wq::nq, val);
	}

M_MODEL& orbtSntype(const std::string &val)
	{return whereOr(B_BASE::cols::sntype, orm::wq::bt, val);
	}

M_MODEL& orbeSntype(const std::string &val)
	{return whereOr(B_BASE::cols::sntype, orm::wq::be, val);
	}

M_MODEL& orltSntype(const std::string &val)
	{return whereOr(B_BASE::cols::sntype, orm::wq::lt, val);
	}

M_MODEL& orleSntype(const std::string &val)
	{return whereOr(B_BASE::cols::sntype, orm::wq::le, val);
	}

M_MODEL& orlikeSntype(const std::string &val)
	{return whereOr(B_BASE::cols::sntype, orm::wq::like, val);
	}

M_MODEL& ornullSntype()
	{return whereOrNull(B_BASE::cols::sntype);
	}

M_MODEL& ornotnullSntype()
	{return whereOrNotNull(B_BASE::cols::sntype);
	}

M_MODEL& eqName(const std::string &val)
	{return where(B_BASE::cols::name, orm::wq::eq, val);
	}

M_MODEL& nqName(const std::string &val)
	{return where(B_BASE::cols::name, orm::wq::nq, val);
	}

M_MODEL& btName(const std::string &val)
	{return where(B_BASE::cols::name, orm::wq::bt, val);
	}

M_MODEL& beName(const std::string &val)
	{return where(B_BASE::cols::name, orm::wq::be, val);
	}

M_MODEL& ltName(const std::string &val)
	{return where(B_BASE::cols::name, orm::wq::lt, val);
	}

M_MODEL& leName(const std::string &val)
	{return where(B_BASE::cols::name, orm::wq::le, val);
	}

M_MODEL& likeName(const std::string &val)
	{return where(B_BASE::cols::name, orm::wq::like, val);
	}

M_MODEL& nullName()
	{return whereNull(B_BASE::cols::name);
	}

M_MODEL& notnullName()
	{return whereNotNull(B_BASE::cols::name);
	}

M_MODEL& oreqName(const std::string &val)
	{return whereOr(B_BASE::cols::name, orm::wq::eq, val);
	}

M_MODEL& ornqName(const std::string &val)
	{return whereOr(B_BASE::cols::name, orm::wq::nq, val);
	}

M_MODEL& orbtName(const std::string &val)
	{return whereOr(B_BASE::cols::name, orm::wq::bt, val);
	}

M_MODEL& orbeName(const std::string &val)
	{return whereOr(B_BASE::cols::name, orm::wq::be, val);
	}

M_MODEL& orltName(const std::string &val)
	{return whereOr(B_BASE::cols::name, orm::wq::lt, val);
	}

M_MODEL& orleName(const std::string &val)
	{return whereOr(B_BASE::cols::name, orm::wq::le, val);
	}

M_MODEL& orlikeName(const std::string &val)
	{return whereOr(B_BASE::cols::name, orm::wq::like, val);
	}

M_MODEL& ornullName()
	{return whereOrNull(B_BASE::cols::name);
	}

M_MODEL& ornotnullName()
...
M_MODEL& eqListimg(const std::string &val)
	{return where(B_BASE::cols::listimg, orm::wq::eq, val);
	}

M_MODEL& nqListimg(const std::string &val)
	{return where(B_BASE::cols::listimg, orm::wq::nq, val);
	}

M_MODEL& btListimg(const std::string &val)
	{return where(B_BASE::cols::listimg, orm::wq::bt, val);
	}

M_MODEL& beListimg(const std::string &val)
	{return where(B_BASE::cols::listimg, orm::wq::be, val);
	}

M_MODEL& ltListimg(const std::string &val)
	{return where(B_BASE::cols::listimg, orm::wq::lt, val);
	}

M_MODEL& leListimg(const std::string &val)
	{return where(B_BASE::cols::listimg, orm::wq::le, val);
	}

M_MODEL& likeListimg(const std::string &val)
	{return where(B_BASE::cols::listimg, orm::wq::like, val);
	}

M_MODEL& nullListimg()
	{return whereNull(B_BASE::cols::listimg);
	}

M_MODEL& notnullListimg()
	{return whereNotNull(B_BASE::cols::listimg);
	}

M_MODEL& oreqListimg(const std::string &val)
	{return whereOr(B_BASE::cols::listimg, orm::wq::eq, val);
	}

M_MODEL& ornqListimg(const std::string &val)
	{return whereOr(B_BASE::cols::listimg, orm::wq::nq, val);
	}

M_MODEL& orbtListimg(const std::string &val)
	{return whereOr(B_BASE::cols::listimg, orm::wq::bt, val);
	}

M_MODEL& orbeListimg(const std::string &val)
	{return whereOr(B_BASE::cols::listimg, orm::wq::be, val);
	}

M_MODEL& orltListimg(const std::string &val)
	{return whereOr(B_BASE::cols::listimg, orm::wq::lt, val);
	}

M_MODEL& orleListimg(const std::string &val)
	{return whereOr(B_BASE::cols::listimg, orm::wq::le, val);
	}

M_MODEL& orlikeListimg(const std::string &val)
	{return whereOr(B_BASE::cols::listimg, orm::wq::like, val);
	}

M_MODEL& ornullListimg()
	{return whereOrNull(B_BASE::cols::listimg);
	}

M_MODEL& ornotnullListimg()
	{return whereOrNotNull(B_BASE::cols::listimg);
	}

M_MODEL& eqBigimg(const std::string &val)
	{return where(B_BASE::cols::bigimg, orm::wq::eq, val);
	}

M_MODEL& nqBigimg(const std::string &val)
	{return where(B_BASE::cols::bigimg, orm::wq::nq, val);
	}

M_MODEL& btBigimg(const std::string &val)
	{return where(B_BASE::cols::bigimg, orm::wq::bt, val);
	}

M_MODEL& beBigimg(const std::string &val)
	{return where(B_BASE::cols::bigimg, orm::wq::be, val);
	}

M_MODEL& ltBigimg(const std::string &val)
	{return where(B_BASE::cols::bigimg, orm::wq::lt, val);
	}

M_MODEL& leBigimg(const std::string &val)
	{return where(B_BASE::cols::bigimg, orm::wq::le, val);
	}

M_MODEL& likeBigimg(const std::string &val)
	{return where(B_BASE::cols::bigimg, orm::wq::like, val);
	}

M_MODEL& nullBigimg()
	{return whereNull(B_BASE::cols::bigimg);
	}

M_MODEL& notnullBigimg()
	{return whereNotNull(B_BASE::cols::bigimg);
	}

M_MODEL& oreqBigimg(const std::string &val)
	{return whereOr(B_BASE::cols::bigimg, orm::wq::eq, val);
	}

M_MODEL& ornqBigimg(const std::string &val)
	{return whereOr(B_BASE::cols::bigimg, orm::wq::nq, val);
	}

M_MODEL& orbtBigimg(const std::string &val)
	{return whereOr(B_BASE::cols::bigimg, orm::wq::bt, val);
	}

M_MODEL& orbeBigimg(const std::string &val)
	{return whereOr(B_BASE::cols::bigimg, orm::wq::be, val);
	}

M_MODEL& orltBigimg(const std::string &val)
	{return whereOr(B_BASE::cols::bigimg, orm::wq::lt, val);
	}

M_MODEL& orleBigimg(const std::string &val)
	{return whereOr(B_BASE::cols::bigimg, orm::wq::le, val);
	}

M_MODEL& orlikeBigimg(const std::string &val)
	{return whereOr(B_BASE::cols::bigimg, orm::wq::like, val);
	}

M_MODEL& ornullBigimg()
	{return whereOrNull(B_BASE::cols::bigimg);
	}

M_MODEL& ornotnullBigimg()
	{return whereOrNotNull(B_BASE::cols::bigimg);
	}

M_MODEL& eqMaincontent(const std::string &val)
	{return where(B_BASE::cols::maincontent, orm::wq::eq, val);
	}

M_MODEL& nqMaincontent(const std::string &val)
	{return where(B_BASE::cols::maincontent, orm::wq::nq, val);
	}

M_MODEL& btMaincontent(const std::string &val)
	{return where(B_BASE::cols::maincontent, orm::wq::bt, val);
	}

M_MODEL& beMaincontent(const std::string &val)
	{return where(B_BASE::cols::maincontent, orm::wq::be, val);
	}

M_MODEL& ltMaincontent(const std::string &val)
	{return where(B_BASE::cols::maincontent, orm::wq::lt, val);
	}

M_MODEL& leMaincontent(const std::string &val)
	{return where(B_BASE::cols::maincontent, orm::wq::le, val);
	}

M_MODEL& likeMaincontent(const std::string &val)
	{return where(B_BASE::cols::maincontent, orm::wq::like, val);
	}

M_MODEL& nullMaincontent()
	{return whereNull(B_BASE::cols::maincontent);
	}

M_MODEL& notnullMaincontent()
	{return whereNotNull(B_BASE::cols::maincontent);
	}

M_MODEL& oreqMaincontent(const std::string &val)
	{return whereOr(B_BASE::cols::maincontent, orm::wq::eq, val);
	}

M_MODEL& ornqMaincontent(const std::string &val)
	{return whereOr(B_BASE::cols::maincontent, orm::wq::nq, val);
	}

M_MODEL& orbtMaincontent(const std::string &val)
	{return whereOr(B_BASE::cols::maincontent, orm::wq::bt, val);
	}

M_MODEL& orbeMaincontent(const std::string &val)
	{return whereOr(B_BASE::cols::maincontent, orm::wq::be, val);
	}

M_MODEL& orltMaincontent(const std::string &val)
	{return whereOr(B_BASE::cols::maincontent, orm::wq::lt, val);
	}

M_MODEL& orleMaincontent(const std::string &val)
	{return whereOr(B_BASE::cols::maincontent, orm::wq::le, val);
	}

M_MODEL& orlikeMaincontent(const std::string &val)
	{return whereOr(B_BASE::cols::maincontent, orm::wq::like, val);
	}

M_MODEL& ornullMaincontent()
	{return whereOrNull(B_BASE::cols::maincontent);
	}

M_MODEL& ornotnullMaincontent()
	{return whereOrNotNull(B_BASE::cols::maincontent);
	}

M_MODEL& eqParacontent(const std::string &val)
	{return where(B_BASE::cols::paracontent, orm::wq::eq, val);
	}

M_MODEL& nqParacontent(const std::string &val)
	{return where(B_BASE::cols::paracontent, orm::wq::nq, val);
	}

M_MODEL& btParacontent(const std::string &val)
	{return where(B_BASE::cols::paracontent, orm::wq::bt, val);
	}

M_MODEL& beParacontent(const std::string &val)
	{return where(B_BASE::cols::paracontent, orm::wq::be, val);
	}

M_MODEL& ltParacontent(const std::string &val)
	{return where(B_BASE::cols::paracontent, orm::wq::lt, val);
	}

M_MODEL& leParacontent(const std::string &val)
	{return where(B_BASE::cols::paracontent, orm::wq::le, val);
	}

M_MODEL& likeParacontent(const std::string &val)
	{return where(B_BASE::cols::paracontent, orm::wq::like, val);
	}

M_MODEL& nullParacontent()
	{return whereNull(B_BASE::cols::paracontent);
	}

M_MODEL& notnullParacontent()
	{return whereNotNull(B_BASE::cols::paracontent);
	}

M_MODEL& oreqParacontent(const std::string &val)
	{return whereOr(B_BASE::cols::paracontent, orm::wq::eq, val);
	}

M_MODEL& ornqParacontent(const std::string &val)
	{return whereOr(B_BASE::cols::paracontent, orm::wq::nq, val);
	}

M_MODEL& orbtParacontent(const std::string &val)
	{return whereOr(B_BASE::cols::paracontent, orm::wq::bt, val);
	}

M_MODEL& orbeParacontent(const std::string &val)
	{return whereOr(B_BASE::cols::paracontent, orm::wq::be, val);
	}

M_MODEL& orltParacontent(const std::string &val)
	{return whereOr(B_BASE::cols::paracontent, orm::wq::lt, val);
	}

M_MODEL& orleParacontent(const std::string &val)
	{return whereOr(B_BASE::cols::paracontent, orm::wq::le, val);
	}

M_MODEL& orlikeParacontent(const std::string &val)
	{return whereOr(B_BASE::cols::paracontent, orm::wq::like, val);
	}

M_MODEL& ornullParacontent()
	{return whereOrNull(B_BASE::cols::paracontent);
	}

M_MODEL& ornotnullParacontent()
	{return whereOrNotNull(B_BASE::cols::paracontent);
	}

M_MODEL& eqSamepro(const std::string &val)
	{return where(B_BASE::cols::samepro, orm::wq::eq, val);
	}

M_MODEL& nqSamepro(const std::string &val)
	{return where(B_BASE::cols::samepro, orm::wq::nq, val);
	}

M_MODEL& btSamepro(const std::string &val)
	{return where(B_BASE::cols::samepro, orm::wq::bt, val);
	}

M_MODEL& beSamepro(const std::string &val)
	{return where(B_BASE::cols::samepro, orm::wq::be, val);
	}

M_MODEL& ltSamepro(const std::string &val)
	{return where(B_BASE::cols::samepro, orm::wq::lt, val);
	}

M_MODEL& leSamepro(const std::string &val)
	{return where(B_BASE::cols::samepro, orm::wq::le, val);
	}

M_MODEL& likeSamepro(const std::string &val)
	{return where(B_BASE::cols::samepro, orm::wq::like, val);
	}

M_MODEL& nullSamepro()
	{return whereNull(B_BASE::cols::samepro);
	}

M_MODEL& notnullSamepro()
	{return whereNotNull(B_BASE::cols::samepro);
	}

M_MODEL& oreqSamepro(const std::string &val)
	{return whereOr(B_BASE::cols::samepro, orm::wq::eq, val);
	}

M_MODEL& ornqSamepro(const std::string &val)
	{return whereOr(B_BASE::cols::samepro, orm::wq::nq, val);
	}

M_MODEL& orbtSamepro(const std::string &val)
	{return whereOr(B_BASE::cols::samepro, orm::wq::bt, val);
	}

M_MODEL& orbeSamepro(const std::string &val)
	{return whereOr(B_BASE::cols::samepro, orm::wq::be, val);
	}

M_MODEL& orltSamepro(const std::string &val)
	{return whereOr(B_BASE::cols::samepro, orm::wq::lt, val);
	}

M_MODEL& orleSamepro(const std::string &val)
	{return whereOr(B_BASE::cols::samepro, orm::wq::le, val);
	}

M_MODEL& orlikeSamepro(const std::string &val)
	{return whereOr(B_BASE::cols::samepro, orm::wq::like, val);
	}

M_MODEL& ornullSamepro()
	{return whereOrNull(B_BASE::cols::samepro);
	}

M_MODEL& ornotnullSamepro()
	{return whereOrNotNull(B_BASE::cols::samepro);
	}

M_MODEL& eqAttatchfiles(const std::string &val)
	{return where(B_BASE::cols::attatchfiles, orm::wq::eq, val);
	}

M_MODEL& nqAttatchfiles(const std::string &val)
	{return where(B_BASE::cols::attatchfiles, orm::wq::nq, val);
	}

M_MODEL& btAttatchfiles(const std::string &val)
	{return where(B_BASE::cols::attatchfiles, orm::wq::bt, val);
	}

M_MODEL& beAttatchfiles(const std::string &val)
	{return where(B_BASE::cols::attatchfiles, orm::wq::be, val);
	}

M_MODEL& ltAttatchfiles(const std::string &val)
	{return where(B_BASE::cols::attatchfiles, orm::wq::lt, val);
	}

M_MODEL& leAttatchfiles(const std::string &val)
	{return where(B_BASE::cols::attatchfiles, orm::wq::le, val);
	}

M_MODEL& likeAttatchfiles(const std::string &val)
	{return where(B_BASE::cols::attatchfiles, orm::wq::like, val);
	}

M_MODEL& nullAttatchfiles()
	{return whereNull(B_BASE::cols::attatchfiles);
	}

M_MODEL& notnullAttatchfiles()
	{return whereNotNull(B_BASE::cols::attatchfiles);
	}

M_MODEL& oreqAttatchfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attatchfiles, orm::wq::eq, val);
	}

M_MODEL& ornqAttatchfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attatchfiles, orm::wq::nq, val);
	}

M_MODEL& orbtAttatchfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attatchfiles, orm::wq::bt, val);
	}

M_MODEL& orbeAttatchfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attatchfiles, orm::wq::be, val);
	}

M_MODEL& orltAttatchfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attatchfiles, orm::wq::lt, val);
	}

M_MODEL& orleAttatchfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attatchfiles, orm::wq::le, val);
	}

M_MODEL& orlikeAttatchfiles(const std::string &val)
	{return whereOr(B_BASE::cols::attatchfiles, orm::wq::like, val);
	}

M_MODEL& ornullAttatchfiles()
	{return whereOrNull(B_BASE::cols::attatchfiles);
	}

M_MODEL& ornotnullAttatchfiles()
	{return whereOrNotNull(B_BASE::cols::attatchfiles);
	}

M_MODEL& eqPrice(const std::string &val)
	{return where(B_BASE::cols::price, orm::wq::eq, val);
	}

M_MODEL& nqPrice(const std::string &val)
	{return where(B_BASE::cols::price, orm::wq::nq, val);
	}

M_MODEL& btPrice(const std::string &val)
	{return where(B_BASE::cols::price, orm::wq::bt, val);
	}

M_MODEL& bePrice(const std::string &val)
	{return where(B_BASE::cols::price, orm::wq::be, val);
	}

M_MODEL& ltPrice(const std::string &val)
	{return where(B_BASE::cols::price, orm::wq::lt, val);
	}

M_MODEL& lePrice(const std::string &val)
	{return where(B_BASE::cols::price, orm::wq::le, val);
	}
...
M_MODEL& nullPrice()
	{return whereNull(B_BASE::cols::price);
	}

M_MODEL& notnullPrice()
	{return whereNotNull(B_BASE::cols::price);
	}

M_MODEL& oreqPrice(const std::string &val)
	{return whereOr(B_BASE::cols::price, orm::wq::eq, val);
	}

M_MODEL& ornqPrice(const std::string &val)
	{return whereOr(B_BASE::cols::price, orm::wq::nq, val);
	}

M_MODEL& orbtPrice(const std::string &val)
	{return whereOr(B_BASE::cols::price, orm::wq::bt, val);
	}

M_MODEL& orbePrice(const std::string &val)
	{return whereOr(B_BASE::cols::price, orm::wq::be, val);
	}

M_MODEL& orltPrice(const std::string &val)
	{return whereOr(B_BASE::cols::price, orm::wq::lt, val);
	}

M_MODEL& orlePrice(const std::string &val)
	{return whereOr(B_BASE::cols::price, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqPrice(T val)
	{return whereOr(B_BASE::cols::price, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqPrice(T val)
	{return whereOr(B_BASE::cols::price, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtPrice(T val)
	{return whereOr(B_BASE::cols::price, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbePrice(T val)
	{return whereOr(B_BASE::cols::price, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltPrice(T val)
	{return whereOr(B_BASE::cols::price, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orlePrice(T val)
	{return whereOr(B_BASE::cols::price, orm::wq::le, val);
	}

M_MODEL& ornullPrice()
	{return whereOrNull(B_BASE::cols::price);
	}

M_MODEL& ornotnullPrice()
	{return whereOrNotNull(B_BASE::cols::price);
	}
...
M_MODEL& eqAdddate(const std::string &val)
	{return where(B_BASE::cols::adddate, orm::wq::eq, val);
	}

M_MODEL& nqAdddate(const std::string &val)
	{return where(B_BASE::cols::adddate, orm::wq::nq, val);
	}

M_MODEL& btAdddate(const std::string &val)
	{return where(B_BASE::cols::adddate, orm::wq::bt, val);
	}

M_MODEL& beAdddate(const std::string &val)
	{return where(B_BASE::cols::adddate, orm::wq::be, val);
	}

M_MODEL& ltAdddate(const std::string &val)
	{return where(B_BASE::cols::adddate, orm::wq::lt, val);
	}

M_MODEL& leAdddate(const std::string &val)
	{return where(B_BASE::cols::adddate, orm::wq::le, val);
	}

M_MODEL& likeAdddate(const std::string &val)
	{return where(B_BASE::cols::adddate, orm::wq::like, val);
	}

M_MODEL& nullAdddate()
	{return whereNull(B_BASE::cols::adddate);
	}

M_MODEL& notnullAdddate()
	{return whereNotNull(B_BASE::cols::adddate);
	}

M_MODEL& oreqAdddate(const std::string &val)
	{return whereOr(B_BASE::cols::adddate, orm::wq::eq, val);
	}

M_MODEL& ornqAdddate(const std::string &val)
	{return whereOr(B_BASE::cols::adddate, orm::wq::nq, val);
	}

M_MODEL& orbtAdddate(const std::string &val)
	{return whereOr(B_BASE::cols::adddate, orm::wq::bt, val);
	}

M_MODEL& orbeAdddate(const std::string &val)
	{return whereOr(B_BASE::cols::adddate, orm::wq::be, val);
	}

M_MODEL& orltAdddate(const std::string &val)
	{return whereOr(B_BASE::cols::adddate, orm::wq::lt, val);
	}

M_MODEL& orleAdddate(const std::string &val)
	{return whereOr(B_BASE::cols::adddate, orm::wq::le, val);
	}

M_MODEL& orlikeAdddate(const std::string &val)
	{return whereOr(B_BASE::cols::adddate, orm::wq::like, val);
	}

M_MODEL& ornullAdddate()
	{return whereOrNull(B_BASE::cols::adddate);
	}

M_MODEL& ornotnullAdddate()
	{return whereOrNotNull(B_BASE::cols::adddate);
	}

M_MODEL& eqEditdate(const std::string &val)
	{return where(B_BASE::cols::editdate, orm::wq::eq, val);
	}

M_MODEL& nqEditdate(const std::string &val)
	{return where(B_BASE::cols::editdate, orm::wq::nq, val);
	}

M_MODEL& btEditdate(const std::string &val)
	{return where(B_BASE::cols::editdate, orm::wq::bt, val);
	}

M_MODEL& beEditdate(const std::string &val)
	{return where(B_BASE::cols::editdate, orm::wq::be, val);
	}

M_MODEL& ltEditdate(const std::string &val)
	{return where(B_BASE::cols::editdate, orm::wq::lt, val);
	}

M_MODEL& leEditdate(const std::string &val)
	{return where(B_BASE::cols::editdate, orm::wq::le, val);
	}

M_MODEL& likeEditdate(const std::string &val)
	{return where(B_BASE::cols::editdate, orm::wq::like, val);
	}

M_MODEL& nullEditdate()
	{return whereNull(B_BASE::cols::editdate);
	}

M_MODEL& notnullEditdate()
	{return whereNotNull(B_BASE::cols::editdate);
	}

M_MODEL& oreqEditdate(const std::string &val)
	{return whereOr(B_BASE::cols::editdate, orm::wq::eq, val);
	}

M_MODEL& ornqEditdate(const std::string &val)
	{return whereOr(B_BASE::cols::editdate, orm::wq::nq, val);
	}

M_MODEL& orbtEditdate(const std::string &val)
	{return whereOr(B_BASE::cols::editdate, orm::wq::bt, val);
	}

M_MODEL& orbeEditdate(const std::string &val)
	{return whereOr(B_BASE::cols::editdate, orm::wq::be, val);
	}

M_MODEL& orltEditdate(const std::string &val)
	{return whereOr(B_BASE::cols::editdate, orm::wq::lt, val);
	}

M_MODEL& orleEditdate(const std::string &val)
	{return whereOr(B_BASE::cols::editdate, orm::wq::le, val);
	}

M_MODEL& orlikeEditdate(const std::string &val)
	{return whereOr(B_BASE::cols::editdate, orm::wq::like, val);
	}

M_MODEL& ornullEditdate()
	{return whereOrNull(B_BASE::cols::editdate);
	}

M_MODEL& ornotnullEditdate()
	{return whereOrNotNull(B_BASE::cols::editdate);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = product_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &product_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &product_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(product_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(product_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(product_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(product_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &product_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &product_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &product_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &product_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &product_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(product_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &product_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(product_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &product_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(product_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &product_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(product_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &product_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(product_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(product_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(product_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(product_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(product_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(product_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(product_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(product_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(product_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(product_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &product_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &product_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &product_info::col_names> field1, orm::table_col<B_BASE, &product_info::col_names> field2)
        {
        ...
        M_MODEL &asc(product_info::cols field, product_info::cols field2)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(product_info::cols field, product_info::cols field2)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &product_info::col_names> field1, orm::table_col<B_BASE, &product_info::col_names> field2)
        {
        ...
        M_MODEL &order(orm::table_col<B_BASE, &product_info::col_names> wq, const std::string &asc_or_desc)
        {
            ordersql.append(" ORDER BY ");
            ordersql.append(wq);
            ordersql.append(" ");
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &product_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &product_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &product_info::col_names> wq)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &product_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
        ...
        {
            model_meta_cache<product_info::meta> &data_cache = model_meta_cache<product_info::meta>::getinstance();
            data_cache.remove_exptime();

            model_meta_cache<std::vector<product_info::meta>> &record_cache = model_meta_cache<std::vector<product_info::meta>>::getinstance();
        ...
        bool save_data_cache(const std::string &cache_key_name, const product_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<product_info::meta> &temp_cache = model_meta_cache<product_info::meta>::getinstance();
        ...
        bool save_cache(std::size_t cache_key_name, const std::vector<product_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<product_info::meta>> &temp_cache = model_meta_cache<std::vector<product_info::meta>>::getinstance();
            temp_cache.save(cache_key_name, cache_data, exp_time);
            return true;
        }
        bool save_cache(const std::string cache_key_name, const std::vector<product_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<product_info::meta>> &temp_cache = model_meta_cache<std::vector<product_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_vector_cache(const std::string cache_key_name, const std::vector<product_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<product_info::meta>> &temp_cache = model_meta_cache<std::vector<product_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_cache(const std::string cache_key_name, const product_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<product_info::meta> &temp_cache = model_meta_cache<product_info::meta>::getinstance();
            std::size_t sqlhashid                               = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        std::tuple<unsigned int, unsigned long long> insert(product_info::meta &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(product_info::meta &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_data_insert_sql(insert_data);
        ...
        std::tuple<unsigned int, unsigned long long> insert(std::vector<product_info::meta> &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(std::vector<product_info::meta> &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_vector_insert_sql(insert_data);
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = product_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(product_info::meta &insert_data)
        {
        ...
        M_MODEL &set_data(product_info::meta indata)
        {
            B_BASE::data = indata;
            return *mod;
        }
        ...
        product_opsql(const product_opsql &)            = delete;
        product_opsql &operator=(const product_opsql &) = delete;
        product_opsql(product_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        product_opsql &operator=(product_opsql &&o) noexcept
        {
            if (this != &o)
            {
        ...
        template <typename T>
        M_MODEL &AND(orm::table_col<B_BASE, &product_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = opwq;
        ...
        template <typename T>
        M_MODEL &OR(orm::table_col<B_BASE, &product_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = opwq;
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < product_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == product_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(product_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(product_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = product_info::col_names.size();
