#ifndef _ORM_CMS_ARTICLE_OPERATE_H
#define _ORM_CMS_ARTICLE_OPERATE_H
/*
 ...
 *  Creation time Thu, 17 Sep 2026 23:22:06 GMT
 */
#include <iostream>
#include <mutex>
#include <string>
...
{ /*tagnamespace_replace*/
    ...
    class article_opsql : public B_BASE
    {
      public:
        article_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        article_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, article_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            break;
                case 5:
            data_temp.topicname.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 6:
            data_temp.title.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 7:
            data_temp.keywords.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 8:
            data_temp.fromsource.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 9:
            data_temp.author.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 10:
            data_temp.addip.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 11:
            data_temp.createtime.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 12:
             {
               data_temp.addtime = 0;
            ...
            break;
                case 15:
            data_temp.icoimg.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 16:
            data_temp.content.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 17:
            data_temp.mdcontent.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 18:
             {
               data_temp.isopen = 0;
            ...
            break;
                case 22:
            data_temp.fromlocal.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 23:
            data_temp.texturl.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 24:
            data_temp.summary.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 25:
            data_temp.editauthor.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 26:
            data_temp.relatecontent.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                
        }
    }
    
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, article_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.aid = static_cast<decltype(data_temp.aid )>(v);
            ...
            case 1: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.topicid = static_cast<decltype(data_temp.topicid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.topicid = static_cast<decltype(data_temp.topicid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.topicid = static_cast<decltype(data_temp.topicid )>(v);
            ...
            case 2: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.classtype = static_cast<decltype(data_temp.classtype )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.classtype = static_cast<decltype(data_temp.classtype )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.classtype = static_cast<decltype(data_temp.classtype )>(v);
            ...
            case 4: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.sortid = static_cast<decltype(data_temp.sortid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.sortid = static_cast<decltype(data_temp.sortid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.sortid = static_cast<decltype(data_temp.sortid )>(v);
            ...
            case 5: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.topicname.assign(v.data(), v.size());
                    else
                        data_temp.topicname.clear();
                }, val);
            } break;
            ...
            case 7: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.keywords.assign(v.data(), v.size());
                    else
                        data_temp.keywords.clear();
                }, val);
            } break;
            case 8: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.fromsource.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.fromsource.assign(v.data(), v.size());
                    else
                        data_temp.fromsource.clear();
                }, val);
            } break;
            case 9: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.author.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.author.assign(v.data(), v.size());
                    else
                        data_temp.author.clear();
                }, val);
            } break;
            ...
            case 11: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.createtime.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.createtime.assign(v.data(), v.size());
                    else
                        data_temp.createtime.clear();
                }, val);
            } break;
            case 12: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.addtime = static_cast<decltype(data_temp.addtime )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.addtime = static_cast<decltype(data_temp.addtime )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.addtime = static_cast<decltype(data_temp.addtime )>(v);
            ...
            case 13: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.readnum = static_cast<decltype(data_temp.readnum )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.readnum = static_cast<decltype(data_temp.readnum )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.readnum = static_cast<decltype(data_temp.readnum )>(v);
            ...
            case 14: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.review = static_cast<decltype(data_temp.review )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.review = static_cast<decltype(data_temp.review )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.review = static_cast<decltype(data_temp.review )>(v);
            ...
            case 15: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.icoimg.assign(v.data(), v.size());
                    else
                        data_temp.icoimg.clear();
                }, val);
            } break;
            case 16: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.content.assign(v.data(), v.size());
                    else
                        data_temp.content.clear();
                }, val);
            } break;
            case 17: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.mdcontent.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.mdcontent.assign(v.data(), v.size());
                    else
                        data_temp.mdcontent.clear();
                }, val);
            } break;
            ...
            case 19: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.ishome = static_cast<decltype(data_temp.ishome )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.ishome = static_cast<decltype(data_temp.ishome )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.ishome = static_cast<decltype(data_temp.ishome )>(v);
            ...
            case 20: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.iscomment = static_cast<decltype(data_temp.iscomment )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.iscomment = static_cast<decltype(data_temp.iscomment )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.iscomment = static_cast<decltype(data_temp.iscomment )>(v);
            ...
            case 21: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.showtype = static_cast<decltype(data_temp.showtype )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.showtype = static_cast<decltype(data_temp.showtype )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.showtype = static_cast<decltype(data_temp.showtype )>(v);
            ...
            case 22: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.fromlocal.assign(v.data(), v.size());
                    else
                        data_temp.fromlocal.clear();
                }, val);
            } break;
            case 23: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.texturl.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.texturl.assign(v.data(), v.size());
                    else
                        data_temp.texturl.clear();
                }, val);
            } break;
            case 24: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.summary.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.summary.assign(v.data(), v.size());
                    else
                        data_temp.summary.clear();
                }, val);
            } break;
            case 25: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.editauthor.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.editauthor.assign(v.data(), v.size());
                    else
                        data_temp.editauthor.clear();
                }, val);
            } break;
            case 26: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.relatecontent.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.relatecontent.assign(v.data(), v.size());
                    else
                        data_temp.relatecontent.clear();
                }, val);
            } break;
        ...
        http::obj_val get_field_value(unsigned char index_pos, const article_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
                return http::obj_val(static_cast<long long>(data_temp.aid));
                break;
            case 1:
                return http::obj_val(static_cast<long long>(data_temp.topicid));
                break;
            case 2:
                return http::obj_val(static_cast<long long>(data_temp.classtype));
                break;
            case 3:
                return http::obj_val(static_cast<long long>(data_temp.userid));
                break;
            case 4:
                return http::obj_val(static_cast<long long>(data_temp.sortid));
                break;
            case 5:
                return http::obj_val(std::string(data_temp.topicname));
                break;
            case 6:
                return http::obj_val(std::string(data_temp.title));
                break;
            case 7:
                return http::obj_val(std::string(data_temp.keywords));
                break;
            case 8:
                return http::obj_val(std::string(data_temp.fromsource));
                break;
            case 9:
                return http::obj_val(std::string(data_temp.author));
                break;
            case 10:
                return http::obj_val(std::string(data_temp.addip));
                break;
            case 11:
                return http::obj_val(std::string(data_temp.createtime));
                break;
            case 12:
                return http::obj_val(static_cast<long long>(data_temp.addtime));
                break;
            case 13:
                return http::obj_val(static_cast<long long>(data_temp.readnum));
                break;
            case 14:
                return http::obj_val(static_cast<long long>(data_temp.review));
                break;
            case 15:
                return http::obj_val(std::string(data_temp.icoimg));
                break;
            case 16:
                return http::obj_val(std::string(data_temp.content));
                break;
            case 17:
                return http::obj_val(std::string(data_temp.mdcontent));
                break;
            case 18:
                return http::obj_val(static_cast<long long>(data_temp.isopen));
                break;
            case 19:
                return http::obj_val(static_cast<long long>(data_temp.ishome));
                break;
            case 20:
                return http::obj_val(static_cast<long long>(data_temp.iscomment));
                break;
            case 21:
                return http::obj_val(static_cast<long long>(data_temp.showtype));
                break;
            case 22:
                return http::obj_val(std::string(data_temp.fromlocal));
                break;
            case 23:
                return http::obj_val(std::string(data_temp.texturl));
                break;
            case 24:
                return http::obj_val(std::string(data_temp.summary));
                break;
            case 25:
                return http::obj_val(std::string(data_temp.editauthor));
                break;
            case 26:
                return http::obj_val(std::string(data_temp.relatecontent));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const article_info::meta &data_temp)
    {
        // Auto-increment primary key aid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.aid == 0)
            return http::obj_val(nullptr);
        return get_field_value(index_pos, data_temp);
    }
    

M_MODEL& eqAid(const std::string &val)
	{return where(B_BASE::cols::aid, orm::wq::eq, val);
	}

M_MODEL& nqAid(const std::string &val)
	{return where(B_BASE::cols::aid, orm::wq::nq, val);
	}

M_MODEL& btAid(const std::string &val)
	{return where(B_BASE::cols::aid, orm::wq::bt, val);
	}

M_MODEL& beAid(const std::string &val)
	{return where(B_BASE::cols::aid, orm::wq::be, val);
	}

M_MODEL& ltAid(const std::string &val)
	{return where(B_BASE::cols::aid, orm::wq::lt, val);
	}

M_MODEL& leAid(const std::string &val)
	{return where(B_BASE::cols::aid, orm::wq::le, val);
	}
...
M_MODEL& nullAid()
	{return whereNull(B_BASE::cols::aid);
	}

M_MODEL& notnullAid()
	{return whereNotNull(B_BASE::cols::aid);
	}

M_MODEL& oreqAid(const std::string &val)
	{return whereOr(B_BASE::cols::aid, orm::wq::eq, val);
	}

M_MODEL& ornqAid(const std::string &val)
	{return whereOr(B_BASE::cols::aid, orm::wq::nq, val);
	}

M_MODEL& orbtAid(const std::string &val)
	{return whereOr(B_BASE::cols::aid, orm::wq::bt, val);
	}

M_MODEL& orbeAid(const std::string &val)
	{return whereOr(B_BASE::cols::aid, orm::wq::be, val);
	}

M_MODEL& orltAid(const std::string &val)
	{return whereOr(B_BASE::cols::aid, orm::wq::lt, val);
	}

M_MODEL& orleAid(const std::string &val)
	{return whereOr(B_BASE::cols::aid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqAid(T val)
	{return whereOr(B_BASE::cols::aid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqAid(T val)
	{return whereOr(B_BASE::cols::aid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtAid(T val)
	{return whereOr(B_BASE::cols::aid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeAid(T val)
	{return whereOr(B_BASE::cols::aid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltAid(T val)
	{return whereOr(B_BASE::cols::aid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleAid(T val)
	{return whereOr(B_BASE::cols::aid, orm::wq::le, val);
	}

M_MODEL& ornullAid()
	{return whereOrNull(B_BASE::cols::aid);
	}

M_MODEL& ornotnullAid()
	{return whereOrNotNull(B_BASE::cols::aid);
	}

M_MODEL& eqTopicid(const std::string &val)
	{return where(B_BASE::cols::topicid, orm::wq::eq, val);
	}

M_MODEL& nqTopicid(const std::string &val)
	{return where(B_BASE::cols::topicid, orm::wq::nq, val);
	}

M_MODEL& btTopicid(const std::string &val)
	{return where(B_BASE::cols::topicid, orm::wq::bt, val);
	}

M_MODEL& beTopicid(const std::string &val)
	{return where(B_BASE::cols::topicid, orm::wq::be, val);
	}

M_MODEL& ltTopicid(const std::string &val)
	{return where(B_BASE::cols::topicid, orm::wq::lt, val);
	}

M_MODEL& leTopicid(const std::string &val)
	{return where(B_BASE::cols::topicid, orm::wq::le, val);
	}
...
M_MODEL& nullTopicid()
	{return whereNull(B_BASE::cols::topicid);
	}

M_MODEL& notnullTopicid()
	{return whereNotNull(B_BASE::cols::topicid);
	}

M_MODEL& oreqTopicid(const std::string &val)
	{return whereOr(B_BASE::cols::topicid, orm::wq::eq, val);
	}

M_MODEL& ornqTopicid(const std::string &val)
	{return whereOr(B_BASE::cols::topicid, orm::wq::nq, val);
	}

M_MODEL& orbtTopicid(const std::string &val)
	{return whereOr(B_BASE::cols::topicid, orm::wq::bt, val);
	}

M_MODEL& orbeTopicid(const std::string &val)
	{return whereOr(B_BASE::cols::topicid, orm::wq::be, val);
	}

M_MODEL& orltTopicid(const std::string &val)
	{return whereOr(B_BASE::cols::topicid, orm::wq::lt, val);
	}

M_MODEL& orleTopicid(const std::string &val)
	{return whereOr(B_BASE::cols::topicid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqTopicid(T val)
	{return whereOr(B_BASE::cols::topicid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqTopicid(T val)
	{return whereOr(B_BASE::cols::topicid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtTopicid(T val)
	{return whereOr(B_BASE::cols::topicid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeTopicid(T val)
	{return whereOr(B_BASE::cols::topicid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltTopicid(T val)
	{return whereOr(B_BASE::cols::topicid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleTopicid(T val)
	{return whereOr(B_BASE::cols::topicid, orm::wq::le, val);
	}

M_MODEL& ornullTopicid()
	{return whereOrNull(B_BASE::cols::topicid);
	}

M_MODEL& ornotnullTopicid()
	{return whereOrNotNull(B_BASE::cols::topicid);
	}

M_MODEL& eqClasstype(const std::string &val)
	{return where(B_BASE::cols::classtype, orm::wq::eq, val);
	}

M_MODEL& nqClasstype(const std::string &val)
	{return where(B_BASE::cols::classtype, orm::wq::nq, val);
	}

M_MODEL& btClasstype(const std::string &val)
	{return where(B_BASE::cols::classtype, orm::wq::bt, val);
	}

M_MODEL& beClasstype(const std::string &val)
	{return where(B_BASE::cols::classtype, orm::wq::be, val);
	}

M_MODEL& ltClasstype(const std::string &val)
	{return where(B_BASE::cols::classtype, orm::wq::lt, val);
	}

M_MODEL& leClasstype(const std::string &val)
	{return where(B_BASE::cols::classtype, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqClasstype(T val)
	{return where(B_BASE::cols::classtype, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqClasstype(T val)
	{return where(B_BASE::cols::classtype, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btClasstype(T val)
	{return where(B_BASE::cols::classtype, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beClasstype(T val)
	{return where(B_BASE::cols::classtype, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltClasstype(T val)
	{return where(B_BASE::cols::classtype, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leClasstype(T val)
	{return where(B_BASE::cols::classtype, orm::wq::le, val);
	}

M_MODEL& nullClasstype()
	{return whereNull(B_BASE::cols::classtype);
	}

M_MODEL& notnullClasstype()
	{return whereNotNull(B_BASE::cols::classtype);
	}

M_MODEL& oreqClasstype(const std::string &val)
	{return whereOr(B_BASE::cols::classtype, orm::wq::eq, val);
	}

M_MODEL& ornqClasstype(const std::string &val)
	{return whereOr(B_BASE::cols::classtype, orm::wq::nq, val);
	}

M_MODEL& orbtClasstype(const std::string &val)
	{return whereOr(B_BASE::cols::classtype, orm::wq::bt, val);
	}

M_MODEL& orbeClasstype(const std::string &val)
	{return whereOr(B_BASE::cols::classtype, orm::wq::be, val);
	}

M_MODEL& orltClasstype(const std::string &val)
	{return whereOr(B_BASE::cols::classtype, orm::wq::lt, val);
	}

M_MODEL& orleClasstype(const std::string &val)
	{return whereOr(B_BASE::cols::classtype, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqClasstype(T val)
	{return whereOr(B_BASE::cols::classtype, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqClasstype(T val)
	{return whereOr(B_BASE::cols::classtype, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtClasstype(T val)
	{return whereOr(B_BASE::cols::classtype, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeClasstype(T val)
	{return whereOr(B_BASE::cols::classtype, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltClasstype(T val)
	{return whereOr(B_BASE::cols::classtype, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleClasstype(T val)
	{return whereOr(B_BASE::cols::classtype, orm::wq::le, val);
	}

M_MODEL& ornullClasstype()
	{return whereOrNull(B_BASE::cols::classtype);
	}

M_MODEL& ornotnullClasstype()
	{return whereOrNotNull(B_BASE::cols::classtype);
	}
...
M_MODEL& eqSortid(const std::string &val)
	{return where(B_BASE::cols::sortid, orm::wq::eq, val);
	}

M_MODEL& nqSortid(const std::string &val)
	{return where(B_BASE::cols::sortid, orm::wq::nq, val);
	}

M_MODEL& btSortid(const std::string &val)
	{return where(B_BASE::cols::sortid, orm::wq::bt, val);
	}

M_MODEL& beSortid(const std::string &val)
	{return where(B_BASE::cols::sortid, orm::wq::be, val);
	}

M_MODEL& ltSortid(const std::string &val)
	{return where(B_BASE::cols::sortid, orm::wq::lt, val);
	}

M_MODEL& leSortid(const std::string &val)
	{return where(B_BASE::cols::sortid, orm::wq::le, val);
	}
...
M_MODEL& nullSortid()
	{return whereNull(B_BASE::cols::sortid);
	}

M_MODEL& notnullSortid()
	{return whereNotNull(B_BASE::cols::sortid);
	}

M_MODEL& oreqSortid(const std::string &val)
	{return whereOr(B_BASE::cols::sortid, orm::wq::eq, val);
	}

M_MODEL& ornqSortid(const std::string &val)
	{return whereOr(B_BASE::cols::sortid, orm::wq::nq, val);
	}

M_MODEL& orbtSortid(const std::string &val)
	{return whereOr(B_BASE::cols::sortid, orm::wq::bt, val);
	}

M_MODEL& orbeSortid(const std::string &val)
	{return whereOr(B_BASE::cols::sortid, orm::wq::be, val);
	}

M_MODEL& orltSortid(const std::string &val)
	{return whereOr(B_BASE::cols::sortid, orm::wq::lt, val);
	}

M_MODEL& orleSortid(const std::string &val)
	{return whereOr(B_BASE::cols::sortid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqSortid(T val)
	{return whereOr(B_BASE::cols::sortid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqSortid(T val)
	{return whereOr(B_BASE::cols::sortid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtSortid(T val)
	{return whereOr(B_BASE::cols::sortid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeSortid(T val)
	{return whereOr(B_BASE::cols::sortid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltSortid(T val)
	{return whereOr(B_BASE::cols::sortid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleSortid(T val)
	{return whereOr(B_BASE::cols::sortid, orm::wq::le, val);
	}

M_MODEL& ornullSortid()
	{return whereOrNull(B_BASE::cols::sortid);
	}

M_MODEL& ornotnullSortid()
	{return whereOrNotNull(B_BASE::cols::sortid);
	}

M_MODEL& eqTopicname(const std::string &val)
	{return where(B_BASE::cols::topicname, orm::wq::eq, val);
	}

M_MODEL& nqTopicname(const std::string &val)
	{return where(B_BASE::cols::topicname, orm::wq::nq, val);
	}

M_MODEL& btTopicname(const std::string &val)
	{return where(B_BASE::cols::topicname, orm::wq::bt, val);
	}

M_MODEL& beTopicname(const std::string &val)
	{return where(B_BASE::cols::topicname, orm::wq::be, val);
	}

M_MODEL& ltTopicname(const std::string &val)
	{return where(B_BASE::cols::topicname, orm::wq::lt, val);
	}

M_MODEL& leTopicname(const std::string &val)
	{return where(B_BASE::cols::topicname, orm::wq::le, val);
	}

M_MODEL& likeTopicname(const std::string &val)
	{return where(B_BASE::cols::topicname, orm::wq::like, val);
	}

M_MODEL& nullTopicname()
	{return whereNull(B_BASE::cols::topicname);
	}

M_MODEL& notnullTopicname()
	{return whereNotNull(B_BASE::cols::topicname);
	}

M_MODEL& oreqTopicname(const std::string &val)
	{return whereOr(B_BASE::cols::topicname, orm::wq::eq, val);
	}

M_MODEL& ornqTopicname(const std::string &val)
	{return whereOr(B_BASE::cols::topicname, orm::wq::nq, val);
	}

M_MODEL& orbtTopicname(const std::string &val)
	{return whereOr(B_BASE::cols::topicname, orm::wq::bt, val);
	}

M_MODEL& orbeTopicname(const std::string &val)
	{return whereOr(B_BASE::cols::topicname, orm::wq::be, val);
	}

M_MODEL& orltTopicname(const std::string &val)
	{return whereOr(B_BASE::cols::topicname, orm::wq::lt, val);
	}

M_MODEL& orleTopicname(const std::string &val)
	{return whereOr(B_BASE::cols::topicname, orm::wq::le, val);
	}

M_MODEL& orlikeTopicname(const std::string &val)
	{return whereOr(B_BASE::cols::topicname, orm::wq::like, val);
	}

M_MODEL& ornullTopicname()
	{return whereOrNull(B_BASE::cols::topicname);
	}

M_MODEL& ornotnullTopicname()
	{return whereOrNotNull(B_BASE::cols::topicname);
	}
...
M_MODEL& eqKeywords(const std::string &val)
	{return where(B_BASE::cols::keywords, orm::wq::eq, val);
	}

M_MODEL& nqKeywords(const std::string &val)
	{return where(B_BASE::cols::keywords, orm::wq::nq, val);
	}

M_MODEL& btKeywords(const std::string &val)
	{return where(B_BASE::cols::keywords, orm::wq::bt, val);
	}

M_MODEL& beKeywords(const std::string &val)
	{return where(B_BASE::cols::keywords, orm::wq::be, val);
	}

M_MODEL& ltKeywords(const std::string &val)
	{return where(B_BASE::cols::keywords, orm::wq::lt, val);
	}

M_MODEL& leKeywords(const std::string &val)
	{return where(B_BASE::cols::keywords, orm::wq::le, val);
	}

M_MODEL& likeKeywords(const std::string &val)
	{return where(B_BASE::cols::keywords, orm::wq::like, val);
	}

M_MODEL& nullKeywords()
	{return whereNull(B_BASE::cols::keywords);
	}

M_MODEL& notnullKeywords()
	{return whereNotNull(B_BASE::cols::keywords);
	}

M_MODEL& oreqKeywords(const std::string &val)
	{return whereOr(B_BASE::cols::keywords, orm::wq::eq, val);
	}

M_MODEL& ornqKeywords(const std::string &val)
	{return whereOr(B_BASE::cols::keywords, orm::wq::nq, val);
	}

M_MODEL& orbtKeywords(const std::string &val)
	{return whereOr(B_BASE::cols::keywords, orm::wq::bt, val);
	}

M_MODEL& orbeKeywords(const std::string &val)
	{return whereOr(B_BASE::cols::keywords, orm::wq::be, val);
	}

M_MODEL& orltKeywords(const std::string &val)
	{return whereOr(B_BASE::cols::keywords, orm::wq::lt, val);
	}

M_MODEL& orleKeywords(const std::string &val)
	{return whereOr(B_BASE::cols::keywords, orm::wq::le, val);
	}

M_MODEL& orlikeKeywords(const std::string &val)
	{return whereOr(B_BASE::cols::keywords, orm::wq::like, val);
	}

M_MODEL& ornullKeywords()
	{return whereOrNull(B_BASE::cols::keywords);
	}

M_MODEL& ornotnullKeywords()
	{return whereOrNotNull(B_BASE::cols::keywords);
	}

M_MODEL& eqFromsource(const std::string &val)
	{return where(B_BASE::cols::fromsource, orm::wq::eq, val);
	}

M_MODEL& nqFromsource(const std::string &val)
	{return where(B_BASE::cols::fromsource, orm::wq::nq, val);
	}

M_MODEL& btFromsource(const std::string &val)
	{return where(B_BASE::cols::fromsource, orm::wq::bt, val);
	}

M_MODEL& beFromsource(const std::string &val)
	{return where(B_BASE::cols::fromsource, orm::wq::be, val);
	}

M_MODEL& ltFromsource(const std::string &val)
	{return where(B_BASE::cols::fromsource, orm::wq::lt, val);
	}

M_MODEL& leFromsource(const std::string &val)
	{return where(B_BASE::cols::fromsource, orm::wq::le, val);
	}

M_MODEL& likeFromsource(const std::string &val)
	{return where(B_BASE::cols::fromsource, orm::wq::like, val);
	}

M_MODEL& nullFromsource()
	{return whereNull(B_BASE::cols::fromsource);
	}

M_MODEL& notnullFromsource()
	{return whereNotNull(B_BASE::cols::fromsource);
	}

M_MODEL& oreqFromsource(const std::string &val)
	{return whereOr(B_BASE::cols::fromsource, orm::wq::eq, val);
	}

M_MODEL& ornqFromsource(const std::string &val)
	{return whereOr(B_BASE::cols::fromsource, orm::wq::nq, val);
	}

M_MODEL& orbtFromsource(const std::string &val)
	{return whereOr(B_BASE::cols::fromsource, orm::wq::bt, val);
	}

M_MODEL& orbeFromsource(const std::string &val)
	{return whereOr(B_BASE::cols::fromsource, orm::wq::be, val);
	}

M_MODEL& orltFromsource(const std::string &val)
	{return whereOr(B_BASE::cols::fromsource, orm::wq::lt, val);
	}

M_MODEL& orleFromsource(const std::string &val)
	{return whereOr(B_BASE::cols::fromsource, orm::wq::le, val);
	}

M_MODEL& orlikeFromsource(const std::string &val)
	{return whereOr(B_BASE::cols::fromsource, orm::wq::like, val);
	}

M_MODEL& ornullFromsource()
	{return whereOrNull(B_BASE::cols::fromsource);
	}

M_MODEL& ornotnullFromsource()
	{return whereOrNotNull(B_BASE::cols::fromsource);
	}

M_MODEL& eqAuthor(const std::string &val)
	{return where(B_BASE::cols::author, orm::wq::eq, val);
	}

M_MODEL& nqAuthor(const std::string &val)
	{return where(B_BASE::cols::author, orm::wq::nq, val);
	}

M_MODEL& btAuthor(const std::string &val)
	{return where(B_BASE::cols::author, orm::wq::bt, val);
	}

M_MODEL& beAuthor(const std::string &val)
	{return where(B_BASE::cols::author, orm::wq::be, val);
	}

M_MODEL& ltAuthor(const std::string &val)
	{return where(B_BASE::cols::author, orm::wq::lt, val);
	}

M_MODEL& leAuthor(const std::string &val)
	{return where(B_BASE::cols::author, orm::wq::le, val);
	}

M_MODEL& likeAuthor(const std::string &val)
	{return where(B_BASE::cols::author, orm::wq::like, val);
	}

M_MODEL& nullAuthor()
	{return whereNull(B_BASE::cols::author);
	}

M_MODEL& notnullAuthor()
	{return whereNotNull(B_BASE::cols::author);
	}

M_MODEL& oreqAuthor(const std::string &val)
	{return whereOr(B_BASE::cols::author, orm::wq::eq, val);
	}

M_MODEL& ornqAuthor(const std::string &val)
	{return whereOr(B_BASE::cols::author, orm::wq::nq, val);
	}

M_MODEL& orbtAuthor(const std::string &val)
	{return whereOr(B_BASE::cols::author, orm::wq::bt, val);
	}

M_MODEL& orbeAuthor(const std::string &val)
	{return whereOr(B_BASE::cols::author, orm::wq::be, val);
	}

M_MODEL& orltAuthor(const std::string &val)
	{return whereOr(B_BASE::cols::author, orm::wq::lt, val);
	}

M_MODEL& orleAuthor(const std::string &val)
	{return whereOr(B_BASE::cols::author, orm::wq::le, val);
	}

M_MODEL& orlikeAuthor(const std::string &val)
	{return whereOr(B_BASE::cols::author, orm::wq::like, val);
	}

M_MODEL& ornullAuthor()
	{return whereOrNull(B_BASE::cols::author);
	}

M_MODEL& ornotnullAuthor()
	{return whereOrNotNull(B_BASE::cols::author);
	}
...
M_MODEL& eqCreatetime(const std::string &val)
	{return where(B_BASE::cols::createtime, orm::wq::eq, val);
	}

M_MODEL& nqCreatetime(const std::string &val)
	{return where(B_BASE::cols::createtime, orm::wq::nq, val);
	}

M_MODEL& btCreatetime(const std::string &val)
	{return where(B_BASE::cols::createtime, orm::wq::bt, val);
	}

M_MODEL& beCreatetime(const std::string &val)
	{return where(B_BASE::cols::createtime, orm::wq::be, val);
	}

M_MODEL& ltCreatetime(const std::string &val)
	{return where(B_BASE::cols::createtime, orm::wq::lt, val);
	}

M_MODEL& leCreatetime(const std::string &val)
	{return where(B_BASE::cols::createtime, orm::wq::le, val);
	}

M_MODEL& likeCreatetime(const std::string &val)
	{return where(B_BASE::cols::createtime, orm::wq::like, val);
	}

M_MODEL& nullCreatetime()
	{return whereNull(B_BASE::cols::createtime);
	}

M_MODEL& notnullCreatetime()
	{return whereNotNull(B_BASE::cols::createtime);
	}

M_MODEL& oreqCreatetime(const std::string &val)
	{return whereOr(B_BASE::cols::createtime, orm::wq::eq, val);
	}

M_MODEL& ornqCreatetime(const std::string &val)
	{return whereOr(B_BASE::cols::createtime, orm::wq::nq, val);
	}

M_MODEL& orbtCreatetime(const std::string &val)
	{return whereOr(B_BASE::cols::createtime, orm::wq::bt, val);
	}

M_MODEL& orbeCreatetime(const std::string &val)
	{return whereOr(B_BASE::cols::createtime, orm::wq::be, val);
	}

M_MODEL& orltCreatetime(const std::string &val)
	{return whereOr(B_BASE::cols::createtime, orm::wq::lt, val);
	}

M_MODEL& orleCreatetime(const std::string &val)
	{return whereOr(B_BASE::cols::createtime, orm::wq::le, val);
	}

M_MODEL& orlikeCreatetime(const std::string &val)
	{return whereOr(B_BASE::cols::createtime, orm::wq::like, val);
	}

M_MODEL& ornullCreatetime()
	{return whereOrNull(B_BASE::cols::createtime);
	}

M_MODEL& ornotnullCreatetime()
	{return whereOrNotNull(B_BASE::cols::createtime);
	}
...
template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqAddtime(T val)
	{return whereOr(B_BASE::cols::addtime, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqAddtime(T val)
	{return whereOr(B_BASE::cols::addtime, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtAddtime(T val)
	{return whereOr(B_BASE::cols::addtime, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeAddtime(T val)
	{return whereOr(B_BASE::cols::addtime, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltAddtime(T val)
	{return whereOr(B_BASE::cols::addtime, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleAddtime(T val)
	{return whereOr(B_BASE::cols::addtime, orm::wq::le, val);
	}
...
M_MODEL& eqReadnum(const std::string &val)
	{return where(B_BASE::cols::readnum, orm::wq::eq, val);
	}

M_MODEL& nqReadnum(const std::string &val)
	{return where(B_BASE::cols::readnum, orm::wq::nq, val);
	}

M_MODEL& btReadnum(const std::string &val)
	{return where(B_BASE::cols::readnum, orm::wq::bt, val);
	}

M_MODEL& beReadnum(const std::string &val)
	{return where(B_BASE::cols::readnum, orm::wq::be, val);
	}

M_MODEL& ltReadnum(const std::string &val)
	{return where(B_BASE::cols::readnum, orm::wq::lt, val);
	}

M_MODEL& leReadnum(const std::string &val)
	{return where(B_BASE::cols::readnum, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqReadnum(T val)
	{return where(B_BASE::cols::readnum, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqReadnum(T val)
	{return where(B_BASE::cols::readnum, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btReadnum(T val)
	{return where(B_BASE::cols::readnum, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beReadnum(T val)
	{return where(B_BASE::cols::readnum, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltReadnum(T val)
	{return where(B_BASE::cols::readnum, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leReadnum(T val)
	{return where(B_BASE::cols::readnum, orm::wq::le, val);
	}

M_MODEL& nullReadnum()
	{return whereNull(B_BASE::cols::readnum);
	}

M_MODEL& notnullReadnum()
	{return whereNotNull(B_BASE::cols::readnum);
	}

M_MODEL& oreqReadnum(const std::string &val)
	{return whereOr(B_BASE::cols::readnum, orm::wq::eq, val);
	}

M_MODEL& ornqReadnum(const std::string &val)
	{return whereOr(B_BASE::cols::readnum, orm::wq::nq, val);
	}

M_MODEL& orbtReadnum(const std::string &val)
	{return whereOr(B_BASE::cols::readnum, orm::wq::bt, val);
	}

M_MODEL& orbeReadnum(const std::string &val)
	{return whereOr(B_BASE::cols::readnum, orm::wq::be, val);
	}

M_MODEL& orltReadnum(const std::string &val)
	{return whereOr(B_BASE::cols::readnum, orm::wq::lt, val);
	}

M_MODEL& orleReadnum(const std::string &val)
	{return whereOr(B_BASE::cols::readnum, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqReadnum(T val)
	{return whereOr(B_BASE::cols::readnum, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqReadnum(T val)
	{return whereOr(B_BASE::cols::readnum, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtReadnum(T val)
	{return whereOr(B_BASE::cols::readnum, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeReadnum(T val)
	{return whereOr(B_BASE::cols::readnum, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltReadnum(T val)
	{return whereOr(B_BASE::cols::readnum, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleReadnum(T val)
	{return whereOr(B_BASE::cols::readnum, orm::wq::le, val);
	}

M_MODEL& ornullReadnum()
	{return whereOrNull(B_BASE::cols::readnum);
	}

M_MODEL& ornotnullReadnum()
	{return whereOrNotNull(B_BASE::cols::readnum);
	}

M_MODEL& eqReview(const std::string &val)
	{return where(B_BASE::cols::review, orm::wq::eq, val);
	}

M_MODEL& nqReview(const std::string &val)
	{return where(B_BASE::cols::review, orm::wq::nq, val);
	}

M_MODEL& btReview(const std::string &val)
	{return where(B_BASE::cols::review, orm::wq::bt, val);
	}

M_MODEL& beReview(const std::string &val)
	{return where(B_BASE::cols::review, orm::wq::be, val);
	}

M_MODEL& ltReview(const std::string &val)
	{return where(B_BASE::cols::review, orm::wq::lt, val);
	}

M_MODEL& leReview(const std::string &val)
	{return where(B_BASE::cols::review, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqReview(T val)
	{return where(B_BASE::cols::review, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqReview(T val)
	{return where(B_BASE::cols::review, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btReview(T val)
	{return where(B_BASE::cols::review, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beReview(T val)
	{return where(B_BASE::cols::review, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltReview(T val)
	{return where(B_BASE::cols::review, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leReview(T val)
	{return where(B_BASE::cols::review, orm::wq::le, val);
	}

M_MODEL& nullReview()
	{return whereNull(B_BASE::cols::review);
	}

M_MODEL& notnullReview()
	{return whereNotNull(B_BASE::cols::review);
	}

M_MODEL& oreqReview(const std::string &val)
	{return whereOr(B_BASE::cols::review, orm::wq::eq, val);
	}

M_MODEL& ornqReview(const std::string &val)
	{return whereOr(B_BASE::cols::review, orm::wq::nq, val);
	}

M_MODEL& orbtReview(const std::string &val)
	{return whereOr(B_BASE::cols::review, orm::wq::bt, val);
	}

M_MODEL& orbeReview(const std::string &val)
	{return whereOr(B_BASE::cols::review, orm::wq::be, val);
	}

M_MODEL& orltReview(const std::string &val)
	{return whereOr(B_BASE::cols::review, orm::wq::lt, val);
	}

M_MODEL& orleReview(const std::string &val)
	{return whereOr(B_BASE::cols::review, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqReview(T val)
	{return whereOr(B_BASE::cols::review, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqReview(T val)
	{return whereOr(B_BASE::cols::review, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtReview(T val)
	{return whereOr(B_BASE::cols::review, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeReview(T val)
	{return whereOr(B_BASE::cols::review, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltReview(T val)
	{return whereOr(B_BASE::cols::review, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleReview(T val)
	{return whereOr(B_BASE::cols::review, orm::wq::le, val);
	}

M_MODEL& ornullReview()
	{return whereOrNull(B_BASE::cols::review);
	}

M_MODEL& ornotnullReview()
	{return whereOrNotNull(B_BASE::cols::review);
	}

M_MODEL& eqIcoimg(const std::string &val)
	{return where(B_BASE::cols::icoimg, orm::wq::eq, val);
	}

M_MODEL& nqIcoimg(const std::string &val)
	{return where(B_BASE::cols::icoimg, orm::wq::nq, val);
	}

M_MODEL& btIcoimg(const std::string &val)
	{return where(B_BASE::cols::icoimg, orm::wq::bt, val);
	}

M_MODEL& beIcoimg(const std::string &val)
	{return where(B_BASE::cols::icoimg, orm::wq::be, val);
	}

M_MODEL& ltIcoimg(const std::string &val)
	{return where(B_BASE::cols::icoimg, orm::wq::lt, val);
	}

M_MODEL& leIcoimg(const std::string &val)
	{return where(B_BASE::cols::icoimg, orm::wq::le, val);
	}

M_MODEL& likeIcoimg(const std::string &val)
	{return where(B_BASE::cols::icoimg, orm::wq::like, val);
	}

M_MODEL& nullIcoimg()
	{return whereNull(B_BASE::cols::icoimg);
	}

M_MODEL& notnullIcoimg()
	{return whereNotNull(B_BASE::cols::icoimg);
	}

M_MODEL& oreqIcoimg(const std::string &val)
	{return whereOr(B_BASE::cols::icoimg, orm::wq::eq, val);
	}

M_MODEL& ornqIcoimg(const std::string &val)
	{return whereOr(B_BASE::cols::icoimg, orm::wq::nq, val);
	}

M_MODEL& orbtIcoimg(const std::string &val)
	{return whereOr(B_BASE::cols::icoimg, orm::wq::bt, val);
	}

M_MODEL& orbeIcoimg(const std::string &val)
	{return whereOr(B_BASE::cols::icoimg, orm::wq::be, val);
	}

M_MODEL& orltIcoimg(const std::string &val)
	{return whereOr(B_BASE::cols::icoimg, orm::wq::lt, val);
	}

M_MODEL& orleIcoimg(const std::string &val)
	{return whereOr(B_BASE::cols::icoimg, orm::wq::le, val);
	}

M_MODEL& orlikeIcoimg(const std::string &val)
	{return whereOr(B_BASE::cols::icoimg, orm::wq::like, val);
	}

M_MODEL& ornullIcoimg()
	{return whereOrNull(B_BASE::cols::icoimg);
	}

M_MODEL& ornotnullIcoimg()
	{return whereOrNotNull(B_BASE::cols::icoimg);
	}

M_MODEL& eqContent(const std::string &val)
	{return where(B_BASE::cols::content, orm::wq::eq, val);
	}

M_MODEL& nqContent(const std::string &val)
	{return where(B_BASE::cols::content, orm::wq::nq, val);
	}

M_MODEL& btContent(const std::string &val)
	{return where(B_BASE::cols::content, orm::wq::bt, val);
	}

M_MODEL& beContent(const std::string &val)
	{return where(B_BASE::cols::content, orm::wq::be, val);
	}

M_MODEL& ltContent(const std::string &val)
	{return where(B_BASE::cols::content, orm::wq::lt, val);
	}

M_MODEL& leContent(const std::string &val)
	{return where(B_BASE::cols::content, orm::wq::le, val);
	}

M_MODEL& likeContent(const std::string &val)
	{return where(B_BASE::cols::content, orm::wq::like, val);
	}

M_MODEL& nullContent()
	{return whereNull(B_BASE::cols::content);
	}

M_MODEL& notnullContent()
	{return whereNotNull(B_BASE::cols::content);
	}

M_MODEL& oreqContent(const std::string &val)
	{return whereOr(B_BASE::cols::content, orm::wq::eq, val);
	}

M_MODEL& ornqContent(const std::string &val)
	{return whereOr(B_BASE::cols::content, orm::wq::nq, val);
	}

M_MODEL& orbtContent(const std::string &val)
	{return whereOr(B_BASE::cols::content, orm::wq::bt, val);
	}

M_MODEL& orbeContent(const std::string &val)
	{return whereOr(B_BASE::cols::content, orm::wq::be, val);
	}

M_MODEL& orltContent(const std::string &val)
	{return whereOr(B_BASE::cols::content, orm::wq::lt, val);
	}

M_MODEL& orleContent(const std::string &val)
	{return whereOr(B_BASE::cols::content, orm::wq::le, val);
	}

M_MODEL& orlikeContent(const std::string &val)
	{return whereOr(B_BASE::cols::content, orm::wq::like, val);
	}

M_MODEL& ornullContent()
	{return whereOrNull(B_BASE::cols::content);
	}

M_MODEL& ornotnullContent()
	{return whereOrNotNull(B_BASE::cols::content);
	}

M_MODEL& eqMdcontent(const std::string &val)
	{return where(B_BASE::cols::mdcontent, orm::wq::eq, val);
	}

M_MODEL& nqMdcontent(const std::string &val)
	{return where(B_BASE::cols::mdcontent, orm::wq::nq, val);
	}

M_MODEL& btMdcontent(const std::string &val)
	{return where(B_BASE::cols::mdcontent, orm::wq::bt, val);
	}

M_MODEL& beMdcontent(const std::string &val)
	{return where(B_BASE::cols::mdcontent, orm::wq::be, val);
	}

M_MODEL& ltMdcontent(const std::string &val)
	{return where(B_BASE::cols::mdcontent, orm::wq::lt, val);
	}

M_MODEL& leMdcontent(const std::string &val)
	{return where(B_BASE::cols::mdcontent, orm::wq::le, val);
	}

M_MODEL& likeMdcontent(const std::string &val)
	{return where(B_BASE::cols::mdcontent, orm::wq::like, val);
	}

M_MODEL& nullMdcontent()
	{return whereNull(B_BASE::cols::mdcontent);
	}

M_MODEL& notnullMdcontent()
	{return whereNotNull(B_BASE::cols::mdcontent);
	}

M_MODEL& oreqMdcontent(const std::string &val)
	{return whereOr(B_BASE::cols::mdcontent, orm::wq::eq, val);
	}

M_MODEL& ornqMdcontent(const std::string &val)
	{return whereOr(B_BASE::cols::mdcontent, orm::wq::nq, val);
	}

M_MODEL& orbtMdcontent(const std::string &val)
	{return whereOr(B_BASE::cols::mdcontent, orm::wq::bt, val);
	}

M_MODEL& orbeMdcontent(const std::string &val)
	{return whereOr(B_BASE::cols::mdcontent, orm::wq::be, val);
	}

M_MODEL& orltMdcontent(const std::string &val)
	{return whereOr(B_BASE::cols::mdcontent, orm::wq::lt, val);
	}

M_MODEL& orleMdcontent(const std::string &val)
	{return whereOr(B_BASE::cols::mdcontent, orm::wq::le, val);
	}

M_MODEL& orlikeMdcontent(const std::string &val)
	{return whereOr(B_BASE::cols::mdcontent, orm::wq::like, val);
	}

M_MODEL& ornullMdcontent()
	{return whereOrNull(B_BASE::cols::mdcontent);
	}

M_MODEL& ornotnullMdcontent()
	{return whereOrNotNull(B_BASE::cols::mdcontent);
	}
...
M_MODEL& eqIshome(const std::string &val)
	{return where(B_BASE::cols::ishome, orm::wq::eq, val);
	}

M_MODEL& nqIshome(const std::string &val)
	{return where(B_BASE::cols::ishome, orm::wq::nq, val);
	}

M_MODEL& btIshome(const std::string &val)
	{return where(B_BASE::cols::ishome, orm::wq::bt, val);
	}

M_MODEL& beIshome(const std::string &val)
	{return where(B_BASE::cols::ishome, orm::wq::be, val);
	}

M_MODEL& ltIshome(const std::string &val)
	{return where(B_BASE::cols::ishome, orm::wq::lt, val);
	}

M_MODEL& leIshome(const std::string &val)
	{return where(B_BASE::cols::ishome, orm::wq::le, val);
	}
...
M_MODEL& nullIshome()
	{return whereNull(B_BASE::cols::ishome);
	}

M_MODEL& notnullIshome()
	{return whereNotNull(B_BASE::cols::ishome);
	}

M_MODEL& oreqIshome(const std::string &val)
	{return whereOr(B_BASE::cols::ishome, orm::wq::eq, val);
	}

M_MODEL& ornqIshome(const std::string &val)
	{return whereOr(B_BASE::cols::ishome, orm::wq::nq, val);
	}

M_MODEL& orbtIshome(const std::string &val)
	{return whereOr(B_BASE::cols::ishome, orm::wq::bt, val);
	}

M_MODEL& orbeIshome(const std::string &val)
	{return whereOr(B_BASE::cols::ishome, orm::wq::be, val);
	}

M_MODEL& orltIshome(const std::string &val)
	{return whereOr(B_BASE::cols::ishome, orm::wq::lt, val);
	}

M_MODEL& orleIshome(const std::string &val)
	{return whereOr(B_BASE::cols::ishome, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqIshome(T val)
	{return whereOr(B_BASE::cols::ishome, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqIshome(T val)
	{return whereOr(B_BASE::cols::ishome, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtIshome(T val)
	{return whereOr(B_BASE::cols::ishome, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeIshome(T val)
	{return whereOr(B_BASE::cols::ishome, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltIshome(T val)
	{return whereOr(B_BASE::cols::ishome, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleIshome(T val)
	{return whereOr(B_BASE::cols::ishome, orm::wq::le, val);
	}

M_MODEL& ornullIshome()
	{return whereOrNull(B_BASE::cols::ishome);
	}

M_MODEL& ornotnullIshome()
	{return whereOrNotNull(B_BASE::cols::ishome);
	}

M_MODEL& eqIscomment(const std::string &val)
	{return where(B_BASE::cols::iscomment, orm::wq::eq, val);
	}

M_MODEL& nqIscomment(const std::string &val)
	{return where(B_BASE::cols::iscomment, orm::wq::nq, val);
	}

M_MODEL& btIscomment(const std::string &val)
	{return where(B_BASE::cols::iscomment, orm::wq::bt, val);
	}

M_MODEL& beIscomment(const std::string &val)
	{return where(B_BASE::cols::iscomment, orm::wq::be, val);
	}

M_MODEL& ltIscomment(const std::string &val)
	{return where(B_BASE::cols::iscomment, orm::wq::lt, val);
	}

M_MODEL& leIscomment(const std::string &val)
	{return where(B_BASE::cols::iscomment, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqIscomment(T val)
	{return where(B_BASE::cols::iscomment, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqIscomment(T val)
	{return where(B_BASE::cols::iscomment, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btIscomment(T val)
	{return where(B_BASE::cols::iscomment, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beIscomment(T val)
	{return where(B_BASE::cols::iscomment, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltIscomment(T val)
	{return where(B_BASE::cols::iscomment, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leIscomment(T val)
	{return where(B_BASE::cols::iscomment, orm::wq::le, val);
	}

M_MODEL& nullIscomment()
	{return whereNull(B_BASE::cols::iscomment);
	}

M_MODEL& notnullIscomment()
	{return whereNotNull(B_BASE::cols::iscomment);
	}

M_MODEL& oreqIscomment(const std::string &val)
	{return whereOr(B_BASE::cols::iscomment, orm::wq::eq, val);
	}

M_MODEL& ornqIscomment(const std::string &val)
	{return whereOr(B_BASE::cols::iscomment, orm::wq::nq, val);
	}

M_MODEL& orbtIscomment(const std::string &val)
	{return whereOr(B_BASE::cols::iscomment, orm::wq::bt, val);
	}

M_MODEL& orbeIscomment(const std::string &val)
	{return whereOr(B_BASE::cols::iscomment, orm::wq::be, val);
	}

M_MODEL& orltIscomment(const std::string &val)
	{return whereOr(B_BASE::cols::iscomment, orm::wq::lt, val);
	}

M_MODEL& orleIscomment(const std::string &val)
	{return whereOr(B_BASE::cols::iscomment, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqIscomment(T val)
	{return whereOr(B_BASE::cols::iscomment, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqIscomment(T val)
	{return whereOr(B_BASE::cols::iscomment, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtIscomment(T val)
	{return whereOr(B_BASE::cols::iscomment, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeIscomment(T val)
	{return whereOr(B_BASE::cols::iscomment, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltIscomment(T val)
	{return whereOr(B_BASE::cols::iscomment, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleIscomment(T val)
	{return whereOr(B_BASE::cols::iscomment, orm::wq::le, val);
	}

M_MODEL& ornullIscomment()
	{return whereOrNull(B_BASE::cols::iscomment);
	}

M_MODEL& ornotnullIscomment()
	{return whereOrNotNull(B_BASE::cols::iscomment);
	}

M_MODEL& eqShowtype(const std::string &val)
	{return where(B_BASE::cols::showtype, orm::wq::eq, val);
	}

M_MODEL& nqShowtype(const std::string &val)
	{return where(B_BASE::cols::showtype, orm::wq::nq, val);
	}

M_MODEL& btShowtype(const std::string &val)
	{return where(B_BASE::cols::showtype, orm::wq::bt, val);
	}

M_MODEL& beShowtype(const std::string &val)
	{return where(B_BASE::cols::showtype, orm::wq::be, val);
	}

M_MODEL& ltShowtype(const std::string &val)
	{return where(B_BASE::cols::showtype, orm::wq::lt, val);
	}

M_MODEL& leShowtype(const std::string &val)
	{return where(B_BASE::cols::showtype, orm::wq::le, val);
	}
...
M_MODEL& nullShowtype()
	{return whereNull(B_BASE::cols::showtype);
	}

M_MODEL& notnullShowtype()
	{return whereNotNull(B_BASE::cols::showtype);
	}

M_MODEL& oreqShowtype(const std::string &val)
	{return whereOr(B_BASE::cols::showtype, orm::wq::eq, val);
	}

M_MODEL& ornqShowtype(const std::string &val)
	{return whereOr(B_BASE::cols::showtype, orm::wq::nq, val);
	}

M_MODEL& orbtShowtype(const std::string &val)
	{return whereOr(B_BASE::cols::showtype, orm::wq::bt, val);
	}

M_MODEL& orbeShowtype(const std::string &val)
	{return whereOr(B_BASE::cols::showtype, orm::wq::be, val);
	}

M_MODEL& orltShowtype(const std::string &val)
	{return whereOr(B_BASE::cols::showtype, orm::wq::lt, val);
	}

M_MODEL& orleShowtype(const std::string &val)
	{return whereOr(B_BASE::cols::showtype, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqShowtype(T val)
	{return whereOr(B_BASE::cols::showtype, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqShowtype(T val)
	{return whereOr(B_BASE::cols::showtype, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtShowtype(T val)
	{return whereOr(B_BASE::cols::showtype, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeShowtype(T val)
	{return whereOr(B_BASE::cols::showtype, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltShowtype(T val)
	{return whereOr(B_BASE::cols::showtype, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleShowtype(T val)
	{return whereOr(B_BASE::cols::showtype, orm::wq::le, val);
	}

M_MODEL& ornullShowtype()
	{return whereOrNull(B_BASE::cols::showtype);
	}

M_MODEL& ornotnullShowtype()
	{return whereOrNotNull(B_BASE::cols::showtype);
	}

M_MODEL& eqFromlocal(const std::string &val)
	{return where(B_BASE::cols::fromlocal, orm::wq::eq, val);
	}

M_MODEL& nqFromlocal(const std::string &val)
	{return where(B_BASE::cols::fromlocal, orm::wq::nq, val);
	}

M_MODEL& btFromlocal(const std::string &val)
	{return where(B_BASE::cols::fromlocal, orm::wq::bt, val);
	}

M_MODEL& beFromlocal(const std::string &val)
	{return where(B_BASE::cols::fromlocal, orm::wq::be, val);
	}

M_MODEL& ltFromlocal(const std::string &val)
	{return where(B_BASE::cols::fromlocal, orm::wq::lt, val);
	}

M_MODEL& leFromlocal(const std::string &val)
	{return where(B_BASE::cols::fromlocal, orm::wq::le, val);
	}

M_MODEL& likeFromlocal(const std::string &val)
	{return where(B_BASE::cols::fromlocal, orm::wq::like, val);
	}

M_MODEL& nullFromlocal()
	{return whereNull(B_BASE::cols::fromlocal);
	}

M_MODEL& notnullFromlocal()
	{return whereNotNull(B_BASE::cols::fromlocal);
	}

M_MODEL& oreqFromlocal(const std::string &val)
	{return whereOr(B_BASE::cols::fromlocal, orm::wq::eq, val);
	}

M_MODEL& ornqFromlocal(const std::string &val)
	{return whereOr(B_BASE::cols::fromlocal, orm::wq::nq, val);
	}

M_MODEL& orbtFromlocal(const std::string &val)
	{return whereOr(B_BASE::cols::fromlocal, orm::wq::bt, val);
	}

M_MODEL& orbeFromlocal(const std::string &val)
	{return whereOr(B_BASE::cols::fromlocal, orm::wq::be, val);
	}

M_MODEL& orltFromlocal(const std::string &val)
	{return whereOr(B_BASE::cols::fromlocal, orm::wq::lt, val);
	}

M_MODEL& orleFromlocal(const std::string &val)
	{return whereOr(B_BASE::cols::fromlocal, orm::wq::le, val);
	}

M_MODEL& orlikeFromlocal(const std::string &val)
	{return whereOr(B_BASE::cols::fromlocal, orm::wq::like, val);
	}

M_MODEL& ornullFromlocal()
	{return whereOrNull(B_BASE::cols::fromlocal);
	}

M_MODEL& ornotnullFromlocal()
	{return whereOrNotNull(B_BASE::cols::fromlocal);
	}

M_MODEL& eqTexturl(const std::string &val)
	{return where(B_BASE::cols::texturl, orm::wq::eq, val);
	}

M_MODEL& nqTexturl(const std::string &val)
	{return where(B_BASE::cols::texturl, orm::wq::nq, val);
	}

M_MODEL& btTexturl(const std::string &val)
	{return where(B_BASE::cols::texturl, orm::wq::bt, val);
	}

M_MODEL& beTexturl(const std::string &val)
	{return where(B_BASE::cols::texturl, orm::wq::be, val);
	}

M_MODEL& ltTexturl(const std::string &val)
	{return where(B_BASE::cols::texturl, orm::wq::lt, val);
	}

M_MODEL& leTexturl(const std::string &val)
	{return where(B_BASE::cols::texturl, orm::wq::le, val);
	}

M_MODEL& likeTexturl(const std::string &val)
	{return where(B_BASE::cols::texturl, orm::wq::like, val);
	}

M_MODEL& nullTexturl()
	{return whereNull(B_BASE::cols::texturl);
	}

M_MODEL& notnullTexturl()
	{return whereNotNull(B_BASE::cols::texturl);
	}

M_MODEL& oreqTexturl(const std::string &val)
	{return whereOr(B_BASE::cols::texturl, orm::wq::eq, val);
	}

M_MODEL& ornqTexturl(const std::string &val)
	{return whereOr(B_BASE::cols::texturl, orm::wq::nq, val);
	}

M_MODEL& orbtTexturl(const std::string &val)
	{return whereOr(B_BASE::cols::texturl, orm::wq::bt, val);
	}

M_MODEL& orbeTexturl(const std::string &val)
	{return whereOr(B_BASE::cols::texturl, orm::wq::be, val);
	}

M_MODEL& orltTexturl(const std::string &val)
	{return whereOr(B_BASE::cols::texturl, orm::wq::lt, val);
	}

M_MODEL& orleTexturl(const std::string &val)
	{return whereOr(B_BASE::cols::texturl, orm::wq::le, val);
	}

M_MODEL& orlikeTexturl(const std::string &val)
	{return whereOr(B_BASE::cols::texturl, orm::wq::like, val);
	}

M_MODEL& ornullTexturl()
	{return whereOrNull(B_BASE::cols::texturl);
	}

M_MODEL& ornotnullTexturl()
	{return whereOrNotNull(B_BASE::cols::texturl);
	}

M_MODEL& eqSummary(const std::string &val)
	{return where(B_BASE::cols::summary, orm::wq::eq, val);
	}

M_MODEL& nqSummary(const std::string &val)
	{return where(B_BASE::cols::summary, orm::wq::nq, val);
	}

M_MODEL& btSummary(const std::string &val)
	{return where(B_BASE::cols::summary, orm::wq::bt, val);
	}

M_MODEL& beSummary(const std::string &val)
	{return where(B_BASE::cols::summary, orm::wq::be, val);
	}

M_MODEL& ltSummary(const std::string &val)
	{return where(B_BASE::cols::summary, orm::wq::lt, val);
	}

M_MODEL& leSummary(const std::string &val)
	{return where(B_BASE::cols::summary, orm::wq::le, val);
	}

M_MODEL& likeSummary(const std::string &val)
	{return where(B_BASE::cols::summary, orm::wq::like, val);
	}

M_MODEL& nullSummary()
	{return whereNull(B_BASE::cols::summary);
	}

M_MODEL& notnullSummary()
	{return whereNotNull(B_BASE::cols::summary);
	}

M_MODEL& oreqSummary(const std::string &val)
	{return whereOr(B_BASE::cols::summary, orm::wq::eq, val);
	}

M_MODEL& ornqSummary(const std::string &val)
	{return whereOr(B_BASE::cols::summary, orm::wq::nq, val);
	}

M_MODEL& orbtSummary(const std::string &val)
	{return whereOr(B_BASE::cols::summary, orm::wq::bt, val);
	}

M_MODEL& orbeSummary(const std::string &val)
	{return whereOr(B_BASE::cols::summary, orm::wq::be, val);
	}

M_MODEL& orltSummary(const std::string &val)
	{return whereOr(B_BASE::cols::summary, orm::wq::lt, val);
	}

M_MODEL& orleSummary(const std::string &val)
	{return whereOr(B_BASE::cols::summary, orm::wq::le, val);
	}

M_MODEL& orlikeSummary(const std::string &val)
	{return whereOr(B_BASE::cols::summary, orm::wq::like, val);
	}

M_MODEL& ornullSummary()
	{return whereOrNull(B_BASE::cols::summary);
	}

M_MODEL& ornotnullSummary()
	{return whereOrNotNull(B_BASE::cols::summary);
	}

M_MODEL& eqEditauthor(const std::string &val)
	{return where(B_BASE::cols::editauthor, orm::wq::eq, val);
	}

M_MODEL& nqEditauthor(const std::string &val)
	{return where(B_BASE::cols::editauthor, orm::wq::nq, val);
	}

M_MODEL& btEditauthor(const std::string &val)
	{return where(B_BASE::cols::editauthor, orm::wq::bt, val);
	}

M_MODEL& beEditauthor(const std::string &val)
	{return where(B_BASE::cols::editauthor, orm::wq::be, val);
	}

M_MODEL& ltEditauthor(const std::string &val)
	{return where(B_BASE::cols::editauthor, orm::wq::lt, val);
	}

M_MODEL& leEditauthor(const std::string &val)
	{return where(B_BASE::cols::editauthor, orm::wq::le, val);
	}

M_MODEL& likeEditauthor(const std::string &val)
	{return where(B_BASE::cols::editauthor, orm::wq::like, val);
	}

M_MODEL& nullEditauthor()
	{return whereNull(B_BASE::cols::editauthor);
	}

M_MODEL& notnullEditauthor()
	{return whereNotNull(B_BASE::cols::editauthor);
	}

M_MODEL& oreqEditauthor(const std::string &val)
	{return whereOr(B_BASE::cols::editauthor, orm::wq::eq, val);
	}

M_MODEL& ornqEditauthor(const std::string &val)
	{return whereOr(B_BASE::cols::editauthor, orm::wq::nq, val);
	}

M_MODEL& orbtEditauthor(const std::string &val)
	{return whereOr(B_BASE::cols::editauthor, orm::wq::bt, val);
	}

M_MODEL& orbeEditauthor(const std::string &val)
	{return whereOr(B_BASE::cols::editauthor, orm::wq::be, val);
	}

M_MODEL& orltEditauthor(const std::string &val)
	{return whereOr(B_BASE::cols::editauthor, orm::wq::lt, val);
	}

M_MODEL& orleEditauthor(const std::string &val)
	{return whereOr(B_BASE::cols::editauthor, orm::wq::le, val);
	}

M_MODEL& orlikeEditauthor(const std::string &val)
	{return whereOr(B_BASE::cols::editauthor, orm::wq::like, val);
	}

M_MODEL& ornullEditauthor()
	{return whereOrNull(B_BASE::cols::editauthor);
	}

M_MODEL& ornotnullEditauthor()
	{return whereOrNotNull(B_BASE::cols::editauthor);
	}

M_MODEL& eqRelatecontent(const std::string &val)
	{return where(B_BASE::cols::relatecontent, orm::wq::eq, val);
	}

M_MODEL& nqRelatecontent(const std::string &val)
	{return where(B_BASE::cols::relatecontent, orm::wq::nq, val);
	}

M_MODEL& btRelatecontent(const std::string &val)
	{return where(B_BASE::cols::relatecontent, orm::wq::bt, val);
	}

M_MODEL& beRelatecontent(const std::string &val)
	{return where(B_BASE::cols::relatecontent, orm::wq::be, val);
	}

M_MODEL& ltRelatecontent(const std::string &val)
	{return where(B_BASE::cols::relatecontent, orm::wq::lt, val);
	}

M_MODEL& leRelatecontent(const std::string &val)
	{return where(B_BASE::cols::relatecontent, orm::wq::le, val);
	}

M_MODEL& likeRelatecontent(const std::string &val)
	{return where(B_BASE::cols::relatecontent, orm::wq::like, val);
	}

M_MODEL& nullRelatecontent()
	{return whereNull(B_BASE::cols::relatecontent);
	}

M_MODEL& notnullRelatecontent()
	{return whereNotNull(B_BASE::cols::relatecontent);
	}

M_MODEL& oreqRelatecontent(const std::string &val)
	{return whereOr(B_BASE::cols::relatecontent, orm::wq::eq, val);
	}

M_MODEL& ornqRelatecontent(const std::string &val)
	{return whereOr(B_BASE::cols::relatecontent, orm::wq::nq, val);
	}

M_MODEL& orbtRelatecontent(const std::string &val)
	{return whereOr(B_BASE::cols::relatecontent, orm::wq::bt, val);
	}

M_MODEL& orbeRelatecontent(const std::string &val)
	{return whereOr(B_BASE::cols::relatecontent, orm::wq::be, val);
	}

M_MODEL& orltRelatecontent(const std::string &val)
	{return whereOr(B_BASE::cols::relatecontent, orm::wq::lt, val);
	}

M_MODEL& orleRelatecontent(const std::string &val)
	{return whereOr(B_BASE::cols::relatecontent, orm::wq::le, val);
	}

M_MODEL& orlikeRelatecontent(const std::string &val)
	{return whereOr(B_BASE::cols::relatecontent, orm::wq::like, val);
	}

M_MODEL& ornullRelatecontent()
	{return whereOrNull(B_BASE::cols::relatecontent);
	}

M_MODEL& ornotnullRelatecontent()
	{return whereOrNotNull(B_BASE::cols::relatecontent);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = article_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &article_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &article_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(article_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(article_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(article_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(article_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &article_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &article_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &article_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &article_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &article_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(article_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &article_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(article_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &article_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(article_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &article_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(article_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &article_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(article_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(article_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(article_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(article_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(article_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(article_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(article_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(article_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(article_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(article_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &article_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &article_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &order(article_info::cols field, const std::string &asc_or_desc)
        {
        ...
        M_MODEL &asc(article_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &article_info::col_names> field1, orm::table_col<B_BASE, &article_info::col_names> field2)
        {
        ...
        M_MODEL &asc(article_info::cols field, article_info::cols field2)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(article_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(article_info::cols field, article_info::cols field2)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &article_info::col_names> field1, orm::table_col<B_BASE, &article_info::col_names> field2)
        {
        ...
        M_MODEL &order(orm::table_col<B_BASE, &article_info::col_names> wq, const std::string &asc_or_desc)
        {
            ordersql.append(" ORDER BY ");
            ordersql.append(wq);
            ordersql.append(" ");
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &article_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &article_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &article_info::col_names> wq)
        {
        ...
        M_MODEL &having(article_info::cols field)
        {
            groupsql.append(" HAVING ");
            switch (field)
            {
			...
			case article_info::cols::keywords:
				...
				break;
			case article_info::cols::fromsource:
				groupsql.append("fromsource");
				break;
			case article_info::cols::author:
				groupsql.append("author");
				break;
			case article_info::cols::addip:
				groupsql.append("addip");
				break;
			case article_info::cols::createtime:
				groupsql.append("createtime");
				break;
			case article_info::cols::addtime:
				groupsql.append("addtime");
				break;
			case article_info::cols::readnum:
				groupsql.append("readnum");
				break;
			case article_info::cols::review:
				groupsql.append("review");
				break;
			case article_info::cols::icoimg:
				groupsql.append("icoimg");
				break;
			case article_info::cols::content:
				groupsql.append("content");
				break;
			case article_info::cols::mdcontent:
				groupsql.append("mdcontent");
				break;
			case article_info::cols::isopen:
			...
			case article_info::cols::ishome:
				...
				break;
			case article_info::cols::iscomment:
				groupsql.append("iscomment");
				break;
			case article_info::cols::showtype:
				groupsql.append("showtype");
				break;
			case article_info::cols::fromlocal:
				groupsql.append("fromlocal");
				break;
			case article_info::cols::texturl:
				groupsql.append("texturl");
				break;
			case article_info::cols::summary:
				groupsql.append("summary");
				break;
			case article_info::cols::editauthor:
				groupsql.append("editauthor");
				break;
			case article_info::cols::relatecontent:
				groupsql.append("relatecontent");
				break;
            default:
                return *mod;
                break;
        ...
        M_MODEL &group(article_info::cols field)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &article_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
        ...
        {
            model_meta_cache<article_info::meta> &data_cache = model_meta_cache<article_info::meta>::getinstance();
            data_cache.remove_exptime();

            model_meta_cache<std::vector<article_info::meta>> &record_cache = model_meta_cache<std::vector<article_info::meta>>::getinstance();
        ...
        {
            model_meta_cache<article_info::meta> &temp_cache = model_meta_cache<article_info::meta>::getinstance();
        ...
        {
            model_meta_cache<std::vector<article_info::meta>> &temp_cache = model_meta_cache<std::vector<article_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(sqlstring);
        ...
        bool save_data_cache(const std::string &cache_key_name, const article_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<article_info::meta> &temp_cache = model_meta_cache<article_info::meta>::getinstance();
        ...
        bool save_cache(std::size_t cache_key_name, const std::vector<article_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<article_info::meta>> &temp_cache = model_meta_cache<std::vector<article_info::meta>>::getinstance();
            temp_cache.save(cache_key_name, cache_data, exp_time);
            return true;
        }
        bool save_cache(const std::string cache_key_name, const std::vector<article_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<article_info::meta>> &temp_cache = model_meta_cache<std::vector<article_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_vector_cache(const std::string cache_key_name, const std::vector<article_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<article_info::meta>> &temp_cache = model_meta_cache<std::vector<article_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_cache(const std::string cache_key_name, const article_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<article_info::meta> &temp_cache = model_meta_cache<article_info::meta>::getinstance();
            std::size_t sqlhashid                               = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        const article_info::meta get_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<article_info::meta> &temp_cache = model_meta_cache<article_info::meta>::getinstance();
        ...
        const std::vector<article_info::meta> get_vector_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<std::vector<article_info::meta>> &temp_cache = model_meta_cache<std::vector<article_info::meta>>::getinstance();
        ...
        std::tuple<unsigned int, unsigned long long> insert(article_info::meta &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(article_info::meta &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_data_insert_sql(insert_data);
        ...
        std::tuple<unsigned int, unsigned long long> insert(std::vector<article_info::meta> &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(std::vector<article_info::meta> &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_vector_insert_sql(insert_data);
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = article_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(article_info::meta &insert_data)
        {
        ...
        M_MODEL &set_data(article_info::meta indata)
        {
            B_BASE::data = indata;
            return *mod;
        }
        ...
        article_opsql(const article_opsql &)            = delete;
        article_opsql &operator=(const article_opsql &) = delete;
        article_opsql(article_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        article_opsql &operator=(article_opsql &&o) noexcept
        {
            if (this != &o)
            {
        ...
        template <typename T>
        M_MODEL &AND(orm::table_col<B_BASE, &article_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = opwq;
        ...
        template <typename T>
        M_MODEL &OR(orm::table_col<B_BASE, &article_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = opwq;
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < article_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == article_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(article_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(article_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = article_info::col_names.size();
