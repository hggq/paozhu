...
{ /*tagnamespace_replace*/
    ...
    class testb_mysql : public B_BASE
    {
      public:
        testb_mysql(const std::string &tag) : dbtag(tag)
        {
        ...
        testb_mysql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned int value_size, testb_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
