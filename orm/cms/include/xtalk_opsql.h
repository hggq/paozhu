...
{ /*tagnamespace_replace*/
    ...
    class xtalk_opsql : public B_BASE
    {
      public:
        xtalk_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        xtalk_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, xtalk_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
        ...
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, xtalk_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.talkid = static_cast<decltype(data_temp.talkid )>(v);
            ...
            case 3: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.taskid = static_cast<decltype(data_temp.taskid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.taskid = static_cast<decltype(data_temp.taskid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.taskid = static_cast<decltype(data_temp.taskid )>(v);
            ...
            case 4: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.projectid = static_cast<decltype(data_temp.projectid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.projectid = static_cast<decltype(data_temp.projectid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.projectid = static_cast<decltype(data_temp.projectid )>(v);
            ...
            case 6: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.isdelete = static_cast<decltype(data_temp.isdelete )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.isdelete = static_cast<decltype(data_temp.isdelete )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.isdelete = static_cast<decltype(data_temp.isdelete )>(v);
            ...
            case 8: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.update_at = static_cast<decltype(data_temp.update_at )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.update_at = static_cast<decltype(data_temp.update_at )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.update_at = static_cast<decltype(data_temp.update_at )>(v);
            ...
            case 9: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.islock = static_cast<decltype(data_temp.islock )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.islock = static_cast<decltype(data_temp.islock )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.islock = static_cast<decltype(data_temp.islock )>(v);
            ...
            case 10: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.replyid = static_cast<decltype(data_temp.replyid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.replyid = static_cast<decltype(data_temp.replyid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.replyid = static_cast<decltype(data_temp.replyid )>(v);
        ...
        http::obj_val get_field_value(unsigned char index_pos, const xtalk_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
                return http::obj_val(static_cast<long long>(data_temp.talkid));
                break;
            case 1:
            ...
            case 2:
                ...
                break;
            case 3:
                return http::obj_val(static_cast<long long>(data_temp.taskid));
                break;
            case 4:
                return http::obj_val(static_cast<long long>(data_temp.projectid));
                break;
            case 5:
                return http::obj_val(std::string(data_temp.content));
                break;
            case 6:
                return http::obj_val(static_cast<long long>(data_temp.isdelete));
                break;
            case 7:
                return http::obj_val(static_cast<long long>(data_temp.addtime));
                break;
            case 8:
                return http::obj_val(static_cast<long long>(data_temp.update_at));
                break;
            case 9:
                return http::obj_val(static_cast<long long>(data_temp.islock));
                break;
            case 10:
                return http::obj_val(static_cast<long long>(data_temp.replyid));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const xtalk_info::meta &data_temp)
    {
        // Auto-increment primary key talkid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.talkid == 0)
            return http::obj_val(nullptr);
...
M_MODEL& eqTalkid(const std::string &val)
	{return where(B_BASE::cols::talkid, orm::wq::eq, val);
	}

M_MODEL& nqTalkid(const std::string &val)
	{return where(B_BASE::cols::talkid, orm::wq::nq, val);
	}

M_MODEL& btTalkid(const std::string &val)
	{return where(B_BASE::cols::talkid, orm::wq::bt, val);
	}

M_MODEL& beTalkid(const std::string &val)
	{return where(B_BASE::cols::talkid, orm::wq::be, val);
	}

M_MODEL& ltTalkid(const std::string &val)
	{return where(B_BASE::cols::talkid, orm::wq::lt, val);
	}

M_MODEL& leTalkid(const std::string &val)
	{return where(B_BASE::cols::talkid, orm::wq::le, val);
	}
...
M_MODEL& nullTalkid()
	{return whereNull(B_BASE::cols::talkid);
	}

M_MODEL& notnullTalkid()
	{return whereNotNull(B_BASE::cols::talkid);
	}

M_MODEL& oreqTalkid(const std::string &val)
	{return whereOr(B_BASE::cols::talkid, orm::wq::eq, val);
	}

M_MODEL& ornqTalkid(const std::string &val)
	{return whereOr(B_BASE::cols::talkid, orm::wq::nq, val);
	}

M_MODEL& orbtTalkid(const std::string &val)
	{return whereOr(B_BASE::cols::talkid, orm::wq::bt, val);
	}

M_MODEL& orbeTalkid(const std::string &val)
	{return whereOr(B_BASE::cols::talkid, orm::wq::be, val);
	}

M_MODEL& orltTalkid(const std::string &val)
	{return whereOr(B_BASE::cols::talkid, orm::wq::lt, val);
	}

M_MODEL& orleTalkid(const std::string &val)
	{return whereOr(B_BASE::cols::talkid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqTalkid(T val)
	{return whereOr(B_BASE::cols::talkid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqTalkid(T val)
	{return whereOr(B_BASE::cols::talkid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtTalkid(T val)
	{return whereOr(B_BASE::cols::talkid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeTalkid(T val)
	{return whereOr(B_BASE::cols::talkid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltTalkid(T val)
	{return whereOr(B_BASE::cols::talkid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleTalkid(T val)
	{return whereOr(B_BASE::cols::talkid, orm::wq::le, val);
	}

M_MODEL& ornullTalkid()
	{return whereOrNull(B_BASE::cols::talkid);
	}

M_MODEL& ornotnullTalkid()
	{return whereOrNotNull(B_BASE::cols::talkid);
	}
...
M_MODEL& eqTaskid(const std::string &val)
	{return where(B_BASE::cols::taskid, orm::wq::eq, val);
	}

M_MODEL& nqTaskid(const std::string &val)
	{return where(B_BASE::cols::taskid, orm::wq::nq, val);
	}

M_MODEL& btTaskid(const std::string &val)
	{return where(B_BASE::cols::taskid, orm::wq::bt, val);
	}

M_MODEL& beTaskid(const std::string &val)
	{return where(B_BASE::cols::taskid, orm::wq::be, val);
	}

M_MODEL& ltTaskid(const std::string &val)
	{return where(B_BASE::cols::taskid, orm::wq::lt, val);
	}

M_MODEL& leTaskid(const std::string &val)
	{return where(B_BASE::cols::taskid, orm::wq::le, val);
	}
...
M_MODEL& nullTaskid()
	{return whereNull(B_BASE::cols::taskid);
	}

M_MODEL& notnullTaskid()
	{return whereNotNull(B_BASE::cols::taskid);
	}

M_MODEL& oreqTaskid(const std::string &val)
	{return whereOr(B_BASE::cols::taskid, orm::wq::eq, val);
	}

M_MODEL& ornqTaskid(const std::string &val)
	{return whereOr(B_BASE::cols::taskid, orm::wq::nq, val);
	}

M_MODEL& orbtTaskid(const std::string &val)
	{return whereOr(B_BASE::cols::taskid, orm::wq::bt, val);
	}

M_MODEL& orbeTaskid(const std::string &val)
	{return whereOr(B_BASE::cols::taskid, orm::wq::be, val);
	}

M_MODEL& orltTaskid(const std::string &val)
	{return whereOr(B_BASE::cols::taskid, orm::wq::lt, val);
	}

M_MODEL& orleTaskid(const std::string &val)
	{return whereOr(B_BASE::cols::taskid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqTaskid(T val)
	{return whereOr(B_BASE::cols::taskid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqTaskid(T val)
	{return whereOr(B_BASE::cols::taskid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtTaskid(T val)
	{return whereOr(B_BASE::cols::taskid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeTaskid(T val)
	{return whereOr(B_BASE::cols::taskid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltTaskid(T val)
	{return whereOr(B_BASE::cols::taskid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleTaskid(T val)
	{return whereOr(B_BASE::cols::taskid, orm::wq::le, val);
	}

M_MODEL& ornullTaskid()
	{return whereOrNull(B_BASE::cols::taskid);
	}

M_MODEL& ornotnullTaskid()
	{return whereOrNotNull(B_BASE::cols::taskid);
	}

M_MODEL& eqProjectid(const std::string &val)
	{return where(B_BASE::cols::projectid, orm::wq::eq, val);
	}

M_MODEL& nqProjectid(const std::string &val)
	{return where(B_BASE::cols::projectid, orm::wq::nq, val);
	}

M_MODEL& btProjectid(const std::string &val)
	{return where(B_BASE::cols::projectid, orm::wq::bt, val);
	}

M_MODEL& beProjectid(const std::string &val)
	{return where(B_BASE::cols::projectid, orm::wq::be, val);
	}

M_MODEL& ltProjectid(const std::string &val)
	{return where(B_BASE::cols::projectid, orm::wq::lt, val);
	}

M_MODEL& leProjectid(const std::string &val)
	{return where(B_BASE::cols::projectid, orm::wq::le, val);
	}
...
M_MODEL& nullProjectid()
	{return whereNull(B_BASE::cols::projectid);
	}

M_MODEL& notnullProjectid()
	{return whereNotNull(B_BASE::cols::projectid);
	}

M_MODEL& oreqProjectid(const std::string &val)
	{return whereOr(B_BASE::cols::projectid, orm::wq::eq, val);
	}

M_MODEL& ornqProjectid(const std::string &val)
	{return whereOr(B_BASE::cols::projectid, orm::wq::nq, val);
	}

M_MODEL& orbtProjectid(const std::string &val)
	{return whereOr(B_BASE::cols::projectid, orm::wq::bt, val);
	}

M_MODEL& orbeProjectid(const std::string &val)
	{return whereOr(B_BASE::cols::projectid, orm::wq::be, val);
	}

M_MODEL& orltProjectid(const std::string &val)
	{return whereOr(B_BASE::cols::projectid, orm::wq::lt, val);
	}

M_MODEL& orleProjectid(const std::string &val)
	{return whereOr(B_BASE::cols::projectid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqProjectid(T val)
	{return whereOr(B_BASE::cols::projectid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqProjectid(T val)
	{return whereOr(B_BASE::cols::projectid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtProjectid(T val)
	{return whereOr(B_BASE::cols::projectid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeProjectid(T val)
	{return whereOr(B_BASE::cols::projectid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltProjectid(T val)
	{return whereOr(B_BASE::cols::projectid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleProjectid(T val)
	{return whereOr(B_BASE::cols::projectid, orm::wq::le, val);
	}

M_MODEL& ornullProjectid()
	{return whereOrNull(B_BASE::cols::projectid);
	}

M_MODEL& ornotnullProjectid()
	{return whereOrNotNull(B_BASE::cols::projectid);
	}
...
M_MODEL& eqIsdelete(const std::string &val)
	{return where(B_BASE::cols::isdelete, orm::wq::eq, val);
	}

M_MODEL& nqIsdelete(const std::string &val)
	{return where(B_BASE::cols::isdelete, orm::wq::nq, val);
	}

M_MODEL& btIsdelete(const std::string &val)
	{return where(B_BASE::cols::isdelete, orm::wq::bt, val);
	}

M_MODEL& beIsdelete(const std::string &val)
	{return where(B_BASE::cols::isdelete, orm::wq::be, val);
	}

M_MODEL& ltIsdelete(const std::string &val)
	{return where(B_BASE::cols::isdelete, orm::wq::lt, val);
	}

M_MODEL& leIsdelete(const std::string &val)
	{return where(B_BASE::cols::isdelete, orm::wq::le, val);
	}
...
M_MODEL& nullIsdelete()
	{return whereNull(B_BASE::cols::isdelete);
	}

M_MODEL& notnullIsdelete()
	{return whereNotNull(B_BASE::cols::isdelete);
	}

M_MODEL& oreqIsdelete(const std::string &val)
	{return whereOr(B_BASE::cols::isdelete, orm::wq::eq, val);
	}

M_MODEL& ornqIsdelete(const std::string &val)
	{return whereOr(B_BASE::cols::isdelete, orm::wq::nq, val);
	}

M_MODEL& orbtIsdelete(const std::string &val)
	{return whereOr(B_BASE::cols::isdelete, orm::wq::bt, val);
	}

M_MODEL& orbeIsdelete(const std::string &val)
	{return whereOr(B_BASE::cols::isdelete, orm::wq::be, val);
	}

M_MODEL& orltIsdelete(const std::string &val)
	{return whereOr(B_BASE::cols::isdelete, orm::wq::lt, val);
	}

M_MODEL& orleIsdelete(const std::string &val)
	{return whereOr(B_BASE::cols::isdelete, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqIsdelete(T val)
	{return whereOr(B_BASE::cols::isdelete, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqIsdelete(T val)
	{return whereOr(B_BASE::cols::isdelete, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtIsdelete(T val)
	{return whereOr(B_BASE::cols::isdelete, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeIsdelete(T val)
	{return whereOr(B_BASE::cols::isdelete, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltIsdelete(T val)
	{return whereOr(B_BASE::cols::isdelete, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleIsdelete(T val)
	{return whereOr(B_BASE::cols::isdelete, orm::wq::le, val);
	}

M_MODEL& ornullIsdelete()
	{return whereOrNull(B_BASE::cols::isdelete);
	}

M_MODEL& ornotnullIsdelete()
	{return whereOrNotNull(B_BASE::cols::isdelete);
	}
...
M_MODEL& eqUpdateAt(const std::string &val)
	{return where(B_BASE::cols::update_at, orm::wq::eq, val);
	}

M_MODEL& nqUpdateAt(const std::string &val)
	{return where(B_BASE::cols::update_at, orm::wq::nq, val);
	}

M_MODEL& btUpdateAt(const std::string &val)
	{return where(B_BASE::cols::update_at, orm::wq::bt, val);
	}

M_MODEL& beUpdateAt(const std::string &val)
	{return where(B_BASE::cols::update_at, orm::wq::be, val);
	}

M_MODEL& ltUpdateAt(const std::string &val)
	{return where(B_BASE::cols::update_at, orm::wq::lt, val);
	}

M_MODEL& leUpdateAt(const std::string &val)
	{return where(B_BASE::cols::update_at, orm::wq::le, val);
	}
...
M_MODEL& nullUpdateAt()
	{return whereNull(B_BASE::cols::update_at);
	}

M_MODEL& notnullUpdateAt()
	{return whereNotNull(B_BASE::cols::update_at);
	}

M_MODEL& oreqUpdateAt(const std::string &val)
	{return whereOr(B_BASE::cols::update_at, orm::wq::eq, val);
	}

M_MODEL& ornqUpdateAt(const std::string &val)
	{return whereOr(B_BASE::cols::update_at, orm::wq::nq, val);
	}

M_MODEL& orbtUpdateAt(const std::string &val)
	{return whereOr(B_BASE::cols::update_at, orm::wq::bt, val);
	}

M_MODEL& orbeUpdateAt(const std::string &val)
	{return whereOr(B_BASE::cols::update_at, orm::wq::be, val);
	}

M_MODEL& orltUpdateAt(const std::string &val)
	{return whereOr(B_BASE::cols::update_at, orm::wq::lt, val);
	}

M_MODEL& orleUpdateAt(const std::string &val)
	{return whereOr(B_BASE::cols::update_at, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqUpdateAt(T val)
	{return whereOr(B_BASE::cols::update_at, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqUpdateAt(T val)
	{return whereOr(B_BASE::cols::update_at, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtUpdateAt(T val)
	{return whereOr(B_BASE::cols::update_at, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeUpdateAt(T val)
	{return whereOr(B_BASE::cols::update_at, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltUpdateAt(T val)
	{return whereOr(B_BASE::cols::update_at, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleUpdateAt(T val)
	{return whereOr(B_BASE::cols::update_at, orm::wq::le, val);
	}

M_MODEL& ornullUpdateAt()
	{return whereOrNull(B_BASE::cols::update_at);
	}

M_MODEL& ornotnullUpdateAt()
	{return whereOrNotNull(B_BASE::cols::update_at);
	}

M_MODEL& eqIslock(const std::string &val)
	{return where(B_BASE::cols::islock, orm::wq::eq, val);
	}

M_MODEL& nqIslock(const std::string &val)
	{return where(B_BASE::cols::islock, orm::wq::nq, val);
	}

M_MODEL& btIslock(const std::string &val)
	{return where(B_BASE::cols::islock, orm::wq::bt, val);
	}

M_MODEL& beIslock(const std::string &val)
	{return where(B_BASE::cols::islock, orm::wq::be, val);
	}

M_MODEL& ltIslock(const std::string &val)
	{return where(B_BASE::cols::islock, orm::wq::lt, val);
	}

M_MODEL& leIslock(const std::string &val)
	{return where(B_BASE::cols::islock, orm::wq::le, val);
	}
...
M_MODEL& nullIslock()
	{return whereNull(B_BASE::cols::islock);
	}

M_MODEL& notnullIslock()
	{return whereNotNull(B_BASE::cols::islock);
	}

M_MODEL& oreqIslock(const std::string &val)
	{return whereOr(B_BASE::cols::islock, orm::wq::eq, val);
	}

M_MODEL& ornqIslock(const std::string &val)
	{return whereOr(B_BASE::cols::islock, orm::wq::nq, val);
	}

M_MODEL& orbtIslock(const std::string &val)
	{return whereOr(B_BASE::cols::islock, orm::wq::bt, val);
	}

M_MODEL& orbeIslock(const std::string &val)
	{return whereOr(B_BASE::cols::islock, orm::wq::be, val);
	}

M_MODEL& orltIslock(const std::string &val)
	{return whereOr(B_BASE::cols::islock, orm::wq::lt, val);
	}

M_MODEL& orleIslock(const std::string &val)
	{return whereOr(B_BASE::cols::islock, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqIslock(T val)
	{return whereOr(B_BASE::cols::islock, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqIslock(T val)
	{return whereOr(B_BASE::cols::islock, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtIslock(T val)
	{return whereOr(B_BASE::cols::islock, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeIslock(T val)
	{return whereOr(B_BASE::cols::islock, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltIslock(T val)
	{return whereOr(B_BASE::cols::islock, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleIslock(T val)
	{return whereOr(B_BASE::cols::islock, orm::wq::le, val);
	}

M_MODEL& ornullIslock()
	{return whereOrNull(B_BASE::cols::islock);
	}

M_MODEL& ornotnullIslock()
	{return whereOrNotNull(B_BASE::cols::islock);
	}

M_MODEL& eqReplyid(const std::string &val)
	{return where(B_BASE::cols::replyid, orm::wq::eq, val);
	}

M_MODEL& nqReplyid(const std::string &val)
	{return where(B_BASE::cols::replyid, orm::wq::nq, val);
	}

M_MODEL& btReplyid(const std::string &val)
	{return where(B_BASE::cols::replyid, orm::wq::bt, val);
	}

M_MODEL& beReplyid(const std::string &val)
	{return where(B_BASE::cols::replyid, orm::wq::be, val);
	}

M_MODEL& ltReplyid(const std::string &val)
	{return where(B_BASE::cols::replyid, orm::wq::lt, val);
	}

M_MODEL& leReplyid(const std::string &val)
	{return where(B_BASE::cols::replyid, orm::wq::le, val);
	}
...
M_MODEL& nullReplyid()
	{return whereNull(B_BASE::cols::replyid);
	}

M_MODEL& notnullReplyid()
	{return whereNotNull(B_BASE::cols::replyid);
	}

M_MODEL& oreqReplyid(const std::string &val)
	{return whereOr(B_BASE::cols::replyid, orm::wq::eq, val);
	}

M_MODEL& ornqReplyid(const std::string &val)
	{return whereOr(B_BASE::cols::replyid, orm::wq::nq, val);
	}

M_MODEL& orbtReplyid(const std::string &val)
	{return whereOr(B_BASE::cols::replyid, orm::wq::bt, val);
	}

M_MODEL& orbeReplyid(const std::string &val)
	{return whereOr(B_BASE::cols::replyid, orm::wq::be, val);
	}

M_MODEL& orltReplyid(const std::string &val)
	{return whereOr(B_BASE::cols::replyid, orm::wq::lt, val);
	}

M_MODEL& orleReplyid(const std::string &val)
	{return whereOr(B_BASE::cols::replyid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqReplyid(T val)
	{return whereOr(B_BASE::cols::replyid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqReplyid(T val)
	{return whereOr(B_BASE::cols::replyid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtReplyid(T val)
	{return whereOr(B_BASE::cols::replyid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeReplyid(T val)
	{return whereOr(B_BASE::cols::replyid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltReplyid(T val)
	{return whereOr(B_BASE::cols::replyid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleReplyid(T val)
	{return whereOr(B_BASE::cols::replyid, orm::wq::le, val);
	}

M_MODEL& ornullReplyid()
	{return whereOrNull(B_BASE::cols::replyid);
	}

M_MODEL& ornotnullReplyid()
	{return whereOrNotNull(B_BASE::cols::replyid);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = xtalk_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &xtalk_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &xtalk_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(xtalk_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(xtalk_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(xtalk_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(xtalk_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &xtalk_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &xtalk_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &xtalk_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &xtalk_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &xtalk_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(xtalk_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &xtalk_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(xtalk_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &xtalk_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(xtalk_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &xtalk_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(xtalk_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &xtalk_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(xtalk_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(xtalk_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(xtalk_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(xtalk_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(xtalk_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(xtalk_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(xtalk_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(xtalk_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(xtalk_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(xtalk_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &xtalk_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &xtalk_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &order(xtalk_info::cols field, const std::string &asc_or_desc)
        {
        ...
        M_MODEL &asc(xtalk_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &xtalk_info::col_names> field1, orm::table_col<B_BASE, &xtalk_info::col_names> field2)
        {
        ...
        M_MODEL &asc(xtalk_info::cols field, xtalk_info::cols field2)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(xtalk_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(xtalk_info::cols field, xtalk_info::cols field2)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &xtalk_info::col_names> field1, orm::table_col<B_BASE, &xtalk_info::col_names> field2)
        {
        ...
        M_MODEL &order(orm::table_col<B_BASE, &xtalk_info::col_names> wq, const std::string &asc_or_desc)
        {
            ordersql.append(" ORDER BY ");
            ordersql.append(wq);
            ordersql.append(" ");
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &xtalk_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &xtalk_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &xtalk_info::col_names> wq)
        {
        ...
        M_MODEL &having(xtalk_info::cols field)
        {
            groupsql.append(" HAVING ");
            switch (field)
            {
			...
			case xtalk_info::cols::adminid:
				...
				break;
			case xtalk_info::cols::taskid:
				groupsql.append("taskid");
				break;
			case xtalk_info::cols::projectid:
				groupsql.append("projectid");
				break;
			case xtalk_info::cols::content:
				groupsql.append("content");
				break;
			case xtalk_info::cols::isdelete:
				groupsql.append("isdelete");
				break;
			case xtalk_info::cols::addtime:
				groupsql.append("addtime");
				break;
			case xtalk_info::cols::update_at:
				groupsql.append("update_at");
				break;
			case xtalk_info::cols::islock:
				groupsql.append("islock");
				break;
			case xtalk_info::cols::replyid:
				groupsql.append("replyid");
				break;
            default:
                return *mod;
                break;
        ...
        M_MODEL &group(xtalk_info::cols field)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &xtalk_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
        ...
        {
            model_meta_cache<xtalk_info::meta> &data_cache = model_meta_cache<xtalk_info::meta>::getinstance();
            data_cache.remove_exptime();

            model_meta_cache<std::vector<xtalk_info::meta>> &record_cache = model_meta_cache<std::vector<xtalk_info::meta>>::getinstance();
        ...
        bool save_data_cache(const std::string &cache_key_name, const xtalk_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<xtalk_info::meta> &temp_cache = model_meta_cache<xtalk_info::meta>::getinstance();
        ...
        bool save_cache(std::size_t cache_key_name, const std::vector<xtalk_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<xtalk_info::meta>> &temp_cache = model_meta_cache<std::vector<xtalk_info::meta>>::getinstance();
            temp_cache.save(cache_key_name, cache_data, exp_time);
            return true;
        }
        bool save_cache(const std::string cache_key_name, const std::vector<xtalk_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<xtalk_info::meta>> &temp_cache = model_meta_cache<std::vector<xtalk_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_vector_cache(const std::string cache_key_name, const std::vector<xtalk_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<xtalk_info::meta>> &temp_cache = model_meta_cache<std::vector<xtalk_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_cache(const std::string cache_key_name, const xtalk_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<xtalk_info::meta> &temp_cache = model_meta_cache<xtalk_info::meta>::getinstance();
            std::size_t sqlhashid                               = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        std::tuple<unsigned int, unsigned long long> insert(xtalk_info::meta &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(xtalk_info::meta &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_data_insert_sql(insert_data);
        ...
        std::tuple<unsigned int, unsigned long long> insert(std::vector<xtalk_info::meta> &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(std::vector<xtalk_info::meta> &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_vector_insert_sql(insert_data);
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = xtalk_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(xtalk_info::meta &insert_data)
        {
        ...
        M_MODEL &set_data(xtalk_info::meta indata)
        {
            B_BASE::data = indata;
            return *mod;
        }
        ...
        xtalk_opsql(const xtalk_opsql &)            = delete;
        xtalk_opsql &operator=(const xtalk_opsql &) = delete;
        xtalk_opsql(xtalk_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        xtalk_opsql &operator=(xtalk_opsql &&o) noexcept
        {
            if (this != &o)
            {
        ...
        template <typename T>
        M_MODEL &AND(orm::table_col<B_BASE, &xtalk_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = opwq;
        ...
        template <typename T>
        M_MODEL &OR(orm::table_col<B_BASE, &xtalk_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = opwq;
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < xtalk_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == xtalk_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(xtalk_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(xtalk_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = xtalk_info::col_names.size();
