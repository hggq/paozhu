...
{ /*tagnamespace_replace*/
    ...
    class xmeet_opsql : public B_BASE
    {
      public:
        xmeet_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        xmeet_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, xmeet_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            break;
                case 6:
            data_temp.zhuchi.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 7:
            data_temp.jilu.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 8:
            data_temp.meetnotice.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 9:
            data_temp.meetfiles.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 10:
            data_temp.address.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 11:
            data_temp.meettime.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 12:
             {
               data_temp.regdate = 0;
            ...
            break;
                case 15:
            data_temp.presents.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 16:
            data_temp.content.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 17:
            data_temp.postresult.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 18:
            data_temp.postfiles.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 19:
            data_temp.jiluphoto.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                
        }
    }
    
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, xmeet_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.xmeetid = static_cast<decltype(data_temp.xmeetid )>(v);
            ...
            case 6: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.zhuchi.assign(v.data(), v.size());
                    else
                        data_temp.zhuchi.clear();
                }, val);
            } break;
            case 7: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.jilu.assign(v.data(), v.size());
                    else
                        data_temp.jilu.clear();
                }, val);
            } break;
            case 8: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.meetnotice.assign(v.data(), v.size());
                    else
                        data_temp.meetnotice.clear();
                }, val);
            } break;
            case 9: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.meetfiles.assign(v.data(), v.size());
                    else
                        data_temp.meetfiles.clear();
                }, val);
            } break;
            ...
            case 11: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.meettime.assign(v.data(), v.size());
                    else
                        data_temp.meettime.clear();
                }, val);
            } break;
            ...
            case 13: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.expecttime = static_cast<decltype(data_temp.expecttime )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.expecttime = static_cast<decltype(data_temp.expecttime )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.expecttime = static_cast<decltype(data_temp.expecttime )>(v);
            ...
            case 14: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.endtime = static_cast<decltype(data_temp.endtime )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.endtime = static_cast<decltype(data_temp.endtime )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.endtime = static_cast<decltype(data_temp.endtime )>(v);
            ...
            case 15: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.presents.assign(v.data(), v.size());
                    else
                        data_temp.presents.clear();
                }, val);
            } break;
            ...
            case 17: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.postresult.assign(v.data(), v.size());
                    else
                        data_temp.postresult.clear();
                }, val);
            } break;
            case 18: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.postfiles.assign(v.data(), v.size());
                    else
                        data_temp.postfiles.clear();
                }, val);
            } break;
            case 19: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.jiluphoto.assign(v.data(), v.size());
                    else
                        data_temp.jiluphoto.clear();
                }, val);
            } break;
        ...
        http::obj_val get_field_value(unsigned char index_pos, const xmeet_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
                return http::obj_val(static_cast<long long>(data_temp.xmeetid));
                break;
            case 1:
            ...
            case 5:
                ...
                break;
            case 6:
                return http::obj_val(std::string(data_temp.zhuchi));
                break;
            case 7:
                return http::obj_val(std::string(data_temp.jilu));
                break;
            case 8:
                return http::obj_val(std::string(data_temp.meetnotice));
                break;
            case 9:
                return http::obj_val(std::string(data_temp.meetfiles));
                break;
            case 10:
                return http::obj_val(std::string(data_temp.address));
                break;
            case 11:
                return http::obj_val(std::string(data_temp.meettime));
                break;
            case 12:
                return http::obj_val(static_cast<long long>(data_temp.regdate));
                break;
            case 13:
                return http::obj_val(static_cast<long long>(data_temp.expecttime));
                break;
            case 14:
                return http::obj_val(static_cast<long long>(data_temp.endtime));
                break;
            case 15:
                return http::obj_val(std::string(data_temp.presents));
                break;
            case 16:
                return http::obj_val(std::string(data_temp.content));
                break;
            case 17:
                return http::obj_val(std::string(data_temp.postresult));
                break;
            case 18:
                return http::obj_val(std::string(data_temp.postfiles));
                break;
            case 19:
                return http::obj_val(std::string(data_temp.jiluphoto));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const xmeet_info::meta &data_temp)
    {
        // Auto-increment primary key xmeetid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.xmeetid == 0)
            return http::obj_val(nullptr);
...
M_MODEL& eqXmeetid(const std::string &val)
	{return where(B_BASE::cols::xmeetid, orm::wq::eq, val);
	}

M_MODEL& nqXmeetid(const std::string &val)
	{return where(B_BASE::cols::xmeetid, orm::wq::nq, val);
	}

M_MODEL& btXmeetid(const std::string &val)
	{return where(B_BASE::cols::xmeetid, orm::wq::bt, val);
	}

M_MODEL& beXmeetid(const std::string &val)
	{return where(B_BASE::cols::xmeetid, orm::wq::be, val);
	}

M_MODEL& ltXmeetid(const std::string &val)
	{return where(B_BASE::cols::xmeetid, orm::wq::lt, val);
	}

M_MODEL& leXmeetid(const std::string &val)
	{return where(B_BASE::cols::xmeetid, orm::wq::le, val);
	}
...
M_MODEL& nullXmeetid()
	{return whereNull(B_BASE::cols::xmeetid);
	}

M_MODEL& notnullXmeetid()
	{return whereNotNull(B_BASE::cols::xmeetid);
	}

M_MODEL& oreqXmeetid(const std::string &val)
	{return whereOr(B_BASE::cols::xmeetid, orm::wq::eq, val);
	}

M_MODEL& ornqXmeetid(const std::string &val)
	{return whereOr(B_BASE::cols::xmeetid, orm::wq::nq, val);
	}

M_MODEL& orbtXmeetid(const std::string &val)
	{return whereOr(B_BASE::cols::xmeetid, orm::wq::bt, val);
	}

M_MODEL& orbeXmeetid(const std::string &val)
	{return whereOr(B_BASE::cols::xmeetid, orm::wq::be, val);
	}

M_MODEL& orltXmeetid(const std::string &val)
	{return whereOr(B_BASE::cols::xmeetid, orm::wq::lt, val);
	}

M_MODEL& orleXmeetid(const std::string &val)
	{return whereOr(B_BASE::cols::xmeetid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqXmeetid(T val)
	{return whereOr(B_BASE::cols::xmeetid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqXmeetid(T val)
	{return whereOr(B_BASE::cols::xmeetid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtXmeetid(T val)
	{return whereOr(B_BASE::cols::xmeetid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeXmeetid(T val)
	{return whereOr(B_BASE::cols::xmeetid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltXmeetid(T val)
	{return whereOr(B_BASE::cols::xmeetid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleXmeetid(T val)
	{return whereOr(B_BASE::cols::xmeetid, orm::wq::le, val);
	}

M_MODEL& ornullXmeetid()
	{return whereOrNull(B_BASE::cols::xmeetid);
	}

M_MODEL& ornotnullXmeetid()
	{return whereOrNotNull(B_BASE::cols::xmeetid);
	}
...
M_MODEL& eqZhuchi(const std::string &val)
	{return where(B_BASE::cols::zhuchi, orm::wq::eq, val);
	}

M_MODEL& nqZhuchi(const std::string &val)
	{return where(B_BASE::cols::zhuchi, orm::wq::nq, val);
	}

M_MODEL& btZhuchi(const std::string &val)
	{return where(B_BASE::cols::zhuchi, orm::wq::bt, val);
	}

M_MODEL& beZhuchi(const std::string &val)
	{return where(B_BASE::cols::zhuchi, orm::wq::be, val);
	}

M_MODEL& ltZhuchi(const std::string &val)
	{return where(B_BASE::cols::zhuchi, orm::wq::lt, val);
	}

M_MODEL& leZhuchi(const std::string &val)
	{return where(B_BASE::cols::zhuchi, orm::wq::le, val);
	}

M_MODEL& likeZhuchi(const std::string &val)
	{return where(B_BASE::cols::zhuchi, orm::wq::like, val);
	}

M_MODEL& nullZhuchi()
	{return whereNull(B_BASE::cols::zhuchi);
	}

M_MODEL& notnullZhuchi()
	{return whereNotNull(B_BASE::cols::zhuchi);
	}

M_MODEL& oreqZhuchi(const std::string &val)
	{return whereOr(B_BASE::cols::zhuchi, orm::wq::eq, val);
	}

M_MODEL& ornqZhuchi(const std::string &val)
	{return whereOr(B_BASE::cols::zhuchi, orm::wq::nq, val);
	}

M_MODEL& orbtZhuchi(const std::string &val)
	{return whereOr(B_BASE::cols::zhuchi, orm::wq::bt, val);
	}

M_MODEL& orbeZhuchi(const std::string &val)
	{return whereOr(B_BASE::cols::zhuchi, orm::wq::be, val);
	}

M_MODEL& orltZhuchi(const std::string &val)
	{return whereOr(B_BASE::cols::zhuchi, orm::wq::lt, val);
	}

M_MODEL& orleZhuchi(const std::string &val)
	{return whereOr(B_BASE::cols::zhuchi, orm::wq::le, val);
	}

M_MODEL& orlikeZhuchi(const std::string &val)
	{return whereOr(B_BASE::cols::zhuchi, orm::wq::like, val);
	}

M_MODEL& ornullZhuchi()
	{return whereOrNull(B_BASE::cols::zhuchi);
	}

M_MODEL& ornotnullZhuchi()
	{return whereOrNotNull(B_BASE::cols::zhuchi);
	}

M_MODEL& eqJilu(const std::string &val)
	{return where(B_BASE::cols::jilu, orm::wq::eq, val);
	}

M_MODEL& nqJilu(const std::string &val)
	{return where(B_BASE::cols::jilu, orm::wq::nq, val);
	}

M_MODEL& btJilu(const std::string &val)
	{return where(B_BASE::cols::jilu, orm::wq::bt, val);
	}

M_MODEL& beJilu(const std::string &val)
	{return where(B_BASE::cols::jilu, orm::wq::be, val);
	}

M_MODEL& ltJilu(const std::string &val)
	{return where(B_BASE::cols::jilu, orm::wq::lt, val);
	}

M_MODEL& leJilu(const std::string &val)
	{return where(B_BASE::cols::jilu, orm::wq::le, val);
	}

M_MODEL& likeJilu(const std::string &val)
	{return where(B_BASE::cols::jilu, orm::wq::like, val);
	}

M_MODEL& nullJilu()
	{return whereNull(B_BASE::cols::jilu);
	}

M_MODEL& notnullJilu()
	{return whereNotNull(B_BASE::cols::jilu);
	}

M_MODEL& oreqJilu(const std::string &val)
	{return whereOr(B_BASE::cols::jilu, orm::wq::eq, val);
	}

M_MODEL& ornqJilu(const std::string &val)
	{return whereOr(B_BASE::cols::jilu, orm::wq::nq, val);
	}

M_MODEL& orbtJilu(const std::string &val)
	{return whereOr(B_BASE::cols::jilu, orm::wq::bt, val);
	}

M_MODEL& orbeJilu(const std::string &val)
	{return whereOr(B_BASE::cols::jilu, orm::wq::be, val);
	}

M_MODEL& orltJilu(const std::string &val)
	{return whereOr(B_BASE::cols::jilu, orm::wq::lt, val);
	}

M_MODEL& orleJilu(const std::string &val)
	{return whereOr(B_BASE::cols::jilu, orm::wq::le, val);
	}

M_MODEL& orlikeJilu(const std::string &val)
	{return whereOr(B_BASE::cols::jilu, orm::wq::like, val);
	}

M_MODEL& ornullJilu()
	{return whereOrNull(B_BASE::cols::jilu);
	}

M_MODEL& ornotnullJilu()
	{return whereOrNotNull(B_BASE::cols::jilu);
	}

M_MODEL& eqMeetnotice(const std::string &val)
	{return where(B_BASE::cols::meetnotice, orm::wq::eq, val);
	}

M_MODEL& nqMeetnotice(const std::string &val)
	{return where(B_BASE::cols::meetnotice, orm::wq::nq, val);
	}

M_MODEL& btMeetnotice(const std::string &val)
	{return where(B_BASE::cols::meetnotice, orm::wq::bt, val);
	}

M_MODEL& beMeetnotice(const std::string &val)
	{return where(B_BASE::cols::meetnotice, orm::wq::be, val);
	}

M_MODEL& ltMeetnotice(const std::string &val)
	{return where(B_BASE::cols::meetnotice, orm::wq::lt, val);
	}

M_MODEL& leMeetnotice(const std::string &val)
	{return where(B_BASE::cols::meetnotice, orm::wq::le, val);
	}

M_MODEL& likeMeetnotice(const std::string &val)
	{return where(B_BASE::cols::meetnotice, orm::wq::like, val);
	}

M_MODEL& nullMeetnotice()
	{return whereNull(B_BASE::cols::meetnotice);
	}

M_MODEL& notnullMeetnotice()
	{return whereNotNull(B_BASE::cols::meetnotice);
	}

M_MODEL& oreqMeetnotice(const std::string &val)
	{return whereOr(B_BASE::cols::meetnotice, orm::wq::eq, val);
	}

M_MODEL& ornqMeetnotice(const std::string &val)
	{return whereOr(B_BASE::cols::meetnotice, orm::wq::nq, val);
	}

M_MODEL& orbtMeetnotice(const std::string &val)
	{return whereOr(B_BASE::cols::meetnotice, orm::wq::bt, val);
	}

M_MODEL& orbeMeetnotice(const std::string &val)
	{return whereOr(B_BASE::cols::meetnotice, orm::wq::be, val);
	}

M_MODEL& orltMeetnotice(const std::string &val)
	{return whereOr(B_BASE::cols::meetnotice, orm::wq::lt, val);
	}

M_MODEL& orleMeetnotice(const std::string &val)
	{return whereOr(B_BASE::cols::meetnotice, orm::wq::le, val);
	}

M_MODEL& orlikeMeetnotice(const std::string &val)
	{return whereOr(B_BASE::cols::meetnotice, orm::wq::like, val);
	}

M_MODEL& ornullMeetnotice()
	{return whereOrNull(B_BASE::cols::meetnotice);
	}

M_MODEL& ornotnullMeetnotice()
	{return whereOrNotNull(B_BASE::cols::meetnotice);
	}

M_MODEL& eqMeetfiles(const std::string &val)
	{return where(B_BASE::cols::meetfiles, orm::wq::eq, val);
	}

M_MODEL& nqMeetfiles(const std::string &val)
	{return where(B_BASE::cols::meetfiles, orm::wq::nq, val);
	}

M_MODEL& btMeetfiles(const std::string &val)
	{return where(B_BASE::cols::meetfiles, orm::wq::bt, val);
	}

M_MODEL& beMeetfiles(const std::string &val)
	{return where(B_BASE::cols::meetfiles, orm::wq::be, val);
	}

M_MODEL& ltMeetfiles(const std::string &val)
	{return where(B_BASE::cols::meetfiles, orm::wq::lt, val);
	}

M_MODEL& leMeetfiles(const std::string &val)
	{return where(B_BASE::cols::meetfiles, orm::wq::le, val);
	}

M_MODEL& likeMeetfiles(const std::string &val)
	{return where(B_BASE::cols::meetfiles, orm::wq::like, val);
	}

M_MODEL& nullMeetfiles()
	{return whereNull(B_BASE::cols::meetfiles);
	}

M_MODEL& notnullMeetfiles()
	{return whereNotNull(B_BASE::cols::meetfiles);
	}

M_MODEL& oreqMeetfiles(const std::string &val)
	{return whereOr(B_BASE::cols::meetfiles, orm::wq::eq, val);
	}

M_MODEL& ornqMeetfiles(const std::string &val)
	{return whereOr(B_BASE::cols::meetfiles, orm::wq::nq, val);
	}

M_MODEL& orbtMeetfiles(const std::string &val)
	{return whereOr(B_BASE::cols::meetfiles, orm::wq::bt, val);
	}

M_MODEL& orbeMeetfiles(const std::string &val)
	{return whereOr(B_BASE::cols::meetfiles, orm::wq::be, val);
	}

M_MODEL& orltMeetfiles(const std::string &val)
	{return whereOr(B_BASE::cols::meetfiles, orm::wq::lt, val);
	}

M_MODEL& orleMeetfiles(const std::string &val)
	{return whereOr(B_BASE::cols::meetfiles, orm::wq::le, val);
	}

M_MODEL& orlikeMeetfiles(const std::string &val)
	{return whereOr(B_BASE::cols::meetfiles, orm::wq::like, val);
	}

M_MODEL& ornullMeetfiles()
	{return whereOrNull(B_BASE::cols::meetfiles);
	}

M_MODEL& ornotnullMeetfiles()
	{return whereOrNotNull(B_BASE::cols::meetfiles);
	}
...
M_MODEL& eqMeettime(const std::string &val)
	{return where(B_BASE::cols::meettime, orm::wq::eq, val);
	}

M_MODEL& nqMeettime(const std::string &val)
	{return where(B_BASE::cols::meettime, orm::wq::nq, val);
	}

M_MODEL& btMeettime(const std::string &val)
	{return where(B_BASE::cols::meettime, orm::wq::bt, val);
	}

M_MODEL& beMeettime(const std::string &val)
	{return where(B_BASE::cols::meettime, orm::wq::be, val);
	}

M_MODEL& ltMeettime(const std::string &val)
	{return where(B_BASE::cols::meettime, orm::wq::lt, val);
	}

M_MODEL& leMeettime(const std::string &val)
	{return where(B_BASE::cols::meettime, orm::wq::le, val);
	}

M_MODEL& likeMeettime(const std::string &val)
	{return where(B_BASE::cols::meettime, orm::wq::like, val);
	}

M_MODEL& nullMeettime()
	{return whereNull(B_BASE::cols::meettime);
	}

M_MODEL& notnullMeettime()
	{return whereNotNull(B_BASE::cols::meettime);
	}

M_MODEL& oreqMeettime(const std::string &val)
	{return whereOr(B_BASE::cols::meettime, orm::wq::eq, val);
	}

M_MODEL& ornqMeettime(const std::string &val)
	{return whereOr(B_BASE::cols::meettime, orm::wq::nq, val);
	}

M_MODEL& orbtMeettime(const std::string &val)
	{return whereOr(B_BASE::cols::meettime, orm::wq::bt, val);
	}

M_MODEL& orbeMeettime(const std::string &val)
	{return whereOr(B_BASE::cols::meettime, orm::wq::be, val);
	}

M_MODEL& orltMeettime(const std::string &val)
	{return whereOr(B_BASE::cols::meettime, orm::wq::lt, val);
	}

M_MODEL& orleMeettime(const std::string &val)
	{return whereOr(B_BASE::cols::meettime, orm::wq::le, val);
	}

M_MODEL& orlikeMeettime(const std::string &val)
	{return whereOr(B_BASE::cols::meettime, orm::wq::like, val);
	}

M_MODEL& ornullMeettime()
	{return whereOrNull(B_BASE::cols::meettime);
	}

M_MODEL& ornotnullMeettime()
	{return whereOrNotNull(B_BASE::cols::meettime);
	}
...
M_MODEL& eqExpecttime(const std::string &val)
	{return where(B_BASE::cols::expecttime, orm::wq::eq, val);
	}

M_MODEL& nqExpecttime(const std::string &val)
	{return where(B_BASE::cols::expecttime, orm::wq::nq, val);
	}

M_MODEL& btExpecttime(const std::string &val)
	{return where(B_BASE::cols::expecttime, orm::wq::bt, val);
	}

M_MODEL& beExpecttime(const std::string &val)
	{return where(B_BASE::cols::expecttime, orm::wq::be, val);
	}

M_MODEL& ltExpecttime(const std::string &val)
	{return where(B_BASE::cols::expecttime, orm::wq::lt, val);
	}

M_MODEL& leExpecttime(const std::string &val)
	{return where(B_BASE::cols::expecttime, orm::wq::le, val);
	}
...
M_MODEL& nullExpecttime()
	{return whereNull(B_BASE::cols::expecttime);
	}

M_MODEL& notnullExpecttime()
	{return whereNotNull(B_BASE::cols::expecttime);
	}

M_MODEL& oreqExpecttime(const std::string &val)
	{return whereOr(B_BASE::cols::expecttime, orm::wq::eq, val);
	}

M_MODEL& ornqExpecttime(const std::string &val)
	{return whereOr(B_BASE::cols::expecttime, orm::wq::nq, val);
	}

M_MODEL& orbtExpecttime(const std::string &val)
	{return whereOr(B_BASE::cols::expecttime, orm::wq::bt, val);
	}

M_MODEL& orbeExpecttime(const std::string &val)
	{return whereOr(B_BASE::cols::expecttime, orm::wq::be, val);
	}

M_MODEL& orltExpecttime(const std::string &val)
	{return whereOr(B_BASE::cols::expecttime, orm::wq::lt, val);
	}

M_MODEL& orleExpecttime(const std::string &val)
	{return whereOr(B_BASE::cols::expecttime, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqExpecttime(T val)
	{return whereOr(B_BASE::cols::expecttime, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqExpecttime(T val)
	{return whereOr(B_BASE::cols::expecttime, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtExpecttime(T val)
	{return whereOr(B_BASE::cols::expecttime, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeExpecttime(T val)
	{return whereOr(B_BASE::cols::expecttime, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltExpecttime(T val)
	{return whereOr(B_BASE::cols::expecttime, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleExpecttime(T val)
	{return whereOr(B_BASE::cols::expecttime, orm::wq::le, val);
	}

M_MODEL& ornullExpecttime()
	{return whereOrNull(B_BASE::cols::expecttime);
	}

M_MODEL& ornotnullExpecttime()
	{return whereOrNotNull(B_BASE::cols::expecttime);
	}

M_MODEL& eqEndtime(const std::string &val)
	{return where(B_BASE::cols::endtime, orm::wq::eq, val);
	}

M_MODEL& nqEndtime(const std::string &val)
	{return where(B_BASE::cols::endtime, orm::wq::nq, val);
	}

M_MODEL& btEndtime(const std::string &val)
	{return where(B_BASE::cols::endtime, orm::wq::bt, val);
	}

M_MODEL& beEndtime(const std::string &val)
	{return where(B_BASE::cols::endtime, orm::wq::be, val);
	}

M_MODEL& ltEndtime(const std::string &val)
	{return where(B_BASE::cols::endtime, orm::wq::lt, val);
	}

M_MODEL& leEndtime(const std::string &val)
	{return where(B_BASE::cols::endtime, orm::wq::le, val);
	}
...
M_MODEL& nullEndtime()
	{return whereNull(B_BASE::cols::endtime);
	}

M_MODEL& notnullEndtime()
	{return whereNotNull(B_BASE::cols::endtime);
	}

M_MODEL& oreqEndtime(const std::string &val)
	{return whereOr(B_BASE::cols::endtime, orm::wq::eq, val);
	}

M_MODEL& ornqEndtime(const std::string &val)
	{return whereOr(B_BASE::cols::endtime, orm::wq::nq, val);
	}

M_MODEL& orbtEndtime(const std::string &val)
	{return whereOr(B_BASE::cols::endtime, orm::wq::bt, val);
	}

M_MODEL& orbeEndtime(const std::string &val)
	{return whereOr(B_BASE::cols::endtime, orm::wq::be, val);
	}

M_MODEL& orltEndtime(const std::string &val)
	{return whereOr(B_BASE::cols::endtime, orm::wq::lt, val);
	}

M_MODEL& orleEndtime(const std::string &val)
	{return whereOr(B_BASE::cols::endtime, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqEndtime(T val)
	{return whereOr(B_BASE::cols::endtime, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqEndtime(T val)
	{return whereOr(B_BASE::cols::endtime, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtEndtime(T val)
	{return whereOr(B_BASE::cols::endtime, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeEndtime(T val)
	{return whereOr(B_BASE::cols::endtime, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltEndtime(T val)
	{return whereOr(B_BASE::cols::endtime, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleEndtime(T val)
	{return whereOr(B_BASE::cols::endtime, orm::wq::le, val);
	}

M_MODEL& ornullEndtime()
	{return whereOrNull(B_BASE::cols::endtime);
	}

M_MODEL& ornotnullEndtime()
	{return whereOrNotNull(B_BASE::cols::endtime);
	}

M_MODEL& eqPresents(const std::string &val)
	{return where(B_BASE::cols::presents, orm::wq::eq, val);
	}

M_MODEL& nqPresents(const std::string &val)
	{return where(B_BASE::cols::presents, orm::wq::nq, val);
	}

M_MODEL& btPresents(const std::string &val)
	{return where(B_BASE::cols::presents, orm::wq::bt, val);
	}

M_MODEL& bePresents(const std::string &val)
	{return where(B_BASE::cols::presents, orm::wq::be, val);
	}

M_MODEL& ltPresents(const std::string &val)
	{return where(B_BASE::cols::presents, orm::wq::lt, val);
	}

M_MODEL& lePresents(const std::string &val)
	{return where(B_BASE::cols::presents, orm::wq::le, val);
	}

M_MODEL& likePresents(const std::string &val)
	{return where(B_BASE::cols::presents, orm::wq::like, val);
	}

M_MODEL& nullPresents()
	{return whereNull(B_BASE::cols::presents);
	}

M_MODEL& notnullPresents()
	{return whereNotNull(B_BASE::cols::presents);
	}

M_MODEL& oreqPresents(const std::string &val)
	{return whereOr(B_BASE::cols::presents, orm::wq::eq, val);
	}

M_MODEL& ornqPresents(const std::string &val)
	{return whereOr(B_BASE::cols::presents, orm::wq::nq, val);
	}

M_MODEL& orbtPresents(const std::string &val)
	{return whereOr(B_BASE::cols::presents, orm::wq::bt, val);
	}

M_MODEL& orbePresents(const std::string &val)
	{return whereOr(B_BASE::cols::presents, orm::wq::be, val);
	}

M_MODEL& orltPresents(const std::string &val)
	{return whereOr(B_BASE::cols::presents, orm::wq::lt, val);
	}

M_MODEL& orlePresents(const std::string &val)
	{return whereOr(B_BASE::cols::presents, orm::wq::le, val);
	}

M_MODEL& orlikePresents(const std::string &val)
	{return whereOr(B_BASE::cols::presents, orm::wq::like, val);
	}

M_MODEL& ornullPresents()
	{return whereOrNull(B_BASE::cols::presents);
	}

M_MODEL& ornotnullPresents()
	{return whereOrNotNull(B_BASE::cols::presents);
	}
...
M_MODEL& eqPostresult(const std::string &val)
	{return where(B_BASE::cols::postresult, orm::wq::eq, val);
	}

M_MODEL& nqPostresult(const std::string &val)
	{return where(B_BASE::cols::postresult, orm::wq::nq, val);
	}

M_MODEL& btPostresult(const std::string &val)
	{return where(B_BASE::cols::postresult, orm::wq::bt, val);
	}

M_MODEL& bePostresult(const std::string &val)
	{return where(B_BASE::cols::postresult, orm::wq::be, val);
	}

M_MODEL& ltPostresult(const std::string &val)
	{return where(B_BASE::cols::postresult, orm::wq::lt, val);
	}

M_MODEL& lePostresult(const std::string &val)
	{return where(B_BASE::cols::postresult, orm::wq::le, val);
	}

M_MODEL& likePostresult(const std::string &val)
	{return where(B_BASE::cols::postresult, orm::wq::like, val);
	}

M_MODEL& nullPostresult()
	{return whereNull(B_BASE::cols::postresult);
	}

M_MODEL& notnullPostresult()
	{return whereNotNull(B_BASE::cols::postresult);
	}

M_MODEL& oreqPostresult(const std::string &val)
	{return whereOr(B_BASE::cols::postresult, orm::wq::eq, val);
	}

M_MODEL& ornqPostresult(const std::string &val)
	{return whereOr(B_BASE::cols::postresult, orm::wq::nq, val);
	}

M_MODEL& orbtPostresult(const std::string &val)
	{return whereOr(B_BASE::cols::postresult, orm::wq::bt, val);
	}

M_MODEL& orbePostresult(const std::string &val)
	{return whereOr(B_BASE::cols::postresult, orm::wq::be, val);
	}

M_MODEL& orltPostresult(const std::string &val)
	{return whereOr(B_BASE::cols::postresult, orm::wq::lt, val);
	}

M_MODEL& orlePostresult(const std::string &val)
	{return whereOr(B_BASE::cols::postresult, orm::wq::le, val);
	}

M_MODEL& orlikePostresult(const std::string &val)
	{return whereOr(B_BASE::cols::postresult, orm::wq::like, val);
	}

M_MODEL& ornullPostresult()
	{return whereOrNull(B_BASE::cols::postresult);
	}

M_MODEL& ornotnullPostresult()
	{return whereOrNotNull(B_BASE::cols::postresult);
	}

M_MODEL& eqPostfiles(const std::string &val)
	{return where(B_BASE::cols::postfiles, orm::wq::eq, val);
	}

M_MODEL& nqPostfiles(const std::string &val)
	{return where(B_BASE::cols::postfiles, orm::wq::nq, val);
	}

M_MODEL& btPostfiles(const std::string &val)
	{return where(B_BASE::cols::postfiles, orm::wq::bt, val);
	}

M_MODEL& bePostfiles(const std::string &val)
	{return where(B_BASE::cols::postfiles, orm::wq::be, val);
	}

M_MODEL& ltPostfiles(const std::string &val)
	{return where(B_BASE::cols::postfiles, orm::wq::lt, val);
	}

M_MODEL& lePostfiles(const std::string &val)
	{return where(B_BASE::cols::postfiles, orm::wq::le, val);
	}

M_MODEL& likePostfiles(const std::string &val)
	{return where(B_BASE::cols::postfiles, orm::wq::like, val);
	}

M_MODEL& nullPostfiles()
	{return whereNull(B_BASE::cols::postfiles);
	}

M_MODEL& notnullPostfiles()
	{return whereNotNull(B_BASE::cols::postfiles);
	}

M_MODEL& oreqPostfiles(const std::string &val)
	{return whereOr(B_BASE::cols::postfiles, orm::wq::eq, val);
	}

M_MODEL& ornqPostfiles(const std::string &val)
	{return whereOr(B_BASE::cols::postfiles, orm::wq::nq, val);
	}

M_MODEL& orbtPostfiles(const std::string &val)
	{return whereOr(B_BASE::cols::postfiles, orm::wq::bt, val);
	}

M_MODEL& orbePostfiles(const std::string &val)
	{return whereOr(B_BASE::cols::postfiles, orm::wq::be, val);
	}

M_MODEL& orltPostfiles(const std::string &val)
	{return whereOr(B_BASE::cols::postfiles, orm::wq::lt, val);
	}

M_MODEL& orlePostfiles(const std::string &val)
	{return whereOr(B_BASE::cols::postfiles, orm::wq::le, val);
	}

M_MODEL& orlikePostfiles(const std::string &val)
	{return whereOr(B_BASE::cols::postfiles, orm::wq::like, val);
	}

M_MODEL& ornullPostfiles()
	{return whereOrNull(B_BASE::cols::postfiles);
	}

M_MODEL& ornotnullPostfiles()
	{return whereOrNotNull(B_BASE::cols::postfiles);
	}

M_MODEL& eqJiluphoto(const std::string &val)
	{return where(B_BASE::cols::jiluphoto, orm::wq::eq, val);
	}

M_MODEL& nqJiluphoto(const std::string &val)
	{return where(B_BASE::cols::jiluphoto, orm::wq::nq, val);
	}

M_MODEL& btJiluphoto(const std::string &val)
	{return where(B_BASE::cols::jiluphoto, orm::wq::bt, val);
	}

M_MODEL& beJiluphoto(const std::string &val)
	{return where(B_BASE::cols::jiluphoto, orm::wq::be, val);
	}

M_MODEL& ltJiluphoto(const std::string &val)
	{return where(B_BASE::cols::jiluphoto, orm::wq::lt, val);
	}

M_MODEL& leJiluphoto(const std::string &val)
	{return where(B_BASE::cols::jiluphoto, orm::wq::le, val);
	}

M_MODEL& likeJiluphoto(const std::string &val)
	{return where(B_BASE::cols::jiluphoto, orm::wq::like, val);
	}

M_MODEL& nullJiluphoto()
	{return whereNull(B_BASE::cols::jiluphoto);
	}

M_MODEL& notnullJiluphoto()
	{return whereNotNull(B_BASE::cols::jiluphoto);
	}

M_MODEL& oreqJiluphoto(const std::string &val)
	{return whereOr(B_BASE::cols::jiluphoto, orm::wq::eq, val);
	}

M_MODEL& ornqJiluphoto(const std::string &val)
	{return whereOr(B_BASE::cols::jiluphoto, orm::wq::nq, val);
	}

M_MODEL& orbtJiluphoto(const std::string &val)
	{return whereOr(B_BASE::cols::jiluphoto, orm::wq::bt, val);
	}

M_MODEL& orbeJiluphoto(const std::string &val)
	{return whereOr(B_BASE::cols::jiluphoto, orm::wq::be, val);
	}

M_MODEL& orltJiluphoto(const std::string &val)
	{return whereOr(B_BASE::cols::jiluphoto, orm::wq::lt, val);
	}

M_MODEL& orleJiluphoto(const std::string &val)
	{return whereOr(B_BASE::cols::jiluphoto, orm::wq::le, val);
	}

M_MODEL& orlikeJiluphoto(const std::string &val)
	{return whereOr(B_BASE::cols::jiluphoto, orm::wq::like, val);
	}

M_MODEL& ornullJiluphoto()
	{return whereOrNull(B_BASE::cols::jiluphoto);
	}

M_MODEL& ornotnullJiluphoto()
	{return whereOrNotNull(B_BASE::cols::jiluphoto);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = xmeet_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &xmeet_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &xmeet_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(xmeet_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(xmeet_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(xmeet_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(xmeet_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &xmeet_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &xmeet_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &xmeet_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &xmeet_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &xmeet_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(xmeet_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &xmeet_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(xmeet_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &xmeet_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(xmeet_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &xmeet_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(xmeet_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &xmeet_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(xmeet_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(xmeet_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(xmeet_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(xmeet_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(xmeet_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(xmeet_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(xmeet_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(xmeet_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(xmeet_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(xmeet_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &xmeet_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &xmeet_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &order(xmeet_info::cols field, const std::string &asc_or_desc)
        {
        ...
        M_MODEL &asc(xmeet_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(xmeet_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &xmeet_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &xmeet_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &xmeet_info::col_names> wq)
        {
        ...
        M_MODEL &having(xmeet_info::cols field)
        {
            groupsql.append(" HAVING ");
            switch (field)
            {
			...
			case xmeet_info::cols::title:
				...
				break;
			case xmeet_info::cols::zhuchi:
				groupsql.append("zhuchi");
				break;
			case xmeet_info::cols::jilu:
				groupsql.append("jilu");
				break;
			case xmeet_info::cols::meetnotice:
				groupsql.append("meetnotice");
				break;
			case xmeet_info::cols::meetfiles:
				groupsql.append("meetfiles");
				break;
			case xmeet_info::cols::address:
				groupsql.append("address");
				break;
			case xmeet_info::cols::meettime:
				groupsql.append("meettime");
				break;
			case xmeet_info::cols::regdate:
				groupsql.append("regdate");
				break;
			case xmeet_info::cols::expecttime:
				groupsql.append("expecttime");
				break;
			case xmeet_info::cols::endtime:
				groupsql.append("endtime");
				break;
			case xmeet_info::cols::presents:
				groupsql.append("presents");
				break;
			case xmeet_info::cols::content:
				groupsql.append("content");
				break;
			case xmeet_info::cols::postresult:
				groupsql.append("postresult");
				break;
			case xmeet_info::cols::postfiles:
				groupsql.append("postfiles");
				break;
			case xmeet_info::cols::jiluphoto:
				groupsql.append("jiluphoto");
				break;
            default:
                return *mod;
                break;
        ...
        M_MODEL &group(xmeet_info::cols field)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &xmeet_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
        ...
        {
            model_meta_cache<xmeet_info::meta> &temp_cache = model_meta_cache<xmeet_info::meta>::getinstance();
        ...
        {
            model_meta_cache<std::vector<xmeet_info::meta>> &temp_cache = model_meta_cache<std::vector<xmeet_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(sqlstring);
        ...
        bool save_data_cache(const std::string &cache_key_name, const xmeet_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<xmeet_info::meta> &temp_cache = model_meta_cache<xmeet_info::meta>::getinstance();
        ...
        bool save_cache(std::size_t cache_key_name, const std::vector<xmeet_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<xmeet_info::meta>> &temp_cache = model_meta_cache<std::vector<xmeet_info::meta>>::getinstance();
            temp_cache.save(cache_key_name, cache_data, exp_time);
            return true;
        }
        bool save_cache(const std::string cache_key_name, const std::vector<xmeet_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<xmeet_info::meta>> &temp_cache = model_meta_cache<std::vector<xmeet_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_vector_cache(const std::string cache_key_name, const std::vector<xmeet_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<xmeet_info::meta>> &temp_cache = model_meta_cache<std::vector<xmeet_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_cache(const std::string cache_key_name, const xmeet_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<xmeet_info::meta> &temp_cache = model_meta_cache<xmeet_info::meta>::getinstance();
            std::size_t sqlhashid                               = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        const xmeet_info::meta get_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<xmeet_info::meta> &temp_cache = model_meta_cache<xmeet_info::meta>::getinstance();
        ...
        const std::vector<xmeet_info::meta> get_vector_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<std::vector<xmeet_info::meta>> &temp_cache = model_meta_cache<std::vector<xmeet_info::meta>>::getinstance();
        ...
        std::tuple<unsigned int, unsigned long long> insert(xmeet_info::meta &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(xmeet_info::meta &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_data_insert_sql(insert_data);
        ...
        std::tuple<unsigned int, unsigned long long> insert(std::vector<xmeet_info::meta> &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(std::vector<xmeet_info::meta> &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_vector_insert_sql(insert_data);
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = xmeet_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(xmeet_info::meta &insert_data)
        {
        ...
        M_MODEL &set_data(xmeet_info::meta indata)
        {
            B_BASE::data = indata;
            return *mod;
        }
        ...
        xmeet_opsql(const xmeet_opsql &)            = delete;
        xmeet_opsql &operator=(const xmeet_opsql &) = delete;
        xmeet_opsql(xmeet_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        xmeet_opsql &operator=(xmeet_opsql &&o) noexcept
        {
            if (this != &o)
            {
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < xmeet_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == xmeet_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(xmeet_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(xmeet_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = xmeet_info::col_names.size();
