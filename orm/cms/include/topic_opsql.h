...
{ /*tagnamespace_replace*/
    ...
    class topic_opsql : public B_BASE
    {
      public:
        topic_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        topic_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, topic_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            break;
                case 10:
            data_temp.twotitle.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 11:
            data_temp.memo.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 12:
            data_temp.templatename.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 13:
            data_temp.url.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 14:
            data_temp.urlpath.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 15:
            data_temp.imgurl.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 16:
            data_temp.topimg.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 17:
             {
        ...
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, topic_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
            ...
            case 3: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.cateid = static_cast<decltype(data_temp.cateid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.cateid = static_cast<decltype(data_temp.cateid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.cateid = static_cast<decltype(data_temp.cateid )>(v);
            ...
            case 4: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.sorttype = static_cast<decltype(data_temp.sorttype )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.sorttype = static_cast<decltype(data_temp.sorttype )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.sorttype = static_cast<decltype(data_temp.sorttype )>(v);
            ...
            case 7: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.isside = static_cast<decltype(data_temp.isside )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.isside = static_cast<decltype(data_temp.isside )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.isside = static_cast<decltype(data_temp.isside )>(v);
            ...
            case 10: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.twotitle.assign(v.data(), v.size());
                    else
                        data_temp.twotitle.clear();
                }, val);
            } break;
            ...
            case 12: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.templatename.assign(v.data(), v.size());
                    else
                        data_temp.templatename.clear();
                }, val);
            } break;
            case 13: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.url.assign(v.data(), v.size());
                    else
                        data_temp.url.clear();
                }, val);
            } break;
            ...
            case 15: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.imgurl.assign(v.data(), v.size());
                    else
                        data_temp.imgurl.clear();
                }, val);
            } break;
            case 16: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.topimg.assign(v.data(), v.size());
                    else
                        data_temp.topimg.clear();
                }, val);
            } break;
            case 17: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.accesscode = static_cast<decltype(data_temp.accesscode )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.accesscode = static_cast<decltype(data_temp.accesscode )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.accesscode = static_cast<decltype(data_temp.accesscode )>(v);
        ...
        http::obj_val get_field_value(unsigned char index_pos, const topic_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            case 2:
                ...
                break;
            case 3:
                return http::obj_val(static_cast<long long>(data_temp.cateid));
                break;
            case 4:
                return http::obj_val(static_cast<long long>(data_temp.sorttype));
                break;
            case 5:
            ...
            case 6:
                ...
                break;
            case 7:
                return http::obj_val(static_cast<long long>(data_temp.isside));
                break;
            case 8:
            ...
            case 9:
                ...
                break;
            case 10:
                return http::obj_val(std::string(data_temp.twotitle));
                break;
            case 11:
                return http::obj_val(std::string(data_temp.memo));
                break;
            case 12:
                return http::obj_val(std::string(data_temp.templatename));
                break;
            case 13:
                return http::obj_val(std::string(data_temp.url));
                break;
            case 14:
                return http::obj_val(std::string(data_temp.urlpath));
                break;
            case 15:
                return http::obj_val(std::string(data_temp.imgurl));
                break;
            case 16:
                return http::obj_val(std::string(data_temp.topimg));
                break;
            case 17:
                return http::obj_val(static_cast<long long>(data_temp.accesscode));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const topic_info::meta &data_temp)
    {
        // Auto-increment primary key topicid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.topicid == 0)
            return http::obj_val(nullptr);
...
M_MODEL& eqCateid(const std::string &val)
	{return where(B_BASE::cols::cateid, orm::wq::eq, val);
	}

M_MODEL& nqCateid(const std::string &val)
	{return where(B_BASE::cols::cateid, orm::wq::nq, val);
	}

M_MODEL& btCateid(const std::string &val)
	{return where(B_BASE::cols::cateid, orm::wq::bt, val);
	}

M_MODEL& beCateid(const std::string &val)
	{return where(B_BASE::cols::cateid, orm::wq::be, val);
	}

M_MODEL& ltCateid(const std::string &val)
	{return where(B_BASE::cols::cateid, orm::wq::lt, val);
	}

M_MODEL& leCateid(const std::string &val)
	{return where(B_BASE::cols::cateid, orm::wq::le, val);
	}
...
M_MODEL& nullCateid()
	{return whereNull(B_BASE::cols::cateid);
	}

M_MODEL& notnullCateid()
	{return whereNotNull(B_BASE::cols::cateid);
	}

M_MODEL& oreqCateid(const std::string &val)
	{return whereOr(B_BASE::cols::cateid, orm::wq::eq, val);
	}

M_MODEL& ornqCateid(const std::string &val)
	{return whereOr(B_BASE::cols::cateid, orm::wq::nq, val);
	}

M_MODEL& orbtCateid(const std::string &val)
	{return whereOr(B_BASE::cols::cateid, orm::wq::bt, val);
	}

M_MODEL& orbeCateid(const std::string &val)
	{return whereOr(B_BASE::cols::cateid, orm::wq::be, val);
	}

M_MODEL& orltCateid(const std::string &val)
	{return whereOr(B_BASE::cols::cateid, orm::wq::lt, val);
	}

M_MODEL& orleCateid(const std::string &val)
	{return whereOr(B_BASE::cols::cateid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqCateid(T val)
	{return whereOr(B_BASE::cols::cateid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqCateid(T val)
	{return whereOr(B_BASE::cols::cateid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtCateid(T val)
	{return whereOr(B_BASE::cols::cateid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeCateid(T val)
	{return whereOr(B_BASE::cols::cateid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltCateid(T val)
	{return whereOr(B_BASE::cols::cateid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleCateid(T val)
	{return whereOr(B_BASE::cols::cateid, orm::wq::le, val);
	}

M_MODEL& ornullCateid()
	{return whereOrNull(B_BASE::cols::cateid);
	}

M_MODEL& ornotnullCateid()
	{return whereOrNotNull(B_BASE::cols::cateid);
	}

M_MODEL& eqSorttype(const std::string &val)
	{return where(B_BASE::cols::sorttype, orm::wq::eq, val);
	}

M_MODEL& nqSorttype(const std::string &val)
	{return where(B_BASE::cols::sorttype, orm::wq::nq, val);
	}

M_MODEL& btSorttype(const std::string &val)
	{return where(B_BASE::cols::sorttype, orm::wq::bt, val);
	}

M_MODEL& beSorttype(const std::string &val)
	{return where(B_BASE::cols::sorttype, orm::wq::be, val);
	}

M_MODEL& ltSorttype(const std::string &val)
	{return where(B_BASE::cols::sorttype, orm::wq::lt, val);
	}

M_MODEL& leSorttype(const std::string &val)
	{return where(B_BASE::cols::sorttype, orm::wq::le, val);
	}
...
M_MODEL& nullSorttype()
	{return whereNull(B_BASE::cols::sorttype);
	}

M_MODEL& notnullSorttype()
	{return whereNotNull(B_BASE::cols::sorttype);
	}

M_MODEL& oreqSorttype(const std::string &val)
	{return whereOr(B_BASE::cols::sorttype, orm::wq::eq, val);
	}

M_MODEL& ornqSorttype(const std::string &val)
	{return whereOr(B_BASE::cols::sorttype, orm::wq::nq, val);
	}

M_MODEL& orbtSorttype(const std::string &val)
	{return whereOr(B_BASE::cols::sorttype, orm::wq::bt, val);
	}

M_MODEL& orbeSorttype(const std::string &val)
	{return whereOr(B_BASE::cols::sorttype, orm::wq::be, val);
	}

M_MODEL& orltSorttype(const std::string &val)
	{return whereOr(B_BASE::cols::sorttype, orm::wq::lt, val);
	}

M_MODEL& orleSorttype(const std::string &val)
	{return whereOr(B_BASE::cols::sorttype, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqSorttype(T val)
	{return whereOr(B_BASE::cols::sorttype, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqSorttype(T val)
	{return whereOr(B_BASE::cols::sorttype, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtSorttype(T val)
	{return whereOr(B_BASE::cols::sorttype, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeSorttype(T val)
	{return whereOr(B_BASE::cols::sorttype, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltSorttype(T val)
	{return whereOr(B_BASE::cols::sorttype, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleSorttype(T val)
	{return whereOr(B_BASE::cols::sorttype, orm::wq::le, val);
	}

M_MODEL& ornullSorttype()
	{return whereOrNull(B_BASE::cols::sorttype);
	}

M_MODEL& ornotnullSorttype()
	{return whereOrNotNull(B_BASE::cols::sorttype);
	}
...
M_MODEL& eqIsside(const std::string &val)
	{return where(B_BASE::cols::isside, orm::wq::eq, val);
	}

M_MODEL& nqIsside(const std::string &val)
	{return where(B_BASE::cols::isside, orm::wq::nq, val);
	}

M_MODEL& btIsside(const std::string &val)
	{return where(B_BASE::cols::isside, orm::wq::bt, val);
	}

M_MODEL& beIsside(const std::string &val)
	{return where(B_BASE::cols::isside, orm::wq::be, val);
	}

M_MODEL& ltIsside(const std::string &val)
	{return where(B_BASE::cols::isside, orm::wq::lt, val);
	}

M_MODEL& leIsside(const std::string &val)
	{return where(B_BASE::cols::isside, orm::wq::le, val);
	}
...
M_MODEL& nullIsside()
	{return whereNull(B_BASE::cols::isside);
	}

M_MODEL& notnullIsside()
	{return whereNotNull(B_BASE::cols::isside);
	}

M_MODEL& oreqIsside(const std::string &val)
	{return whereOr(B_BASE::cols::isside, orm::wq::eq, val);
	}

M_MODEL& ornqIsside(const std::string &val)
	{return whereOr(B_BASE::cols::isside, orm::wq::nq, val);
	}

M_MODEL& orbtIsside(const std::string &val)
	{return whereOr(B_BASE::cols::isside, orm::wq::bt, val);
	}

M_MODEL& orbeIsside(const std::string &val)
	{return whereOr(B_BASE::cols::isside, orm::wq::be, val);
	}

M_MODEL& orltIsside(const std::string &val)
	{return whereOr(B_BASE::cols::isside, orm::wq::lt, val);
	}

M_MODEL& orleIsside(const std::string &val)
	{return whereOr(B_BASE::cols::isside, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqIsside(T val)
	{return whereOr(B_BASE::cols::isside, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqIsside(T val)
	{return whereOr(B_BASE::cols::isside, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtIsside(T val)
	{return whereOr(B_BASE::cols::isside, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeIsside(T val)
	{return whereOr(B_BASE::cols::isside, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltIsside(T val)
	{return whereOr(B_BASE::cols::isside, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleIsside(T val)
	{return whereOr(B_BASE::cols::isside, orm::wq::le, val);
	}

M_MODEL& ornullIsside()
	{return whereOrNull(B_BASE::cols::isside);
	}

M_MODEL& ornotnullIsside()
	{return whereOrNotNull(B_BASE::cols::isside);
	}
...
M_MODEL& eqTwotitle(const std::string &val)
	{return where(B_BASE::cols::twotitle, orm::wq::eq, val);
	}

M_MODEL& nqTwotitle(const std::string &val)
	{return where(B_BASE::cols::twotitle, orm::wq::nq, val);
	}

M_MODEL& btTwotitle(const std::string &val)
	{return where(B_BASE::cols::twotitle, orm::wq::bt, val);
	}

M_MODEL& beTwotitle(const std::string &val)
	{return where(B_BASE::cols::twotitle, orm::wq::be, val);
	}

M_MODEL& ltTwotitle(const std::string &val)
	{return where(B_BASE::cols::twotitle, orm::wq::lt, val);
	}

M_MODEL& leTwotitle(const std::string &val)
	{return where(B_BASE::cols::twotitle, orm::wq::le, val);
	}

M_MODEL& likeTwotitle(const std::string &val)
	{return where(B_BASE::cols::twotitle, orm::wq::like, val);
	}

M_MODEL& nullTwotitle()
	{return whereNull(B_BASE::cols::twotitle);
	}

M_MODEL& notnullTwotitle()
	{return whereNotNull(B_BASE::cols::twotitle);
	}

M_MODEL& oreqTwotitle(const std::string &val)
	{return whereOr(B_BASE::cols::twotitle, orm::wq::eq, val);
	}

M_MODEL& ornqTwotitle(const std::string &val)
	{return whereOr(B_BASE::cols::twotitle, orm::wq::nq, val);
	}

M_MODEL& orbtTwotitle(const std::string &val)
	{return whereOr(B_BASE::cols::twotitle, orm::wq::bt, val);
	}

M_MODEL& orbeTwotitle(const std::string &val)
	{return whereOr(B_BASE::cols::twotitle, orm::wq::be, val);
	}

M_MODEL& orltTwotitle(const std::string &val)
	{return whereOr(B_BASE::cols::twotitle, orm::wq::lt, val);
	}

M_MODEL& orleTwotitle(const std::string &val)
	{return whereOr(B_BASE::cols::twotitle, orm::wq::le, val);
	}

M_MODEL& orlikeTwotitle(const std::string &val)
	{return whereOr(B_BASE::cols::twotitle, orm::wq::like, val);
	}

M_MODEL& ornullTwotitle()
	{return whereOrNull(B_BASE::cols::twotitle);
	}

M_MODEL& ornotnullTwotitle()
	{return whereOrNotNull(B_BASE::cols::twotitle);
	}
...
M_MODEL& eqTemplatename(const std::string &val)
	{return where(B_BASE::cols::templatename, orm::wq::eq, val);
	}

M_MODEL& nqTemplatename(const std::string &val)
	{return where(B_BASE::cols::templatename, orm::wq::nq, val);
	}

M_MODEL& btTemplatename(const std::string &val)
	{return where(B_BASE::cols::templatename, orm::wq::bt, val);
	}

M_MODEL& beTemplatename(const std::string &val)
	{return where(B_BASE::cols::templatename, orm::wq::be, val);
	}

M_MODEL& ltTemplatename(const std::string &val)
	{return where(B_BASE::cols::templatename, orm::wq::lt, val);
	}

M_MODEL& leTemplatename(const std::string &val)
	{return where(B_BASE::cols::templatename, orm::wq::le, val);
	}

M_MODEL& likeTemplatename(const std::string &val)
	{return where(B_BASE::cols::templatename, orm::wq::like, val);
	}

M_MODEL& nullTemplatename()
	{return whereNull(B_BASE::cols::templatename);
	}

M_MODEL& notnullTemplatename()
	{return whereNotNull(B_BASE::cols::templatename);
	}

M_MODEL& oreqTemplatename(const std::string &val)
	{return whereOr(B_BASE::cols::templatename, orm::wq::eq, val);
	}

M_MODEL& ornqTemplatename(const std::string &val)
	{return whereOr(B_BASE::cols::templatename, orm::wq::nq, val);
	}

M_MODEL& orbtTemplatename(const std::string &val)
	{return whereOr(B_BASE::cols::templatename, orm::wq::bt, val);
	}

M_MODEL& orbeTemplatename(const std::string &val)
	{return whereOr(B_BASE::cols::templatename, orm::wq::be, val);
	}

M_MODEL& orltTemplatename(const std::string &val)
	{return whereOr(B_BASE::cols::templatename, orm::wq::lt, val);
	}

M_MODEL& orleTemplatename(const std::string &val)
	{return whereOr(B_BASE::cols::templatename, orm::wq::le, val);
	}

M_MODEL& orlikeTemplatename(const std::string &val)
	{return whereOr(B_BASE::cols::templatename, orm::wq::like, val);
	}

M_MODEL& ornullTemplatename()
	{return whereOrNull(B_BASE::cols::templatename);
	}

M_MODEL& ornotnullTemplatename()
	{return whereOrNotNull(B_BASE::cols::templatename);
	}

M_MODEL& eqUrl(const std::string &val)
	{return where(B_BASE::cols::url, orm::wq::eq, val);
	}

M_MODEL& nqUrl(const std::string &val)
	{return where(B_BASE::cols::url, orm::wq::nq, val);
	}

M_MODEL& btUrl(const std::string &val)
	{return where(B_BASE::cols::url, orm::wq::bt, val);
	}

M_MODEL& beUrl(const std::string &val)
	{return where(B_BASE::cols::url, orm::wq::be, val);
	}

M_MODEL& ltUrl(const std::string &val)
	{return where(B_BASE::cols::url, orm::wq::lt, val);
	}

M_MODEL& leUrl(const std::string &val)
	{return where(B_BASE::cols::url, orm::wq::le, val);
	}

M_MODEL& likeUrl(const std::string &val)
	{return where(B_BASE::cols::url, orm::wq::like, val);
	}

M_MODEL& nullUrl()
	{return whereNull(B_BASE::cols::url);
	}

M_MODEL& notnullUrl()
	{return whereNotNull(B_BASE::cols::url);
	}

M_MODEL& oreqUrl(const std::string &val)
	{return whereOr(B_BASE::cols::url, orm::wq::eq, val);
	}

M_MODEL& ornqUrl(const std::string &val)
	{return whereOr(B_BASE::cols::url, orm::wq::nq, val);
	}

M_MODEL& orbtUrl(const std::string &val)
	{return whereOr(B_BASE::cols::url, orm::wq::bt, val);
	}

M_MODEL& orbeUrl(const std::string &val)
	{return whereOr(B_BASE::cols::url, orm::wq::be, val);
	}

M_MODEL& orltUrl(const std::string &val)
	{return whereOr(B_BASE::cols::url, orm::wq::lt, val);
	}

M_MODEL& orleUrl(const std::string &val)
	{return whereOr(B_BASE::cols::url, orm::wq::le, val);
	}

M_MODEL& orlikeUrl(const std::string &val)
	{return whereOr(B_BASE::cols::url, orm::wq::like, val);
	}

M_MODEL& ornullUrl()
	{return whereOrNull(B_BASE::cols::url);
	}

M_MODEL& ornotnullUrl()
	{return whereOrNotNull(B_BASE::cols::url);
	}
...
M_MODEL& eqImgurl(const std::string &val)
	{return where(B_BASE::cols::imgurl, orm::wq::eq, val);
	}

M_MODEL& nqImgurl(const std::string &val)
	{return where(B_BASE::cols::imgurl, orm::wq::nq, val);
	}

M_MODEL& btImgurl(const std::string &val)
	{return where(B_BASE::cols::imgurl, orm::wq::bt, val);
	}

M_MODEL& beImgurl(const std::string &val)
	{return where(B_BASE::cols::imgurl, orm::wq::be, val);
	}

M_MODEL& ltImgurl(const std::string &val)
	{return where(B_BASE::cols::imgurl, orm::wq::lt, val);
	}

M_MODEL& leImgurl(const std::string &val)
	{return where(B_BASE::cols::imgurl, orm::wq::le, val);
	}

M_MODEL& likeImgurl(const std::string &val)
	{return where(B_BASE::cols::imgurl, orm::wq::like, val);
	}

M_MODEL& nullImgurl()
	{return whereNull(B_BASE::cols::imgurl);
	}

M_MODEL& notnullImgurl()
	{return whereNotNull(B_BASE::cols::imgurl);
	}

M_MODEL& oreqImgurl(const std::string &val)
	{return whereOr(B_BASE::cols::imgurl, orm::wq::eq, val);
	}

M_MODEL& ornqImgurl(const std::string &val)
	{return whereOr(B_BASE::cols::imgurl, orm::wq::nq, val);
	}

M_MODEL& orbtImgurl(const std::string &val)
	{return whereOr(B_BASE::cols::imgurl, orm::wq::bt, val);
	}

M_MODEL& orbeImgurl(const std::string &val)
	{return whereOr(B_BASE::cols::imgurl, orm::wq::be, val);
	}

M_MODEL& orltImgurl(const std::string &val)
	{return whereOr(B_BASE::cols::imgurl, orm::wq::lt, val);
	}

M_MODEL& orleImgurl(const std::string &val)
	{return whereOr(B_BASE::cols::imgurl, orm::wq::le, val);
	}

M_MODEL& orlikeImgurl(const std::string &val)
	{return whereOr(B_BASE::cols::imgurl, orm::wq::like, val);
	}

M_MODEL& ornullImgurl()
	{return whereOrNull(B_BASE::cols::imgurl);
	}

M_MODEL& ornotnullImgurl()
	{return whereOrNotNull(B_BASE::cols::imgurl);
	}

M_MODEL& eqTopimg(const std::string &val)
	{return where(B_BASE::cols::topimg, orm::wq::eq, val);
	}

M_MODEL& nqTopimg(const std::string &val)
	{return where(B_BASE::cols::topimg, orm::wq::nq, val);
	}

M_MODEL& btTopimg(const std::string &val)
	{return where(B_BASE::cols::topimg, orm::wq::bt, val);
	}

M_MODEL& beTopimg(const std::string &val)
	{return where(B_BASE::cols::topimg, orm::wq::be, val);
	}

M_MODEL& ltTopimg(const std::string &val)
	{return where(B_BASE::cols::topimg, orm::wq::lt, val);
	}

M_MODEL& leTopimg(const std::string &val)
	{return where(B_BASE::cols::topimg, orm::wq::le, val);
	}

M_MODEL& likeTopimg(const std::string &val)
	{return where(B_BASE::cols::topimg, orm::wq::like, val);
	}

M_MODEL& nullTopimg()
	{return whereNull(B_BASE::cols::topimg);
	}

M_MODEL& notnullTopimg()
	{return whereNotNull(B_BASE::cols::topimg);
	}

M_MODEL& oreqTopimg(const std::string &val)
	{return whereOr(B_BASE::cols::topimg, orm::wq::eq, val);
	}

M_MODEL& ornqTopimg(const std::string &val)
	{return whereOr(B_BASE::cols::topimg, orm::wq::nq, val);
	}

M_MODEL& orbtTopimg(const std::string &val)
	{return whereOr(B_BASE::cols::topimg, orm::wq::bt, val);
	}

M_MODEL& orbeTopimg(const std::string &val)
	{return whereOr(B_BASE::cols::topimg, orm::wq::be, val);
	}

M_MODEL& orltTopimg(const std::string &val)
	{return whereOr(B_BASE::cols::topimg, orm::wq::lt, val);
	}

M_MODEL& orleTopimg(const std::string &val)
	{return whereOr(B_BASE::cols::topimg, orm::wq::le, val);
	}

M_MODEL& orlikeTopimg(const std::string &val)
	{return whereOr(B_BASE::cols::topimg, orm::wq::like, val);
	}

M_MODEL& ornullTopimg()
	{return whereOrNull(B_BASE::cols::topimg);
	}

M_MODEL& ornotnullTopimg()
	{return whereOrNotNull(B_BASE::cols::topimg);
	}

M_MODEL& eqAccesscode(const std::string &val)
	{return where(B_BASE::cols::accesscode, orm::wq::eq, val);
	}

M_MODEL& nqAccesscode(const std::string &val)
	{return where(B_BASE::cols::accesscode, orm::wq::nq, val);
	}

M_MODEL& btAccesscode(const std::string &val)
	{return where(B_BASE::cols::accesscode, orm::wq::bt, val);
	}

M_MODEL& beAccesscode(const std::string &val)
	{return where(B_BASE::cols::accesscode, orm::wq::be, val);
	}

M_MODEL& ltAccesscode(const std::string &val)
	{return where(B_BASE::cols::accesscode, orm::wq::lt, val);
	}

M_MODEL& leAccesscode(const std::string &val)
	{return where(B_BASE::cols::accesscode, orm::wq::le, val);
	}
...
M_MODEL& nullAccesscode()
	{return whereNull(B_BASE::cols::accesscode);
	}

M_MODEL& notnullAccesscode()
	{return whereNotNull(B_BASE::cols::accesscode);
	}

M_MODEL& oreqAccesscode(const std::string &val)
	{return whereOr(B_BASE::cols::accesscode, orm::wq::eq, val);
	}

M_MODEL& ornqAccesscode(const std::string &val)
	{return whereOr(B_BASE::cols::accesscode, orm::wq::nq, val);
	}

M_MODEL& orbtAccesscode(const std::string &val)
	{return whereOr(B_BASE::cols::accesscode, orm::wq::bt, val);
	}

M_MODEL& orbeAccesscode(const std::string &val)
	{return whereOr(B_BASE::cols::accesscode, orm::wq::be, val);
	}

M_MODEL& orltAccesscode(const std::string &val)
	{return whereOr(B_BASE::cols::accesscode, orm::wq::lt, val);
	}

M_MODEL& orleAccesscode(const std::string &val)
	{return whereOr(B_BASE::cols::accesscode, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqAccesscode(T val)
	{return whereOr(B_BASE::cols::accesscode, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqAccesscode(T val)
	{return whereOr(B_BASE::cols::accesscode, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtAccesscode(T val)
	{return whereOr(B_BASE::cols::accesscode, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeAccesscode(T val)
	{return whereOr(B_BASE::cols::accesscode, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltAccesscode(T val)
	{return whereOr(B_BASE::cols::accesscode, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleAccesscode(T val)
	{return whereOr(B_BASE::cols::accesscode, orm::wq::le, val);
	}

M_MODEL& ornullAccesscode()
	{return whereOrNull(B_BASE::cols::accesscode);
	}

M_MODEL& ornotnullAccesscode()
	{return whereOrNotNull(B_BASE::cols::accesscode);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = topic_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &topic_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &topic_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(topic_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(topic_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(topic_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(topic_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &topic_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &topic_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &topic_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &topic_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &topic_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(topic_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &topic_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(topic_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &topic_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(topic_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &topic_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(topic_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &topic_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(topic_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(topic_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(topic_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(topic_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(topic_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(topic_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(topic_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(topic_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(topic_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(topic_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &topic_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &topic_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &order(topic_info::cols field, const std::string &asc_or_desc)
        {
        ...
        M_MODEL &asc(topic_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(topic_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &topic_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &topic_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &topic_info::col_names> wq)
        {
        ...
        M_MODEL &having(topic_info::cols field)
        {
            groupsql.append(" HAVING ");
            switch (field)
            {
			...
			case topic_info::cols::parentid:
				...
				break;
			case topic_info::cols::cateid:
				groupsql.append("cateid");
				break;
			case topic_info::cols::sorttype:
				groupsql.append("sorttype");
				break;
			case topic_info::cols::languagetype:
			...
			case topic_info::cols::title:
				...
				break;
			case topic_info::cols::twotitle:
				groupsql.append("twotitle");
				break;
			case topic_info::cols::memo:
				groupsql.append("memo");
				break;
			case topic_info::cols::templatename:
				groupsql.append("templatename");
				break;
			case topic_info::cols::url:
				groupsql.append("url");
				break;
			case topic_info::cols::urlpath:
				...
				break;
			case topic_info::cols::imgurl:
				groupsql.append("imgurl");
				break;
			case topic_info::cols::topimg:
				groupsql.append("topimg");
				break;
			case topic_info::cols::accesscode:
				groupsql.append("accesscode");
				break;
            default:
                return *mod;
                break;
        ...
        M_MODEL &group(topic_info::cols field)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &topic_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
        ...
        bool save_data_cache(const std::string &cache_key_name, const topic_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<topic_info::meta> &temp_cache = model_meta_cache<topic_info::meta>::getinstance();
        ...
        bool save_cache(std::size_t cache_key_name, const std::vector<topic_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<topic_info::meta>> &temp_cache = model_meta_cache<std::vector<topic_info::meta>>::getinstance();
            temp_cache.save(cache_key_name, cache_data, exp_time);
            return true;
        }
        bool save_cache(const std::string cache_key_name, const std::vector<topic_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<topic_info::meta>> &temp_cache = model_meta_cache<std::vector<topic_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_vector_cache(const std::string cache_key_name, const std::vector<topic_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<topic_info::meta>> &temp_cache = model_meta_cache<std::vector<topic_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_cache(const std::string cache_key_name, const topic_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<topic_info::meta> &temp_cache = model_meta_cache<topic_info::meta>::getinstance();
            std::size_t sqlhashid                               = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        std::tuple<unsigned int, unsigned long long> insert(topic_info::meta &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(topic_info::meta &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_data_insert_sql(insert_data);
        ...
        std::tuple<unsigned int, unsigned long long> insert(std::vector<topic_info::meta> &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(std::vector<topic_info::meta> &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_vector_insert_sql(insert_data);
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = topic_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(topic_info::meta &insert_data)
        {
        ...
        M_MODEL &set_data(topic_info::meta indata)
        {
            B_BASE::data = indata;
            return *mod;
        }
        ...
        topic_opsql(const topic_opsql &)            = delete;
        topic_opsql &operator=(const topic_opsql &) = delete;
        topic_opsql(topic_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        topic_opsql &operator=(topic_opsql &&o) noexcept
        {
            if (this != &o)
            {
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < topic_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == topic_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(topic_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(topic_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = topic_info::col_names.size();
