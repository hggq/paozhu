#ifndef _ORM_CMS_HOMEBLOCK_OPERATE_H
#define _ORM_CMS_HOMEBLOCK_OPERATE_H
/*
...
{ /*tagnamespace_replace*/
    ...
    class homeblock_opsql : public B_BASE
    {
      public:
        homeblock_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        homeblock_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, homeblock_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            break;
                case 4:
            data_temp.jsonconfig.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 5:
             {
               data_temp.viewtype = 0;
        ...
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, homeblock_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.hbid = static_cast<decltype(data_temp.hbid )>(v);
            ...
            case 4: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.jsonconfig.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.jsonconfig.assign(v.data(), v.size());
                    else
                        data_temp.jsonconfig.clear();
                }, val);
            } break;
            case 5: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.viewtype = static_cast<decltype(data_temp.viewtype )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.viewtype = static_cast<decltype(data_temp.viewtype )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.viewtype = static_cast<decltype(data_temp.viewtype )>(v);
            ...
            case 6: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.gettype = static_cast<decltype(data_temp.gettype )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.gettype = static_cast<decltype(data_temp.gettype )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.gettype = static_cast<decltype(data_temp.gettype )>(v);
            ...
            case 7: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.rownum = static_cast<decltype(data_temp.rownum )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.rownum = static_cast<decltype(data_temp.rownum )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.rownum = static_cast<decltype(data_temp.rownum )>(v);
            ...
            case 8: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.width = static_cast<decltype(data_temp.width )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.width = static_cast<decltype(data_temp.width )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.width = static_cast<decltype(data_temp.width )>(v);
            ...
            case 9: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.height = static_cast<decltype(data_temp.height )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.height = static_cast<decltype(data_temp.height )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.height = static_cast<decltype(data_temp.height )>(v);
            ...
            case 10: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.strlength = static_cast<decltype(data_temp.strlength )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.strlength = static_cast<decltype(data_temp.strlength )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.strlength = static_cast<decltype(data_temp.strlength )>(v);
        ...
        http::obj_val get_field_value(unsigned char index_pos, const homeblock_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
                return http::obj_val(static_cast<long long>(data_temp.hbid));
                break;
            case 1:
            ...
            case 3:
                ...
                break;
            case 4:
                return http::obj_val(std::string(data_temp.jsonconfig));
                break;
            case 5:
                return http::obj_val(static_cast<long long>(data_temp.viewtype));
                break;
            case 6:
                return http::obj_val(static_cast<long long>(data_temp.gettype));
                break;
            case 7:
                return http::obj_val(static_cast<long long>(data_temp.rownum));
                break;
            case 8:
                return http::obj_val(static_cast<long long>(data_temp.width));
                break;
            case 9:
                return http::obj_val(static_cast<long long>(data_temp.height));
                break;
            case 10:
                return http::obj_val(static_cast<long long>(data_temp.strlength));
                break;
            case 11:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const homeblock_info::meta &data_temp)
    {
        // Auto-increment primary key hbid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.hbid == 0)
            return http::obj_val(nullptr);
        return get_field_value(index_pos, data_temp);
    }
    

M_MODEL& eqHbid(const std::string &val)
	{return where(B_BASE::cols::hbid, orm::wq::eq, val);
	}

M_MODEL& nqHbid(const std::string &val)
	{return where(B_BASE::cols::hbid, orm::wq::nq, val);
	}

M_MODEL& btHbid(const std::string &val)
	{return where(B_BASE::cols::hbid, orm::wq::bt, val);
	}

M_MODEL& beHbid(const std::string &val)
	{return where(B_BASE::cols::hbid, orm::wq::be, val);
	}

M_MODEL& ltHbid(const std::string &val)
	{return where(B_BASE::cols::hbid, orm::wq::lt, val);
	}

M_MODEL& leHbid(const std::string &val)
	{return where(B_BASE::cols::hbid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqHbid(T val)
	{return where(B_BASE::cols::hbid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqHbid(T val)
	{return where(B_BASE::cols::hbid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btHbid(T val)
	{return where(B_BASE::cols::hbid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beHbid(T val)
	{return where(B_BASE::cols::hbid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltHbid(T val)
	{return where(B_BASE::cols::hbid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leHbid(T val)
	{return where(B_BASE::cols::hbid, orm::wq::le, val);
	}

M_MODEL& nullHbid()
	{return whereNull(B_BASE::cols::hbid);
	}

M_MODEL& notnullHbid()
	{return whereNotNull(B_BASE::cols::hbid);
	}

M_MODEL& oreqHbid(const std::string &val)
	{return whereOr(B_BASE::cols::hbid, orm::wq::eq, val);
	}

M_MODEL& ornqHbid(const std::string &val)
	{return whereOr(B_BASE::cols::hbid, orm::wq::nq, val);
	}

M_MODEL& orbtHbid(const std::string &val)
	{return whereOr(B_BASE::cols::hbid, orm::wq::bt, val);
	}

M_MODEL& orbeHbid(const std::string &val)
	{return whereOr(B_BASE::cols::hbid, orm::wq::be, val);
	}

M_MODEL& orltHbid(const std::string &val)
	{return whereOr(B_BASE::cols::hbid, orm::wq::lt, val);
	}

M_MODEL& orleHbid(const std::string &val)
	{return whereOr(B_BASE::cols::hbid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqHbid(T val)
	{return whereOr(B_BASE::cols::hbid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqHbid(T val)
	{return whereOr(B_BASE::cols::hbid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtHbid(T val)
	{return whereOr(B_BASE::cols::hbid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeHbid(T val)
	{return whereOr(B_BASE::cols::hbid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltHbid(T val)
	{return whereOr(B_BASE::cols::hbid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleHbid(T val)
	{return whereOr(B_BASE::cols::hbid, orm::wq::le, val);
	}

M_MODEL& ornullHbid()
	{return whereOrNull(B_BASE::cols::hbid);
	}

M_MODEL& ornotnullHbid()
	{return whereOrNotNull(B_BASE::cols::hbid);
	}
...
M_MODEL& eqJsonconfig(const std::string &val)
	{return where(B_BASE::cols::jsonconfig, orm::wq::eq, val);
	}

M_MODEL& nqJsonconfig(const std::string &val)
	{return where(B_BASE::cols::jsonconfig, orm::wq::nq, val);
	}

M_MODEL& btJsonconfig(const std::string &val)
	{return where(B_BASE::cols::jsonconfig, orm::wq::bt, val);
	}

M_MODEL& beJsonconfig(const std::string &val)
	{return where(B_BASE::cols::jsonconfig, orm::wq::be, val);
	}

M_MODEL& ltJsonconfig(const std::string &val)
	{return where(B_BASE::cols::jsonconfig, orm::wq::lt, val);
	}

M_MODEL& leJsonconfig(const std::string &val)
	{return where(B_BASE::cols::jsonconfig, orm::wq::le, val);
	}

M_MODEL& likeJsonconfig(const std::string &val)
	{return where(B_BASE::cols::jsonconfig, orm::wq::like, val);
	}

M_MODEL& nullJsonconfig()
	{return whereNull(B_BASE::cols::jsonconfig);
	}

M_MODEL& notnullJsonconfig()
	{return whereNotNull(B_BASE::cols::jsonconfig);
	}

M_MODEL& oreqJsonconfig(const std::string &val)
	{return whereOr(B_BASE::cols::jsonconfig, orm::wq::eq, val);
	}

M_MODEL& ornqJsonconfig(const std::string &val)
	{return whereOr(B_BASE::cols::jsonconfig, orm::wq::nq, val);
	}

M_MODEL& orbtJsonconfig(const std::string &val)
	{return whereOr(B_BASE::cols::jsonconfig, orm::wq::bt, val);
	}

M_MODEL& orbeJsonconfig(const std::string &val)
	{return whereOr(B_BASE::cols::jsonconfig, orm::wq::be, val);
	}

M_MODEL& orltJsonconfig(const std::string &val)
	{return whereOr(B_BASE::cols::jsonconfig, orm::wq::lt, val);
	}

M_MODEL& orleJsonconfig(const std::string &val)
	{return whereOr(B_BASE::cols::jsonconfig, orm::wq::le, val);
	}

M_MODEL& orlikeJsonconfig(const std::string &val)
	{return whereOr(B_BASE::cols::jsonconfig, orm::wq::like, val);
	}

M_MODEL& ornullJsonconfig()
	{return whereOrNull(B_BASE::cols::jsonconfig);
	}

M_MODEL& ornotnullJsonconfig()
	{return whereOrNotNull(B_BASE::cols::jsonconfig);
	}

M_MODEL& eqViewtype(const std::string &val)
	{return where(B_BASE::cols::viewtype, orm::wq::eq, val);
	}

M_MODEL& nqViewtype(const std::string &val)
	{return where(B_BASE::cols::viewtype, orm::wq::nq, val);
	}

M_MODEL& btViewtype(const std::string &val)
	{return where(B_BASE::cols::viewtype, orm::wq::bt, val);
	}

M_MODEL& beViewtype(const std::string &val)
	{return where(B_BASE::cols::viewtype, orm::wq::be, val);
	}

M_MODEL& ltViewtype(const std::string &val)
	{return where(B_BASE::cols::viewtype, orm::wq::lt, val);
	}

M_MODEL& leViewtype(const std::string &val)
	{return where(B_BASE::cols::viewtype, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqViewtype(T val)
	{return where(B_BASE::cols::viewtype, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqViewtype(T val)
	{return where(B_BASE::cols::viewtype, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btViewtype(T val)
	{return where(B_BASE::cols::viewtype, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beViewtype(T val)
	{return where(B_BASE::cols::viewtype, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltViewtype(T val)
	{return where(B_BASE::cols::viewtype, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leViewtype(T val)
	{return where(B_BASE::cols::viewtype, orm::wq::le, val);
	}

M_MODEL& nullViewtype()
	{return whereNull(B_BASE::cols::viewtype);
	}

M_MODEL& notnullViewtype()
	{return whereNotNull(B_BASE::cols::viewtype);
	}

M_MODEL& oreqViewtype(const std::string &val)
	{return whereOr(B_BASE::cols::viewtype, orm::wq::eq, val);
	}

M_MODEL& ornqViewtype(const std::string &val)
	{return whereOr(B_BASE::cols::viewtype, orm::wq::nq, val);
	}

M_MODEL& orbtViewtype(const std::string &val)
	{return whereOr(B_BASE::cols::viewtype, orm::wq::bt, val);
	}

M_MODEL& orbeViewtype(const std::string &val)
	{return whereOr(B_BASE::cols::viewtype, orm::wq::be, val);
	}

M_MODEL& orltViewtype(const std::string &val)
	{return whereOr(B_BASE::cols::viewtype, orm::wq::lt, val);
	}

M_MODEL& orleViewtype(const std::string &val)
	{return whereOr(B_BASE::cols::viewtype, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqViewtype(T val)
	{return whereOr(B_BASE::cols::viewtype, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqViewtype(T val)
	{return whereOr(B_BASE::cols::viewtype, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtViewtype(T val)
	{return whereOr(B_BASE::cols::viewtype, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeViewtype(T val)
	{return whereOr(B_BASE::cols::viewtype, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltViewtype(T val)
	{return whereOr(B_BASE::cols::viewtype, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleViewtype(T val)
	{return whereOr(B_BASE::cols::viewtype, orm::wq::le, val);
	}

M_MODEL& ornullViewtype()
	{return whereOrNull(B_BASE::cols::viewtype);
	}

M_MODEL& ornotnullViewtype()
	{return whereOrNotNull(B_BASE::cols::viewtype);
	}

M_MODEL& eqGettype(const std::string &val)
	{return where(B_BASE::cols::gettype, orm::wq::eq, val);
	}

M_MODEL& nqGettype(const std::string &val)
	{return where(B_BASE::cols::gettype, orm::wq::nq, val);
	}

M_MODEL& btGettype(const std::string &val)
	{return where(B_BASE::cols::gettype, orm::wq::bt, val);
	}

M_MODEL& beGettype(const std::string &val)
	{return where(B_BASE::cols::gettype, orm::wq::be, val);
	}

M_MODEL& ltGettype(const std::string &val)
	{return where(B_BASE::cols::gettype, orm::wq::lt, val);
	}

M_MODEL& leGettype(const std::string &val)
	{return where(B_BASE::cols::gettype, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqGettype(T val)
	{return where(B_BASE::cols::gettype, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqGettype(T val)
	{return where(B_BASE::cols::gettype, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btGettype(T val)
	{return where(B_BASE::cols::gettype, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beGettype(T val)
	{return where(B_BASE::cols::gettype, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltGettype(T val)
	{return where(B_BASE::cols::gettype, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leGettype(T val)
	{return where(B_BASE::cols::gettype, orm::wq::le, val);
	}

M_MODEL& nullGettype()
	{return whereNull(B_BASE::cols::gettype);
	}

M_MODEL& notnullGettype()
	{return whereNotNull(B_BASE::cols::gettype);
	}

M_MODEL& oreqGettype(const std::string &val)
	{return whereOr(B_BASE::cols::gettype, orm::wq::eq, val);
	}

M_MODEL& ornqGettype(const std::string &val)
	{return whereOr(B_BASE::cols::gettype, orm::wq::nq, val);
	}

M_MODEL& orbtGettype(const std::string &val)
	{return whereOr(B_BASE::cols::gettype, orm::wq::bt, val);
	}

M_MODEL& orbeGettype(const std::string &val)
	{return whereOr(B_BASE::cols::gettype, orm::wq::be, val);
	}

M_MODEL& orltGettype(const std::string &val)
	{return whereOr(B_BASE::cols::gettype, orm::wq::lt, val);
	}

M_MODEL& orleGettype(const std::string &val)
	{return whereOr(B_BASE::cols::gettype, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqGettype(T val)
	{return whereOr(B_BASE::cols::gettype, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqGettype(T val)
	{return whereOr(B_BASE::cols::gettype, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtGettype(T val)
	{return whereOr(B_BASE::cols::gettype, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeGettype(T val)
	{return whereOr(B_BASE::cols::gettype, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltGettype(T val)
	{return whereOr(B_BASE::cols::gettype, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleGettype(T val)
	{return whereOr(B_BASE::cols::gettype, orm::wq::le, val);
	}

M_MODEL& ornullGettype()
	{return whereOrNull(B_BASE::cols::gettype);
	}

M_MODEL& ornotnullGettype()
	{return whereOrNotNull(B_BASE::cols::gettype);
	}

M_MODEL& eqRownum(const std::string &val)
	{return where(B_BASE::cols::rownum, orm::wq::eq, val);
	}

M_MODEL& nqRownum(const std::string &val)
	{return where(B_BASE::cols::rownum, orm::wq::nq, val);
	}

M_MODEL& btRownum(const std::string &val)
	{return where(B_BASE::cols::rownum, orm::wq::bt, val);
	}

M_MODEL& beRownum(const std::string &val)
	{return where(B_BASE::cols::rownum, orm::wq::be, val);
	}

M_MODEL& ltRownum(const std::string &val)
	{return where(B_BASE::cols::rownum, orm::wq::lt, val);
	}

M_MODEL& leRownum(const std::string &val)
	{return where(B_BASE::cols::rownum, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqRownum(T val)
	{return where(B_BASE::cols::rownum, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqRownum(T val)
	{return where(B_BASE::cols::rownum, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btRownum(T val)
	{return where(B_BASE::cols::rownum, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beRownum(T val)
	{return where(B_BASE::cols::rownum, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltRownum(T val)
	{return where(B_BASE::cols::rownum, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leRownum(T val)
	{return where(B_BASE::cols::rownum, orm::wq::le, val);
	}

M_MODEL& nullRownum()
	{return whereNull(B_BASE::cols::rownum);
	}

M_MODEL& notnullRownum()
	{return whereNotNull(B_BASE::cols::rownum);
	}

M_MODEL& oreqRownum(const std::string &val)
	{return whereOr(B_BASE::cols::rownum, orm::wq::eq, val);
	}

M_MODEL& ornqRownum(const std::string &val)
	{return whereOr(B_BASE::cols::rownum, orm::wq::nq, val);
	}

M_MODEL& orbtRownum(const std::string &val)
	{return whereOr(B_BASE::cols::rownum, orm::wq::bt, val);
	}

M_MODEL& orbeRownum(const std::string &val)
	{return whereOr(B_BASE::cols::rownum, orm::wq::be, val);
	}

M_MODEL& orltRownum(const std::string &val)
	{return whereOr(B_BASE::cols::rownum, orm::wq::lt, val);
	}

M_MODEL& orleRownum(const std::string &val)
	{return whereOr(B_BASE::cols::rownum, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqRownum(T val)
	{return whereOr(B_BASE::cols::rownum, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqRownum(T val)
	{return whereOr(B_BASE::cols::rownum, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtRownum(T val)
	{return whereOr(B_BASE::cols::rownum, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeRownum(T val)
	{return whereOr(B_BASE::cols::rownum, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltRownum(T val)
	{return whereOr(B_BASE::cols::rownum, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleRownum(T val)
	{return whereOr(B_BASE::cols::rownum, orm::wq::le, val);
	}

M_MODEL& ornullRownum()
	{return whereOrNull(B_BASE::cols::rownum);
	}

M_MODEL& ornotnullRownum()
	{return whereOrNotNull(B_BASE::cols::rownum);
	}

M_MODEL& eqWidth(const std::string &val)
	{return where(B_BASE::cols::width, orm::wq::eq, val);
	}

M_MODEL& nqWidth(const std::string &val)
	{return where(B_BASE::cols::width, orm::wq::nq, val);
	}

M_MODEL& btWidth(const std::string &val)
	{return where(B_BASE::cols::width, orm::wq::bt, val);
	}

M_MODEL& beWidth(const std::string &val)
	{return where(B_BASE::cols::width, orm::wq::be, val);
	}

M_MODEL& ltWidth(const std::string &val)
	{return where(B_BASE::cols::width, orm::wq::lt, val);
	}

M_MODEL& leWidth(const std::string &val)
	{return where(B_BASE::cols::width, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqWidth(T val)
	{return where(B_BASE::cols::width, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqWidth(T val)
	{return where(B_BASE::cols::width, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btWidth(T val)
	{return where(B_BASE::cols::width, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beWidth(T val)
	{return where(B_BASE::cols::width, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltWidth(T val)
	{return where(B_BASE::cols::width, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leWidth(T val)
	{return where(B_BASE::cols::width, orm::wq::le, val);
	}

M_MODEL& nullWidth()
	{return whereNull(B_BASE::cols::width);
	}

M_MODEL& notnullWidth()
	{return whereNotNull(B_BASE::cols::width);
	}

M_MODEL& oreqWidth(const std::string &val)
	{return whereOr(B_BASE::cols::width, orm::wq::eq, val);
	}

M_MODEL& ornqWidth(const std::string &val)
	{return whereOr(B_BASE::cols::width, orm::wq::nq, val);
	}

M_MODEL& orbtWidth(const std::string &val)
	{return whereOr(B_BASE::cols::width, orm::wq::bt, val);
	}

M_MODEL& orbeWidth(const std::string &val)
	{return whereOr(B_BASE::cols::width, orm::wq::be, val);
	}

M_MODEL& orltWidth(const std::string &val)
	{return whereOr(B_BASE::cols::width, orm::wq::lt, val);
	}

M_MODEL& orleWidth(const std::string &val)
	{return whereOr(B_BASE::cols::width, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqWidth(T val)
	{return whereOr(B_BASE::cols::width, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqWidth(T val)
	{return whereOr(B_BASE::cols::width, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtWidth(T val)
	{return whereOr(B_BASE::cols::width, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeWidth(T val)
	{return whereOr(B_BASE::cols::width, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltWidth(T val)
	{return whereOr(B_BASE::cols::width, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleWidth(T val)
	{return whereOr(B_BASE::cols::width, orm::wq::le, val);
	}

M_MODEL& ornullWidth()
	{return whereOrNull(B_BASE::cols::width);
	}

M_MODEL& ornotnullWidth()
	{return whereOrNotNull(B_BASE::cols::width);
	}

M_MODEL& eqHeight(const std::string &val)
	{return where(B_BASE::cols::height, orm::wq::eq, val);
	}

M_MODEL& nqHeight(const std::string &val)
	{return where(B_BASE::cols::height, orm::wq::nq, val);
	}

M_MODEL& btHeight(const std::string &val)
	{return where(B_BASE::cols::height, orm::wq::bt, val);
	}

M_MODEL& beHeight(const std::string &val)
	{return where(B_BASE::cols::height, orm::wq::be, val);
	}

M_MODEL& ltHeight(const std::string &val)
	{return where(B_BASE::cols::height, orm::wq::lt, val);
	}

M_MODEL& leHeight(const std::string &val)
	{return where(B_BASE::cols::height, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqHeight(T val)
	{return where(B_BASE::cols::height, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqHeight(T val)
	{return where(B_BASE::cols::height, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btHeight(T val)
	{return where(B_BASE::cols::height, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beHeight(T val)
	{return where(B_BASE::cols::height, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltHeight(T val)
	{return where(B_BASE::cols::height, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leHeight(T val)
	{return where(B_BASE::cols::height, orm::wq::le, val);
	}

M_MODEL& nullHeight()
	{return whereNull(B_BASE::cols::height);
	}

M_MODEL& notnullHeight()
	{return whereNotNull(B_BASE::cols::height);
	}

M_MODEL& oreqHeight(const std::string &val)
	{return whereOr(B_BASE::cols::height, orm::wq::eq, val);
	}

M_MODEL& ornqHeight(const std::string &val)
	{return whereOr(B_BASE::cols::height, orm::wq::nq, val);
	}

M_MODEL& orbtHeight(const std::string &val)
	{return whereOr(B_BASE::cols::height, orm::wq::bt, val);
	}

M_MODEL& orbeHeight(const std::string &val)
	{return whereOr(B_BASE::cols::height, orm::wq::be, val);
	}

M_MODEL& orltHeight(const std::string &val)
	{return whereOr(B_BASE::cols::height, orm::wq::lt, val);
	}

M_MODEL& orleHeight(const std::string &val)
	{return whereOr(B_BASE::cols::height, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqHeight(T val)
	{return whereOr(B_BASE::cols::height, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqHeight(T val)
	{return whereOr(B_BASE::cols::height, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtHeight(T val)
	{return whereOr(B_BASE::cols::height, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeHeight(T val)
	{return whereOr(B_BASE::cols::height, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltHeight(T val)
	{return whereOr(B_BASE::cols::height, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleHeight(T val)
	{return whereOr(B_BASE::cols::height, orm::wq::le, val);
	}

M_MODEL& ornullHeight()
	{return whereOrNull(B_BASE::cols::height);
	}

M_MODEL& ornotnullHeight()
	{return whereOrNotNull(B_BASE::cols::height);
	}

M_MODEL& eqStrlength(const std::string &val)
	{return where(B_BASE::cols::strlength, orm::wq::eq, val);
	}

M_MODEL& nqStrlength(const std::string &val)
	{return where(B_BASE::cols::strlength, orm::wq::nq, val);
	}

M_MODEL& btStrlength(const std::string &val)
	{return where(B_BASE::cols::strlength, orm::wq::bt, val);
	}

M_MODEL& beStrlength(const std::string &val)
	{return where(B_BASE::cols::strlength, orm::wq::be, val);
	}

M_MODEL& ltStrlength(const std::string &val)
	{return where(B_BASE::cols::strlength, orm::wq::lt, val);
	}

M_MODEL& leStrlength(const std::string &val)
	{return where(B_BASE::cols::strlength, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqStrlength(T val)
	{return where(B_BASE::cols::strlength, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqStrlength(T val)
	{return where(B_BASE::cols::strlength, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btStrlength(T val)
	{return where(B_BASE::cols::strlength, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beStrlength(T val)
	{return where(B_BASE::cols::strlength, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltStrlength(T val)
	{return where(B_BASE::cols::strlength, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leStrlength(T val)
	{return where(B_BASE::cols::strlength, orm::wq::le, val);
	}

M_MODEL& nullStrlength()
	{return whereNull(B_BASE::cols::strlength);
	}

M_MODEL& notnullStrlength()
	{return whereNotNull(B_BASE::cols::strlength);
	}

M_MODEL& oreqStrlength(const std::string &val)
	{return whereOr(B_BASE::cols::strlength, orm::wq::eq, val);
	}

M_MODEL& ornqStrlength(const std::string &val)
	{return whereOr(B_BASE::cols::strlength, orm::wq::nq, val);
	}

M_MODEL& orbtStrlength(const std::string &val)
	{return whereOr(B_BASE::cols::strlength, orm::wq::bt, val);
	}

M_MODEL& orbeStrlength(const std::string &val)
	{return whereOr(B_BASE::cols::strlength, orm::wq::be, val);
	}

M_MODEL& orltStrlength(const std::string &val)
	{return whereOr(B_BASE::cols::strlength, orm::wq::lt, val);
	}

M_MODEL& orleStrlength(const std::string &val)
	{return whereOr(B_BASE::cols::strlength, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqStrlength(T val)
	{return whereOr(B_BASE::cols::strlength, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqStrlength(T val)
	{return whereOr(B_BASE::cols::strlength, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtStrlength(T val)
	{return whereOr(B_BASE::cols::strlength, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeStrlength(T val)
	{return whereOr(B_BASE::cols::strlength, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltStrlength(T val)
	{return whereOr(B_BASE::cols::strlength, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleStrlength(T val)
	{return whereOr(B_BASE::cols::strlength, orm::wq::le, val);
	}

M_MODEL& ornullStrlength()
	{return whereOrNull(B_BASE::cols::strlength);
	}

M_MODEL& ornotnullStrlength()
	{return whereOrNotNull(B_BASE::cols::strlength);
	}
...
M_MODEL& ornotnullSortid()
	{return whereOrNotNull(B_BASE::cols::sortid);
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = homeblock_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &homeblock_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &homeblock_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(homeblock_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(homeblock_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(homeblock_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(homeblock_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &homeblock_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &homeblock_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &homeblock_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &homeblock_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &homeblock_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(homeblock_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &homeblock_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(homeblock_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &homeblock_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(homeblock_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &homeblock_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(homeblock_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &homeblock_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(homeblock_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(homeblock_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(homeblock_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(homeblock_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(homeblock_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(homeblock_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(homeblock_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(homeblock_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(homeblock_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(homeblock_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &homeblock_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &homeblock_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &order(homeblock_info::cols field, const std::string &asc_or_desc)
        {
        ...
        M_MODEL &asc(homeblock_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(homeblock_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &homeblock_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &homeblock_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &homeblock_info::col_names> wq)
        {
        ...
        M_MODEL &having(homeblock_info::cols field)
        {
            groupsql.append(" HAVING ");
            switch (field)
            {
			...
			case homeblock_info::cols::content:
				...
				break;
			case homeblock_info::cols::jsonconfig:
				groupsql.append("jsonconfig");
				break;
			case homeblock_info::cols::viewtype:
				groupsql.append("viewtype");
				break;
			case homeblock_info::cols::gettype:
				groupsql.append("gettype");
				break;
			case homeblock_info::cols::rownum:
				groupsql.append("rownum");
				break;
			case homeblock_info::cols::width:
				groupsql.append("width");
				break;
			case homeblock_info::cols::height:
				groupsql.append("height");
				break;
			case homeblock_info::cols::strlength:
				groupsql.append("strlength");
				break;
			case homeblock_info::cols::sortid:
        ...
        M_MODEL &group(homeblock_info::cols field)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &homeblock_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
        ...
        {
            model_meta_cache<homeblock_info::meta> &temp_cache = model_meta_cache<homeblock_info::meta>::getinstance();
        ...
        {
            model_meta_cache<std::vector<homeblock_info::meta>> &temp_cache = model_meta_cache<std::vector<homeblock_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(sqlstring);
        ...
        bool save_data_cache(const std::string &cache_key_name, const homeblock_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<homeblock_info::meta> &temp_cache = model_meta_cache<homeblock_info::meta>::getinstance();
        ...
        bool save_cache(std::size_t cache_key_name, const std::vector<homeblock_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<homeblock_info::meta>> &temp_cache = model_meta_cache<std::vector<homeblock_info::meta>>::getinstance();
            temp_cache.save(cache_key_name, cache_data, exp_time);
            return true;
        }
        bool save_cache(const std::string cache_key_name, const std::vector<homeblock_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<homeblock_info::meta>> &temp_cache = model_meta_cache<std::vector<homeblock_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_vector_cache(const std::string cache_key_name, const std::vector<homeblock_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<homeblock_info::meta>> &temp_cache = model_meta_cache<std::vector<homeblock_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_cache(const std::string cache_key_name, const homeblock_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<homeblock_info::meta> &temp_cache = model_meta_cache<homeblock_info::meta>::getinstance();
            std::size_t sqlhashid                               = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        const homeblock_info::meta get_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<homeblock_info::meta> &temp_cache = model_meta_cache<homeblock_info::meta>::getinstance();
        ...
        const std::vector<homeblock_info::meta> get_vector_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<std::vector<homeblock_info::meta>> &temp_cache = model_meta_cache<std::vector<homeblock_info::meta>>::getinstance();
        ...
        std::tuple<unsigned int, unsigned long long> insert(homeblock_info::meta &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(homeblock_info::meta &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_data_insert_sql(insert_data);
        ...
        std::tuple<unsigned int, unsigned long long> insert(std::vector<homeblock_info::meta> &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(std::vector<homeblock_info::meta> &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_vector_insert_sql(insert_data);
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = homeblock_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(homeblock_info::meta &insert_data)
        {
        ...
        M_MODEL &set_data(homeblock_info::meta indata)
        {
            B_BASE::data = indata;
            return *mod;
        }
        ...
        homeblock_opsql(const homeblock_opsql &)            = delete;
        homeblock_opsql &operator=(const homeblock_opsql &) = delete;
        homeblock_opsql(homeblock_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        homeblock_opsql &operator=(homeblock_opsql &&o) noexcept
        {
            if (this != &o)
            {
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < homeblock_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == homeblock_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(homeblock_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(homeblock_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = homeblock_info::col_names.size();
