...
{ /*tagnamespace_replace*/
    ...
    class brand_mysql : public B_BASE
    {
      public:
        brand_mysql(const std::string &tag) : dbtag(tag)
        {
        ...
        brand_mysql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned int value_size, brand_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            data_temp.logo.resize(value_size);
            
            std::memcpy(data_temp.logo.data(), result_temp_data, value_size);
            break;
                case 5:
            ...
            data_temp.detailcontent.resize(value_size);
            
            std::memcpy(data_temp.detailcontent.data(), result_temp_data, value_size);
            break;
...
M_MODEL& nullDetailcontent()
	{
        ...
        wheresql.append(" detailcontent = NULL ");
...
M_MODEL& or_leDetailcontent(T val)
	{
        ...
        template <typename T2>
        M_MODEL &where(brand_info::cols field1, orm::wq opwq, T2 &&field2)
        {
            if (wheresql.empty())
            {
            }
        ...
        template <typename T2>
        M_MODEL &whereOr(brand_info::cols field1, orm::wq opwq, T2 &&field2)
        {
            if (wheresql.empty())
            {
            }
