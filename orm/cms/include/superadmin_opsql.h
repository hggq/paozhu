...
{ /*tagnamespace_replace*/
    ...
    class superadmin_opsql : public B_BASE
    {
      public:
        superadmin_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        superadmin_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, superadmin_info::meta &data_temp)
    {
        switch(index_pos)
        {
            ...
            break;
                case 7:
            data_temp.regdate.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 8:
            ...
            break;
                case 13:
            data_temp.basesitepath.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                
        }
    }
    
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, superadmin_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
            ...
            case 7: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.regdate.assign(v.data(), v.size());
                    else
                        data_temp.regdate.clear();
                }, val);
            } break;
            ...
            case 10: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.loginnum = static_cast<decltype(data_temp.loginnum )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.loginnum = static_cast<decltype(data_temp.loginnum )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.loginnum = static_cast<decltype(data_temp.loginnum )>(v);
            ...
            case 13: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.basesitepath.assign(v.data(), v.size());
                    else
                        data_temp.basesitepath.clear();
                }, val);
            } break;
        ...
        http::obj_val get_field_value(unsigned char index_pos, const superadmin_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            case 6:
                ...
                break;
            case 7:
                return http::obj_val(std::string(data_temp.regdate));
                break;
            case 8:
            ...
            case 9:
                ...
                break;
            case 10:
                return http::obj_val(static_cast<long long>(data_temp.loginnum));
                break;
            case 11:
            ...
            case 12:
                ...
                break;
            case 13:
                return http::obj_val(std::string(data_temp.basesitepath));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const superadmin_info::meta &data_temp)
    {
...
M_MODEL& likeRegdate(const std::string &val)
	{return where(B_BASE::cols::regdate, orm::wq::like, val);
	}
...
M_MODEL& orlikeRegdate(const std::string &val)
	{return whereOr(B_BASE::cols::regdate, orm::wq::like, val);
	}
...
M_MODEL& eqLoginnum(const std::string &val)
	{return where(B_BASE::cols::loginnum, orm::wq::eq, val);
	}

M_MODEL& nqLoginnum(const std::string &val)
	{return where(B_BASE::cols::loginnum, orm::wq::nq, val);
	}

M_MODEL& btLoginnum(const std::string &val)
	{return where(B_BASE::cols::loginnum, orm::wq::bt, val);
	}

M_MODEL& beLoginnum(const std::string &val)
	{return where(B_BASE::cols::loginnum, orm::wq::be, val);
	}

M_MODEL& ltLoginnum(const std::string &val)
	{return where(B_BASE::cols::loginnum, orm::wq::lt, val);
	}

M_MODEL& leLoginnum(const std::string &val)
	{return where(B_BASE::cols::loginnum, orm::wq::le, val);
	}
...
M_MODEL& nullLoginnum()
	{return whereNull(B_BASE::cols::loginnum);
	}

M_MODEL& notnullLoginnum()
	{return whereNotNull(B_BASE::cols::loginnum);
	}

M_MODEL& oreqLoginnum(const std::string &val)
	{return whereOr(B_BASE::cols::loginnum, orm::wq::eq, val);
	}

M_MODEL& ornqLoginnum(const std::string &val)
	{return whereOr(B_BASE::cols::loginnum, orm::wq::nq, val);
	}

M_MODEL& orbtLoginnum(const std::string &val)
	{return whereOr(B_BASE::cols::loginnum, orm::wq::bt, val);
	}

M_MODEL& orbeLoginnum(const std::string &val)
	{return whereOr(B_BASE::cols::loginnum, orm::wq::be, val);
	}

M_MODEL& orltLoginnum(const std::string &val)
	{return whereOr(B_BASE::cols::loginnum, orm::wq::lt, val);
	}

M_MODEL& orleLoginnum(const std::string &val)
	{return whereOr(B_BASE::cols::loginnum, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqLoginnum(T val)
	{return whereOr(B_BASE::cols::loginnum, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqLoginnum(T val)
	{return whereOr(B_BASE::cols::loginnum, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtLoginnum(T val)
	{return whereOr(B_BASE::cols::loginnum, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeLoginnum(T val)
	{return whereOr(B_BASE::cols::loginnum, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltLoginnum(T val)
	{return whereOr(B_BASE::cols::loginnum, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleLoginnum(T val)
	{return whereOr(B_BASE::cols::loginnum, orm::wq::le, val);
	}

M_MODEL& ornullLoginnum()
	{return whereOrNull(B_BASE::cols::loginnum);
	}

M_MODEL& ornotnullLoginnum()
	{return whereOrNotNull(B_BASE::cols::loginnum);
	}
...
M_MODEL& eqBasesitepath(const std::string &val)
	{return where(B_BASE::cols::basesitepath, orm::wq::eq, val);
	}

M_MODEL& nqBasesitepath(const std::string &val)
	{return where(B_BASE::cols::basesitepath, orm::wq::nq, val);
	}

M_MODEL& btBasesitepath(const std::string &val)
	{return where(B_BASE::cols::basesitepath, orm::wq::bt, val);
	}

M_MODEL& beBasesitepath(const std::string &val)
	{return where(B_BASE::cols::basesitepath, orm::wq::be, val);
	}

M_MODEL& ltBasesitepath(const std::string &val)
	{return where(B_BASE::cols::basesitepath, orm::wq::lt, val);
	}

M_MODEL& leBasesitepath(const std::string &val)
	{return where(B_BASE::cols::basesitepath, orm::wq::le, val);
	}

M_MODEL& likeBasesitepath(const std::string &val)
	{return where(B_BASE::cols::basesitepath, orm::wq::like, val);
	}

M_MODEL& nullBasesitepath()
	{return whereNull(B_BASE::cols::basesitepath);
	}

M_MODEL& notnullBasesitepath()
	{return whereNotNull(B_BASE::cols::basesitepath);
	}

M_MODEL& oreqBasesitepath(const std::string &val)
	{return whereOr(B_BASE::cols::basesitepath, orm::wq::eq, val);
	}

M_MODEL& ornqBasesitepath(const std::string &val)
	{return whereOr(B_BASE::cols::basesitepath, orm::wq::nq, val);
	}

M_MODEL& orbtBasesitepath(const std::string &val)
	{return whereOr(B_BASE::cols::basesitepath, orm::wq::bt, val);
	}

M_MODEL& orbeBasesitepath(const std::string &val)
	{return whereOr(B_BASE::cols::basesitepath, orm::wq::be, val);
	}

M_MODEL& orltBasesitepath(const std::string &val)
	{return whereOr(B_BASE::cols::basesitepath, orm::wq::lt, val);
	}

M_MODEL& orleBasesitepath(const std::string &val)
	{return whereOr(B_BASE::cols::basesitepath, orm::wq::le, val);
	}

M_MODEL& orlikeBasesitepath(const std::string &val)
	{return whereOr(B_BASE::cols::basesitepath, orm::wq::like, val);
	}

M_MODEL& ornullBasesitepath()
	{return whereOrNull(B_BASE::cols::basesitepath);
	}

M_MODEL& ornotnullBasesitepath()
	{return whereOrNotNull(B_BASE::cols::basesitepath);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = superadmin_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &superadmin_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &superadmin_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(superadmin_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(superadmin_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(superadmin_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(superadmin_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &superadmin_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &superadmin_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &superadmin_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &superadmin_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &superadmin_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(superadmin_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &superadmin_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(superadmin_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &superadmin_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(superadmin_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &superadmin_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(superadmin_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &superadmin_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(superadmin_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(superadmin_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(superadmin_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(superadmin_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(superadmin_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(superadmin_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(superadmin_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(superadmin_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(superadmin_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(superadmin_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &superadmin_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &superadmin_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &order(superadmin_info::cols field, const std::string &asc_or_desc)
        {
        ...
        M_MODEL &asc(superadmin_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(superadmin_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &superadmin_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &superadmin_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &superadmin_info::col_names> wq)
        {
        ...
        M_MODEL &having(superadmin_info::cols field)
        {
            groupsql.append(" HAVING ");
            switch (field)
            {
        ...
        M_MODEL &group(superadmin_info::cols field)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &superadmin_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = superadmin_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        superadmin_opsql(const superadmin_opsql &)            = delete;
        superadmin_opsql &operator=(const superadmin_opsql &) = delete;
        superadmin_opsql(superadmin_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        superadmin_opsql &operator=(superadmin_opsql &&o) noexcept
        {
            if (this != &o)
            {
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < superadmin_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == superadmin_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(superadmin_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(superadmin_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = superadmin_info::col_names.size();
