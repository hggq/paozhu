#ifndef _ORM_CMS_DEPARTMENT_OPERATE_H
#define _ORM_CMS_DEPARTMENT_OPERATE_H
/*
...
{ /*tagnamespace_replace*/
    ...
    class department_opsql : public B_BASE
    {
      public:
        department_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        department_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, department_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            break;
                case 4:
            data_temp.depart_code.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 5:
             {
               data_temp.bianzhi_num = 0;
            
                    auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            data_temp.bianzhi_num);
                        if (result.ec == std::errc()) {
            ...
            break;
                case 7:
             {
               data_temp.quan_weight = 0;
            
                    auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            data_temp.quan_weight);
                        if (result.ec == std::errc()) {
            ...
            break;
                case 9:
            data_temp.memo.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 10:
             {
            ...
            break;
                case 15:
            data_temp.linkdpid.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                
        }
    }
    
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, department_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
            ...
            case 4: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.depart_code.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.depart_code.assign(v.data(), v.size());
                    else
                        data_temp.depart_code.clear();
                }, val);
            } break;
            case 5: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.bianzhi_num = static_cast<decltype(data_temp.bianzhi_num )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.bianzhi_num = static_cast<decltype(data_temp.bianzhi_num )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.bianzhi_num = static_cast<decltype(data_temp.bianzhi_num )>(v);
            ...
            case 6: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.real_num = static_cast<decltype(data_temp.real_num )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.real_num = static_cast<decltype(data_temp.real_num )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.real_num = static_cast<decltype(data_temp.real_num )>(v);
            ...
            case 7: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.quan_weight = static_cast<decltype(data_temp.quan_weight )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.quan_weight = static_cast<decltype(data_temp.quan_weight )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.quan_weight = static_cast<decltype(data_temp.quan_weight )>(v);
            ...
            case 9: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.memo.assign(v.data(), v.size());
                    else
                        data_temp.memo.clear();
                }, val);
            } break;
            case 10: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.created_time = static_cast<decltype(data_temp.created_time )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.created_time = static_cast<decltype(data_temp.created_time )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.created_time = static_cast<decltype(data_temp.created_time )>(v);
            ...
            case 11: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.created_user = static_cast<decltype(data_temp.created_user )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.created_user = static_cast<decltype(data_temp.created_user )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.created_user = static_cast<decltype(data_temp.created_user )>(v);
            ...
            case 12: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.updated_time = static_cast<decltype(data_temp.updated_time )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.updated_time = static_cast<decltype(data_temp.updated_time )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.updated_time = static_cast<decltype(data_temp.updated_time )>(v);
            ...
            case 13: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.updated_user = static_cast<decltype(data_temp.updated_user )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.updated_user = static_cast<decltype(data_temp.updated_user )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.updated_user = static_cast<decltype(data_temp.updated_user )>(v);
            ...
            case 14: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.isvirtual = static_cast<decltype(data_temp.isvirtual )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.isvirtual = static_cast<decltype(data_temp.isvirtual )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.isvirtual = static_cast<decltype(data_temp.isvirtual )>(v);
            ...
            case 15: {
                std::visit([&](auto&& v){
                    using T = std::decay_t<decltype(v)>;
                    if constexpr (std::is_same_v<T, std::monostate>)
                        data_temp.linkdpid.clear();
                    else if constexpr (std::is_same_v<T, std::string_view>)
                        data_temp.linkdpid.assign(v.data(), v.size());
                    else
                        data_temp.linkdpid.clear();
                }, val);
            } break;
        ...
        http::obj_val get_field_value(unsigned char index_pos, const department_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            case 3:
                ...
                break;
            case 4:
                return http::obj_val(std::string(data_temp.depart_code));
                break;
            case 5:
                return http::obj_val(static_cast<long long>(data_temp.bianzhi_num));
                break;
            case 6:
                return http::obj_val(static_cast<long long>(data_temp.real_num));
                break;
            case 7:
                return http::obj_val(static_cast<long long>(data_temp.quan_weight));
                break;
            case 8:
                return http::obj_val(static_cast<long long>(data_temp.isopen));
                break;
            case 9:
                return http::obj_val(std::string(data_temp.memo));
                break;
            case 10:
                return http::obj_val(static_cast<long long>(data_temp.created_time));
                break;
            case 11:
                return http::obj_val(static_cast<long long>(data_temp.created_user));
                break;
            case 12:
                return http::obj_val(static_cast<long long>(data_temp.updated_time));
                break;
            case 13:
                return http::obj_val(static_cast<long long>(data_temp.updated_user));
                break;
            case 14:
                return http::obj_val(static_cast<long long>(data_temp.isvirtual));
                break;
            case 15:
                return http::obj_val(std::string(data_temp.linkdpid));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const department_info::meta &data_temp)
    {
        // Auto-increment primary key dpid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.dpid == 0)
            return http::obj_val(nullptr);
...
M_MODEL& eqDepartCode(const std::string &val)
	{return where(B_BASE::cols::depart_code, orm::wq::eq, val);
	}

M_MODEL& nqDepartCode(const std::string &val)
	{return where(B_BASE::cols::depart_code, orm::wq::nq, val);
	}

M_MODEL& btDepartCode(const std::string &val)
	{return where(B_BASE::cols::depart_code, orm::wq::bt, val);
	}

M_MODEL& beDepartCode(const std::string &val)
	{return where(B_BASE::cols::depart_code, orm::wq::be, val);
	}

M_MODEL& ltDepartCode(const std::string &val)
	{return where(B_BASE::cols::depart_code, orm::wq::lt, val);
	}

M_MODEL& leDepartCode(const std::string &val)
	{return where(B_BASE::cols::depart_code, orm::wq::le, val);
	}

M_MODEL& likeDepartCode(const std::string &val)
	{return where(B_BASE::cols::depart_code, orm::wq::like, val);
	}

M_MODEL& nullDepartCode()
	{return whereNull(B_BASE::cols::depart_code);
	}

M_MODEL& notnullDepartCode()
	{return whereNotNull(B_BASE::cols::depart_code);
	}

M_MODEL& oreqDepartCode(const std::string &val)
	{return whereOr(B_BASE::cols::depart_code, orm::wq::eq, val);
	}

M_MODEL& ornqDepartCode(const std::string &val)
	{return whereOr(B_BASE::cols::depart_code, orm::wq::nq, val);
	}

M_MODEL& orbtDepartCode(const std::string &val)
	{return whereOr(B_BASE::cols::depart_code, orm::wq::bt, val);
	}

M_MODEL& orbeDepartCode(const std::string &val)
	{return whereOr(B_BASE::cols::depart_code, orm::wq::be, val);
	}

M_MODEL& orltDepartCode(const std::string &val)
	{return whereOr(B_BASE::cols::depart_code, orm::wq::lt, val);
	}

M_MODEL& orleDepartCode(const std::string &val)
	{return whereOr(B_BASE::cols::depart_code, orm::wq::le, val);
	}

M_MODEL& orlikeDepartCode(const std::string &val)
	{return whereOr(B_BASE::cols::depart_code, orm::wq::like, val);
	}

M_MODEL& ornullDepartCode()
	{return whereOrNull(B_BASE::cols::depart_code);
	}

M_MODEL& ornotnullDepartCode()
	{return whereOrNotNull(B_BASE::cols::depart_code);
	}

M_MODEL& eqBianzhiNum(const std::string &val)
	{return where(B_BASE::cols::bianzhi_num, orm::wq::eq, val);
	}

M_MODEL& nqBianzhiNum(const std::string &val)
	{return where(B_BASE::cols::bianzhi_num, orm::wq::nq, val);
	}

M_MODEL& btBianzhiNum(const std::string &val)
	{return where(B_BASE::cols::bianzhi_num, orm::wq::bt, val);
	}

M_MODEL& beBianzhiNum(const std::string &val)
	{return where(B_BASE::cols::bianzhi_num, orm::wq::be, val);
	}

M_MODEL& ltBianzhiNum(const std::string &val)
	{return where(B_BASE::cols::bianzhi_num, orm::wq::lt, val);
	}

M_MODEL& leBianzhiNum(const std::string &val)
	{return where(B_BASE::cols::bianzhi_num, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqBianzhiNum(T val)
	{return where(B_BASE::cols::bianzhi_num, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqBianzhiNum(T val)
	{return where(B_BASE::cols::bianzhi_num, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btBianzhiNum(T val)
	{return where(B_BASE::cols::bianzhi_num, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beBianzhiNum(T val)
	{return where(B_BASE::cols::bianzhi_num, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltBianzhiNum(T val)
	{return where(B_BASE::cols::bianzhi_num, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leBianzhiNum(T val)
	{return where(B_BASE::cols::bianzhi_num, orm::wq::le, val);
	}

M_MODEL& nullBianzhiNum()
	{return whereNull(B_BASE::cols::bianzhi_num);
	}

M_MODEL& notnullBianzhiNum()
	{return whereNotNull(B_BASE::cols::bianzhi_num);
	}

M_MODEL& oreqBianzhiNum(const std::string &val)
	{return whereOr(B_BASE::cols::bianzhi_num, orm::wq::eq, val);
	}

M_MODEL& ornqBianzhiNum(const std::string &val)
	{return whereOr(B_BASE::cols::bianzhi_num, orm::wq::nq, val);
	}

M_MODEL& orbtBianzhiNum(const std::string &val)
	{return whereOr(B_BASE::cols::bianzhi_num, orm::wq::bt, val);
	}

M_MODEL& orbeBianzhiNum(const std::string &val)
	{return whereOr(B_BASE::cols::bianzhi_num, orm::wq::be, val);
	}

M_MODEL& orltBianzhiNum(const std::string &val)
	{return whereOr(B_BASE::cols::bianzhi_num, orm::wq::lt, val);
	}

M_MODEL& orleBianzhiNum(const std::string &val)
	{return whereOr(B_BASE::cols::bianzhi_num, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqBianzhiNum(T val)
	{return whereOr(B_BASE::cols::bianzhi_num, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqBianzhiNum(T val)
	{return whereOr(B_BASE::cols::bianzhi_num, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtBianzhiNum(T val)
	{return whereOr(B_BASE::cols::bianzhi_num, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeBianzhiNum(T val)
	{return whereOr(B_BASE::cols::bianzhi_num, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltBianzhiNum(T val)
	{return whereOr(B_BASE::cols::bianzhi_num, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleBianzhiNum(T val)
	{return whereOr(B_BASE::cols::bianzhi_num, orm::wq::le, val);
	}

M_MODEL& ornullBianzhiNum()
	{return whereOrNull(B_BASE::cols::bianzhi_num);
	}

M_MODEL& ornotnullBianzhiNum()
	{return whereOrNotNull(B_BASE::cols::bianzhi_num);
	}

M_MODEL& eqRealNum(const std::string &val)
	{return where(B_BASE::cols::real_num, orm::wq::eq, val);
	}

M_MODEL& nqRealNum(const std::string &val)
	{return where(B_BASE::cols::real_num, orm::wq::nq, val);
	}

M_MODEL& btRealNum(const std::string &val)
	{return where(B_BASE::cols::real_num, orm::wq::bt, val);
	}

M_MODEL& beRealNum(const std::string &val)
	{return where(B_BASE::cols::real_num, orm::wq::be, val);
	}

M_MODEL& ltRealNum(const std::string &val)
	{return where(B_BASE::cols::real_num, orm::wq::lt, val);
	}

M_MODEL& leRealNum(const std::string &val)
	{return where(B_BASE::cols::real_num, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqRealNum(T val)
	{return where(B_BASE::cols::real_num, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqRealNum(T val)
	{return where(B_BASE::cols::real_num, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btRealNum(T val)
	{return where(B_BASE::cols::real_num, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beRealNum(T val)
	{return where(B_BASE::cols::real_num, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltRealNum(T val)
	{return where(B_BASE::cols::real_num, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leRealNum(T val)
	{return where(B_BASE::cols::real_num, orm::wq::le, val);
	}

M_MODEL& nullRealNum()
	{return whereNull(B_BASE::cols::real_num);
	}

M_MODEL& notnullRealNum()
	{return whereNotNull(B_BASE::cols::real_num);
	}

M_MODEL& oreqRealNum(const std::string &val)
	{return whereOr(B_BASE::cols::real_num, orm::wq::eq, val);
	}

M_MODEL& ornqRealNum(const std::string &val)
	{return whereOr(B_BASE::cols::real_num, orm::wq::nq, val);
	}

M_MODEL& orbtRealNum(const std::string &val)
	{return whereOr(B_BASE::cols::real_num, orm::wq::bt, val);
	}

M_MODEL& orbeRealNum(const std::string &val)
	{return whereOr(B_BASE::cols::real_num, orm::wq::be, val);
	}

M_MODEL& orltRealNum(const std::string &val)
	{return whereOr(B_BASE::cols::real_num, orm::wq::lt, val);
	}

M_MODEL& orleRealNum(const std::string &val)
	{return whereOr(B_BASE::cols::real_num, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqRealNum(T val)
	{return whereOr(B_BASE::cols::real_num, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqRealNum(T val)
	{return whereOr(B_BASE::cols::real_num, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtRealNum(T val)
	{return whereOr(B_BASE::cols::real_num, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeRealNum(T val)
	{return whereOr(B_BASE::cols::real_num, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltRealNum(T val)
	{return whereOr(B_BASE::cols::real_num, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleRealNum(T val)
	{return whereOr(B_BASE::cols::real_num, orm::wq::le, val);
	}

M_MODEL& ornullRealNum()
	{return whereOrNull(B_BASE::cols::real_num);
	}

M_MODEL& ornotnullRealNum()
	{return whereOrNotNull(B_BASE::cols::real_num);
	}

M_MODEL& eqQuanWeight(const std::string &val)
	{return where(B_BASE::cols::quan_weight, orm::wq::eq, val);
	}

M_MODEL& nqQuanWeight(const std::string &val)
	{return where(B_BASE::cols::quan_weight, orm::wq::nq, val);
	}

M_MODEL& btQuanWeight(const std::string &val)
	{return where(B_BASE::cols::quan_weight, orm::wq::bt, val);
	}

M_MODEL& beQuanWeight(const std::string &val)
	{return where(B_BASE::cols::quan_weight, orm::wq::be, val);
	}

M_MODEL& ltQuanWeight(const std::string &val)
	{return where(B_BASE::cols::quan_weight, orm::wq::lt, val);
	}

M_MODEL& leQuanWeight(const std::string &val)
	{return where(B_BASE::cols::quan_weight, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqQuanWeight(T val)
	{return where(B_BASE::cols::quan_weight, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqQuanWeight(T val)
	{return where(B_BASE::cols::quan_weight, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btQuanWeight(T val)
	{return where(B_BASE::cols::quan_weight, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beQuanWeight(T val)
	{return where(B_BASE::cols::quan_weight, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltQuanWeight(T val)
	{return where(B_BASE::cols::quan_weight, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leQuanWeight(T val)
	{return where(B_BASE::cols::quan_weight, orm::wq::le, val);
	}

M_MODEL& nullQuanWeight()
	{return whereNull(B_BASE::cols::quan_weight);
	}

M_MODEL& notnullQuanWeight()
	{return whereNotNull(B_BASE::cols::quan_weight);
	}

M_MODEL& oreqQuanWeight(const std::string &val)
	{return whereOr(B_BASE::cols::quan_weight, orm::wq::eq, val);
	}

M_MODEL& ornqQuanWeight(const std::string &val)
	{return whereOr(B_BASE::cols::quan_weight, orm::wq::nq, val);
	}

M_MODEL& orbtQuanWeight(const std::string &val)
	{return whereOr(B_BASE::cols::quan_weight, orm::wq::bt, val);
	}

M_MODEL& orbeQuanWeight(const std::string &val)
	{return whereOr(B_BASE::cols::quan_weight, orm::wq::be, val);
	}

M_MODEL& orltQuanWeight(const std::string &val)
	{return whereOr(B_BASE::cols::quan_weight, orm::wq::lt, val);
	}

M_MODEL& orleQuanWeight(const std::string &val)
	{return whereOr(B_BASE::cols::quan_weight, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqQuanWeight(T val)
	{return whereOr(B_BASE::cols::quan_weight, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqQuanWeight(T val)
	{return whereOr(B_BASE::cols::quan_weight, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtQuanWeight(T val)
	{return whereOr(B_BASE::cols::quan_weight, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeQuanWeight(T val)
	{return whereOr(B_BASE::cols::quan_weight, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltQuanWeight(T val)
	{return whereOr(B_BASE::cols::quan_weight, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleQuanWeight(T val)
	{return whereOr(B_BASE::cols::quan_weight, orm::wq::le, val);
	}

M_MODEL& ornullQuanWeight()
	{return whereOrNull(B_BASE::cols::quan_weight);
	}

M_MODEL& ornotnullQuanWeight()
	{return whereOrNotNull(B_BASE::cols::quan_weight);
	}
...
M_MODEL& eqMemo(const std::string &val)
	{return where(B_BASE::cols::memo, orm::wq::eq, val);
	}

M_MODEL& nqMemo(const std::string &val)
	{return where(B_BASE::cols::memo, orm::wq::nq, val);
	}

M_MODEL& btMemo(const std::string &val)
	{return where(B_BASE::cols::memo, orm::wq::bt, val);
	}

M_MODEL& beMemo(const std::string &val)
	{return where(B_BASE::cols::memo, orm::wq::be, val);
	}

M_MODEL& ltMemo(const std::string &val)
	{return where(B_BASE::cols::memo, orm::wq::lt, val);
	}

M_MODEL& leMemo(const std::string &val)
	{return where(B_BASE::cols::memo, orm::wq::le, val);
	}

M_MODEL& likeMemo(const std::string &val)
	{return where(B_BASE::cols::memo, orm::wq::like, val);
	}

M_MODEL& nullMemo()
	{return whereNull(B_BASE::cols::memo);
	}

M_MODEL& notnullMemo()
	{return whereNotNull(B_BASE::cols::memo);
	}

M_MODEL& oreqMemo(const std::string &val)
	{return whereOr(B_BASE::cols::memo, orm::wq::eq, val);
	}

M_MODEL& ornqMemo(const std::string &val)
	{return whereOr(B_BASE::cols::memo, orm::wq::nq, val);
	}

M_MODEL& orbtMemo(const std::string &val)
	{return whereOr(B_BASE::cols::memo, orm::wq::bt, val);
	}

M_MODEL& orbeMemo(const std::string &val)
	{return whereOr(B_BASE::cols::memo, orm::wq::be, val);
	}

M_MODEL& orltMemo(const std::string &val)
	{return whereOr(B_BASE::cols::memo, orm::wq::lt, val);
	}

M_MODEL& orleMemo(const std::string &val)
	{return whereOr(B_BASE::cols::memo, orm::wq::le, val);
	}

M_MODEL& orlikeMemo(const std::string &val)
	{return whereOr(B_BASE::cols::memo, orm::wq::like, val);
	}

M_MODEL& ornullMemo()
	{return whereOrNull(B_BASE::cols::memo);
	}

M_MODEL& ornotnullMemo()
	{return whereOrNotNull(B_BASE::cols::memo);
	}

M_MODEL& eqCreatedTime(const std::string &val)
	{return where(B_BASE::cols::created_time, orm::wq::eq, val);
	}

M_MODEL& nqCreatedTime(const std::string &val)
	{return where(B_BASE::cols::created_time, orm::wq::nq, val);
	}

M_MODEL& btCreatedTime(const std::string &val)
	{return where(B_BASE::cols::created_time, orm::wq::bt, val);
	}

M_MODEL& beCreatedTime(const std::string &val)
	{return where(B_BASE::cols::created_time, orm::wq::be, val);
	}

M_MODEL& ltCreatedTime(const std::string &val)
	{return where(B_BASE::cols::created_time, orm::wq::lt, val);
	}

M_MODEL& leCreatedTime(const std::string &val)
	{return where(B_BASE::cols::created_time, orm::wq::le, val);
	}
...
M_MODEL& nullCreatedTime()
	{return whereNull(B_BASE::cols::created_time);
	}

M_MODEL& notnullCreatedTime()
	{return whereNotNull(B_BASE::cols::created_time);
	}

M_MODEL& oreqCreatedTime(const std::string &val)
	{return whereOr(B_BASE::cols::created_time, orm::wq::eq, val);
	}

M_MODEL& ornqCreatedTime(const std::string &val)
	{return whereOr(B_BASE::cols::created_time, orm::wq::nq, val);
	}

M_MODEL& orbtCreatedTime(const std::string &val)
	{return whereOr(B_BASE::cols::created_time, orm::wq::bt, val);
	}

M_MODEL& orbeCreatedTime(const std::string &val)
	{return whereOr(B_BASE::cols::created_time, orm::wq::be, val);
	}

M_MODEL& orltCreatedTime(const std::string &val)
	{return whereOr(B_BASE::cols::created_time, orm::wq::lt, val);
	}

M_MODEL& orleCreatedTime(const std::string &val)
	{return whereOr(B_BASE::cols::created_time, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqCreatedTime(T val)
	{return whereOr(B_BASE::cols::created_time, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqCreatedTime(T val)
	{return whereOr(B_BASE::cols::created_time, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtCreatedTime(T val)
	{return whereOr(B_BASE::cols::created_time, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeCreatedTime(T val)
	{return whereOr(B_BASE::cols::created_time, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltCreatedTime(T val)
	{return whereOr(B_BASE::cols::created_time, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleCreatedTime(T val)
	{return whereOr(B_BASE::cols::created_time, orm::wq::le, val);
	}

M_MODEL& ornullCreatedTime()
	{return whereOrNull(B_BASE::cols::created_time);
	}

M_MODEL& ornotnullCreatedTime()
	{return whereOrNotNull(B_BASE::cols::created_time);
	}

M_MODEL& eqCreatedUser(const std::string &val)
	{return where(B_BASE::cols::created_user, orm::wq::eq, val);
	}

M_MODEL& nqCreatedUser(const std::string &val)
	{return where(B_BASE::cols::created_user, orm::wq::nq, val);
	}

M_MODEL& btCreatedUser(const std::string &val)
	{return where(B_BASE::cols::created_user, orm::wq::bt, val);
	}

M_MODEL& beCreatedUser(const std::string &val)
	{return where(B_BASE::cols::created_user, orm::wq::be, val);
	}

M_MODEL& ltCreatedUser(const std::string &val)
	{return where(B_BASE::cols::created_user, orm::wq::lt, val);
	}

M_MODEL& leCreatedUser(const std::string &val)
	{return where(B_BASE::cols::created_user, orm::wq::le, val);
	}
...
M_MODEL& nullCreatedUser()
	{return whereNull(B_BASE::cols::created_user);
	}

M_MODEL& notnullCreatedUser()
	{return whereNotNull(B_BASE::cols::created_user);
	}

M_MODEL& oreqCreatedUser(const std::string &val)
	{return whereOr(B_BASE::cols::created_user, orm::wq::eq, val);
	}

M_MODEL& ornqCreatedUser(const std::string &val)
	{return whereOr(B_BASE::cols::created_user, orm::wq::nq, val);
	}

M_MODEL& orbtCreatedUser(const std::string &val)
	{return whereOr(B_BASE::cols::created_user, orm::wq::bt, val);
	}

M_MODEL& orbeCreatedUser(const std::string &val)
	{return whereOr(B_BASE::cols::created_user, orm::wq::be, val);
	}

M_MODEL& orltCreatedUser(const std::string &val)
	{return whereOr(B_BASE::cols::created_user, orm::wq::lt, val);
	}

M_MODEL& orleCreatedUser(const std::string &val)
	{return whereOr(B_BASE::cols::created_user, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqCreatedUser(T val)
	{return whereOr(B_BASE::cols::created_user, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqCreatedUser(T val)
	{return whereOr(B_BASE::cols::created_user, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtCreatedUser(T val)
	{return whereOr(B_BASE::cols::created_user, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeCreatedUser(T val)
	{return whereOr(B_BASE::cols::created_user, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltCreatedUser(T val)
	{return whereOr(B_BASE::cols::created_user, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleCreatedUser(T val)
	{return whereOr(B_BASE::cols::created_user, orm::wq::le, val);
	}

M_MODEL& ornullCreatedUser()
	{return whereOrNull(B_BASE::cols::created_user);
	}

M_MODEL& ornotnullCreatedUser()
	{return whereOrNotNull(B_BASE::cols::created_user);
	}

M_MODEL& eqUpdatedTime(const std::string &val)
	{return where(B_BASE::cols::updated_time, orm::wq::eq, val);
	}

M_MODEL& nqUpdatedTime(const std::string &val)
	{return where(B_BASE::cols::updated_time, orm::wq::nq, val);
	}

M_MODEL& btUpdatedTime(const std::string &val)
	{return where(B_BASE::cols::updated_time, orm::wq::bt, val);
	}

M_MODEL& beUpdatedTime(const std::string &val)
	{return where(B_BASE::cols::updated_time, orm::wq::be, val);
	}

M_MODEL& ltUpdatedTime(const std::string &val)
	{return where(B_BASE::cols::updated_time, orm::wq::lt, val);
	}

M_MODEL& leUpdatedTime(const std::string &val)
	{return where(B_BASE::cols::updated_time, orm::wq::le, val);
	}
...
M_MODEL& nullUpdatedTime()
	{return whereNull(B_BASE::cols::updated_time);
	}

M_MODEL& notnullUpdatedTime()
	{return whereNotNull(B_BASE::cols::updated_time);
	}

M_MODEL& oreqUpdatedTime(const std::string &val)
	{return whereOr(B_BASE::cols::updated_time, orm::wq::eq, val);
	}

M_MODEL& ornqUpdatedTime(const std::string &val)
	{return whereOr(B_BASE::cols::updated_time, orm::wq::nq, val);
	}

M_MODEL& orbtUpdatedTime(const std::string &val)
	{return whereOr(B_BASE::cols::updated_time, orm::wq::bt, val);
	}

M_MODEL& orbeUpdatedTime(const std::string &val)
	{return whereOr(B_BASE::cols::updated_time, orm::wq::be, val);
	}

M_MODEL& orltUpdatedTime(const std::string &val)
	{return whereOr(B_BASE::cols::updated_time, orm::wq::lt, val);
	}

M_MODEL& orleUpdatedTime(const std::string &val)
	{return whereOr(B_BASE::cols::updated_time, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqUpdatedTime(T val)
	{return whereOr(B_BASE::cols::updated_time, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqUpdatedTime(T val)
	{return whereOr(B_BASE::cols::updated_time, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtUpdatedTime(T val)
	{return whereOr(B_BASE::cols::updated_time, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeUpdatedTime(T val)
	{return whereOr(B_BASE::cols::updated_time, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltUpdatedTime(T val)
	{return whereOr(B_BASE::cols::updated_time, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleUpdatedTime(T val)
	{return whereOr(B_BASE::cols::updated_time, orm::wq::le, val);
	}

M_MODEL& ornullUpdatedTime()
	{return whereOrNull(B_BASE::cols::updated_time);
	}

M_MODEL& ornotnullUpdatedTime()
	{return whereOrNotNull(B_BASE::cols::updated_time);
	}

M_MODEL& eqUpdatedUser(const std::string &val)
	{return where(B_BASE::cols::updated_user, orm::wq::eq, val);
	}

M_MODEL& nqUpdatedUser(const std::string &val)
	{return where(B_BASE::cols::updated_user, orm::wq::nq, val);
	}

M_MODEL& btUpdatedUser(const std::string &val)
	{return where(B_BASE::cols::updated_user, orm::wq::bt, val);
	}

M_MODEL& beUpdatedUser(const std::string &val)
	{return where(B_BASE::cols::updated_user, orm::wq::be, val);
	}

M_MODEL& ltUpdatedUser(const std::string &val)
	{return where(B_BASE::cols::updated_user, orm::wq::lt, val);
	}

M_MODEL& leUpdatedUser(const std::string &val)
	{return where(B_BASE::cols::updated_user, orm::wq::le, val);
	}
...
M_MODEL& nullUpdatedUser()
	{return whereNull(B_BASE::cols::updated_user);
	}

M_MODEL& notnullUpdatedUser()
	{return whereNotNull(B_BASE::cols::updated_user);
	}

M_MODEL& oreqUpdatedUser(const std::string &val)
	{return whereOr(B_BASE::cols::updated_user, orm::wq::eq, val);
	}

M_MODEL& ornqUpdatedUser(const std::string &val)
	{return whereOr(B_BASE::cols::updated_user, orm::wq::nq, val);
	}

M_MODEL& orbtUpdatedUser(const std::string &val)
	{return whereOr(B_BASE::cols::updated_user, orm::wq::bt, val);
	}

M_MODEL& orbeUpdatedUser(const std::string &val)
	{return whereOr(B_BASE::cols::updated_user, orm::wq::be, val);
	}

M_MODEL& orltUpdatedUser(const std::string &val)
	{return whereOr(B_BASE::cols::updated_user, orm::wq::lt, val);
	}

M_MODEL& orleUpdatedUser(const std::string &val)
	{return whereOr(B_BASE::cols::updated_user, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqUpdatedUser(T val)
	{return whereOr(B_BASE::cols::updated_user, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqUpdatedUser(T val)
	{return whereOr(B_BASE::cols::updated_user, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtUpdatedUser(T val)
	{return whereOr(B_BASE::cols::updated_user, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeUpdatedUser(T val)
	{return whereOr(B_BASE::cols::updated_user, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltUpdatedUser(T val)
	{return whereOr(B_BASE::cols::updated_user, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleUpdatedUser(T val)
	{return whereOr(B_BASE::cols::updated_user, orm::wq::le, val);
	}

M_MODEL& ornullUpdatedUser()
	{return whereOrNull(B_BASE::cols::updated_user);
	}

M_MODEL& ornotnullUpdatedUser()
	{return whereOrNotNull(B_BASE::cols::updated_user);
	}

M_MODEL& eqIsvirtual(const std::string &val)
	{return where(B_BASE::cols::isvirtual, orm::wq::eq, val);
	}

M_MODEL& nqIsvirtual(const std::string &val)
	{return where(B_BASE::cols::isvirtual, orm::wq::nq, val);
	}

M_MODEL& btIsvirtual(const std::string &val)
	{return where(B_BASE::cols::isvirtual, orm::wq::bt, val);
	}

M_MODEL& beIsvirtual(const std::string &val)
	{return where(B_BASE::cols::isvirtual, orm::wq::be, val);
	}

M_MODEL& ltIsvirtual(const std::string &val)
	{return where(B_BASE::cols::isvirtual, orm::wq::lt, val);
	}

M_MODEL& leIsvirtual(const std::string &val)
	{return where(B_BASE::cols::isvirtual, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqIsvirtual(T val)
	{return where(B_BASE::cols::isvirtual, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqIsvirtual(T val)
	{return where(B_BASE::cols::isvirtual, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btIsvirtual(T val)
	{return where(B_BASE::cols::isvirtual, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beIsvirtual(T val)
	{return where(B_BASE::cols::isvirtual, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltIsvirtual(T val)
	{return where(B_BASE::cols::isvirtual, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leIsvirtual(T val)
	{return where(B_BASE::cols::isvirtual, orm::wq::le, val);
	}

M_MODEL& nullIsvirtual()
	{return whereNull(B_BASE::cols::isvirtual);
	}

M_MODEL& notnullIsvirtual()
	{return whereNotNull(B_BASE::cols::isvirtual);
	}

M_MODEL& oreqIsvirtual(const std::string &val)
	{return whereOr(B_BASE::cols::isvirtual, orm::wq::eq, val);
	}

M_MODEL& ornqIsvirtual(const std::string &val)
	{return whereOr(B_BASE::cols::isvirtual, orm::wq::nq, val);
	}

M_MODEL& orbtIsvirtual(const std::string &val)
	{return whereOr(B_BASE::cols::isvirtual, orm::wq::bt, val);
	}

M_MODEL& orbeIsvirtual(const std::string &val)
	{return whereOr(B_BASE::cols::isvirtual, orm::wq::be, val);
	}

M_MODEL& orltIsvirtual(const std::string &val)
	{return whereOr(B_BASE::cols::isvirtual, orm::wq::lt, val);
	}

M_MODEL& orleIsvirtual(const std::string &val)
	{return whereOr(B_BASE::cols::isvirtual, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqIsvirtual(T val)
	{return whereOr(B_BASE::cols::isvirtual, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqIsvirtual(T val)
	{return whereOr(B_BASE::cols::isvirtual, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtIsvirtual(T val)
	{return whereOr(B_BASE::cols::isvirtual, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeIsvirtual(T val)
	{return whereOr(B_BASE::cols::isvirtual, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltIsvirtual(T val)
	{return whereOr(B_BASE::cols::isvirtual, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleIsvirtual(T val)
	{return whereOr(B_BASE::cols::isvirtual, orm::wq::le, val);
	}

M_MODEL& ornullIsvirtual()
	{return whereOrNull(B_BASE::cols::isvirtual);
	}

M_MODEL& ornotnullIsvirtual()
	{return whereOrNotNull(B_BASE::cols::isvirtual);
	}

M_MODEL& eqLinkdpid(const std::string &val)
	{return where(B_BASE::cols::linkdpid, orm::wq::eq, val);
	}

M_MODEL& nqLinkdpid(const std::string &val)
	{return where(B_BASE::cols::linkdpid, orm::wq::nq, val);
	}

M_MODEL& btLinkdpid(const std::string &val)
	{return where(B_BASE::cols::linkdpid, orm::wq::bt, val);
	}

M_MODEL& beLinkdpid(const std::string &val)
	{return where(B_BASE::cols::linkdpid, orm::wq::be, val);
	}

M_MODEL& ltLinkdpid(const std::string &val)
	{return where(B_BASE::cols::linkdpid, orm::wq::lt, val);
	}

M_MODEL& leLinkdpid(const std::string &val)
	{return where(B_BASE::cols::linkdpid, orm::wq::le, val);
	}

M_MODEL& likeLinkdpid(const std::string &val)
	{return where(B_BASE::cols::linkdpid, orm::wq::like, val);
	}

M_MODEL& nullLinkdpid()
	{return whereNull(B_BASE::cols::linkdpid);
	}

M_MODEL& notnullLinkdpid()
	{return whereNotNull(B_BASE::cols::linkdpid);
	}

M_MODEL& oreqLinkdpid(const std::string &val)
	{return whereOr(B_BASE::cols::linkdpid, orm::wq::eq, val);
	}

M_MODEL& ornqLinkdpid(const std::string &val)
	{return whereOr(B_BASE::cols::linkdpid, orm::wq::nq, val);
	}

M_MODEL& orbtLinkdpid(const std::string &val)
	{return whereOr(B_BASE::cols::linkdpid, orm::wq::bt, val);
	}

M_MODEL& orbeLinkdpid(const std::string &val)
	{return whereOr(B_BASE::cols::linkdpid, orm::wq::be, val);
	}

M_MODEL& orltLinkdpid(const std::string &val)
	{return whereOr(B_BASE::cols::linkdpid, orm::wq::lt, val);
	}

M_MODEL& orleLinkdpid(const std::string &val)
	{return whereOr(B_BASE::cols::linkdpid, orm::wq::le, val);
	}

M_MODEL& orlikeLinkdpid(const std::string &val)
	{return whereOr(B_BASE::cols::linkdpid, orm::wq::like, val);
	}

M_MODEL& ornullLinkdpid()
	{return whereOrNull(B_BASE::cols::linkdpid);
	}

M_MODEL& ornotnullLinkdpid()
	{return whereOrNotNull(B_BASE::cols::linkdpid);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = department_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &department_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &department_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(department_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(department_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(department_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(department_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &department_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &department_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &department_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &department_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &department_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(department_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &department_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(department_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &department_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(department_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &department_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(department_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &department_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(department_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(department_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(department_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(department_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(department_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(department_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(department_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(department_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(department_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(department_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &department_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &department_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &order(department_info::cols field, const std::string &asc_or_desc)
        {
        ...
        M_MODEL &asc(department_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(department_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &department_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &department_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &department_info::col_names> wq)
        {
        ...
        M_MODEL &having(department_info::cols field)
        {
            groupsql.append(" HAVING ");
            switch (field)
            {
			...
			case department_info::cols::name:
				...
				break;
			case department_info::cols::depart_code:
				groupsql.append("depart_code");
				break;
			case department_info::cols::bianzhi_num:
				groupsql.append("bianzhi_num");
				break;
			case department_info::cols::real_num:
				groupsql.append("real_num");
				break;
			case department_info::cols::quan_weight:
				groupsql.append("quan_weight");
				break;
			case department_info::cols::isopen:
				groupsql.append("isopen");
				break;
			case department_info::cols::memo:
				groupsql.append("memo");
				break;
			case department_info::cols::created_time:
			...
			case department_info::cols::updated_user:
				...
				break;
			case department_info::cols::isvirtual:
				groupsql.append("isvirtual");
				break;
			case department_info::cols::linkdpid:
				groupsql.append("linkdpid");
				break;
            default:
                return *mod;
                break;
        ...
        M_MODEL &group(department_info::cols field)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &department_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
            ...
            try
            {
                ...
                unsigned int fetch_count = select_conn->fetch_directly(sqlstring,
                                                                       [this, col_pos_map = std::vector<int>{}, first_row = true](int col_count, char **col_names, auto get_data) mutable -> bool
                                                                       {
                                                                           ...
                                                                           department_info::meta data_temp;
                                                                           for (int ij = 0; ij < col_count; ij++)
                                                                           {
                                                                               auto [ptr, len] = get_data(ij);
                                                                               if (ptr == nullptr)
        ...
        {
            model_meta_cache<department_info::meta> &temp_cache = model_meta_cache<department_info::meta>::getinstance();
        ...
        {
            model_meta_cache<std::vector<department_info::meta>> &temp_cache = model_meta_cache<std::vector<department_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(sqlstring);
        ...
        bool save_data_cache(const std::string &cache_key_name, const department_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<department_info::meta> &temp_cache = model_meta_cache<department_info::meta>::getinstance();
        ...
        bool save_cache(std::size_t cache_key_name, const std::vector<department_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<department_info::meta>> &temp_cache = model_meta_cache<std::vector<department_info::meta>>::getinstance();
            temp_cache.save(cache_key_name, cache_data, exp_time);
            return true;
        }
        bool save_cache(const std::string cache_key_name, const std::vector<department_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<department_info::meta>> &temp_cache = model_meta_cache<std::vector<department_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_vector_cache(const std::string cache_key_name, const std::vector<department_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<department_info::meta>> &temp_cache = model_meta_cache<std::vector<department_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_cache(const std::string cache_key_name, const department_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<department_info::meta> &temp_cache = model_meta_cache<department_info::meta>::getinstance();
            std::size_t sqlhashid                               = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        const department_info::meta get_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<department_info::meta> &temp_cache = model_meta_cache<department_info::meta>::getinstance();
        ...
        const std::vector<department_info::meta> get_vector_cache(const std::string &cache_key_name)
        {
            try
            {
                model_meta_cache<std::vector<department_info::meta>> &temp_cache = model_meta_cache<std::vector<department_info::meta>>::getinstance();
        ...
        std::tuple<unsigned int, unsigned long long> insert(department_info::meta &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(department_info::meta &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_data_insert_sql(insert_data);
        ...
        std::tuple<unsigned int, unsigned long long> insert(std::vector<department_info::meta> &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(std::vector<department_info::meta> &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_vector_insert_sql(insert_data);
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = department_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(department_info::meta &insert_data)
        {
        ...
        M_MODEL &set_data(department_info::meta indata)
        {
            B_BASE::data = indata;
            return *mod;
        }
        ...
        department_opsql(const department_opsql &)            = delete;
        department_opsql &operator=(const department_opsql &) = delete;
        department_opsql(department_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        department_opsql &operator=(department_opsql &&o) noexcept
        {
            if (this != &o)
            {
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < department_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == department_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(department_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(department_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = department_info::col_names.size();
