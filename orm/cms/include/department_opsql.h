...
{ /*tagnamespace_replace*/
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, department_info::meta &data_temp)
    {
        switch(index_pos)
        {
            ...
            data_temp.depart_code.resize(value_size);
            
            std::memcpy(data_temp.depart_code.data(), result_temp_data, value_size);
            break;
                case 5:
             {
            ...
            data_temp.linkdpid.resize(value_size);
            
            std::memcpy(data_temp.linkdpid.data(), result_temp_data, value_size);
            break;
...
M_MODEL& or_leLinkdpid(T val)
	{
        ...
        template <typename T2>
        M_MODEL &where(department_info::cols field1, orm::wq opwq, T2 &&field2)
        {
            if (wheresql.empty())
            {
            }
        ...
        template <typename T2>
        M_MODEL &whereOr(department_info::cols field1, orm::wq opwq, T2 &&field2)
        {
            if (wheresql.empty())
            {
            }
