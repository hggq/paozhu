...
{ /*tagnamespace_replace*/
    ...
    class topicset_opsql : public B_BASE
    {
      public:
        topicset_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        topicset_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, topicset_info::meta &data_temp)
    {
        switch(index_pos)
        {
            ...
            break;
                case 6:
            data_temp.sidename.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 7:
            data_temp.txtcontent.assign(reinterpret_cast<const char*>(result_temp_data), value_size);
            break;
                case 8:
             {
               data_temp.sort = 0;
        ...
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, topicset_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.topicsetid = static_cast<decltype(data_temp.topicsetid )>(v);
            ...
            case 3: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.linktopicid = static_cast<decltype(data_temp.linktopicid )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.linktopicid = static_cast<decltype(data_temp.linktopicid )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.linktopicid = static_cast<decltype(data_temp.linktopicid )>(v);
            ...
            case 4: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.linkrownum = static_cast<decltype(data_temp.linkrownum )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.linkrownum = static_cast<decltype(data_temp.linkrownum )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.linkrownum = static_cast<decltype(data_temp.linkrownum )>(v);
            ...
            case 5: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.sidetype = static_cast<decltype(data_temp.sidetype )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.sidetype = static_cast<decltype(data_temp.sidetype )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.sidetype = static_cast<decltype(data_temp.sidetype )>(v);
            ...
            case 6: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.sidename.assign(v.data(), v.size());
                    else
                        data_temp.sidename.clear();
                }, val);
            } break;
            case 7: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.txtcontent.assign(v.data(), v.size());
                    else
                        data_temp.txtcontent.clear();
                }, val);
            } break;
            case 8: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.sort = static_cast<decltype(data_temp.sort )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.sort = static_cast<decltype(data_temp.sort )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.sort = static_cast<decltype(data_temp.sort )>(v);
        ...
        http::obj_val get_field_value(unsigned char index_pos, const topicset_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
                return http::obj_val(static_cast<long long>(data_temp.topicsetid));
                break;
            case 1:
            ...
            case 2:
                ...
                break;
            case 3:
                return http::obj_val(static_cast<long long>(data_temp.linktopicid));
                break;
            case 4:
                return http::obj_val(static_cast<long long>(data_temp.linkrownum));
                break;
            case 5:
                return http::obj_val(static_cast<long long>(data_temp.sidetype));
                break;
            case 6:
                return http::obj_val(std::string(data_temp.sidename));
                break;
            case 7:
                return http::obj_val(std::string(data_temp.txtcontent));
                break;
            case 8:
                return http::obj_val(static_cast<long long>(data_temp.sort));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const topicset_info::meta &data_temp)
    {
        // Auto-increment primary key topicsetid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.topicsetid == 0)
            return http::obj_val(nullptr);
...
M_MODEL& eqTopicsetid(const std::string &val)
	{return where(B_BASE::cols::topicsetid, orm::wq::eq, val);
	}

M_MODEL& nqTopicsetid(const std::string &val)
	{return where(B_BASE::cols::topicsetid, orm::wq::nq, val);
	}

M_MODEL& btTopicsetid(const std::string &val)
	{return where(B_BASE::cols::topicsetid, orm::wq::bt, val);
	}

M_MODEL& beTopicsetid(const std::string &val)
	{return where(B_BASE::cols::topicsetid, orm::wq::be, val);
	}

M_MODEL& ltTopicsetid(const std::string &val)
	{return where(B_BASE::cols::topicsetid, orm::wq::lt, val);
	}

M_MODEL& leTopicsetid(const std::string &val)
	{return where(B_BASE::cols::topicsetid, orm::wq::le, val);
	}
...
M_MODEL& nullTopicsetid()
	{return whereNull(B_BASE::cols::topicsetid);
	}

M_MODEL& notnullTopicsetid()
	{return whereNotNull(B_BASE::cols::topicsetid);
	}

M_MODEL& oreqTopicsetid(const std::string &val)
	{return whereOr(B_BASE::cols::topicsetid, orm::wq::eq, val);
	}

M_MODEL& ornqTopicsetid(const std::string &val)
	{return whereOr(B_BASE::cols::topicsetid, orm::wq::nq, val);
	}

M_MODEL& orbtTopicsetid(const std::string &val)
	{return whereOr(B_BASE::cols::topicsetid, orm::wq::bt, val);
	}

M_MODEL& orbeTopicsetid(const std::string &val)
	{return whereOr(B_BASE::cols::topicsetid, orm::wq::be, val);
	}

M_MODEL& orltTopicsetid(const std::string &val)
	{return whereOr(B_BASE::cols::topicsetid, orm::wq::lt, val);
	}

M_MODEL& orleTopicsetid(const std::string &val)
	{return whereOr(B_BASE::cols::topicsetid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqTopicsetid(T val)
	{return whereOr(B_BASE::cols::topicsetid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqTopicsetid(T val)
	{return whereOr(B_BASE::cols::topicsetid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtTopicsetid(T val)
	{return whereOr(B_BASE::cols::topicsetid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeTopicsetid(T val)
	{return whereOr(B_BASE::cols::topicsetid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltTopicsetid(T val)
	{return whereOr(B_BASE::cols::topicsetid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleTopicsetid(T val)
	{return whereOr(B_BASE::cols::topicsetid, orm::wq::le, val);
	}

M_MODEL& ornullTopicsetid()
	{return whereOrNull(B_BASE::cols::topicsetid);
	}

M_MODEL& ornotnullTopicsetid()
	{return whereOrNotNull(B_BASE::cols::topicsetid);
	}
...
M_MODEL& eqLinktopicid(const std::string &val)
	{return where(B_BASE::cols::linktopicid, orm::wq::eq, val);
	}

M_MODEL& nqLinktopicid(const std::string &val)
	{return where(B_BASE::cols::linktopicid, orm::wq::nq, val);
	}

M_MODEL& btLinktopicid(const std::string &val)
	{return where(B_BASE::cols::linktopicid, orm::wq::bt, val);
	}

M_MODEL& beLinktopicid(const std::string &val)
	{return where(B_BASE::cols::linktopicid, orm::wq::be, val);
	}

M_MODEL& ltLinktopicid(const std::string &val)
	{return where(B_BASE::cols::linktopicid, orm::wq::lt, val);
	}

M_MODEL& leLinktopicid(const std::string &val)
	{return where(B_BASE::cols::linktopicid, orm::wq::le, val);
	}
...
M_MODEL& nullLinktopicid()
	{return whereNull(B_BASE::cols::linktopicid);
	}

M_MODEL& notnullLinktopicid()
	{return whereNotNull(B_BASE::cols::linktopicid);
	}

M_MODEL& oreqLinktopicid(const std::string &val)
	{return whereOr(B_BASE::cols::linktopicid, orm::wq::eq, val);
	}

M_MODEL& ornqLinktopicid(const std::string &val)
	{return whereOr(B_BASE::cols::linktopicid, orm::wq::nq, val);
	}

M_MODEL& orbtLinktopicid(const std::string &val)
	{return whereOr(B_BASE::cols::linktopicid, orm::wq::bt, val);
	}

M_MODEL& orbeLinktopicid(const std::string &val)
	{return whereOr(B_BASE::cols::linktopicid, orm::wq::be, val);
	}

M_MODEL& orltLinktopicid(const std::string &val)
	{return whereOr(B_BASE::cols::linktopicid, orm::wq::lt, val);
	}

M_MODEL& orleLinktopicid(const std::string &val)
	{return whereOr(B_BASE::cols::linktopicid, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqLinktopicid(T val)
	{return whereOr(B_BASE::cols::linktopicid, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqLinktopicid(T val)
	{return whereOr(B_BASE::cols::linktopicid, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtLinktopicid(T val)
	{return whereOr(B_BASE::cols::linktopicid, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeLinktopicid(T val)
	{return whereOr(B_BASE::cols::linktopicid, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltLinktopicid(T val)
	{return whereOr(B_BASE::cols::linktopicid, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleLinktopicid(T val)
	{return whereOr(B_BASE::cols::linktopicid, orm::wq::le, val);
	}

M_MODEL& ornullLinktopicid()
	{return whereOrNull(B_BASE::cols::linktopicid);
	}

M_MODEL& ornotnullLinktopicid()
	{return whereOrNotNull(B_BASE::cols::linktopicid);
	}

M_MODEL& eqLinkrownum(const std::string &val)
	{return where(B_BASE::cols::linkrownum, orm::wq::eq, val);
	}

M_MODEL& nqLinkrownum(const std::string &val)
	{return where(B_BASE::cols::linkrownum, orm::wq::nq, val);
	}

M_MODEL& btLinkrownum(const std::string &val)
	{return where(B_BASE::cols::linkrownum, orm::wq::bt, val);
	}

M_MODEL& beLinkrownum(const std::string &val)
	{return where(B_BASE::cols::linkrownum, orm::wq::be, val);
	}

M_MODEL& ltLinkrownum(const std::string &val)
	{return where(B_BASE::cols::linkrownum, orm::wq::lt, val);
	}

M_MODEL& leLinkrownum(const std::string &val)
	{return where(B_BASE::cols::linkrownum, orm::wq::le, val);
	}
...
M_MODEL& nullLinkrownum()
	{return whereNull(B_BASE::cols::linkrownum);
	}

M_MODEL& notnullLinkrownum()
	{return whereNotNull(B_BASE::cols::linkrownum);
	}

M_MODEL& oreqLinkrownum(const std::string &val)
	{return whereOr(B_BASE::cols::linkrownum, orm::wq::eq, val);
	}

M_MODEL& ornqLinkrownum(const std::string &val)
	{return whereOr(B_BASE::cols::linkrownum, orm::wq::nq, val);
	}

M_MODEL& orbtLinkrownum(const std::string &val)
	{return whereOr(B_BASE::cols::linkrownum, orm::wq::bt, val);
	}

M_MODEL& orbeLinkrownum(const std::string &val)
	{return whereOr(B_BASE::cols::linkrownum, orm::wq::be, val);
	}

M_MODEL& orltLinkrownum(const std::string &val)
	{return whereOr(B_BASE::cols::linkrownum, orm::wq::lt, val);
	}

M_MODEL& orleLinkrownum(const std::string &val)
	{return whereOr(B_BASE::cols::linkrownum, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqLinkrownum(T val)
	{return whereOr(B_BASE::cols::linkrownum, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqLinkrownum(T val)
	{return whereOr(B_BASE::cols::linkrownum, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtLinkrownum(T val)
	{return whereOr(B_BASE::cols::linkrownum, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeLinkrownum(T val)
	{return whereOr(B_BASE::cols::linkrownum, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltLinkrownum(T val)
	{return whereOr(B_BASE::cols::linkrownum, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleLinkrownum(T val)
	{return whereOr(B_BASE::cols::linkrownum, orm::wq::le, val);
	}

M_MODEL& ornullLinkrownum()
	{return whereOrNull(B_BASE::cols::linkrownum);
	}

M_MODEL& ornotnullLinkrownum()
	{return whereOrNotNull(B_BASE::cols::linkrownum);
	}

M_MODEL& eqSidetype(const std::string &val)
	{return where(B_BASE::cols::sidetype, orm::wq::eq, val);
	}

M_MODEL& nqSidetype(const std::string &val)
	{return where(B_BASE::cols::sidetype, orm::wq::nq, val);
	}

M_MODEL& btSidetype(const std::string &val)
	{return where(B_BASE::cols::sidetype, orm::wq::bt, val);
	}

M_MODEL& beSidetype(const std::string &val)
	{return where(B_BASE::cols::sidetype, orm::wq::be, val);
	}

M_MODEL& ltSidetype(const std::string &val)
	{return where(B_BASE::cols::sidetype, orm::wq::lt, val);
	}

M_MODEL& leSidetype(const std::string &val)
	{return where(B_BASE::cols::sidetype, orm::wq::le, val);
	}
...
M_MODEL& nullSidetype()
	{return whereNull(B_BASE::cols::sidetype);
	}

M_MODEL& notnullSidetype()
	{return whereNotNull(B_BASE::cols::sidetype);
	}

M_MODEL& oreqSidetype(const std::string &val)
	{return whereOr(B_BASE::cols::sidetype, orm::wq::eq, val);
	}

M_MODEL& ornqSidetype(const std::string &val)
	{return whereOr(B_BASE::cols::sidetype, orm::wq::nq, val);
	}

M_MODEL& orbtSidetype(const std::string &val)
	{return whereOr(B_BASE::cols::sidetype, orm::wq::bt, val);
	}

M_MODEL& orbeSidetype(const std::string &val)
	{return whereOr(B_BASE::cols::sidetype, orm::wq::be, val);
	}

M_MODEL& orltSidetype(const std::string &val)
	{return whereOr(B_BASE::cols::sidetype, orm::wq::lt, val);
	}

M_MODEL& orleSidetype(const std::string &val)
	{return whereOr(B_BASE::cols::sidetype, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqSidetype(T val)
	{return whereOr(B_BASE::cols::sidetype, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqSidetype(T val)
	{return whereOr(B_BASE::cols::sidetype, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtSidetype(T val)
	{return whereOr(B_BASE::cols::sidetype, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeSidetype(T val)
	{return whereOr(B_BASE::cols::sidetype, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltSidetype(T val)
	{return whereOr(B_BASE::cols::sidetype, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleSidetype(T val)
	{return whereOr(B_BASE::cols::sidetype, orm::wq::le, val);
	}

M_MODEL& ornullSidetype()
	{return whereOrNull(B_BASE::cols::sidetype);
	}

M_MODEL& ornotnullSidetype()
	{return whereOrNotNull(B_BASE::cols::sidetype);
	}

M_MODEL& eqSidename(const std::string &val)
	{return where(B_BASE::cols::sidename, orm::wq::eq, val);
	}

M_MODEL& nqSidename(const std::string &val)
	{return where(B_BASE::cols::sidename, orm::wq::nq, val);
	}

M_MODEL& btSidename(const std::string &val)
	{return where(B_BASE::cols::sidename, orm::wq::bt, val);
	}

M_MODEL& beSidename(const std::string &val)
	{return where(B_BASE::cols::sidename, orm::wq::be, val);
	}

M_MODEL& ltSidename(const std::string &val)
	{return where(B_BASE::cols::sidename, orm::wq::lt, val);
	}

M_MODEL& leSidename(const std::string &val)
	{return where(B_BASE::cols::sidename, orm::wq::le, val);
	}

M_MODEL& likeSidename(const std::string &val)
	{return where(B_BASE::cols::sidename, orm::wq::like, val);
	}

M_MODEL& nullSidename()
	{return whereNull(B_BASE::cols::sidename);
	}

M_MODEL& notnullSidename()
	{return whereNotNull(B_BASE::cols::sidename);
	}

M_MODEL& oreqSidename(const std::string &val)
	{return whereOr(B_BASE::cols::sidename, orm::wq::eq, val);
	}

M_MODEL& ornqSidename(const std::string &val)
	{return whereOr(B_BASE::cols::sidename, orm::wq::nq, val);
	}

M_MODEL& orbtSidename(const std::string &val)
	{return whereOr(B_BASE::cols::sidename, orm::wq::bt, val);
	}

M_MODEL& orbeSidename(const std::string &val)
	{return whereOr(B_BASE::cols::sidename, orm::wq::be, val);
	}

M_MODEL& orltSidename(const std::string &val)
	{return whereOr(B_BASE::cols::sidename, orm::wq::lt, val);
	}

M_MODEL& orleSidename(const std::string &val)
	{return whereOr(B_BASE::cols::sidename, orm::wq::le, val);
	}

M_MODEL& orlikeSidename(const std::string &val)
	{return whereOr(B_BASE::cols::sidename, orm::wq::like, val);
	}

M_MODEL& ornullSidename()
	{return whereOrNull(B_BASE::cols::sidename);
	}

M_MODEL& ornotnullSidename()
	{return whereOrNotNull(B_BASE::cols::sidename);
	}

M_MODEL& eqTxtcontent(const std::string &val)
	{return where(B_BASE::cols::txtcontent, orm::wq::eq, val);
	}

M_MODEL& nqTxtcontent(const std::string &val)
	{return where(B_BASE::cols::txtcontent, orm::wq::nq, val);
	}

M_MODEL& btTxtcontent(const std::string &val)
	{return where(B_BASE::cols::txtcontent, orm::wq::bt, val);
	}

M_MODEL& beTxtcontent(const std::string &val)
	{return where(B_BASE::cols::txtcontent, orm::wq::be, val);
	}

M_MODEL& ltTxtcontent(const std::string &val)
	{return where(B_BASE::cols::txtcontent, orm::wq::lt, val);
	}

M_MODEL& leTxtcontent(const std::string &val)
	{return where(B_BASE::cols::txtcontent, orm::wq::le, val);
	}

M_MODEL& likeTxtcontent(const std::string &val)
	{return where(B_BASE::cols::txtcontent, orm::wq::like, val);
	}

M_MODEL& nullTxtcontent()
	{return whereNull(B_BASE::cols::txtcontent);
	}

M_MODEL& notnullTxtcontent()
	{return whereNotNull(B_BASE::cols::txtcontent);
	}

M_MODEL& oreqTxtcontent(const std::string &val)
	{return whereOr(B_BASE::cols::txtcontent, orm::wq::eq, val);
	}

M_MODEL& ornqTxtcontent(const std::string &val)
	{return whereOr(B_BASE::cols::txtcontent, orm::wq::nq, val);
	}

M_MODEL& orbtTxtcontent(const std::string &val)
	{return whereOr(B_BASE::cols::txtcontent, orm::wq::bt, val);
	}

M_MODEL& orbeTxtcontent(const std::string &val)
	{return whereOr(B_BASE::cols::txtcontent, orm::wq::be, val);
	}

M_MODEL& orltTxtcontent(const std::string &val)
	{return whereOr(B_BASE::cols::txtcontent, orm::wq::lt, val);
	}

M_MODEL& orleTxtcontent(const std::string &val)
	{return whereOr(B_BASE::cols::txtcontent, orm::wq::le, val);
	}

M_MODEL& orlikeTxtcontent(const std::string &val)
	{return whereOr(B_BASE::cols::txtcontent, orm::wq::like, val);
	}

M_MODEL& ornullTxtcontent()
	{return whereOrNull(B_BASE::cols::txtcontent);
	}

M_MODEL& ornotnullTxtcontent()
	{return whereOrNotNull(B_BASE::cols::txtcontent);
	}

M_MODEL& eqSort(const std::string &val)
	{return where(B_BASE::cols::sort, orm::wq::eq, val);
	}

M_MODEL& nqSort(const std::string &val)
	{return where(B_BASE::cols::sort, orm::wq::nq, val);
	}

M_MODEL& btSort(const std::string &val)
	{return where(B_BASE::cols::sort, orm::wq::bt, val);
	}

M_MODEL& beSort(const std::string &val)
	{return where(B_BASE::cols::sort, orm::wq::be, val);
	}

M_MODEL& ltSort(const std::string &val)
	{return where(B_BASE::cols::sort, orm::wq::lt, val);
	}

M_MODEL& leSort(const std::string &val)
	{return where(B_BASE::cols::sort, orm::wq::le, val);
	}
...
M_MODEL& nullSort()
	{return whereNull(B_BASE::cols::sort);
	}

M_MODEL& notnullSort()
	{return whereNotNull(B_BASE::cols::sort);
	}

M_MODEL& oreqSort(const std::string &val)
	{return whereOr(B_BASE::cols::sort, orm::wq::eq, val);
	}

M_MODEL& ornqSort(const std::string &val)
	{return whereOr(B_BASE::cols::sort, orm::wq::nq, val);
	}

M_MODEL& orbtSort(const std::string &val)
	{return whereOr(B_BASE::cols::sort, orm::wq::bt, val);
	}

M_MODEL& orbeSort(const std::string &val)
	{return whereOr(B_BASE::cols::sort, orm::wq::be, val);
	}

M_MODEL& orltSort(const std::string &val)
	{return whereOr(B_BASE::cols::sort, orm::wq::lt, val);
	}

M_MODEL& orleSort(const std::string &val)
	{return whereOr(B_BASE::cols::sort, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqSort(T val)
	{return whereOr(B_BASE::cols::sort, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqSort(T val)
	{return whereOr(B_BASE::cols::sort, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtSort(T val)
	{return whereOr(B_BASE::cols::sort, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeSort(T val)
	{return whereOr(B_BASE::cols::sort, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltSort(T val)
	{return whereOr(B_BASE::cols::sort, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleSort(T val)
	{return whereOr(B_BASE::cols::sort, orm::wq::le, val);
	}

M_MODEL& ornullSort()
	{return whereOrNull(B_BASE::cols::sort);
	}

M_MODEL& ornotnullSort()
	{return whereOrNotNull(B_BASE::cols::sort);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = topicset_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &topicset_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &topicset_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(topicset_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(topicset_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(topicset_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(topicset_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &topicset_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &topicset_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &topicset_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &topicset_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &topicset_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(topicset_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &topicset_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(topicset_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &topicset_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(topicset_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &topicset_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(topicset_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &topicset_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(topicset_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(topicset_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(topicset_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(topicset_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(topicset_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(topicset_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(topicset_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(topicset_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(topicset_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(topicset_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &topicset_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &topicset_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &order(topicset_info::cols field, const std::string &asc_or_desc)
        {
        ...
        M_MODEL &asc(topicset_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &topicset_info::col_names> field1, orm::table_col<B_BASE, &topicset_info::col_names> field2)
        {
        ...
        M_MODEL &asc(topicset_info::cols field, topicset_info::cols field2)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(topicset_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(topicset_info::cols field, topicset_info::cols field2)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &topicset_info::col_names> field1, orm::table_col<B_BASE, &topicset_info::col_names> field2)
        {
        ...
        M_MODEL &order(orm::table_col<B_BASE, &topicset_info::col_names> wq, const std::string &asc_or_desc)
        {
            ordersql.append(" ORDER BY ");
            ordersql.append(wq);
            ordersql.append(" ");
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &topicset_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &topicset_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &topicset_info::col_names> wq)
        {
        ...
        M_MODEL &having(topicset_info::cols field)
        {
            groupsql.append(" HAVING ");
            switch (field)
            {
			...
			case topicset_info::cols::topicid:
				...
				break;
			case topicset_info::cols::linktopicid:
				groupsql.append("linktopicid");
				break;
			case topicset_info::cols::linkrownum:
				groupsql.append("linkrownum");
				break;
			case topicset_info::cols::sidetype:
				groupsql.append("sidetype");
				break;
			case topicset_info::cols::sidename:
				groupsql.append("sidename");
				break;
			case topicset_info::cols::txtcontent:
				groupsql.append("txtcontent");
				break;
			case topicset_info::cols::sort:
				groupsql.append("sort");
				break;
            default:
                return *mod;
                break;
        ...
        M_MODEL &group(topicset_info::cols field)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &topicset_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
        ...
        {
            model_meta_cache<topicset_info::meta> &data_cache = model_meta_cache<topicset_info::meta>::getinstance();
            data_cache.remove_exptime();

            model_meta_cache<std::vector<topicset_info::meta>> &record_cache = model_meta_cache<std::vector<topicset_info::meta>>::getinstance();
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = topicset_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        topicset_opsql(const topicset_opsql &)            = delete;
        topicset_opsql &operator=(const topicset_opsql &) = delete;
        topicset_opsql(topicset_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        topicset_opsql &operator=(topicset_opsql &&o) noexcept
        {
            if (this != &o)
            {
        ...
        template <typename T>
        M_MODEL &AND(orm::table_col<B_BASE, &topicset_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = opwq;
        ...
        template <typename T>
        M_MODEL &OR(orm::table_col<B_BASE, &topicset_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = opwq;
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < topicset_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == topicset_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(topicset_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(topicset_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = topicset_info::col_names.size();
