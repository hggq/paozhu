...
{ /*tagnamespace_replace*/
    ...
    class sysuser_mysql : public B_BASE
    {
      public:
        sysuser_mysql(const std::string &tag) : dbtag(tag)
        {
        ...
        sysuser_mysql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned int value_size, sysuser_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            data_temp.textword.resize(value_size);
            
            std::memcpy(data_temp.textword.data(), result_temp_data, value_size);
            break;
                case 4:
            data_temp.isopen=0;
            ...
            data_temp.level=0;
                    ...
                    {
                        decltype(data_temp.level) _tmp{};
                        auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            _tmp);
                        if (result.ec == std::errc()) {
                            data_temp.level = _tmp;
                        }
                    }
            break;
                case 6:
            data_temp.companyid=0;
                    ...
                    {
                        decltype(data_temp.companyid) _tmp{};
                        auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            _tmp);
                        if (result.ec == std::errc()) {
                            data_temp.companyid = _tmp;
                        }
                    }
            break;
                case 7:
            ...
            data_temp.jobid=0;
                    ...
                    {
                        decltype(data_temp.jobid) _tmp{};
                        auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            _tmp);
                        if (result.ec == std::errc()) {
                            data_temp.jobid = _tmp;
                        }
                    }
            break;
                case 9:
            data_temp.roleid=0;
                    ...
                    {
                        decltype(data_temp.roleid) _tmp{};
                        auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            _tmp);
                        if (result.ec == std::errc()) {
                            data_temp.roleid = _tmp;
                        }
                    }
            break;
                case 10:
            data_temp.postid=0;
                    ...
                    {
                        decltype(data_temp.postid) _tmp{};
                        auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            _tmp);
                        if (result.ec == std::errc()) {
                            data_temp.postid = _tmp;
                        }
                    }
            break;
                case 11:
            data_temp.created_at=0;
                    ...
                    {
                        decltype(data_temp.created_at) _tmp{};
                        auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            _tmp);
                        if (result.ec == std::errc()) {
                            data_temp.created_at = _tmp;
                        }
                    }
            break;
                case 12:
            data_temp.enddate=0;
                    ...
                    {
                        decltype(data_temp.enddate) _tmp{};
                        auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            _tmp);
                        if (result.ec == std::errc()) {
                            data_temp.enddate = _tmp;
                        }
                    }
            break;
                case 13:
            data_temp.qrtemp=0;
                    ...
                    {
                        decltype(data_temp.qrtemp) _tmp{};
                        auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            _tmp);
                        if (result.ec == std::errc()) {
                            data_temp.qrtemp = _tmp;
                        }
                    }
            break;
                case 14:
            data_temp.gender=0;
                    ...
                    {
                        decltype(data_temp.gender) _tmp{};
                        auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            _tmp);
                        if (result.ec == std::errc()) {
                            data_temp.gender = _tmp;
                        }
                    }
            break;
                case 15:
            ...
            data_temp.realname.resize(value_size);
            
            std::memcpy(data_temp.realname.data(), result_temp_data, value_size);
            break;
                case 17:
            data_temp.avatar.clear();
            data_temp.avatar.resize(value_size);
            
            std::memcpy(data_temp.avatar.data(), result_temp_data, value_size);
            break;
                case 18:
