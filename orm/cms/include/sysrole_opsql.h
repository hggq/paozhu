#ifndef _ORM_CMS_SYSROLE_OPERATE_H
#define _ORM_CMS_SYSROLE_OPERATE_H
/*
...
{ /*tagnamespace_replace*/
    ...
    class sysrole_opsql : public B_BASE
    {
      public:
        sysrole_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        sysrole_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, sysrole_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
        ...
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, sysrole_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
            ...
            case 4: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.rolecode = static_cast<decltype(data_temp.rolecode )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.rolecode = static_cast<decltype(data_temp.rolecode )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.rolecode = static_cast<decltype(data_temp.rolecode )>(v);
            ...
            case 6: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.rolevalue = static_cast<decltype(data_temp.rolevalue )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.rolevalue = static_cast<decltype(data_temp.rolevalue )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.rolevalue = static_cast<decltype(data_temp.rolevalue )>(v);
        ...
        http::obj_val get_field_value(unsigned char index_pos, const sysrole_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            case 3:
                ...
                break;
            case 4:
                return http::obj_val(static_cast<long long>(data_temp.rolecode));
                break;
            case 5:
                return http::obj_val(static_cast<long long>(data_temp.sortid));
                break;
            case 6:
                return http::obj_val(static_cast<long long>(data_temp.rolevalue));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const sysrole_info::meta &data_temp)
    {
        // Auto-increment primary key roleid == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.roleid == 0)
            return http::obj_val(nullptr);
...
M_MODEL& eqRolecode(const std::string &val)
	{return where(B_BASE::cols::rolecode, orm::wq::eq, val);
	}

M_MODEL& nqRolecode(const std::string &val)
	{return where(B_BASE::cols::rolecode, orm::wq::nq, val);
	}

M_MODEL& btRolecode(const std::string &val)
	{return where(B_BASE::cols::rolecode, orm::wq::bt, val);
	}

M_MODEL& beRolecode(const std::string &val)
	{return where(B_BASE::cols::rolecode, orm::wq::be, val);
	}

M_MODEL& ltRolecode(const std::string &val)
	{return where(B_BASE::cols::rolecode, orm::wq::lt, val);
	}

M_MODEL& leRolecode(const std::string &val)
	{return where(B_BASE::cols::rolecode, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqRolecode(T val)
	{return where(B_BASE::cols::rolecode, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqRolecode(T val)
	{return where(B_BASE::cols::rolecode, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btRolecode(T val)
	{return where(B_BASE::cols::rolecode, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beRolecode(T val)
	{return where(B_BASE::cols::rolecode, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltRolecode(T val)
	{return where(B_BASE::cols::rolecode, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leRolecode(T val)
	{return where(B_BASE::cols::rolecode, orm::wq::le, val);
	}

M_MODEL& nullRolecode()
	{return whereNull(B_BASE::cols::rolecode);
	}

M_MODEL& notnullRolecode()
	{return whereNotNull(B_BASE::cols::rolecode);
	}

M_MODEL& oreqRolecode(const std::string &val)
	{return whereOr(B_BASE::cols::rolecode, orm::wq::eq, val);
	}

M_MODEL& ornqRolecode(const std::string &val)
	{return whereOr(B_BASE::cols::rolecode, orm::wq::nq, val);
	}

M_MODEL& orbtRolecode(const std::string &val)
	{return whereOr(B_BASE::cols::rolecode, orm::wq::bt, val);
	}

M_MODEL& orbeRolecode(const std::string &val)
	{return whereOr(B_BASE::cols::rolecode, orm::wq::be, val);
	}

M_MODEL& orltRolecode(const std::string &val)
	{return whereOr(B_BASE::cols::rolecode, orm::wq::lt, val);
	}

M_MODEL& orleRolecode(const std::string &val)
	{return whereOr(B_BASE::cols::rolecode, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqRolecode(T val)
	{return whereOr(B_BASE::cols::rolecode, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqRolecode(T val)
	{return whereOr(B_BASE::cols::rolecode, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtRolecode(T val)
	{return whereOr(B_BASE::cols::rolecode, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeRolecode(T val)
	{return whereOr(B_BASE::cols::rolecode, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltRolecode(T val)
	{return whereOr(B_BASE::cols::rolecode, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleRolecode(T val)
	{return whereOr(B_BASE::cols::rolecode, orm::wq::le, val);
	}

M_MODEL& ornullRolecode()
	{return whereOrNull(B_BASE::cols::rolecode);
	}

M_MODEL& ornotnullRolecode()
	{return whereOrNotNull(B_BASE::cols::rolecode);
	}
...
M_MODEL& eqRolevalue(const std::string &val)
	{return where(B_BASE::cols::rolevalue, orm::wq::eq, val);
	}

M_MODEL& nqRolevalue(const std::string &val)
	{return where(B_BASE::cols::rolevalue, orm::wq::nq, val);
	}

M_MODEL& btRolevalue(const std::string &val)
	{return where(B_BASE::cols::rolevalue, orm::wq::bt, val);
	}

M_MODEL& beRolevalue(const std::string &val)
	{return where(B_BASE::cols::rolevalue, orm::wq::be, val);
	}

M_MODEL& ltRolevalue(const std::string &val)
	{return where(B_BASE::cols::rolevalue, orm::wq::lt, val);
	}

M_MODEL& leRolevalue(const std::string &val)
	{return where(B_BASE::cols::rolevalue, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqRolevalue(T val)
	{return where(B_BASE::cols::rolevalue, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqRolevalue(T val)
	{return where(B_BASE::cols::rolevalue, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btRolevalue(T val)
	{return where(B_BASE::cols::rolevalue, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beRolevalue(T val)
	{return where(B_BASE::cols::rolevalue, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltRolevalue(T val)
	{return where(B_BASE::cols::rolevalue, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leRolevalue(T val)
	{return where(B_BASE::cols::rolevalue, orm::wq::le, val);
	}

M_MODEL& nullRolevalue()
	{return whereNull(B_BASE::cols::rolevalue);
	}

M_MODEL& notnullRolevalue()
	{return whereNotNull(B_BASE::cols::rolevalue);
	}

M_MODEL& oreqRolevalue(const std::string &val)
	{return whereOr(B_BASE::cols::rolevalue, orm::wq::eq, val);
	}

M_MODEL& ornqRolevalue(const std::string &val)
	{return whereOr(B_BASE::cols::rolevalue, orm::wq::nq, val);
	}

M_MODEL& orbtRolevalue(const std::string &val)
	{return whereOr(B_BASE::cols::rolevalue, orm::wq::bt, val);
	}

M_MODEL& orbeRolevalue(const std::string &val)
	{return whereOr(B_BASE::cols::rolevalue, orm::wq::be, val);
	}

M_MODEL& orltRolevalue(const std::string &val)
	{return whereOr(B_BASE::cols::rolevalue, orm::wq::lt, val);
	}

M_MODEL& orleRolevalue(const std::string &val)
	{return whereOr(B_BASE::cols::rolevalue, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqRolevalue(T val)
	{return whereOr(B_BASE::cols::rolevalue, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqRolevalue(T val)
	{return whereOr(B_BASE::cols::rolevalue, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtRolevalue(T val)
	{return whereOr(B_BASE::cols::rolevalue, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeRolevalue(T val)
	{return whereOr(B_BASE::cols::rolevalue, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltRolevalue(T val)
	{return whereOr(B_BASE::cols::rolevalue, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleRolevalue(T val)
	{return whereOr(B_BASE::cols::rolevalue, orm::wq::le, val);
	}

M_MODEL& ornullRolevalue()
	{return whereOrNull(B_BASE::cols::rolevalue);
	}

M_MODEL& ornotnullRolevalue()
	{return whereOrNotNull(B_BASE::cols::rolevalue);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = sysrole_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &sysrole_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &sysrole_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(sysrole_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(sysrole_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(sysrole_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(sysrole_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &sysrole_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &sysrole_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &sysrole_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &sysrole_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &sysrole_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(sysrole_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &sysrole_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(sysrole_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &sysrole_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(sysrole_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &sysrole_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(sysrole_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &sysrole_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(sysrole_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(sysrole_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(sysrole_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(sysrole_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(sysrole_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(sysrole_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(sysrole_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(sysrole_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(sysrole_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(sysrole_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &sysrole_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &sysrole_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &order(sysrole_info::cols field, const std::string &asc_or_desc)
        {
        ...
        M_MODEL &asc(sysrole_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &sysrole_info::col_names> field1, orm::table_col<B_BASE, &sysrole_info::col_names> field2)
        {
        ...
        M_MODEL &asc(sysrole_info::cols field, sysrole_info::cols field2)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(sysrole_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(sysrole_info::cols field, sysrole_info::cols field2)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &sysrole_info::col_names> field1, orm::table_col<B_BASE, &sysrole_info::col_names> field2)
        {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &sysrole_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &sysrole_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &sysrole_info::col_names> wq)
        {
        ...
        M_MODEL &having(sysrole_info::cols field)
        {
            groupsql.append(" HAVING ");
            switch (field)
            {
			...
			case sysrole_info::cols::status:
				...
				break;
			case sysrole_info::cols::rolecode:
				groupsql.append("rolecode");
				break;
			case sysrole_info::cols::sortid:
				groupsql.append("sortid");
				break;
			case sysrole_info::cols::rolevalue:
				groupsql.append("rolevalue");
				break;
            default:
                return *mod;
                break;
        ...
        M_MODEL &group(sysrole_info::cols field)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &sysrole_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
        ...
        {
            model_meta_cache<sysrole_info::meta> &data_cache = model_meta_cache<sysrole_info::meta>::getinstance();
            data_cache.remove_exptime();

            model_meta_cache<std::vector<sysrole_info::meta>> &record_cache = model_meta_cache<std::vector<sysrole_info::meta>>::getinstance();
        ...
        bool save_data_cache(const std::string &cache_key_name, const sysrole_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<sysrole_info::meta> &temp_cache = model_meta_cache<sysrole_info::meta>::getinstance();
        ...
        bool save_cache(std::size_t cache_key_name, const std::vector<sysrole_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<sysrole_info::meta>> &temp_cache = model_meta_cache<std::vector<sysrole_info::meta>>::getinstance();
            temp_cache.save(cache_key_name, cache_data, exp_time);
            return true;
        }
        bool save_cache(const std::string cache_key_name, const std::vector<sysrole_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<sysrole_info::meta>> &temp_cache = model_meta_cache<std::vector<sysrole_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_vector_cache(const std::string cache_key_name, const std::vector<sysrole_info::meta> &cache_data, int exp_time = 0)
        {
            model_meta_cache<std::vector<sysrole_info::meta>> &temp_cache = model_meta_cache<std::vector<sysrole_info::meta>>::getinstance();
            std::size_t sqlhashid                                            = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        bool save_cache(const std::string cache_key_name, const sysrole_info::meta &cache_data, int exp_time = 0)
        {
            model_meta_cache<sysrole_info::meta> &temp_cache = model_meta_cache<sysrole_info::meta>::getinstance();
            std::size_t sqlhashid                               = std::hash<std::string>{}(cache_key_name);
            temp_cache.save(sqlhashid, cache_data, exp_time);
        ...
        std::tuple<unsigned int, unsigned long long> insert(sysrole_info::meta &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(sysrole_info::meta &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_data_insert_sql(insert_data);
        ...
        std::tuple<unsigned int, unsigned long long> insert(std::vector<sysrole_info::meta> &insert_data)
        {
            effect_num = 0;
        ...
        asio::awaitable<std::tuple<unsigned int, unsigned long long>> async_insert(std::vector<sysrole_info::meta> &insert_data)
        {
            effect_num = 0;
            sqlstring  = B_BASE::make_vector_insert_sql(insert_data);
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = sysrole_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        M_MODEL &set_data(sysrole_info::meta indata)
        {
            B_BASE::data = indata;
            return *mod;
        }
        ...
        sysrole_opsql(const sysrole_opsql &)            = delete;
        sysrole_opsql &operator=(const sysrole_opsql &) = delete;
        sysrole_opsql(sysrole_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        sysrole_opsql &operator=(sysrole_opsql &&o) noexcept
        {
            if (this != &o)
            {
        ...
        template <typename T>
        M_MODEL &AND(orm::table_col<B_BASE, &sysrole_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = opwq;
        ...
        template <typename T>
        M_MODEL &OR(orm::table_col<B_BASE, &sysrole_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = opwq;
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < sysrole_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == sysrole_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(sysrole_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(sysrole_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = sysrole_info::col_names.size();
