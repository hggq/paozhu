#ifndef _ORM_CMS_TESTA_OPERATE_H
#define _ORM_CMS_TESTA_OPERATE_H
/*
...
{ /*tagnamespace_replace*/
    ...
    class testa_opsql : public B_BASE
    {
      public:
        testa_opsql(const std::string &tag) : dbtag(tag)
        {
        ...
        testa_opsql() : dbtag(B_BASE::_rmstag)
        {
            std::map<std::string, std::shared_ptr<orm_conn_pool>> &conn_pool_obj = get_orm_conn_pool_obj();
            auto iter                                                            = conn_pool_obj.find(dbtag);
            if (iter != conn_pool_obj.end())
        ...
        void assign_field_value(unsigned char index_pos, unsigned char *result_temp_data, unsigned long long value_size, testa_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            break;
                case 5:
             {
               data_temp.deletetime = 0;
            
                    auto result = std::from_chars(
                            reinterpret_cast<const char*>(result_temp_data),
                            reinterpret_cast<const char*>(result_temp_data) + value_size,
                            data_temp.deletetime);
                        if (result.ec == std::errc()) {
        ...
        void assign_field_value_binary(unsigned char index_pos, col_value_variant val, testa_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0: {
            ...
            case 2: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.value_id = static_cast<decltype(data_temp.value_id )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.value_id = static_cast<decltype(data_temp.value_id )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.value_id = static_cast<decltype(data_temp.value_id )>(v);
            ...
            case 4: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.deleted = static_cast<decltype(data_temp.deleted )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.deleted = static_cast<decltype(data_temp.deleted )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.deleted = static_cast<decltype(data_temp.deleted )>(v);
            ...
            case 5: {
                std::visit([&](auto&& v){
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        data_temp.deletetime = static_cast<decltype(data_temp.deletetime )>(v);
                    else if constexpr (std::is_same_v<T, uint64_t>)
                        data_temp.deletetime = static_cast<decltype(data_temp.deletetime )>(v);
                    else if constexpr (std::is_same_v<T, double>)
                        data_temp.deletetime = static_cast<decltype(data_temp.deletetime )>(v);
        ...
        http::obj_val get_field_value(unsigned char index_pos, const testa_info::meta &data_temp)
    {
        switch(index_pos)
        {
            case 0:
            ...
            case 1:
                ...
                break;
            case 2:
                return http::obj_val(static_cast<long long>(data_temp.value_id));
                break;
            case 3:
                return http::obj_val(std::string(data_temp.content));
                break;
            case 4:
                return http::obj_val(static_cast<long long>(data_temp.deleted));
                break;
            case 5:
                return http::obj_val(static_cast<long long>(data_temp.deletetime));
                break;
            default:
        ...
        http::obj_val get_insert_field_value(unsigned char index_pos, const testa_info::meta &data_temp)
    {
        // Auto-increment primary key id == 0 bind NULL at runtime, and let the database automatically generate the primary key
        if (index_pos == 0 && data_temp.id == 0)
            return http::obj_val(nullptr);
...
M_MODEL& eqValueId(const std::string &val)
	{return where(B_BASE::cols::value_id, orm::wq::eq, val);
	}

M_MODEL& nqValueId(const std::string &val)
	{return where(B_BASE::cols::value_id, orm::wq::nq, val);
	}

M_MODEL& btValueId(const std::string &val)
	{return where(B_BASE::cols::value_id, orm::wq::bt, val);
	}

M_MODEL& beValueId(const std::string &val)
	{return where(B_BASE::cols::value_id, orm::wq::be, val);
	}

M_MODEL& ltValueId(const std::string &val)
	{return where(B_BASE::cols::value_id, orm::wq::lt, val);
	}

M_MODEL& leValueId(const std::string &val)
	{return where(B_BASE::cols::value_id, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqValueId(T val)
	{return where(B_BASE::cols::value_id, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqValueId(T val)
	{return where(B_BASE::cols::value_id, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btValueId(T val)
	{return where(B_BASE::cols::value_id, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beValueId(T val)
	{return where(B_BASE::cols::value_id, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltValueId(T val)
	{return where(B_BASE::cols::value_id, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leValueId(T val)
	{return where(B_BASE::cols::value_id, orm::wq::le, val);
	}

M_MODEL& nullValueId()
	{return whereNull(B_BASE::cols::value_id);
	}

M_MODEL& notnullValueId()
	{return whereNotNull(B_BASE::cols::value_id);
	}

M_MODEL& oreqValueId(const std::string &val)
	{return whereOr(B_BASE::cols::value_id, orm::wq::eq, val);
	}

M_MODEL& ornqValueId(const std::string &val)
	{return whereOr(B_BASE::cols::value_id, orm::wq::nq, val);
	}

M_MODEL& orbtValueId(const std::string &val)
	{return whereOr(B_BASE::cols::value_id, orm::wq::bt, val);
	}

M_MODEL& orbeValueId(const std::string &val)
	{return whereOr(B_BASE::cols::value_id, orm::wq::be, val);
	}

M_MODEL& orltValueId(const std::string &val)
	{return whereOr(B_BASE::cols::value_id, orm::wq::lt, val);
	}

M_MODEL& orleValueId(const std::string &val)
	{return whereOr(B_BASE::cols::value_id, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqValueId(T val)
	{return whereOr(B_BASE::cols::value_id, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqValueId(T val)
	{return whereOr(B_BASE::cols::value_id, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtValueId(T val)
	{return whereOr(B_BASE::cols::value_id, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeValueId(T val)
	{return whereOr(B_BASE::cols::value_id, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltValueId(T val)
	{return whereOr(B_BASE::cols::value_id, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleValueId(T val)
	{return whereOr(B_BASE::cols::value_id, orm::wq::le, val);
	}

M_MODEL& ornullValueId()
	{return whereOrNull(B_BASE::cols::value_id);
	}

M_MODEL& ornotnullValueId()
	{return whereOrNotNull(B_BASE::cols::value_id);
	}
...
M_MODEL& eqDeleted(const std::string &val)
	{return where(B_BASE::cols::deleted, orm::wq::eq, val);
	}

M_MODEL& nqDeleted(const std::string &val)
	{return where(B_BASE::cols::deleted, orm::wq::nq, val);
	}

M_MODEL& btDeleted(const std::string &val)
	{return where(B_BASE::cols::deleted, orm::wq::bt, val);
	}

M_MODEL& beDeleted(const std::string &val)
	{return where(B_BASE::cols::deleted, orm::wq::be, val);
	}

M_MODEL& ltDeleted(const std::string &val)
	{return where(B_BASE::cols::deleted, orm::wq::lt, val);
	}

M_MODEL& leDeleted(const std::string &val)
	{return where(B_BASE::cols::deleted, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqDeleted(T val)
	{return where(B_BASE::cols::deleted, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqDeleted(T val)
	{return where(B_BASE::cols::deleted, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btDeleted(T val)
	{return where(B_BASE::cols::deleted, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beDeleted(T val)
	{return where(B_BASE::cols::deleted, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltDeleted(T val)
	{return where(B_BASE::cols::deleted, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leDeleted(T val)
	{return where(B_BASE::cols::deleted, orm::wq::le, val);
	}

M_MODEL& nullDeleted()
	{return whereNull(B_BASE::cols::deleted);
	}

M_MODEL& notnullDeleted()
	{return whereNotNull(B_BASE::cols::deleted);
	}

M_MODEL& oreqDeleted(const std::string &val)
	{return whereOr(B_BASE::cols::deleted, orm::wq::eq, val);
	}

M_MODEL& ornqDeleted(const std::string &val)
	{return whereOr(B_BASE::cols::deleted, orm::wq::nq, val);
	}

M_MODEL& orbtDeleted(const std::string &val)
	{return whereOr(B_BASE::cols::deleted, orm::wq::bt, val);
	}

M_MODEL& orbeDeleted(const std::string &val)
	{return whereOr(B_BASE::cols::deleted, orm::wq::be, val);
	}

M_MODEL& orltDeleted(const std::string &val)
	{return whereOr(B_BASE::cols::deleted, orm::wq::lt, val);
	}

M_MODEL& orleDeleted(const std::string &val)
	{return whereOr(B_BASE::cols::deleted, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqDeleted(T val)
	{return whereOr(B_BASE::cols::deleted, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqDeleted(T val)
	{return whereOr(B_BASE::cols::deleted, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtDeleted(T val)
	{return whereOr(B_BASE::cols::deleted, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeDeleted(T val)
	{return whereOr(B_BASE::cols::deleted, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltDeleted(T val)
	{return whereOr(B_BASE::cols::deleted, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleDeleted(T val)
	{return whereOr(B_BASE::cols::deleted, orm::wq::le, val);
	}

M_MODEL& ornullDeleted()
	{return whereOrNull(B_BASE::cols::deleted);
	}

M_MODEL& ornotnullDeleted()
	{return whereOrNotNull(B_BASE::cols::deleted);
	}

M_MODEL& eqDeletetime(const std::string &val)
	{return where(B_BASE::cols::deletetime, orm::wq::eq, val);
	}

M_MODEL& nqDeletetime(const std::string &val)
	{return where(B_BASE::cols::deletetime, orm::wq::nq, val);
	}

M_MODEL& btDeletetime(const std::string &val)
	{return where(B_BASE::cols::deletetime, orm::wq::bt, val);
	}

M_MODEL& beDeletetime(const std::string &val)
	{return where(B_BASE::cols::deletetime, orm::wq::be, val);
	}

M_MODEL& ltDeletetime(const std::string &val)
	{return where(B_BASE::cols::deletetime, orm::wq::lt, val);
	}

M_MODEL& leDeletetime(const std::string &val)
	{return where(B_BASE::cols::deletetime, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& eqDeletetime(T val)
	{return where(B_BASE::cols::deletetime, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& nqDeletetime(T val)
	{return where(B_BASE::cols::deletetime, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& btDeletetime(T val)
	{return where(B_BASE::cols::deletetime, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& beDeletetime(T val)
	{return where(B_BASE::cols::deletetime, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ltDeletetime(T val)
	{return where(B_BASE::cols::deletetime, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& leDeletetime(T val)
	{return where(B_BASE::cols::deletetime, orm::wq::le, val);
	}

M_MODEL& nullDeletetime()
	{return whereNull(B_BASE::cols::deletetime);
	}

M_MODEL& notnullDeletetime()
	{return whereNotNull(B_BASE::cols::deletetime);
	}

M_MODEL& oreqDeletetime(const std::string &val)
	{return whereOr(B_BASE::cols::deletetime, orm::wq::eq, val);
	}

M_MODEL& ornqDeletetime(const std::string &val)
	{return whereOr(B_BASE::cols::deletetime, orm::wq::nq, val);
	}

M_MODEL& orbtDeletetime(const std::string &val)
	{return whereOr(B_BASE::cols::deletetime, orm::wq::bt, val);
	}

M_MODEL& orbeDeletetime(const std::string &val)
	{return whereOr(B_BASE::cols::deletetime, orm::wq::be, val);
	}

M_MODEL& orltDeletetime(const std::string &val)
	{return whereOr(B_BASE::cols::deletetime, orm::wq::lt, val);
	}

M_MODEL& orleDeletetime(const std::string &val)
	{return whereOr(B_BASE::cols::deletetime, orm::wq::le, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& oreqDeletetime(T val)
	{return whereOr(B_BASE::cols::deletetime, orm::wq::eq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& ornqDeletetime(T val)
	{return whereOr(B_BASE::cols::deletetime, orm::wq::nq, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbtDeletetime(T val)
	{return whereOr(B_BASE::cols::deletetime, orm::wq::bt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orbeDeletetime(T val)
	{return whereOr(B_BASE::cols::deletetime, orm::wq::be, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orltDeletetime(T val)
	{return whereOr(B_BASE::cols::deletetime, orm::wq::lt, val);
	}

template <typename T>
	requires std::is_integral_v<T>
M_MODEL& orleDeletetime(T val)
	{return whereOr(B_BASE::cols::deletetime, orm::wq::le, val);
	}

M_MODEL& ornullDeletetime()
	{return whereOrNull(B_BASE::cols::deletetime);
	}

M_MODEL& ornotnullDeletetime()
	{return whereOrNotNull(B_BASE::cols::deletetime);
	}
        ...
        M_MODEL &where(B_BASE::cols field, orm::wq op, T &&val)
        {
            ...
            item.filed_name  = testa_info::col_names[item.col_idx];
        ...
        template <typename T>
        M_MODEL &where(orm::table_col<B_BASE, &testa_info::col_names> wq, T &&val)
        {
        ...
        template <typename T>
        M_MODEL &whereOr(orm::table_col<B_BASE, &testa_info::col_names> wq, T &&val)
        {
        ...
        M_MODEL &whereIn(testa_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereNotIn(testa_info::cols field, const std::vector<T2> &a)
        {
            return whereIn(field, orm::wq::notin, a);
        }

        template <typename T2>
        M_MODEL &whereOrIn(testa_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::in, a);
        }

        template <typename T2>
        M_MODEL &whereOrNotIn(testa_info::cols field, const std::vector<T2> &a)
        {
            return whereOrIn(field, orm::wq::notin, a);
        }

        M_MODEL &whereIn(orm::table_col<B_BASE, &testa_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::in, a);
        }
        // 2 参数旧版兼容: whereIn("id", "1,2,3") = whereIn("id", split_csv("1,2,3"))
        M_MODEL &whereIn(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view csv_val)
        {
        ...
        template <typename T>
        M_MODEL &whereIn(orm::table_col<B_BASE, &testa_info::col_names> wq, const std::vector<T> &a)
        {
        ...
        M_MODEL &whereNotIn(orm::table_col<B_BASE, &testa_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereIn(wq, orm::wq::notin, a);
        }

        M_MODEL &whereOrIn(orm::table_col<B_BASE, &testa_info::col_names> wq, const std::vector<std::string> &a)
        {
            return whereOrIn(wq, orm::wq::in, a);
        }

        M_MODEL &whereOrNotIn(orm::table_col<B_BASE, &testa_info::col_names> wq, const std::vector<std::string> &a)
        {
        ...
        M_MODEL &whereNull(testa_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNull(orm::table_col<B_BASE, &testa_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(testa_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::isnull;
        ...
        M_MODEL &whereOrNull(orm::table_col<B_BASE, &testa_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::isnull;
        ...
        M_MODEL &whereNotNull(testa_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 1;
        ...
        M_MODEL &whereNotNull(orm::table_col<B_BASE, &testa_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(testa_info::cols field)
        {
            orm_where_sql_t item;
            item.pre_op     = wheresql.empty() ? 0 : 2;
            item.op_type    = orm::wq::notnull;
        ...
        M_MODEL &whereOrNotNull(orm::table_col<B_BASE, &testa_info::col_names> wq)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = orm::wq::notnull;
        ...
        template <typename T2>
        M_MODEL &whereEQ(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereEQ(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereEQ(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereAnd(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::eq, std::forward<T2>(value));
        }

        M_MODEL &whereAnd(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereAnd(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::eq, val);
        }

        template <typename T2>
        M_MODEL &whereBT(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereBT(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBT(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereGT(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereGT(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGT(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereBE(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereBE(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereBE(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereGE(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereGE(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereGE(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereLT(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereLT(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLT(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereLE(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereLE(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLE(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereNQ(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNQ(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNQ(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereNE(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereNE(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNE(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereLike(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereLike(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLike(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereLikeLeft(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeLeft(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereLikeRight(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereLikeRight(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereNotLike(testa_info::cols field, T2 &&value)
        {
            return where(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereNotLike(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereNotLike(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return where(wq, orm::wq::nlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrBT(testa_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::bt, std::forward<T2>(value));
        }

        M_MODEL &whereOrBT(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBT(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::bt, val);
        }

        template <typename T2>
        M_MODEL &whereOrBE(testa_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::be, std::forward<T2>(value));
        }

        M_MODEL &whereOrBE(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrBE(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::be, val);
        }

        template <typename T2>
        M_MODEL &whereOrLT(testa_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::lt, std::forward<T2>(value));
        }

        M_MODEL &whereOrLT(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLT(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::lt, val);
        }

        template <typename T2>
        M_MODEL &whereOrLE(testa_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::le, std::forward<T2>(value));
        }

        M_MODEL &whereOrLE(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLE(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::le, val);
        }

        template <typename T2>
        M_MODEL &whereOrNQ(testa_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nq, std::forward<T2>(value));
        }

        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNQ(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nq, val);
        }

        template <typename T2>
        M_MODEL &whereOrLike(testa_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::like, std::forward<T2>(value));
        }

        M_MODEL &whereOrLike(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLike(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::like, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeLeft(testa_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::llike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeLeft(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::llike, val);
        }

        template <typename T2>
        M_MODEL &whereOrLikeRight(testa_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::rlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrLikeRight(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::rlike, val);
        }

        template <typename T2>
        M_MODEL &whereOrNotLike(testa_info::cols field, T2 &&value)
        {
            return whereOr(field, orm::wq::nlike, std::forward<T2>(value));
        }

        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &testa_info::col_names> wq, std::string_view val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }

        template <typename _SQL_Value>
        M_MODEL &whereOrNotLike(orm::table_col<B_BASE, &testa_info::col_names> wq, _SQL_Value val)
        {
            return whereOr(wq, orm::wq::nlike, val);
        }
        ...
        M_MODEL &order(testa_info::cols field, const std::string &asc_or_desc)
        {
        ...
        M_MODEL &asc(testa_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &testa_info::col_names> field1, orm::table_col<B_BASE, &testa_info::col_names> field2)
        {
        ...
        M_MODEL &asc(testa_info::cols field, testa_info::cols field2)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(testa_info::cols field)
        {
            ordersql.append(" ORDER BY ");
            switch (field)
            {
        ...
        M_MODEL &desc(testa_info::cols field, testa_info::cols field2)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &testa_info::col_names> field1, orm::table_col<B_BASE, &testa_info::col_names> field2)
        {
        ...
        M_MODEL &order(orm::table_col<B_BASE, &testa_info::col_names> wq, const std::string &asc_or_desc)
        {
            ordersql.append(" ORDER BY ");
            ordersql.append(wq);
            ordersql.append(" ");
        ...
        M_MODEL &asc(orm::table_col<B_BASE, &testa_info::col_names> wq)
        {
        ...
        M_MODEL &desc(orm::table_col<B_BASE, &testa_info::col_names> wq)
        {
        ...
        M_MODEL &having(orm::table_col<B_BASE, &testa_info::col_names> wq)
        {
        ...
        M_MODEL &having(testa_info::cols field)
        {
            groupsql.append(" HAVING ");
            switch (field)
            {
			...
			case testa_info::cols::parentid:
				...
				break;
			case testa_info::cols::value_id:
				groupsql.append("value_id");
				break;
			case testa_info::cols::content:
				groupsql.append("content");
				break;
			case testa_info::cols::deleted:
				groupsql.append("deleted");
				break;
			case testa_info::cols::deletetime:
				groupsql.append("deletetime");
				break;
            default:
                return *mod;
                break;
        ...
        M_MODEL &group(testa_info::cols field)
        {
        ...
        M_MODEL &group(orm::table_col<B_BASE, &testa_info::col_names> wq)
        {
            groupsql.append(" GROUP BY ");
            groupsql.append(wq);
            return *mod;
        ...
        {
            model_meta_cache<testa_info::meta> &data_cache = model_meta_cache<testa_info::meta>::getinstance();
            data_cache.remove_exptime();

            model_meta_cache<std::vector<testa_info::meta>> &record_cache = model_meta_cache<std::vector<testa_info::meta>>::getinstance();
        ...
        M_MODEL &joinOn(std::string_view join_field, B_BASE::cols main_field)
        {
            ...
            item.left_filed_name  = testa_info::col_names[item.left_idx];
            item.right_filed_name = join_field;
            join_ptr->on_sqls.push_back(std::move(item));
        ...
        std::string commit_insert(testa_info::meta &insert_data)
        {
        ...
        testa_opsql(const testa_opsql &)            = delete;
        testa_opsql &operator=(const testa_opsql &) = delete;
        testa_opsql(testa_opsql &&o) noexcept : B_BASE(std::move(o))
        {
        ...
        testa_opsql &operator=(testa_opsql &&o) noexcept
        {
            if (this != &o)
            {
        ...
        template <typename T>
        M_MODEL &AND(orm::table_col<B_BASE, &testa_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 1;
            item.op_type = opwq;
        ...
        template <typename T>
        M_MODEL &OR(orm::table_col<B_BASE, &testa_info::col_names> field, orm::wq opwq, T val)
        {
            orm_where_sql_t item;
            item.pre_op  = wheresql.empty() ? 0 : 2;
            item.op_type = opwq;
      ...
      public:
        ...
        unsigned int exec_update()
        {
            ...
            std::vector<http::obj_val> params;
            bool first = true;
            for (unsigned char i = 0; i < testa_info::col_names.size(); i++)
            {
                // 跳过自增主键：全字段更新不修改身份列（“自增主键跳过”约定），避免把主键误改为 0
                if (static_cast<int>(i) == testa_info::auto_pk_index)
                    continue;
                if (!first)
                    sql.append(", ");
                sql.append(testa_info::col_names[i]);
                sql.append(" = ?");
        ...
        unsigned int exec_update_dirty()
        {
            ...
            for (auto idx : dirty_indices)
            {
                if (!first)
                    sql.append(", ");
                sql.append(testa_info::col_names[idx]);
                sql.append(" = ?");
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert_batch()
        {
            ...
            const unsigned char ncols = testa_info::col_names.size();
