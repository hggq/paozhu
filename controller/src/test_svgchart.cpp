...
std::string test_svgchart(std::shared_ptr<httppeer> peer)
{
    ...
    // ============================================================
    {
        ...
        }, "Dataset A", SvgColor(), 7);
        chart.addSeries({
            {8.0, 6.5}, {10.0, 8.0}, {12.0, 9.5}, {6.0, 5.5},
            {14.0, 11.0}, {4.0, 3.5}, {9.0, 7.0}, {11.0, 8.5}
        }, "Dataset B", SvgColor(), 7);
    ...
    // ============================================================
    {
        ...
        chart.setXLabels({"07/14", "07/15", "07/16", "07/17", "07/18",
    ...
    // ============================================================
    {
        ...
        chart.setCell(0, 0, "Revenue",
                      "12.5M\n+8.3% YoY",
                      SvgColor(230, 245, 240), SvgColor(30, 80, 50),
                      SvgColor(60, 140, 80), 14, 11);
        chart.setCell(0, 1, "Cost",
                      "8.2M\n-2.1% YoY",
                      SvgColor(245, 235, 230), SvgColor(140, 60, 30),
                      SvgColor(180, 80, 40), 14, 11);
        chart.setCell(0, 2, "Profit",
                      "4.3M\n+15.6% YoY",
                      SvgColor(230, 240, 255), SvgColor(30, 50, 140),
                      SvgColor(50, 100, 200), 14, 11);

        chart.setCell(1, 0, "Users",
                      "48.2K\n+12.0%",
                      SvgColor(240, 245, 230), SvgColor(80, 100, 30),
                      SvgColor(120, 150, 40), 14, 11);
        chart.setCell(1, 1, "DAU",
                      "12.8K\n+5.3%",
                      SvgColor(240, 230, 245), SvgColor(100, 30, 120),
                      SvgColor(150, 50, 180), 14, 11);
        chart.setCell(1, 2, "Retention",
                      "72.5%\n+1.2%",
                      SvgColor(235, 245, 245), SvgColor(20, 100, 100),
                      SvgColor(30, 140, 140), 14, 11);

        chart.setCell(2, 0, "NPS",
                      "68\n+3 pts",
                      SvgColor(245, 240, 230), SvgColor(120, 90, 20),
                      SvgColor(180, 130, 30), 14, 11);
        chart.setCell(2, 1, "CSAT",
                      "4.8/5.0\n+0.2",
                      SvgColor(255, 235, 240), SvgColor(150, 30, 50),
                      SvgColor(200, 40, 70), 14, 11);
        chart.setCell(2, 2, "Churn",
                      "3.2%\n-0.5%",
                      SvgColor(245, 245, 240), SvgColor(100, 60, 10),
                      SvgColor(150, 90, 20), 14, 11);
...
std::string test_svgstats(std::shared_ptr<httppeer> peer)
{
    ...
    // ---- 示例 1: 线性回归 y = ax + b ----
    {
        ...
        std::vector<std::vector<double>> data = {
            {50, 150}, {70, 210}, {90, 270}, {110, 330}, {130, 390}
        };
    ...
    // ---- 示例 2: 指数回归 y = a * e^(bx) ----
    {
        ...
        oss << std::fixed << std::setprecision(4);
        std::vector<std::vector<double>> data = {
            {1, 4862}, {2, 5294}, {3, 5934}, {4, 7171}, {5, 8964},
            {6, 10202}, {7, 11962}, {8, 14928}, {9, 16909}, {10, 18547},
            {11, 21617}, {12, 26638}, {13, 34634}, {14, 46759}, {15, 58478},
            {16, 67884}, {17, 74462}, {18, 79395}
        };
    ...
    // ---- 示例 3: 对数回归 y = a + b*ln(x) ----
    {
        ...
        oss << std::fixed << std::setprecision(4);
        std::vector<std::vector<double>> data = {
            {1, 5}, {2, 12}, {3, 17}, {5, 22}, {10, 29}, {20, 35}, {50, 42}
        };
    ...
    // ---- 示例 4: 多项式回归 y = a0 + a1*x + a2*x^2 + ... ----
    {
        ...
        oss << std::fixed << std::setprecision(4);
        std::vector<std::vector<double>> data = {
            {0, 0}, {1, 1.2}, {2, 3.8}, {3, 8.9}, {4, 16.1}, {5, 25.0}
        };
    ...
    // ---- 示例 5: 过原点线性回归 y = ax ----
    {
        ...
        oss << std::fixed << std::setprecision(4);
        std::vector<std::vector<double>> data = {
            {1, 3.1}, {2, 5.9}, {3, 9.2}, {4, 11.8}, {5, 15.2}
        };
    ...
    // ---- 示例 6: 多维指定列 ----
    {
        ...
        oss << std::fixed << std::setprecision(4);
        std::vector<std::vector<double>> data = {
            {1, 0, 3}, {2, 0, 5}, {3, 0, 7}, {4, 0, 9}, {5, 0, 11}
        };
        LinearRegression::Config config;
    ...
    // ---- 示例 1: 客户分群 ----
    {
        ...
        std::vector<std::vector<double>> data = {
            {2, 1}, {3, 2}, {2.5, 1},       // 低价值客户
            {15, 8}, {18, 10}, {16, 9},      // 中等客户
            {50, 25}, {55, 30}, {48, 28}     // 高价值客户
        };
    ...
    // ---- 示例 2: 二维散点聚类 + 每个点的分配 ----
    {
        ...
        oss << std::fixed << std::setprecision(4);
        std::vector<std::vector<double>> data = {
            {0.1, 0.2}, {0.2, 0.1}, {0.3, 0.3}, {0.1, 0.4}, {0.5, 0.2},
            {9.8, 9.9}, {9.9, 9.8}, {10.1, 10.0}, {10.0, 10.2}, {9.7, 10.1}
        };
    ...
    // ---- 示例 3: 多维数据聚类 + 指定维度 ----
    {
        ...
        oss << std::fixed << std::setprecision(4);
        std::vector<std::vector<double>> data = {
            {1.0, 2.0, 100}, {1.1, 2.1, 101},
            {10.0, 20.0, 200}, {10.1, 20.1, 201}
        };
    ...
    // ---- 示例 4: 每个点的距离信息 ----
    {
        ...
        oss << std::fixed << std::setprecision(4);
        std::vector<std::vector<double>> data = {
            {0.1, 0.2}, {1.0, 1.0}, {0.3, 0.1},
            {9.8, 9.9}, {9.9, 9.8}, {10.0, 10.0}
        };
        auto result = ClusteringProcess::simpleKMeans(data, 2);
    ...
    client << "<h3>LinearRegression 单元测试</h3>";
    {
        ...
        // Test 1: Simple linear regression y = 2x + 1
        {
            ...
            auto result = LinearRegression::linear(data);
            bool pass = std::abs(result.parameters.gradient - 2.0) < 0.01
                     && std::abs(result.parameters.intercept - 1.0) < 0.01;
        ...
        // Test 3: Exponential regression y = 2 * e^(0.5*x)
        {
            ...
            auto result = LinearRegression::exponential(data);
            bool pass = std::abs(result.parameters.gradient - 0.5) < 0.05
                     && std::abs(result.parameters.intercept - 2.0) < 0.5;
        ...
        // Test 4: Logarithmic regression y = 1 + 2*ln(x)
        {
            ...
            auto result = LinearRegression::logarithmic(data);
            bool pass = std::abs(result.parameters.intercept - 1.0) < 0.01
                     && std::abs(result.parameters.gradient - 2.0) < 0.01;
        ...
        // Test 5: Polynomial regression y = x^2
        {
            ...
            auto result = LinearRegression::polynomial(data, 2);
            bool pass = result.parameters.coefficients.size() == 3
                     && std::abs(result.parameters.coefficients[2] - 1.0) < 0.01;
        ...
        {
            std::vector<std::vector<double>> data = {
                {1, 4862.4}, {2, 5294.7}, {3, 5934.5}, {4, 7171.0},
                {5, 8964.4}, {6, 10202.2}, {7, 11962.5}, {8, 14928.3},
                {9, 16909.2}, {10, 18547.9}, {11, 21617.8}, {12, 26638.1},
                {13, 34634.4}, {14, 46759.4}, {15, 58478.1}, {16, 67884.6},
    ...
    client << "<h3>Clustering 单元测试</h3>";
    {
        ...
        // Test 2: Three clusters
        {
            std::vector<std::vector<double>> data = {
                ...
                {5.0, 5.1}, {5.1, 5.0}, {4.9, 5.2},
                {9.8, 9.9}, {9.9, 9.7}, {10.0, 10.1}
            };
            auto result = ClusteringProcess::simpleKMeans(data, 3);
            bool allNonEmpty = true;
            for (size_t c = 0; c < result.pointsInCluster.size(); ++c) {
                if (result.pointsInCluster[c].size() < 1) allNonEmpty = false;
            }
        ...
        // Test 3: Multi-dimensional data
        {
            std::vector<std::vector<double>> data = {
                ...
                {10.0, 20.0, 30.0}, {10.1, 20.1, 30.1}
            };
            auto result = ClusteringProcess::simpleKMeans(data, 2);
            bool pass = result.centroids.size() == 2
                     && result.clusterAssigned[0].clusterIndex != result.clusterAssigned[2].clusterIndex;
    ...
    client << "<p>21. 线性回归图 - 房价预测</p>";
    {
        ...
        chart.setData({
            {50, 150}, {60, 175}, {70, 210}, {80, 240}, {90, 270},
            {100, 300}, {110, 330}, {120, 360}, {130, 390}, {140, 420}
        });
    ...
    client << "<p>22. 指数回归图 - GDP 增长</p>";
    {
        ...
        chart.setData({
            {1, 2.1}, {2, 2.5}, {3, 3.1}, {4, 4.0}, {5, 5.2},
            {6, 6.8}, {7, 8.9}, {8, 11.5}, {9, 14.8}, {10, 19.0}
        });
    ...
    client << "<p>23. 对数回归图 - 学习曲线</p>";
    {
        ...
        chart.setData({
            {1, 20}, {2, 35}, {3, 45}, {4, 52}, {5, 57},
            {6, 61}, {8, 67}, {10, 71}, {15, 78}, {20, 83}
        });
    ...
    client << "<p>24. 多项式回归图 - 二次曲线</p>";
    {
        ...
        chart.setData({
            ...
            {5, 61.1}, {6, 82.3}, {7, 108.5}, {8, 138.2}, {9, 170.0}
        });
    ...
    client << "<p>25. 聚类散点图 - 客户分群 (3 类)</p>";
    {
        ...
        chart.setData({
            {2, 1}, {3, 2}, {2.5, 1.5}, {4, 3}, {3.5, 2.5},
            {1.5, 1}, {2, 2.5}, {3, 1.5},
            {12, 8}, {15, 10}, {14, 7}, {13, 9}, {16, 11},
            {11, 8}, {15, 8.5}, {14, 10},
            {45, 25}, {50, 30}, {48, 28}, {52, 32}, {55, 30},
            {47, 26}, {50, 27}, {53, 31}
        });
    ...
    client << "<p>26. 聚类散点图 - 4 类</p>";
    {
        ...
        chart.setData({
            {5, 5}, {6, 4}, {4, 6}, {5, 7}, {6, 5},
            {7, 4}, {4, 5},
            {5, 25}, {6, 26}, {4, 24}, {5, 27}, {7, 25},
            {6, 24}, {4, 26},
            {25, 5}, {26, 4}, {24, 6}, {25, 7}, {27, 5},
            {26, 6}, {24, 5},
            {25, 25}, {26, 26}, {24, 24}, {25, 27}, {27, 25},
            {26, 24}, {24, 26}
        });
