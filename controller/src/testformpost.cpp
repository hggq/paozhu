        ...
        if (!sitepath.empty())
        {
            ...
            if (fs::exists(paths))
            {
                ...
                if (is_success)
                {
                    ...
                    if (extfilesize > 3)
                    {
                        if (filename[extfilesize - 1] == 's' && filename[extfilesize - 2] == 'j' &&
                            filename[extfilesize - 3] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                            isshowfile = false;
                        }
                        if (isshowfile && filename[extfilesize - 1] == 'd' && filename[extfilesize - 2] == 'm' &&
                            filename[extfilesize - 3] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                        ...
                        if (isshowfile && filename[extfilesize - 1] == 'g' && filename[extfilesize - 2] == 'n' &&
                            filename[extfilesize - 3] == 'p' && filename[extfilesize - 4] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                            isshowfile = false;
                        }
                        if (isshowfile && filename[extfilesize - 1] == 'g' && filename[extfilesize - 2] == 'p' &&
                            filename[extfilesize - 3] == 'j' && filename[extfilesize - 4] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                            isshowfile = false;
                        }
                        if (isshowfile && filename[extfilesize - 1] == 't' && filename[extfilesize - 2] == 'x' &&
                            filename[extfilesize - 3] == 't' && filename[extfilesize - 4] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                            isshowfile = false;
                        }
                        if (isshowfile && filename[extfilesize - 1] == 'f' && filename[extfilesize - 2] == 'd' &&
                            filename[extfilesize - 3] == 'p' && filename[extfilesize - 4] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                        ...
                        if (isshowfile && filename[extfilesize - 1] == 's' && filename[extfilesize - 2] == 's' &&
                            filename[extfilesize - 3] == 'c' && filename[extfilesize - 4] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                            isshowfile = false;
                        }
                        if (isshowfile && filename[extfilesize - 1] == 'm' && filename[extfilesize - 2] == 't' &&
                            filename[extfilesize - 3] == 'h' && filename[extfilesize - 4] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                            isshowfile = false;
                        }
                        if (isshowfile && filename[extfilesize - 1] == 'g' && filename[extfilesize - 2] == 'v' &&
                            filename[extfilesize - 3] == 's' && filename[extfilesize - 4] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                            isshowfile = false;
                        }
                        if (isshowfile && filename[extfilesize - 1] == 'f' && filename[extfilesize - 2] == 'i' &&
                            filename[extfilesize - 3] == 'g' && filename[extfilesize - 4] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                            isshowfile = false;
                        }
                        if (isshowfile && filename[extfilesize - 1] == 'p' && filename[extfilesize - 2] == 'a' &&
                            filename[extfilesize - 3] == 'm' && filename[extfilesize - 4] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                            isshowfile = false;
                        }
                        if (isshowfile && extfilesize > 4 && filename[extfilesize - 1] == 'l' &&
                            filename[extfilesize - 2] == 'm' && filename[extfilesize - 3] == 't' &&
                            filename[extfilesize - 4] == 'h' && filename[extfilesize - 5] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                            isshowfile = false;
                        }
                        if (isshowfile && extfilesize > 4 && filename[extfilesize - 1] == 'g' &&
                            filename[extfilesize - 2] == 'e' && filename[extfilesize - 3] == 'p' &&
                            filename[extfilesize - 4] == 'j' && filename[extfilesize - 5] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                            isshowfile = false;
                        }
                        if (isshowfile && extfilesize > 4 && filename[extfilesize - 1] == 'p' &&
                            filename[extfilesize - 2] == 'b' && filename[extfilesize - 3] == 'e' &&
                            filename[extfilesize - 4] == 'w' && filename[extfilesize - 5] == '.')
                        {
                            sitepath.push_back('/');
                            sitepath.append(filename);
                            isshowfile = false;
                    ...
                    if (isshowfile)
                    {
                        ...
                        client << "<p><a href=\"" << client.get_hosturl() << "/upload/"
                               << get_filename(client.files["fileField"]["tempfile"].as_string())
                               << ".data\" target=_blank>show</a></p>";
                    }
                    else
                    {
                        client << "<p><a href=\"" << client.get_hosturl() << "/upload/" << filename
