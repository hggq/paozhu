...
class LinearScale
{
  public:
    ...
    {
        double ratio = (domainMax_ == domainMin_) ? 0.0 : (value - domainMin_) / (domainMax_ - domainMin_);
...
class SvgChart
{
  ...
  protected:
    ...
    std::string svgRect(double x, double y, double w, double h, const std::string &fill, double rx = 0) const;
    std::string svgCircle(double cx, double cy, double r, const std::string &fill) const;
    std::string svgLine(double x1, double y1, double x2, double y2, const std::string &stroke, double width = 1) const;
    ...
    std::string svgPath(const std::string &d,
                        ...
                        double width            = 2,
                        const std::string &fill = "none") const;
    std::string svgText(double x, double y, const std::string &text, const std::string &anchor = "middle", const std::string &fill = "#333", int fontSize = 12) const;
...
class SvgPieChart : public SvgChart
{
  public:
    ...
    // 添加数据项
    void addData(const std::string &name, double value, const SvgColor &color = SvgColor());
  ...
  private:
    ...
    std::string buildSectorPath(double cx, double cy, double r, double r0, double startAngle, double endAngle) const;
    // 计算扇区标签位置
    SvgPoint labelPosition(double cx, double cy, double r, double startAngle, double endAngle) const;
};
...
class SvgRadarChart : public SvgChart
{
  ...
  private:
    ...
    SvgPoint coordToPoint(double cx, double cy, double r, double r0, double value, double min, double max, double angle) const;
...
class SvgGaugeChart : public SvgChart
{
  ...
  private:
    ...
    // 绘制圆弧路径
    std::string arcPath(double cx, double cy, double r, double r0, double startAngle, double endAngle, bool clockwise = true) const;

    // 绘制指针
    std::string drawPointer(double cx, double cy, double r, double angleDeg, double pointerLen) const;

    // 绘制锚点
    std::string drawAnchor(double cx, double cy) const;

    // 绘制刻度、分割线和标签
    std::string drawTicks(double cx, double cy, double r, double startAngle, double endAngle) const;
...
struct FunnelDataItem
{
    ...
    double value;
    SvgColor color;
    FunnelDataItem(const std::string &n = "", double v = 0, const SvgColor &c = SvgColor())
        : name(n), value(v), color(c) {}
};
...
class SvgNegativeBarChart : public SvgChart
{
  public:
    ...
    void addData(double value, const std::string &label, const SvgColor &color = SvgColor());
...
class SvgJohariWindowChart : public SvgChart
{
  public:
    ...
    void setQuadrant(Quadrant q, const std::string &title, const std::string &content, const SvgColor &bgColor = SvgColor(), const SvgColor &titleColor = SvgColor(), const SvgColor &contentColor = SvgColor(), int titleFontSize = 14, int contentFontSize = 11);
...
class SvgGridChart : public SvgChart
{
  public:
    ...
    // 设置单个格子（只有标题，居中显示）
    void setCell(int row, int col, const std::string &title, const SvgColor &bgColor = SvgColor(), const SvgColor &titleColor = SvgColor(), int titleFontSize = 12);

    // 设置单个格子（标题 + 内容，标题顶对齐）
    void setCell(int row, int col, const std::string &title, const std::string &content, const SvgColor &bgColor = SvgColor(), const SvgColor &titleColor = SvgColor(), const SvgColor &contentColor = SvgColor(), int titleFontSize = 12, int contentFontSize = 10);
