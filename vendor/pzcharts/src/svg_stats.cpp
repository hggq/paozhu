    ...
    int yDim)
{
    ...
    for (const auto &row : data)
    {
        ...
        double y = row[yDim];
        if (y <= 0)
            continue;// skip non-positive y for log transform
        sumY += y;
        sumXY += x * y;
        sumX2Y += x * x * y;
    ...
    int yDim)
{
    ...
    for (const auto &row : data)
    {
        double x = row[xDim];
        if (x <= 0)
            continue;// skip non-positive x for log
