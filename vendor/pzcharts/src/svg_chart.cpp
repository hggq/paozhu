...
std::string SvgChart::svgRect(double x, double y, double w, double h, const std::string &fill, double rx) const
{
    std::ostringstream oss;
    oss << "<rect x=\"" << x << "\" y=\"" << y
        << "\" width=\"" << w << "\" height=\"" << h
...
std::string SvgChart::svgCircle(double cx, double cy, double r, const std::string &fill) const
{
    std::ostringstream oss;
...
std::string SvgChart::svgLine(double x1, double y1, double x2, double y2, const std::string &stroke, double width) const
{
    std::ostringstream oss;
...
std::string SvgChart::svgText(double x, double y, const std::string &text, const std::string &anchor, const std::string &fill, int fontSize) const
{
...
std::string SvgBarChart::render()
{
    ...
    for (int i = 0; i < maxLen; ++i)
    {
        ...
        {
            if (i >= static_cast<int>(series_[si].data.size()))
                continue;
...
void SvgPieChart::addData(const std::string &name, double value, const SvgColor &color)
{
    PieDataItem item;
    item.name  = name;
    item.value = value;
...
std::string SvgPieChart::buildSectorPath(double cx, double cy, double r, double r0, double startAngle, double endAngle) const
{
    // 将角度转换为弧度坐标
...
SvgPoint SvgPieChart::labelPosition(double cx, double cy, double r, double startAngle, double endAngle) const
{
...
std::string SvgPieChart::render()
{
    ...
    {
        if (std::isnan(data_[i].value) || data_[i].value <= 0)
            continue;
        ...
        oss << svgText(labelPos.x, labelPos.y, data_[i].name, "middle", "#333", 11);
...
SvgPoint SvgRadarChart::coordToPoint(double cx, double cy, double r, double r0, double value, double min, double max, double angle) const
{
    // 将值映射到 [r0, r] 范围
...
std::string SvgRadarChart::render()
{
    ...
    for (int level = 1; level <= splitCount; ++level)
    {
        ...
        {
            double val       = dataMax;
            SvgPoint labelPt = coordToPoint(layout.cx, layout.cy, layout.r, layout.r0, val, dataMin, dataMax, layout.axisAngles[0]);
    ...
    {
        // 轴线
        SvgPoint axisEnd = coordToPoint(layout.cx, layout.cy, layout.r, layout.r0, dataMax, dataMin, dataMax, layout.axisAngles[i]);
    ...
    for (const auto &s : series_)
    {
        ...
        for (int i = 0; i < n && i < static_cast<int>(s.data.size()); ++i)
        {
            ...
            points.push_back(coordToPoint(layout.cx, layout.cy, layout.r, layout.r0, val, dataMin, dataMax, layout.axisAngles[i]));
        }
...
std::string SvgGaugeChart::arcPath(double cx, double cy, double r, double r0, double startAngle, double endAngle, bool clockwise) const
{
...
std::string SvgGaugeChart::drawPointer(double cx, double cy, double r, double angleDeg, double pointerLen) const
{
...
std::string SvgGaugeChart::drawTicks(double cx, double cy, double r, double startAngle, double endAngle) const
{
    std::ostringstream oss;
...
std::string SvgGaugeChart::render()
{
    ...
    if (!colorStops_.empty())
    {
        ...
        for (size_t i = 0; i < colorStops_.size(); ++i)
        {
            ...
            std::string pathD = arcPath(cx, cy, gaugeR, gaugeR - axisLineWidth, prevEndDeg, segEndDeg, true);
    ...
    if (showProgress_)
    {
        ...
        std::string pathD    = arcPath(cx, cy, gaugeR, gaugeR - progressWidth, -startAngle_, valueDeg, true);
        // 进度条颜色：来自 ECharts 全局调色板 (visualStyleAccessPath='itemStyle')
        // 默认使用调色板第一色 #5470c6，可通过 setProgressColor 自定义
        SvgColor progressColor = progressColorSet_ ? progressColor_ : SvgColor(84, 112, 198);// 默认调色板色 #5470c6
...
void SvgFunnelChart::addData(const std::string &name, double value, const SvgColor &color)
{
    FunnelDataItem item;
    item.name  = name;
    item.value = value;
...
std::string SvgFunnelChart::render()
{
    ...
    for (size_t i = 0; i < data_.size(); ++i)
    {
        ...
        oss << svgText(cx, cy + 14, fmt(data_[i].value, 0), "middle", "#fff", 12);
    }
...
std::string SvgDensityChart::render()
{
    ...
    for (size_t si = 0; si < series_.size(); ++si)
    {
        ...
        oss << svgLine(legendX, legendY + 4, legendX + 25, legendY + 4, s.color.toHex(), lineWidth);
    ...
    if (showRug_)
    {
        ...
        for (size_t si = 0; si < series_.size(); ++si)
        {
            ...
            for (double xi : data)
            {
                ...
                oss << svgLine(rx, rugY, rx, rugY + rugHeight, s.color.toHex(), 1);
            }
        }
    }
...
void SvgNegativeBarChart::addData(double value, const std::string &label, const SvgColor &color)
{
    DataItem item;
    item.value = value;
    item.label = label;
...
std::string SvgCandlestickChart::render()
{
    ...
    for (const auto &item : data_)
    {
        if (std::isfinite(item.low))
            yMin = std::min(yMin, item.low);
        if (std::isfinite(item.high))
            yMax = std::max(yMax, item.high);
    }
...
void SvgJohariWindowChart::setQuadrant(Quadrant q, const std::string &title, const std::string &content, const SvgColor &bgColor, const SvgColor &titleColor, const SvgColor &contentColor, int titleFontSize, int contentFontSize)
{
    if (q < 0 || q > 3)
        return;
...
std::string SvgJohariWindowChart::render()
{
    ...
    for (int r = 0; r < 2; ++r)
    {
        for (int c = 0; c < 2; ++c)
        {
            ...
            if (qd.title.empty() && qd.content.empty())
                continue;
...
void SvgGridChart::setCell(int row, int col, const std::string &title, const SvgColor &bgColor, const SvgColor &titleColor, int titleFontSize)
{
...
void SvgGridChart::setCell(int row, int col, const std::string &title, const std::string &content, const SvgColor &bgColor, const SvgColor &titleColor, const SvgColor &contentColor, int titleFontSize, int contentFontSize)
{
    if (row < 0 || row > 2 || col < 0 || col > 2)
        return;
    cells_[row][col].title   = title;
...
std::string SvgGridChart::render()
{
    ...
    if (hasYLabels)
    {
        double labelX      = (yLabelPos_ == "right") ? gridX + gridW + 10 : gridX - 10;
    ...
    if (hasXLabels)
    {
        double labelY      = (xLabelPos_ == "top") ? gridY - 10 : gridY + gridH + 25;
        std::string anchor = "middle";
    ...
    for (int r = 0; r < 3; ++r)
    {
        for (int c = 0; c < 3; ++c)
        {
            ...
            if (cell.title.empty() && cell.content.empty())
                continue;
...
SvgColor SvgHeatmapChart::valueToColor(double value) const
{
    if (gradientColors_.empty())
        return SvgColor(0, 0, 0);
    if (gradientColors_.size() == 1)
        return gradientColors_[0];
...
std::string SvgHeatmapChart::render()
{
    ...
    for (const auto &item : data_)
    {
        if (item.x < 0 || item.x >= xCount || item.y < 0 || item.y >= yCount)
            continue;
        if (!std::isfinite(item.value))
            continue;
...
std::string SvgClusteringChart::render()
{
    ...
    for (size_t i = 0; i < data_.size(); ++i)
    {
        ...
        if (ci < 0 || ci >= static_cast<int>(colors.size()))
            continue;
