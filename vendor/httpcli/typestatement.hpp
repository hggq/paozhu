...
struct obj_reflect_type
{
  ...
  unsigned char
      typevalue; // 0 null 1 struct 2 class 3 func  10 float 11 double 12 char
                 // 13 unsigned char 14 long long  15 unsigned long long  16 int
                 // 17 unsigned  20 string 21 vector 22 map 23 array 30 other
...
public:
  ...
  std::vector<struct obj_reflect_type>
  pickreflectfile(const std::string &methodpathfile)
  {
    std::vector<struct obj_reflect_type> temp;
    std::unique_ptr<std::FILE, decltype(&std::fclose)> fp(
        std::fopen(methodpathfile.c_str(), "rb"), &std::fclose);
    ...
    for (unsigned int i = 0; i < filecontent.size(); i++)
    {
      ...
      // pick namespace
      if (filecontent[i] == 'n' && filecontent[i + 1] == 'a' &&
          filecontent[i + 2] == 'm' && filecontent[i + 3] == 'e' &&
          filecontent[i + 4] == 's' && filecontent[i + 5] == 'p' &&
          filecontent[i + 6] == 'a' && filecontent[i + 7] == 'c' &&
          filecontent[i + 8] == 'e' && filecontent[i + 9] == ' ')
      {
        ...
        {
          if (filecontent[j] == ' ' || filecontent[j] == '\t' ||
              filecontent[j] == '{' || filecontent[j] == 0x0A ||
              filecontent[j] == 0x0D)
          {
            break;
          }
    ...
    for (unsigned int i = 0; i < filecontent.size(); i++)
    {
      ...
      if (filecontent[i] == '/' && filecontent[i + 1] == '/' &&
          filecontent[i + 2] == '@' && filecontent[i + 3] == 'r' &&
          filecontent[i + 4] == 'e' && filecontent[i + 5] == 'f' &&
          filecontent[i + 6] == 'l')
      {
        if (filecontent[i + 7] == 'e' && filecontent[i + 8] == 'c' &&
            filecontent[i + 9] == 't' && filecontent[i + 10] == ' ' &&
            filecontent[i + 11] == 'j' && filecontent[i + 12] == 's' &&
  ...
  std::tuple<unsigned int, obj_reflect_type>
  processclass(const std::string &filecontent, unsigned int offset = 0)
  {
    ...
    {
      // struct
      if (filecontent[offset] == 's' && filecontent[offset + 1] == 't' &&
          filecontent[offset + 2] == 'r' && filecontent[offset + 3] == 'u' &&
          filecontent[offset + 4] == 'c' && filecontent[offset + 5] == 't' &&
      ...
      // class
      if (filecontent[offset] == 'c' && filecontent[offset + 1] == 'l' &&
          filecontent[offset + 2] == 'a' && filecontent[offset + 3] == 's' &&
          filecontent[offset + 4] == 's' && filecontent[offset + 5] == ' ')
      {
        offset = offset + 6;
        for (; offset < filecontent.size(); offset++)
        {
    ...
    if (classname.size() > 0)
    {
      ...
      for (; offset < filecontent.size();)
      {
        ...
        // struct
        bool isobj = false;
        if (linecontent.size() > 5 && linecontent[0] == 's' &&
            linecontent[1] == 't' && linecontent[2] == 'r' &&
            linecontent[3] == 'u' && linecontent[4] == 'c' &&
            linecontent[5] == 't' && linecontent[6] == ' ')
        {
          isobj = true;
        }
        // class
        if (linecontent.size() > 4 && linecontent[0] == 'c' &&
            linecontent[1] == 'l' && linecontent[2] == 'a' &&
            linecontent[3] == 's' && linecontent[4] == 's' &&
            linecontent[5] == ' ')
        {
          isobj = true;
        }
        // public:
        if (linecontent[0] == 'p' && linecontent[1] == 'u' &&
            linecontent[2] == 'b' && linecontent[3] == 'l' &&
            linecontent[4] == 'i' && linecontent[5] == 'c' &&
            linecontent[6] == ':')
        {
          ispublic = true;
          offset   = newoffset;
        ...
        else if (linecontent[0] == 'p' && linecontent[1] == 'r' &&
                 linecontent[2] == 'i' && linecontent[3] == 'v' &&
                 linecontent[4] == 'a' && linecontent[5] == 't' &&
  ...
  processvarname(const std::string &filecontent, unsigned int offset = 0)
  {
    std::string varname;
    for (; offset < filecontent.size(); offset++)
    {
  ...
  unsigned int processfunc(const std::string &filecontent,
                           unsigned int offset = 0)
  {
    ...
    for (; offset < filecontent.size(); offset++)
    {
      ...
      if (filecontent[offset] == '/')
      {
        ...
        else if ((offset + 1) < filecontent.size() &&
                 filecontent[offset + 1] == '/')
        {
          for (; offset < filecontent.size(); offset++)
          {
  ...
  std::tuple<int, obj_reflect_type>
  processclassline(const std::string &linecontent)
  {
    struct obj_reflect_type temp;
    unsigned int offset      = 0;
  ...
  std::tuple<unsigned int, std::string>
  readlinecontent(const std::string &filecontent, unsigned int offset = 0)
  {
  ...
  {
    std::unique_ptr<std::FILE, decltype(&std::fclose)> fp(
        std::fopen(filename.c_str(), "wb"), &std::fclose);
...
#include "unicode.h"
  ...
  std::string
  jsonobjfuncto_json(const struct obj_reflect_type &json_reflect_data,
                     const std::string &jsonobjtypename)
  {
    std::string temp;
    // to_json
    temp = "\n\nstd::string json_encode(const " + jsonobjtypename +
           " &json_reflectobj)\n\t{\n";
    temp += single_obj_to_json(json_reflect_data, jsonobjtypename,
                               "json_reflectobj", true, 1);
    temp += "\n\t}\n";

    temp += "\n\nstd::string json_encode(const std::vector<" + jsonobjtypename +
            "> &json_reflectobj)";
    temp += R"(
        {
  ...
  std::string
  jsonobjfuncfrom_json(const struct obj_reflect_type &json_reflect_data,
                       const std::string &jsonobjtypename)
  {
    std::string temp;
  ...
  std::string
  single_obj_fromjson(const struct obj_reflect_type &json_reflect_data,
                      const std::string &jsonobjtypename,
                      unsigned int levelnum = 0)
  {
    ...
    tempcontent +=
        R"( &json_reflectobj,const std::string &_json_data,unsigned int _offset)
     {           
             bool _isarray=false;
    ...
    for (unsigned int i = 0; i < json_reflect_data.children.size(); i++)
    {
      if (json_reflect_data.children[i].type == "struct")
      {
        ...
        tempcontent += R"(") == 0)
                            {)";
        std::string temp_obj_name_obj =
            tempobjname + json_reflect_data.children[i].varname;

        tempcontent += object_item_fromjson(json_reflect_data.children[i],
                                            temp_obj_name_obj, 1);
    ...
    for (unsigned int i = 0; i < json_reflect_data.children.size(); i++)
    {
      ...
      if (json_reflect_data.children[i].type.size() > 12 &&
          json_reflect_data.children[i].type.starts_with(
              "std::vector<std::string>"))
      {
        ...
        tempcontent += R"(") == 0)
                            {)";
        obj_name_array_tempname =
            tempobjname + json_reflect_data.children[i].name;
        tempcontent += array_item_fromjson(json_reflect_data.children[i],
                                           obj_name_array_tempname, 1, 30);
                   ...
                   "std::vector<int>"))
      {
        ...
        obj_name_array_tempname =
            tempobjname + json_reflect_data.children[i].name;
        tempcontent += array_item_fromjson(json_reflect_data.children[i],
                                           obj_name_array_tempname, 1, 24);

        tempcontent += R"(     
                            }
                            )";
      }
      else if (json_reflect_data.children[i].type ==
  ...
  std::string
  object_item_fromjson(const struct obj_reflect_type &json_reflect_data,
                       const std::string &jsonobjtypename,
                       unsigned int levelnum = 0)
  {
    std::string tempcontent;
  ...
  std::string
  array_item_fromjson(const struct obj_reflect_type &json_reflect_data,
                      const std::string &jsonobjtypename,
                      unsigned int levelnum = 0, unsigned int typenum = 0)
  {
    std::string tempcontent;
    std::string tempobjname = jsonobjtypename + ".";
    unsigned ishas          = 0;
    ...
    if (typenum > 39)
    {
      ...
      tempcontent += array_item_fromjson(json_reflect_data, tempobjname_a,
                                         levelnum, temptype_a);
    }
...
unsigned int json_decode(std::vector<)";
  ...
  std::string
  single_obj_to_json(const struct obj_reflect_type &json_reflect_data,
                     const std::string &jsonobjtypename,
                     std::string objnameval = "", bool isheadcontent = true,
                     unsigned int leveobj = 1)
  {
    ...
    for (unsigned int i = 0; i < json_reflect_data.children.size(); i++)
    {
      if (json_reflect_data.children[i].name.size() > 0 &&
          json_reflect_data.children[i].name[0] == '_')
      {
        continue;
      }
      if (i > 0)
      ...
      if (json_reflect_data.children[i].type == "struct")
      {
        std::string templevel =
            tempobjname + json_reflect_data.children[i].varname;
        maincontent += "_stream <<\"\\\"" +
                       json_reflect_data.children[i].varname + "\\\":\";";
        unsigned int newlevelobj = leveobj + 1;
        maincontent +=
            single_obj_to_json(json_reflect_data.children[i], jsonobjtypename,
                               templevel, false, newlevelobj);
      }
      else
      {
        maincontent +=
            single_obj_itemto_json(json_reflect_data.children[i],
                                   jsonobjtypename, tempobjname, leveobj);
      }
    }
  ...
  std::string
  single_obj_itemto_json(const struct obj_reflect_type &json_reflect_data,
                         const std::string &jsonobjtypename,
                         std::string pre_name = "", unsigned int levelnum = 0)
  {
    ...
    else if (json_reflect_data.type == selfitem)
    {
      ...
      _stream << "]";
             ...
             json_reflect_data.type == "std::vector<float>" ||
             json_reflect_data.type == "std::vector<double>" ||
    ...
    else if (json_reflect_data.type.size() > 3 &&
             json_reflect_data.type[0] != 's' &&
             json_reflect_data.type[1] != 't' &&
             json_reflect_data.type[2] != 'd' &&
    ...
    else if (json_reflect_data.type.size() > 10 &&
             json_reflect_data.type[0] == 's' &&
             json_reflect_data.type[1] == 't' &&
             json_reflect_data.type[2] == 'd' &&
             json_reflect_data.type[3] == ':' &&
             json_reflect_data.type[4] == ':' &&
             json_reflect_data.type[5] == 'v' &&
             json_reflect_data.type[6] == 'e' &&
             json_reflect_data.type[7] == 'c' &&
             json_reflect_data.type[8] == 't' &&
             json_reflect_data.type[9] == 'o' &&
             json_reflect_data.type[10] == 'r' &&
  ...
  std::string get_jsonreflect_header(
      const std::vector<struct obj_reflect_type> &methodpathfile)
  {
    std::string temp;
    std::string namespacename;
    ...
    for (unsigned int i = 0; i < methodpathfile.size(); i++)
    {
      ...
      temp += "\n\nstd::string json_encode(const " + methodpathfile[i].name +
              " &json_reflectobj);\n";
      temp += "\n\nstd::string json_encode(const std::vector<" +
              methodpathfile[i].name + "> &json_reflectobj);";
      temp += "\n\nunsigned int json_decode(" + methodpathfile[i].name +
              " &json_reflectobj,const std::string &_json_data,unsigned int "
              "_offset=0);";
      temp += "\n\nunsigned int json_decode(std::vector<" +
              methodpathfile[i].name +
              "> &json_reflectobj,const std::string &_json_data,unsigned int "
              "_offset=0);";
    }
    if (namespacename.size() > 0)
