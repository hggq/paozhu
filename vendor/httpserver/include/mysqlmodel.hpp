...
namespace orm
{
// 通用操作 类 mysql 方法 在这里接上getSession(); 这里可以操作 data sql count
// page
//  using namespace http;
typedef std::vector<
    std::pair<std::string, std::function<void(long long, long long)>>>
    commit_lists_callback;
...
public:
  void save(std::size_t hashid, BASE_MODEL &data_list, int expnum = 0,
            bool cover_data = false)
  {
    std::map<std::size_t, data_cache_t> &obj =
        get_static_model_cache<data_cache_t>();
  ...
  void save(std::size_t hashid, std::vector<BASE_MODEL> &data_list,
            int expnum = 0, bool cover_data = false)
  {
    std::map<std::size_t, data_cache_t> &obj =
        get_static_model_cache<data_cache_t>();
    struct data_cache_t temp;
...
public:
  ...
  query(const std::string &rawsql, bool isedit = false)
  {
    ...
    try
    {
      ...
      mysql_ping(conn.get());
      long long readnum =
          mysql_real_query(conn.get(), &rawsql[0], rawsql.size());
      ...
      for (unsigned char index = 0; index < num_fields; index++)
      {
        ...
        std::transform(type_temp.begin(), type_temp.end(), type_temp.begin(),
                       ::tolower);
  ...
  long long query_edit(const std::string &rawsql, bool isinsert = false)
  {
    try
    {
      std::unique_ptr<MYSQL, decltype(&mysql_close)> conn =
          http::get_mysqleditexecute(dbhash);
      mysql_ping(conn.get());
      long long readnum =
          mysql_real_query(conn.get(), &rawsql[0], rawsql.size());
...
public:
  ...
  unsigned int count()
  {
    ...
    try
    {
      ...
      std::unique_ptr<MYSQL, decltype(&mysql_close)> conn =
          http::get_mysqlselectexecute(dbhash);
      MYSQL_RES *resultone = nullptr;
      mysql_ping(conn.get());
      long long readnum =
          mysql_real_query(conn.get(), &countsql[0], countsql.size());
  ...
  std::vector<std::map<std::string, std::string>> fetch_obj()
  {
    ...
    try
    {
      ...
      mysql_ping(conn.get());
      long long readnum =
          mysql_real_query(conn.get(), &sqlstring[0], sqlstring.size());
      ...
      while ((json_row = mysql_fetch_row(resultall)))
      {
        ...
        for (unsigned char index = 0; index < num_fields; index++)
        {
          if (json_row[index] != NULL)
          {
            rowtemp.insert(table_fieldname[index],
                           std::string(json_row[index]));
          }
          else
          {
  ...
  std::tuple<std::vector<std::string>, std::map<std::string, unsigned int>,
  ...
  fetch_row()
  {
    ...
    if (iscache)
    {
      ...
      {
        iscache = false;
        model_meta_cache<std::string> &table_cache =
            model_meta_cache<std::string>::getinstance();
        table_fieldname = table_cache.get(sqlhashid);

        model_meta_cache<std::map<std::string, unsigned int>> &tablemap_cache =
            model_meta_cache<
                std::map<std::string, unsigned int>>::getinstance();
      ...
      if (iscache)
      {
        if (exptime > 0)
        {
          if (temprecord.size() > 0)
          {
            ...
            model_meta_cache<std::map<std::string, unsigned int>>
                &tablemap_cache = model_meta_cache<
                    std::map<std::string, unsigned int>>::getinstance();
  ...
  void remove_exptime_cache()
  {
    model_meta_cache<typename base::meta> &temp_cache =
        model_meta_cache<typename base::meta>::getinstance();
  ...
  bool save_cache(std::size_t cache_key_name, typename base::meta &cache_data,
                  int exp_time = 0)
  {
    model_meta_cache<typename base::meta> &temp_cache =
        model_meta_cache<typename base::meta>::getinstance();
  ...
  bool save_cache(std::size_t cache_key_name,
                  std::vector<typename base::meta> &cache_data,
                  int exp_time = 0)
  {
    model_meta_cache<typename base::meta> &temp_cache =
        model_meta_cache<typename base::meta>::getinstance();
  ...
  std::tuple<long long, long long> insert(typename base::meta &insert_data,
                                          bool isclear = true)
  {
    try
    {
  ...
  insert(std::vector<typename base::meta> &insert_data, bool isclear = true)
  {
    try
    {
      sqlstring = base::_makerecordinsertsql(insert_data);
  ...
  int submit_commit(const commit_lists_callback &commit_sqllist)
  {
    ...
    try
    {
      ...
      {
        readnum = mysql_real_query(conn.get(), &commit_sqllist[j].first[0],
                                   commit_sqllist[j].first.size());
        if (readnum != 0)
        {
          error_msg = std::string(mysql_error(conn.get()));
        ...
        if (commit_sqllist[j].first.size() > 2 &&
            commit_sqllist[j].first[0] == 'I' &&
