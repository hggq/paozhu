#ifndef HTTP_POSTGRESQL_ORM_HPP
#define HTTP_POSTGRESQL_ORM_HPP
/*
...
{ /*tagnamespace_replace*/
    ...
    class mysql_orm : public B_BASE
    {
      ...
      private:
        ...
        {
            if (B_BASE::auto_pk_index < 0)
                return false;
            return get_field_value(static_cast<unsigned char>(B_BASE::auto_pk_index), data_temp).to_int() == 0;
        }
      ...
      public:
        ...
        std::tuple<unsigned int, unsigned long long> exec_insert()
        {
            ...
            for (unsigned char i = 0; i < B_BASE::col_names.size(); i++)
            {
                ...
                if (pk_use_default && static_cast<int>(i) == B_BASE::auto_pk_index)
                {
                    sql.append("DEFAULT");
                    continue;
                }
            ...
            for (size_t r = 0; r < nrows; r++)
            {
                ...
                for (unsigned char c = 0; c < ncols; c++)
                {
                    if (c > 0)
                        sql.append(", ");
                    // DEFAULT 不占参数位，后面的列继续按顺序编号
                    if (pk_use_default && static_cast<int>(c) == B_BASE::auto_pk_index)
                    {
                        sql.append("DEFAULT");
                        continue;
                    }
