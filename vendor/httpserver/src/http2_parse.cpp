...
void http2parse::readrst_stream([[maybe_unused]] const unsigned char *buffer, [[maybe_unused]] unsigned int buffersize)
{
    ...
    if (http_data.contains(block_steamid))
    {
        ...
        http_data[block_steamid]->socket_session->http2_send_rst_stream(block_steamid, 1);
    }
}
...
void http2parse::readrawfileformdata(const unsigned char *buffer, unsigned int buffersize)
{
    ...
    if (block_steam_httppeer->content_length < (block_data_info_ptr->upfile->size + 2))
    {
        ...
        {
            // fclose(block_data_info_ptr->uprawfile.get());
            block_data_info_ptr->uprawfile.reset();
        }
        procssformfile();
    }
}
...
void http2parse::data_process()
{
    ...
    {
        if (block_steam_httppeer->content_length > CONST_HTTP_BODY_POST_SIZE)
        {
            error = 41400;
            return;
        }
        if (block_steam_httppeer->upload_length > CONST_HTTP_BODY_POST_SIZE)
        {
            error = 41401;
            return;
        }
        ...
        case 3:
            ...
            if (block_steam_httppeer->rawcontent.size() > CONST_HTTP_JSON_POST_SIZE)
            {
                error = 4140;
                return;
            }
