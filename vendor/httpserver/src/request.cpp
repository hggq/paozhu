...
OBJ_VALUE::OBJ_VALUE(const bool i) : int_v(static_cast<bool>(i)), type_t(BOOL) {}
...
void OBJ_VALUE::push(unsigned long long temp, const OBJ_VALUE &v) { array_v.push(temp, v); }

void OBJ_VALUE::push(std::string &key, OBJ_VALUE &v) { array_v.push(key, v); }
void OBJ_VALUE::push(std::string &key, OBJ_VALUE &&v) { array_v.push(key, std::move(v)); }

void OBJ_VALUE::push(std::string &&key, OBJ_VALUE &v) { array_v.push(std::move(key), v); }
void OBJ_VALUE::push(std::string &&key, OBJ_VALUE &&v) { array_v.push(std::move(key), std::move(v)); }
...
std::string OBJ_VALUE::tag() { return string_v; }
...
bool OBJ_VALUE::isset(const std::string &&key) { return array_v.isset(std::move(key)); }

bool OBJ_VALUE::unset(unsigned long long temp) { return array_v.unset(temp); }

bool OBJ_VALUE::unset(const std::string &key) { return array_v.unset(key); }

bool OBJ_VALUE::unset(const std::string &&key) { return array_v.unset(std::move(key)); }

std::size_t OBJ_VALUE::size() const
{
    switch (type())
    {
    case INT: return sizeof(long long); break;

    case FLOAT: return sizeof(long double); break;

    case NIL: return 0; break;

    case STRING: return string_v.size(); break;

    case ARRAY: return array_v.size(); break;
    default: return 0;
    }
}
...
std::map<unsigned long long, OBJ_VALUE>::iterator OBJ_ARRAY::begin() { return _array.begin(); }

std::map<unsigned long long, OBJ_VALUE>::iterator OBJ_ARRAY::end() { return _array.end(); }

std::map<unsigned long long, OBJ_VALUE>::const_iterator OBJ_ARRAY::begin() const { return _array.begin(); }

std::map<unsigned long long, OBJ_VALUE>::const_iterator OBJ_ARRAY::end() const { return _array.end(); }
...
std::size_t OBJ_ARRAY::size() const { return _array.size(); }
...
void OBJ_ARRAY::push(unsigned long long temp, const OBJ_VALUE &v) { _array[temp] = v; }
...
bool value_write(FILE *f, const OBJ_VALUE &v)
{
    ...
    case STRING:
        ...
        break;
    case ARRAY: array_write(f, v.as_array(), objsize); break;
    }
    return true;
}
...
std::string OBJ_ARRAY::to_json()
{
    ...
    if (is_numarray())
    {
        ...
        for (auto [first, second] : _array)
        {
            ...
            else if (second.is_bool())
            {
                os << (second.as_bool() ? "true" : "false");
            }
        }
...
std::string OBJ_VALUE::to_json()
{
    ...
    case INT: os << int_v; break;
    case BOOL: os << (int_v > 0 ? "true" : "false"); break;
    case FLOAT:
    ...
    case ARRAY: os << array_v.to_json(); break;
    }
    return os.str();
}
...
std::ostream &operator<<(std::ostream &os, const http::OBJ_VALUE &v)
{
    ...
    case http::INT: os << (long long)v; break;
    case http::BOOL: os << (v.as_bool() ? "true" : "false"); break;
    case http::FLOAT:
    ...
    case http::ARRAY: os << v.as_array(); break;
    }
    return os;
}
