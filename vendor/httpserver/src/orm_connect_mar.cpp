...
void orm_connect_mar_t::save_log(const std::string &filename)
{
...
#ifndef _MSC_VER
    int fd = open(filename.c_str(), O_WRONLY | O_CREAT | O_APPEND, 0640);
    if (fd == -1)
    {
        return;
    }
    ...
    if (n_write != temp_file_content.size())
    {
        // 日志写入失败（磁盘满/IO错误），忽略以免影响主流程
        n_write = 0;
    }
...
void orm_connect_mar_t::watch_mysql_connect(std::weak_ptr<mysql_conn_base> conn)
{
    std::unique_lock lk(connect_mutex);
    mysql_conn_list.emplace_back(conn);
    lk.unlock();
}
void orm_connect_mar_t::watch_pg_connect(std::weak_ptr<pg_conn_base> conn)
{
    std::unique_lock lk(connect_mutex);
    pg_conn_list.emplace_back(conn);
    lk.unlock();
}
void orm_connect_mar_t::clear_connect()
{
    ...
    std::unique_lock lk(connect_mutex);
    total_connect = mysql_conn_list.size() + pg_conn_list.size();
    lk.unlock();
    ...
    for (unsigned int i = 0; i < total_connect; i++)
    {
        ...
        for (auto iter = mysql_conn_list.begin(); iter != mysql_conn_list.end();)
        {
