...
namespace http
{
      ...
      // }
      void httpparse::setpeer(std::shared_ptr<httppeer> hpeer)
      {
            peer=hpeer->get_ptr();
      }
      ...
      {
            peer->state.ifmodifiedsince = strgmttotime(header_value);
      }
      ...
      void httpparse::readformfilename(const unsigned char *buffer, unsigned int buffersize)
      {
            ...
            if (isbound == 1)
            {
                  ...
                  if (postfieldtype == 9)
                  {
                        ...
                        header_temp = upfile.filename + std::to_string(timeid()) + header_key + std::to_string(rand_range(100, 999));
