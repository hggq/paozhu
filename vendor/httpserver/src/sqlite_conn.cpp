    ...
    unsigned int &affected_rows_out)
{
    return this->submit_sync<unsigned int>([this, &sql, &fields_out, &rows_out, &affected_rows_out]()
        ...
        {
            field_info_t fi;
            fi.name     = result.column_names[i];
            fi.org_name = result.column_names[i];
            fields_out.push_back(fi);
        }

        for (size_t ri = 0; ri < result.rows.size(); ri++)
        {
            pg_row_data_t rd;
            rd.values = result.rows[ri];
            if (ri < result.is_null.size())
            {
                rd.is_null = result.is_null[ri];
            }
            rows_out.push_back(std::move(rd));
        }
