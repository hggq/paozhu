...
namespace http
{
      ...
      void httppeer::save_session()
      {
            ...
            if (sessionfile.empty())
            {
                  sessionfile = client_ip + std::to_string(client_port) + std::to_string(timeid()) + std::to_string(rand_range(1000, 9999));
      ...
      unsigned char httppeer::get_fileinfo()
      {
            ...
            if (stat(sendfilename.c_str(), &fileinfo) == 0)
            {
                  ...
                  else if (fileinfo.st_mode & S_IFREG)
                  {
                        ...
                        //     pathtype=3;
                        // }
                        if (sysconfigpath.siteusehtmlchache && pathinfos.size() > 2 && sysconfigpath.siteusehtmlchachetime > 10 && sysconfigpath.siteusehtmlchachetime < (timeid() - (unsigned long)fileinfo.st_mtime))
                        {
                              sendfiletype = 3;
                        }
                  }
      ...
      std::shared_ptr<httppeer> httppeer::get_ptr()
      {
      ...
      void httppeer::status(unsigned int http_code)
      {
            status_code = http_code;
      }
      void httppeer::type(const std::string &a)
      {
            content_type = a;
      }
      ...
      std::list<std::string> httppeer::cookietoheader()
      {
            ...
            for (auto &iter : send_cookie)
            {
                  ...
                  if (!samesite.empty())
                  {
                        ...
                        case 'L':
                        case 'l':
                              temph.append("; SameSite=Lax");
                              break;
                        case 'S':
                        case 's':
                              temph.append("; SameSite=Strict");
                              break;
                        }
                  }
      ...
      void httppeer::scheme(unsigned char code)
      {
            isssl = (code == 1) ? 1 : 0;
      }
      void httppeer::length(unsigned long long num)
      {
            content_length = num;
      }
