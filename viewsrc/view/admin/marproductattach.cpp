...
namespace view {
	namespace admin{
 		 std::string marproductattach([[maybe_unused]] const struct view_param &vinfo,[[maybe_unused]] http::OBJ_VALUE &obj)
			{
 ...
 echo<<a.second["name"].to_string(); 
 			 echo<<" </strong></p>\n            ";
 for(auto &b:a.second["profilelist"].as_array()){ 
              std::string attrname=b.second["name"].as_string();  
              if(attrname.size()>0)
              {
            
 			 echo<<"            <p style=\"color: #555555;text-decoration: underline;margin-left: 150px;\"\"><a href=\"";
 echo<<b.second["url"].to_string(); 
 			 echo<<"\" target=\"_blank\">";
 echo<<b.second["name"].as_string(); 
