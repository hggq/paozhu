...
namespace view {
	namespace admin{
 		 std::string edittopicsidepick([[maybe_unused]] const struct view_param &vinfo,[[maybe_unused]] http::obj_val &obj)
			{
...
echo<<obj["info"]["rownum"].to_string(); 
 ...
 echo<<a["id"].to_string(); 
 			 echo<<"\" ";
 if(a["id"].to_int()==obj["info"]["picktopicid"].to_int()){ 
 			 echo<<"selected";
 } 
 			 echo<<"  ";
